...
export class NativeExtensionService extends AbstractExtensionService implements IExtensionService {
	...
	constructor(
		...
		// see https://github.com/microsoft/ProX-Code/issues/41322
		...
		if (extensionHost.kind === ExtensionHostKind.LocalProcess) {
			if (code === ExtensionHostExitCode.VersionMismatch) {
				this._notificationService.prompt(
					...
					[{
						label: nls.localize('relaunch', "Relaunch ProX Code"),
						run: () => {
					...
					choices.push({
						...
						run: () => {
							this._instantiationService.invokeFunction(accessor => {
								...
								openerService.open('https://aka.ms/ProX-Code-extension-bisect');
							});
						}
					});
				}
