...
class BufferSynchronizer {
	...
	constructor(
		...
		pathNormalizer: (path: ProX-Code.Uri) => string | undefined,
	...
	public open(resource: ProX-Code.Uri, args: Proto.OpenRequestArgs) {
	 ...
	 */
	public close(resource: ProX-Code.Uri, filepath: string, scriptKind: ScriptKind | undefined): boolean {
		return this.updatePending(resource, new CloseOperation(filepath, scriptKind));
	}

	public change(resource: ProX-Code.Uri, filepath: string, events: readonly ProX-Code.TextDocumentContentChangeEvent[]) {
		if (!events.length) {
			return;
		}
	...
	private updatePending(resource: ProX-Code.Uri, op: BufferOperation): boolean {
		switch (op.type) {
...
class SyncedBuffer {
	...
	constructor(
		private readonly document: ProX-Code.TextDocument,
	...
	private getProjectRootPath(resource: ProX-Code.Uri): string | undefined {
		...
		if (!workspaceRoot && ProX-Code.workspace.workspaceFolders) {
			for (const root of Array.from(ProX-Code.workspace.workspaceFolders).sort((a, b) => a.uri.path.length - b.uri.path.length)) {
	...
	public onContentChanged(events: readonly ProX-Code.TextDocumentContentChangeEvent[]): void {
		if (this.state !== BufferState.Open) {
...
class SyncedBufferMap extends ResourceMap<SyncedBuffer> {
	...
	public getForPath(filePath: string): SyncedBuffer | undefined {
		return this.get(ProX-Code.Uri.file(filePath));
	}
...
class PendingDiagnostics extends ResourceMap<number> {
	public getOrderedFileSet(): ResourceMap<void | ProX-Code.Range[]> {
		const orderedResources = Array.from(this.entries())
			.sort((a, b) => a.value - b.value)
			.map(entry => entry.resource);

		const map = new ResourceMap<void | ProX-Code.Range[]>(this._normalizePath, this.config);
...
class GetErrRequest {
	...
	public static executeGetErrRequest(
		...
		files: ResourceMap<void | ProX-Code.Range[]>,
		onDone: () => void
	) {
	...
	private readonly _token: ProX-Code.CancellationTokenSource = new ProX-Code.CancellationTokenSource();

	private constructor(
		private readonly client: ITypeScriptServiceClient,
		public readonly files: ResourceMap<void | ProX-Code.Range[]>,
		onDone: () => void
	) {
...
class TabResourceTracker extends Disposable {
	...
	private readonly _onDidChange = this._register(new ProX-Code.EventEmitter<{
		readonly closed: Iterable<ProX-Code.Uri>;
		readonly opened: Iterable<ProX-Code.Uri>;
	}>());
	public readonly onDidChange = this._onDidChange.event;

	private readonly _tabResources: ResourceMap<{ readonly tabs: Set<ProX-Code.Tab> }>;

	constructor(
		normalizePath: (resource: ProX-Code.Uri) => string | undefined,
		config: {
		...
		this._tabResources = new ResourceMap<{ readonly tabs: Set<ProX-Code.Tab> }>(normalizePath, config);

		for (const tabGroup of ProX-Code.window.tabGroups.all) {
			for (const tab of tabGroup.tabs) {
				this.add(tab);
			}
		}

		this._register(ProX-Code.window.tabGroups.onDidChangeTabs(e => {
	...
	public has(resource: ProX-Code.Uri): boolean {
		if (resource.scheme === fileSchemes.ProX-CodeNotebookCell) {
			const notebook = ProX-Code.workspace.notebookDocuments.find(doc =>
	...
	private add(tab: ProX-Code.Tab): ProX-Code.Uri[] {
		const addedResources: ProX-Code.Uri[] = [];
	...
	private delete(tab: ProX-Code.Tab): ProX-Code.Uri[] {
		const closedResources: ProX-Code.Uri[] = [];
		for (const uri of this.getResourcesForTab(tab)) {
			const entry = this._tabResources.get(uri);
			if (!entry) {
				continue;
	...
	private getResourcesForTab(tab: ProX-Code.Tab): ProX-Code.Uri[] {
		if (tab.input instanceof ProX-Code.TabInputText) {
			return [tab.input.uri];
		} else if (tab.input instanceof ProX-Code.TabInputTextDiff) {
			return [tab.input.original, tab.input.modified];
		} else if (tab.input instanceof ProX-Code.TabInputNotebook) {
			return [tab.input.uri];
		} else {
			return [];
		}
...
export default class BufferSyncSupport extends Disposable {
	...
	constructor(
		...
		const pathNormalizer = (path: ProX-Code.Uri) => this.client.toTsFilePath(path);
		...
		ProX-Code.workspace.onDidChangeConfiguration(this.updateConfiguration, this, this._disposables);
	}

	private readonly _onDelete = this._register(new ProX-Code.EventEmitter<ProX-Code.Uri>());
	public readonly onDelete = this._onDelete.event;

	private readonly _onWillChange = this._register(new ProX-Code.EventEmitter<ProX-Code.Uri>());
	...
	public listen(): void {
		...
		this.listening = true;
		ProX-Code.workspace.onDidOpenTextDocument(this.openTextDocument, this, this._disposables);
		ProX-Code.workspace.onDidCloseTextDocument(this.onDidCloseTextDocument, this, this._disposables);
		ProX-Code.workspace.onDidChangeTextDocument(this.onDidChangeTextDocument, this, this._disposables);
		...
		ProX-Code.workspace.textDocuments.forEach(this.openTextDocument, this);
	}

	public handles(resource: ProX-Code.Uri): boolean {
		return this.syncedBuffers.has(resource);
	}

	public ensureHasBuffer(resource: ProX-Code.Uri): boolean {
		if (this.syncedBuffers.has(resource)) {
			return true;
		}

		const existingDocument = ProX-Code.workspace.textDocuments.find(doc => doc.uri.toString() === resource.toString());
		if (existingDocument) {
	...
	public toVsCodeResource(resource: ProX-Code.Uri): ProX-Code.Uri {
	...
	public toResource(filePath: string): ProX-Code.Uri {
		...
		return ProX-Code.Uri.file(filePath);
	}
	...
	public openTextDocument(document: ProX-Code.TextDocument): boolean {
	...
	public closeResource(resource: ProX-Code.Uri): void {
	...
	public lineCount(resource: ProX-Code.Uri): number | undefined {
		return this.syncedBuffers.get(resource)?.lineCount;
	}

	private onDidCloseTextDocument(document: ProX-Code.TextDocument): void {
		this.closeResource(document.uri);
	}

	private onDidChangeTextDocument(e: ProX-Code.TextDocumentChangeEvent): void {
	...
	public getErr(resources: readonly ProX-Code.Uri[]): any {
	...
	public hasPendingDiagnostics(resource: ProX-Code.Uri): boolean {
	...
	private sendPendingDiagnostics(): void {
		...
		for (const buffer of this.syncedBuffers.values()) {
			const editors = ProX-Code.window.visibleTextEditors.filter(editor => editor.document.uri.toString() === buffer.resource.toString());
	...
	private updateConfiguration() {
		const jsConfig = ProX-Code.workspace.getConfiguration('javascript', null);
		const tsConfig = ProX-Code.workspace.getConfiguration('typescript', null);
