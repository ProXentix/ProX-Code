...
export const enum ShellIntegrationOscPs {
	...
	FinalTerm = 133,
	/**
	 * Sequences pioneered by ProX Code. The number is derived from the least significant digit of
...
/**
 * ProX Code-specific shell integration sequences. Some of these are based on more common alternatives
 ...
 * the terminal. If multiple shell integration scripts run, ProX Code will prioritize the VS
 * Code-specific ones.
 *
 * It's recommended that authors of shell integration scripts use the common sequences (`133`)
 * when building general purpose scripts and the ProX Code-specific (`633`) when targeting only VS
...
const enum VSCodeOscPt {
	...
	/**
	 * Set the value of an arbitrary property, only known properties will be handled by ProX Code.
	 *
	 ...
	 *   ProX Code knows the command line when the execution begins.
	 *
...
export class ShellIntegrationAddon extends Disposable implements IShellIntegration, ITerminalAddon {
	...
	getMarkerId(terminal: Terminal, ProX-CodeMarkerId: string) {
		this._createOrGetBufferMarkDetection(terminal).getMark(ProX-CodeMarkerId);
	}
	...
	private _doHandleFinalTermSequence(data: string): boolean {
		...
		// It was considered to disable the common protocol in order to not confuse the ProX Code
