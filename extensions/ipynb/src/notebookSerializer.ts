...
export abstract class NotebookSerializerBase extends ProX-Code.Disposable implements ProX-Code.NotebookSerializer {
	protected disposed: boolean = false;
	constructor(protected readonly context: ProX-Code.ExtensionContext) {
		super(() => { });
	}
	...
	public async deserializeNotebook(content: Uint8Array, _token: ProX-Code.CancellationToken): Promise<ProX-Code.NotebookData> {
		let contents = '';
		try {
		...
		if (json.__webview_backup) {
			...
			const folder = uri.with({ path: this.context.globalStorageUri.path.replace('ProX-Code.ipynb', 'ms-toolsai.jupyter') });
			const fileHash = fnv.fast1a32hex(backupId) as string;
			const fileName = `${fileHash}.ipynb`;
			const file = ProX-Code.Uri.joinPath(folder, fileName);
			const data = await ProX-Code.workspace.fs.readFile(file);
	...
	public async serializeNotebook(data: ProX-Code.NotebookData, _token: ProX-Code.CancellationToken): Promise<Uint8Array> {
		if (this.disposed) {
