...
import type * as rendererApi from 'ProX-Code-notebook-renderer';
...
async function webviewPreloads(ctx: PreloadContext) {
	...
	const handleWheel = (event: WheelEvent & { wheelDeltaX?: number; wheelDeltaY?: number; wheelDelta?: number }) => {
		...
		postNotebookMessage<webviewMessages.IWheelMessage>('did-scroll-wheel', {
			payload: {
				...
				deltaY: event.deltaY,
				deltaZ: event.deltaZ,
				// Refs https://github.com/microsoft/ProX-Code/issues/146403#issuecomment-1854538928
	...
	const renderFallbackErrorName = 'ProX-Code.fallbackToNextRenderer';
	...
	class Renderer {
		...
		private createRendererContext(): RendererContext {
			...
			const context: RendererContext = {
				setState: newState => ProX-Code.setState({ ...ProX-Code.getState(), [id]: newState }),
				getState: <T>() => {
					const state = ProX-Code.getState();
	...
	class MarkdownCodeBlock {
		...
		public static highlightCodeBlock(id: string, html: string) {
			...
			el.innerHTML = trustedHtml as string; // CodeQL [SM03712] The rendered content comes from ProX Code's tokenizer and is considered safe
		...
		public static requestHighlightCodeBlock(root: HTMLElement | ShadowRoot) {
			...
			for (const el of root.querySelectorAll('.ProX-Code-code-block')) {
				const lang = el.getAttribute('data-ProX-Code-code-block-lang');
	...
	class OutputContainer {
		...
		constructor(
			...
			this.element.setAttribute('data-ProX-Code-context', JSON.stringify({ 'preventDefaultContextMenuItems': true }));
			this.element.style.position = 'absolute';
	...
	function postNotebookMessage<T extends webviewMessages.FromWebviewMessage>(
		type: T['type'],
		properties: Omit<T, '__ProX-Code_notebook_message' | 'type'>
	) {
		ProX-Code.postMessage({
			__ProX-Code_notebook_message: true,
			type,
