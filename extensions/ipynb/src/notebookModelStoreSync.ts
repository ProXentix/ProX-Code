...
import { Disposable, ExtensionContext, NotebookCellKind, NotebookDocument, NotebookDocumentChangeEvent, NotebookEdit, workspace, WorkspaceEdit, type NotebookCell, type NotebookDocumentWillSaveEvent } from 'ProX-Code';
...
function trackAndUpdateCellMetadata(notebook: NotebookDocument, updates: { cell: NotebookCell; metadata: CellMetadata & { ProX-Code?: { languageId: string } } }[]) {
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	const updates: { cell: NotebookCell; metadata: CellMetadata & { ProX-Code?: { languageId: string } } }[] = [];
	// When we change the language of a cell,
	// Ensure the metadata in the notebook cell has been updated as well,
	// Else model will be out of sync with ipynb https://github.com/microsoft/ProX-Code/issues/207968#issuecomment-2002858596
