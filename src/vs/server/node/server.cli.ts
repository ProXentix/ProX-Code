...
/*
 * Implements a standalone CLI app that opens ProX Code from a remote terminal.
 ...
 *  - In external terminals for WSL this calls ProX Code on the Windows side.
 *    The ProX Code desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a ProX Code terminal.');
		return;
	}
...
function fatal(message: string, err: any): void {
	console.error('Unable to connect to ProX Code server: ' + message);
	console.error(err);
	process.exit(1);
}
...
function mapFileToRemoteUri(uri: string): string {
	return uri.replace(/^file:\/\//, 'ProX-Code-remote://' + cliRemoteAuthority);
}
