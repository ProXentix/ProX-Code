...
suite('ExtHostLanguageFeatures', function () {
	...
	test('DocumentSymbols, register/deregister', async () => {
		...
		const d1 = extHost.registerDocumentSymbolProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentSymbolProvider {
			provideDocumentSymbols() {
				return <ProX-Code.SymbolInformation[]>[];
			}
		});
	...
	test('DocumentSymbols, evil provider', async () => {
		disposables.add(extHost.registerDocumentSymbolProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentSymbolProvider {
	...
	test('CodeLens, evil provider', async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			disposables.add(extHost.registerCodeLensProvider(defaultExtension, defaultSelector, new class implements ProX-Code.CodeLensProvider {
	...
	test('Definition, data conversion', async () => {
		...
		disposables.add(extHost.registerDefinitionProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DefinitionProvider {
	...
	test('Declaration, data conversion', async () => {
		...
		disposables.add(extHost.registerDeclarationProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DeclarationProvider {
	...
	test('Implementation, data conversion', async () => {
		...
		disposables.add(extHost.registerImplementationProvider(defaultExtension, defaultSelector, new class implements ProX-Code.ImplementationProvider {
	...
	test('Type Definition, data conversion', async () => {
		...
		disposables.add(extHost.registerTypeDefinitionProvider(defaultExtension, defaultSelector, new class implements ProX-Code.TypeDefinitionProvider {
	...
	test('HoverProvider, word range at pos', async () => {
		...
		disposables.add(extHost.registerHoverProvider(defaultExtension, defaultSelector, new class implements ProX-Code.HoverProvider {
			provideHover(): any {
	...
	test('Occurrences, data conversion', async () => {
		...
		disposables.add(extHost.registerDocumentHighlightProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentHighlightProvider {
	...
	test('Occurrences, order 1/2', async () => {
		...
		disposables.add(extHost.registerDocumentHighlightProvider(defaultExtension, '*', new class implements ProX-Code.DocumentHighlightProvider {
			provideDocumentHighlights(): any {
				return [new types.DocumentHighlight(new types.Range(0, 0, 0, 4))];
			}
		}));
	...
	test('References, registration order', async () => {
		...
		disposables.add(extHost.registerReferenceProvider(defaultExtension, defaultSelector, new class implements ProX-Code.ReferenceProvider {
	...
	test('Quick Fix, code action data conversion', async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			disposables.add(extHost.registerCodeActionProvider(defaultExtension, defaultSelector, {
				provideCodeActions(): ProX-Code.CodeAction[] {
					return [
						{
							title: 'Testing1',
	...
	test('Cannot read property \'id\' of undefined, #29469', async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			disposables.add(extHost.registerCodeActionProvider(defaultExtension, defaultSelector, new class implements ProX-Code.CodeActionProvider {
	...
	test('Navigate types, evil provider', async () => {
		...
		disposables.add(extHost.registerWorkspaceSymbolProvider(defaultExtension, new class implements ProX-Code.WorkspaceSymbolProvider {
	...
	test('Navigate types, de-duplicate results', async () => {
		...
		disposables.add(extHost.registerWorkspaceSymbolProvider(defaultExtension, new class implements ProX-Code.WorkspaceSymbolProvider {
			...
			resolveWorkspaceSymbol(a: ProX-Code.SymbolInformation) {
				return a;
			}
		}));
	...
	test('Rename, evil provider 0/2', async () => {
		...
		disposables.add(extHost.registerRenameProvider(defaultExtension, defaultSelector, new class implements ProX-Code.RenameProvider {
	...
	test('Rename, evil provider 2/2', async () => {
		...
		disposables.add(extHost.registerRenameProvider(defaultExtension, '*', new class implements ProX-Code.RenameProvider {
			provideRenameEdits(): any {
				throw Error('evil');
			}
		}));
	...
	test('Multiple RenameProviders don\'t respect all possible PrepareRename handlers 1/2, #98352', async function () {
		...
		disposables.add(extHost.registerRenameProvider(defaultExtension, defaultSelector, new class implements ProX-Code.RenameProvider {
			prepareRename(document: ProX-Code.TextDocument, position: ProX-Code.Position,): ProX-Code.ProviderResult<ProX-Code.Range> {
				called[0] = true;
				const range = document.getWordRangeAtPosition(position);
				return range;
			}

			provideRenameEdits(): ProX-Code.ProviderResult<ProX-Code.WorkspaceEdit> {
				called[1] = true;
				return undefined;
			}
		}));
	...
	test('Multiple RenameProviders don\'t respect all possible PrepareRename handlers 2/2, #98352', async function () {
		...
		disposables.add(extHost.registerRenameProvider(defaultExtension, defaultSelector, new class implements ProX-Code.RenameProvider {
			...
			provideRenameEdits(document: ProX-Code.TextDocument, position: ProX-Code.Position, newName: string,): ProX-Code.ProviderResult<ProX-Code.WorkspaceEdit> {
				called[2] = true;
	...
	test('Parameter Hints, order', async () => {
		...
		disposables.add(extHost.registerSignatureHelpProvider(defaultExtension, defaultSelector, new class implements ProX-Code.SignatureHelpProvider {
		...
		disposables.add(extHost.registerSignatureHelpProvider(defaultExtension, defaultSelector, new class implements ProX-Code.SignatureHelpProvider {
			provideSignatureHelp(): ProX-Code.SignatureHelp {
				return {
					signatures: [],
					activeParameter: 0,
					activeSignature: 0
	...
	test('Suggest, order 1/3', async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			disposables.add(extHost.registerCompletionItemProvider(defaultExtension, '*', new class implements ProX-Code.CompletionItemProvider {
				provideCompletionItems(): any {
					return [new types.CompletionItem('testing1')];
				}
			}, []));

			disposables.add(extHost.registerCompletionItemProvider(defaultExtension, defaultSelector, new class implements ProX-Code.CompletionItemProvider {
				provideCompletionItems(): any {
	...
	test('Format Doc, data conversion', async () => {
		disposables.add(extHost.registerDocumentFormattingEditProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentFormattingEditProvider {
	...
	test('Format Range, data conversion', async () => {
		disposables.add(extHost.registerDocumentRangeFormattingEditProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentRangeFormattingEditProvider {
	...
	test('Format on Type, data conversion', async () => {
		...
		disposables.add(extHost.registerOnTypeFormattingEditProvider(defaultExtension, defaultSelector, new class implements ProX-Code.OnTypeFormattingEditProvider {
	...
	test('Links, data conversion', async () => {
		...
		disposables.add(extHost.registerDocumentLinkProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentLinkProvider {
	...
	test('Document colors, data conversion', async () => {
		...
		disposables.add(extHost.registerColorProvider(defaultExtension, defaultSelector, new class implements ProX-Code.DocumentColorProvider {
			provideDocumentColors(): ProX-Code.ColorInformation[] {
				return [new types.ColorInformation(new types.Range(0, 0, 0, 20), new types.Color(0.1, 0.2, 0.3, 0.4))];
			}
			provideColorPresentations(color: ProX-Code.Color, context: { range: ProX-Code.Range; document: ProX-Code.TextDocument }): ProX-Code.ColorPresentation[] {
				return [];
			}
		}));
	...
	test('Selection Ranges, data conversion', async () => {
		disposables.add(extHost.registerSelectionRangeProvider(defaultExtension, defaultSelector, new class implements ProX-Code.SelectionRangeProvider {
