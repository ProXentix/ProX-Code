...
interface ITestRunner {
	/** Old test runner API, as exported from `ProX-Code/lib/testrunner` */
...
export abstract class AbstractExtHostExtensionService extends Disposable implements ExtHostExtensionServiceShape {
	...
	private readonly _resolvers: { [authorityPrefix: string]: ProX-Code.RemoteAuthorityResolver };
	...
	private _loadExtensionContext(extensionDescription: IExtensionDescription, extensionInternalStore: DisposableStore): Promise<ProX-Code.ExtensionContext> {
		...
		return Promise.all([
			...
			const that = this;
			let extension: ProX-Code.Extension<any> | undefined;

			let messagePassingProtocol: ProX-Code.MessagePassingProtocol | undefined;
			...
			return Object.freeze<ProX-Code.ExtensionContext>({
				globalState,
				workspaceState,
				secrets,
				subscriptions: [],
	...
	private static _callActivate(logService: ILogService, extensionId: ExtensionIdentifier, extensionModule: IExtensionModule, context: ProX-Code.ExtensionContext, extensionInternalStore: IDisposable, activationTimesBuilder: ExtensionActivationTimesBuilder): Promise<ActivatedExtension> {
	...
	private static _callActivateOptional(logService: ILogService, extensionId: ExtensionIdentifier, extensionModule: IExtensionModule, context: ProX-Code.ExtensionContext, activationTimesBuilder: ExtensionActivationTimesBuilder): Promise<IExtensionAPI> {
	...
	private _handleWorkspaceContainsEagerExtensions(folders: ReadonlyArray<ProX-Code.WorkspaceFolder>): Promise<void> {
		if (folders.length === 0) {
			return Promise.resolve(undefined);
		}
	...
	private async _handleWorkspaceContainsEagerExtension(folders: ReadonlyArray<ProX-Code.WorkspaceFolder>, desc: IExtensionDescription): Promise<void> {
	...
	public registerRemoteAuthorityResolver(authorityPrefix: string, resolver: ProX-Code.RemoteAuthorityResolver): ProX-Code.Disposable {
	...
	public async getRemoteExecServer(remoteAuthority: string): Promise<ProX-Code.ExecServer | undefined> {
	...
	private async _activateAndGetResolver(remoteAuthority: string): Promise<{ authorityPrefix: string; resolver: ProX-Code.RemoteAuthorityResolver | undefined }> {
	...
	public async $resolveAuthority(remoteAuthorityChain: string, resolveAttempt: number): Promise<Dto<IResolveAuthorityResult>> {
		...
		let result!: ProX-Code.ResolverResult;
		let execServer: ProX-Code.ExecServer | undefined;
...
export interface IExtHostExtensionService extends AbstractExtHostExtensionService {
	...
	registerRemoteAuthorityResolver(authorityPrefix: string, resolver: ProX-Code.RemoteAuthorityResolver): ProX-Code.Disposable;
	getRemoteExecServer(authority: string): Promise<ProX-Code.ExecServer | undefined>;

	onDidChangeRemoteConnectionData: Event<void>;
	getRemoteConnectionData(): IRemoteConnectionData | null;
}

export class Extension<T extends object | null | undefined> implements ProX-Code.Extension<T> {
	...
	readonly extensionKind: ProX-Code.ExtensionKind;
