...
class TypeScriptSignatureHelpProvider implements ProX-Code.SignatureHelpProvider {
	...
	public async provideSignatureHelp(
		...
		context: ProX-Code.SignatureHelpContext,
	): Promise<ProX-Code.SignatureHelp | undefined> {
		...
		const result = new ProX-Code.SignatureHelp();
	...
	private getActiveSignature(context: ProX-Code.SignatureHelpContext, info: Proto.SignatureHelpItems, signatures: readonly ProX-Code.SignatureInformation[]): number {
	...
	private convertSignature(item: Proto.SignatureHelpItem, baseUri: ProX-Code.Uri) {
		const signature = new ProX-Code.SignatureInformation(
		...
		for (let i = 0; i < item.parameters.length; ++i) {
			...
			signature.parameters.push(
				new ProX-Code.ParameterInformation(
...
function toTsTriggerReason(context: ProX-Code.SignatureHelpContext): Proto.SignatureHelpTriggerReason {
	switch (context.triggerKind) {
		case ProX-Code.SignatureHelpTriggerKind.TriggerCharacter:
		...
		case ProX-Code.SignatureHelpTriggerKind.ContentChange:
			return context.isRetrigger ? { kind: 'retrigger' } : { kind: 'invoked' };

		case ProX-Code.SignatureHelpTriggerKind.Invoke:
		default:
			return { kind: 'invoked' };
	}
}
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerSignatureHelpProvider(selector.syntax,
