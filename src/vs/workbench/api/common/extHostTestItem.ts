...
const testItemPropAccessor = <K extends keyof ProX-Code.TestItem>(
	api: IExtHostTestItemApi,
	defaultValue: ProX-Code.TestItem[K],
	equals: (a: ProX-Code.TestItem[K], b: ProX-Code.TestItem[K]) => boolean,
	toUpdate: (newValue: ProX-Code.TestItem[K], oldValue: ProX-Code.TestItem[K]) => ExtHostTestItemEvent,
) => {
	...
	return {
		...
		set(newValue: ProX-Code.TestItem[K]) {
...
type WritableProps = Pick<ProX-Code.TestItem, 'range' | 'label' | 'description' | 'sortText' | 'canResolveChildren' | 'busy' | 'error' | 'tags'>;

const strictEqualComparator = <T>(a: T, b: T) => a === b;

const propComparators: { [K in keyof Required<WritableProps>]: (a: ProX-Code.TestItem[K], b: ProX-Code.TestItem[K]) => boolean } = {
	range: (a, b) => {
		if (a === b) { return true; }
		if (!a || !b) { return false; }
		return a.isEqual(b);
...
const makePropDescriptors = (api: IExtHostTestItemApi, label: string): { [K in keyof Required<WritableProps>]: PropertyDescriptor } => ({
	range: (() => {
		let value: ProX-Code.Range | undefined;
		const updateProps = evSetProps<ProX-Code.Range | undefined>(r => ({ range: editorRange.Range.lift(Convert.Range.from(r)) }));
		return {
			enumerable: true,
			configurable: false,
			get() {
			...
			set(newValue: ProX-Code.Range | undefined) {
...
export class TestItemImpl implements ProX-Code.TestItem {
	public readonly id!: string;
	public readonly uri!: ProX-Code.Uri | undefined;
	public readonly children!: ITestItemChildren<ProX-Code.TestItem>;
	public readonly parent!: TestItemImpl | undefined;

	public range!: ProX-Code.Range | undefined;
	public description!: string | undefined;
	public sortText!: string | undefined;
	public label!: string;
	public error!: string | ProX-Code.MarkdownString;
	public busy!: boolean;
	public canResolveChildren!: boolean;
	public tags!: readonly ProX-Code.TestTag[];

	/**
	 * Note that data is deprecated and here for back-compat only
	 */
	constructor(controllerId: string, id: string, label: string, uri: ProX-Code.Uri | undefined) {
