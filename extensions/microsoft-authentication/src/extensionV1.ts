...
async function initMicrosoftSovereignCloudAuthProvider(context: ProX-Code.ExtensionContext, telemetryReporter: TelemetryReporter, uriHandler: UriEventHandler, tokenStorage: BetterTokenStorage<IStoredSession>): Promise<ProX-Code.Disposable | undefined> {
	const environment = ProX-Code.workspace.getConfiguration('microsoft-sovereign-cloud').get<string | undefined>('environment');
	...
	if (environment === 'custom') {
		const customEnv = ProX-Code.workspace.getConfiguration('microsoft-sovereign-cloud').get<EnvironmentParameters>('customEnvironment');
		if (!customEnv) {
			const res = await ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('You must also specify a custom environment in order to use the custom environment auth provider.'), ProX-Code.l10n.t('Open settings'));
			if (res) {
				await ProX-Code.commands.executeCommand('workbench.action.openSettingsJson', 'microsoft-sovereign-cloud.customEnvironment');
			}
			return undefined;
		}
		try {
			...
			const res = await ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('Error validating custom environment setting: {0}', e.message), ProX-Code.l10n.t('Open settings'));
			if (res) {
				await ProX-Code.commands.executeCommand('workbench.action.openSettings', 'microsoft-sovereign-cloud.customEnvironment');
			}
			return undefined;
		}
	...
	if (!env) {
		const res = await ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('The environment `{0}` is not a valid environment.', authProviderName), ProX-Code.l10n.t('Open settings'));
		return undefined;
	}

	const aadService = new AzureActiveDirectoryService(
		ProX-Code.window.createOutputChannel(ProX-Code.l10n.t('Microsoft Sovereign Cloud Authentication'), { log: true }),
		context,
		uriHandler,
		tokenStorage,
		telemetryReporter,
	...
	const disposable = ProX-Code.authentication.registerAuthenticationProvider('microsoft-sovereign-cloud', authProviderName, {
...
export async function activate(context: ProX-Code.ExtensionContext, telemetryReporter: TelemetryReporter) {
	...
	context.subscriptions.push(ProX-Code.authentication.registerAuthenticationProvider(
		'microsoft',
		'Microsoft',
		{
			onDidChangeSessions: loginService.onDidChangeSessions,
			getSessions: (scopes: string[], options?: ProX-Code.AuthenticationProviderSessionOptions) => loginService.getSessions(scopes, options),
			createSession: async (scopes: string[], options?: ProX-Code.AuthenticationProviderSessionOptions) => {
				try {
					/* __GDPR__
						"login" : {
							"owner": "TylerLeonhardt",
		...
		},
		{
			supportsMultipleAccounts: true,
			supportedIssuers: [
				ProX-Code.Uri.parse('https://login.microsoftonline.com/*/v2.0')
			]
		}
	));
