...
export function toggleComment(): Thenable<boolean> | undefined {
	...
	return editor.edit(editBuilder => {
		const allEdits: ProX-Code.TextEdit[][] = [];
		...
		let lastEditPosition = new ProX-Code.Position(0, 0);
...
function toggleCommentHTML(document: ProX-Code.TextDocument, selection: ProX-Code.Selection, rootNode: Node): ProX-Code.TextEdit[] {
	...
	edits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, allNodes[0].start, allNodes[0].start), startCommentHTML));
	edits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, allNodes[allNodes.length - 1].end, allNodes[allNodes.length - 1].end), endCommentHTML));

	return edits;
}

function getRangesToUnCommentHTML(node: Node, document: ProX-Code.TextDocument): ProX-Code.TextEdit[] {
	let unCommentTextEdits: ProX-Code.TextEdit[] = [];

	// If current node is commented, then uncomment and return
	if (node.type === 'comment') {
		unCommentTextEdits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, node.start, node.start + startCommentHTML.length), ''));
		unCommentTextEdits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, node.end - endCommentHTML.length, node.end), ''));
...
function toggleCommentStylesheet(document: ProX-Code.TextDocument, selection: ProX-Code.Selection, rootNode: Stylesheet): ProX-Code.TextEdit[] {
	const selectionStart = selection.isReversed ? selection.active : selection.anchor;
	const selectionEnd = selection.isReversed ? selection.anchor : selection.active;
	...
	const rangesToUnComment: ProX-Code.Range[] = [];
	const edits: ProX-Code.TextEdit[] = [];
	rootNode.comments.forEach(comment => {
		...
		if (selection.intersection(commentRange)) {
			...
			edits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, comment.start, comment.start + startCommentStylesheet.length), ''));
			edits.push(new ProX-Code.TextEdit(offsetRangeToVsRange(document, comment.end - endCommentStylesheet.length, comment.end), ''));
		}
	});
	...
	return [
		new ProX-Code.TextEdit(new ProX-Code.Range(selection.start, selection.start), startCommentStylesheet),
		new ProX-Code.TextEdit(new ProX-Code.Range(selection.end, selection.end), endCommentStylesheet)
	];
}

function setupCommentSpacing() {
	const config: boolean | undefined = ProX-Code.workspace.getConfiguration('editor.comments').get('insertSpace');
	if (config) {
