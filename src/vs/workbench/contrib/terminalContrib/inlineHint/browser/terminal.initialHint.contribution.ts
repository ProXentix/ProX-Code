	...
	private _getHintInlineChat() {
		...
		const hintHandler: IContentActionHandler = {
			...
			callback: (index, _event) => {
				// No ops for handler 0 since chat is removed.
			}
		};
		...
		// Chat hint removed. Fallback directly to text hint since chat is gone.
		...
		ariaLabelParts.push(localize('openChatHint', 'start typing to dismiss.'));
