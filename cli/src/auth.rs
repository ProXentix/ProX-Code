...
struct KeyringStorage {
	...
	// on Windows https://github.com/microsoft/ProX-Code-cli/issues/358
...
macro_rules! get_next_entry {
	($self: expr, $i: expr) => {
		match $self.entries.get($i) {
			Some(e) => e,
			None => {
				let e = keyring::Entry::new("ProX-Code-cli", &format!("ProX-Code-cli-{}", $i)).unwrap();
				$self.entries.push(e);
...
#[cfg(feature = "ProX-Code-encrypt")]
fn encrypt(value: &str) -> String {
	ProX-Code_encrypt::encrypt(&HOSTNAME, value.as_bytes()).expect("expected to encrypt")
}

#[cfg(feature = "ProX-Code-encrypt")]
fn decrypt(value: &str) -> Option<String> {
	let b = ProX-Code_encrypt::decrypt(&HOSTNAME, value).ok()?;
	String::from_utf8(b).ok()
}

#[cfg(not(feature = "ProX-Code-encrypt"))]
fn encrypt(value: &str) -> String {
	value.to_owned()
}
