...
export interface IExtHostSearch extends ExtHostSearchShape {
	registerTextSearchProviderOld(scheme: string, provider: ProX-Code.TextSearchProvider): IDisposable;
	registerFileSearchProviderOld(scheme: string, provider: ProX-Code.FileSearchProvider): IDisposable;
	registerTextSearchProvider(scheme: string, provider: ProX-Code.TextSearchProvider2): IDisposable;
	registerAITextSearchProvider(scheme: string, provider: ProX-Code.AITextSearchProvider): IDisposable;
	registerFileSearchProvider(scheme: string, provider: ProX-Code.FileSearchProvider2): IDisposable;
...
export class ExtHostSearch implements IExtHostSearch {
	...
	private readonly _textSearchProvider: Map<number, ProX-Code.TextSearchProvider2>;
	private readonly _textSearchUsedSchemes: Set<string>;

	private readonly _aiTextSearchProvider: Map<number, ProX-Code.AITextSearchProvider>;
	private readonly _aiTextSearchUsedSchemes: Set<string>;

	private readonly _fileSearchProvider: Map<number, ProX-Code.FileSearchProvider2>;
	...
	constructor(
		...
		this._handlePool = 0;
		this._textSearchProvider = new Map<number, ProX-Code.TextSearchProvider2>();
		this._textSearchUsedSchemes = new Set<string>();
		this._aiTextSearchProvider = new Map<number, ProX-Code.AITextSearchProvider>();
		this._aiTextSearchUsedSchemes = new Set<string>();
		this._fileSearchProvider = new Map<number, ProX-Code.FileSearchProvider2>();
	...
	$provideFileSearchResults(handle: number, session: number, rawQuery: IRawFileQuery, token: ProX-Code.CancellationToken): Promise<ISearchCompleteStats> {
	...
	$provideTextSearchResults(handle: number, session: number, rawQuery: IRawTextQuery, token: ProX-Code.CancellationToken): Promise<ISearchCompleteStats> {
	...
	$provideAITextSearchResults(handle: number, session: number, rawQuery: IRawAITextQuery, token: ProX-Code.CancellationToken): Promise<ISearchCompleteStats> {
	...
	protected createTextSearchManager(query: ITextQuery, provider: ProX-Code.TextSearchProvider2): TextSearchManager {
	...
	protected createAITextSearchManager(query: IAITextQuery, provider: ProX-Code.AITextSearchProvider): TextSearchManager {
		return new TextSearchManager({ query, provider }, {
			readdir: resource => Promise.resolve([]),
			toCanonicalName: encoding => encoding
