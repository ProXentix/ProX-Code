...
interface IScanMergedConflict {
	startHeader: ProX-Code.TextLine;
	commonAncestors: ProX-Code.TextLine[];
	splitter?: ProX-Code.TextLine;
	endFooter?: ProX-Code.TextLine;
}

export class MergeConflictParser {

	static scanDocument(document: ProX-Code.TextDocument, telemetryReporter: TelemetryReporter): interfaces.IDocumentMergeConflict[] {
	...
	private static scanItemTolMergeConflictDescriptor(document: ProX-Code.TextDocument, scanned: IScanMergedConflict): interfaces.IDocumentMergeConflictDescriptor | null {
		// Validate we have all the required lines within the scan item.
		if (!scanned.startHeader || !scanned.splitter || !scanned.endFooter) {
			return null;
		}

		const tokenAfterCurrentBlock: ProX-Code.TextLine = scanned.commonAncestors[0] || scanned.splitter;
		...
		return {
			current: {
				...
				decoratorContent: new ProX-Code.Range(
			...
			range: new ProX-Code.Range(scanned.startHeader.range.start, scanned.endFooter.rangeIncludingLineBreak.end)
		};
	}

	static containsConflict(document: ProX-Code.TextDocument): boolean {
		if (!document) {
			return false;
		}
	...
	private static shiftBackOneCharacter(document: ProX-Code.TextDocument, range: ProX-Code.Position, unlessEqual: ProX-Code.Position): ProX-Code.Position {
		...
		return new ProX-Code.Position(line, character);
	}
}
