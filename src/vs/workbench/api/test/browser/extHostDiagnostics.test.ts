...
suite('ExtHostDiagnostics', () => {
	...
	test('diagnostic collection, immutable read', function () {
		...
		collection.forEach((uri: URI, array: readonly ProX-Code.Diagnostic[]): any => {
	...
	test('ProX-Code.languages.onDidChangeDiagnostics Does Not Provide Document URI #49582', async function () {
		const emitter = new Emitter<readonly URI[]>();
		const collection = new DiagnosticCollection('ddd', 'test', 100, 100, versionProvider, extUri, new DiagnosticsShape(), emitter);
	...
	test('ProX-Code.languages.getDiagnostics appears to return old diagnostics in some circumstances #54359', function () {
