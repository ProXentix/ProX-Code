...
/**
 * When clause context set when the current file is managed by ProX-Code's built-in typescript extension.
 */
export default class ManagedFileContextManager extends Disposable {
	...
	private onDidChangeActiveTextEditor(editor?: ProX-Code.TextEditor): void {
		if (editor) {
	...
	private updateContext(newValue: boolean) {
		...
		ProX-Code.commands.executeCommand('setContext', ManagedFileContextManager.contextName, newValue);
