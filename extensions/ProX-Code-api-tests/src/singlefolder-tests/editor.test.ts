...
import { commands, env, Position, Range, Selection, SnippetString, TextDocument, TextEditor, TextEditorCursorStyle, TextEditorLineNumbersStyle, Uri, window, workspace } from 'ProX-Code';
import { assertNoRpc, closeAllEditors, createRandomFile, deleteFile } from '../utils';

suite('ProX-Code API - editors', () => {
	...
	test('TextEditor.edit can control undo/redo stack 1', () => {
		return withRandomFileEditor('Hello world!', async (editor, doc) => {
			...
			if (doc.getText() === 'hello world!') {
				// see https://github.com/microsoft/ProX-Code/issues/109131
