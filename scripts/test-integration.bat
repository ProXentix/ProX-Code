...
set VSCODEUSERDATADIR=%TEMP%\ProX-Codeuserfolder-%RANDOM%-%TIME:~6,2%
...
call "%INTEGRATION_TEST_ELECTRON_PATH%" %~dp0\..\extensions\ProX-Code-api-tests\testWorkspace --enable-proposed-api=ProX-Code.ProX-Code-api-tests --extensionDevelopmentPath=%~dp0\..\extensions\ProX-Code-api-tests --extensionTestsPath=%~dp0\..\extensions\ProX-Code-api-tests\out\singlefolder-tests %API_TESTS_EXTRA_ARGS%
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### API tests (workspace)
call "%INTEGRATION_TEST_ELECTRON_PATH%" %~dp0\..\extensions\ProX-Code-api-tests\testworkspace.code-workspace --enable-proposed-api=ProX-Code.ProX-Code-api-tests --extensionDevelopmentPath=%~dp0\..\extensions\ProX-Code-api-tests --extensionTestsPath=%~dp0\..\extensions\ProX-Code-api-tests\out\workspace-tests %API_TESTS_EXTRA_ARGS%
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### Colorize tests
call npm run test-extension -- -l ProX-Code-colorize-tests
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### Terminal Suggest tests
call npm run test-extension -- -l terminal-suggest --enable-proposed-api=ProX-Code.ProX-Code-api-tests
if %errorlevel% neq 0 exit /b %errorlevel%
