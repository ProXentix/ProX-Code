...
export class LocalStorageSecretStorageProvider implements ISecretStorageProvider {
	...
	private loadAuthSessionFromElement(): Record<string, string> {
		...
		const authSessionElement = mainWindow.document.getElementById('ProX-Code-workbench-auth-session');
		...
		const authAccount = JSON.stringify({ extensionId: 'ProX-Code.github-authentication', key: 'github.auth' });
...
class LocalStorageURLCallbackProvider extends Disposable implements IURLCallbackProvider {
	...
	create(options: Partial<UriComponents> = {}): URI {
		...
		const queryParams: string[] = [`ProX-Code-reqid=${id}`];

		for (const key of LocalStorageURLCallbackProvider.QUERY_KEYS) {
			const value = options[key];

			if (value) {
				queryParams.push(`ProX-Code-${key}=${encodeURIComponent(value)}`);
			}
		}

		// TODO@joao remove eventually
		// https://github.com/microsoft/ProX-Code-dev/issues/62
		// https://github.com/microsoft/ProX-Code/blob/159479eb5ae451a66b5dac3c12d564f32f454796/extensions/github-authentication/src/githubServer.ts#L50-L50
		if (!(options.authority === 'ProX-Code.github-authentication' && options.path === '/dummy')) {
			const key = `ProX-Code-web.url-callbacks[${id}]`;
...
class WorkspaceProvider implements IWorkspaceProvider {
			...
			switch (key) {
				...
				case WorkspaceProvider.QUERY_PARAM_FOLDER:
					if (config.remoteAuthority && value.startsWith(posix.sep)) {
						...
						// is a ProX-Code-remote resource as simplified URL.
						workspace = { folderUri: URI.from({ scheme: Schemas.ProX-CodeRemote, path: value, authority: config.remoteAuthority }) };
					} else {
				...
				case WorkspaceProvider.QUERY_PARAM_WORKSPACE:
					if (config.remoteAuthority && value.startsWith(posix.sep)) {
						...
						// that is a path (begins with a `/`), assume this
						// is a ProX-Code-remote resource as simplified URL.
						workspace = { workspaceUri: URI.from({ scheme: Schemas.ProX-CodeRemote, path: value, authority: config.remoteAuthority }) };
					} else {
	...
	private encodeWorkspacePath(uri: URI): string {
		if (this.config.remoteAuthority && uri.scheme === Schemas.ProX-CodeRemote) {
	...
	hasRemote(): boolean {
		if (this.workspace) {
			if (isFolderToOpen(this.workspace)) {
				return this.workspace.folderUri.scheme === Schemas.ProX-CodeRemote;
			}

			if (isWorkspaceToOpen(this.workspace)) {
				return this.workspace.workspaceUri.scheme === Schemas.ProX-CodeRemote;
			}
		}
...
(function () {
	...
	const configElement = mainWindow.document.getElementById('ProX-Code-workbench-web-configuration');
	const configElementAttribute = configElement ? configElement.getAttribute('data-settings') : undefined;
	if (!configElement || !configElementAttribute) {
		throw new Error('Missing web configuration element');
	}
	const config: IWorkbenchConstructionOptions & { folderUri?: UriComponents; workspaceUri?: UriComponents; callbackRoute: string } = JSON.parse(configElementAttribute);
	const secretStorageKeyPath = readCookie('ProX-Code-secret-key-path');
