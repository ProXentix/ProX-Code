...
export function extractEditorsDropData(e: DragEvent): Array<IDraggedResourceEditorInput> {
	...
	// are being used (https://github.com/microsoft/ProX-Code/issues/128925)
...
export function createDraggedEditorInputFromRawResourcesData(rawResourcesData: string | undefined): IDraggedResourceEditorInput[] {
	...
	if (rawResourcesData) {
		...
		for (const resourceRaw of resourcesRaw) {
			if (resourceRaw.indexOf(':') > 0) { // mitigate https://github.com/microsoft/ProX-Code/issues/124946
...
export function getPathForFile(file: File): string | undefined {
	if (isNative && typeof (globalThis as any).ProX-Code?.webUtils?.getPathForFile === 'function') {
		return (globalThis as any).ProX-Code.webUtils.getPathForFile(file);
	}
