...
import * as EmmetHelper from '@ProX-Code/emmet-helper';
import { TextDocument as LSTextDocument } from 'ProX-Code-languageserver-textdocument';
import { getRootNode } from './parseDocument';

let _emmetHelper: typeof EmmetHelper;
let _currentExtensionsPath: string[] | undefined;

let _homeDir: ProX-Code.Uri | undefined;


export function setHomeDir(homeDir: ProX-Code.Uri) {
	_homeDir = homeDir;
}

export function getEmmetHelper() {
	// Lazy load ProX-Code-emmet-helper instead of importing it
	// directly to reduce the start-up time of the extension
	if (!_emmetHelper) {
		_emmetHelper = require('@ProX-Code/emmet-helper');
	}
	return _emmetHelper;
}
...
export function updateEmmetExtensionsPath(forceRefresh: boolean = false) {
	...
	let extensionsPath = ProX-Code.workspace.getConfiguration('emmet').get<string[]>('extensionsPath');
	if (!extensionsPath) {
		extensionsPath = [];
	}
	if (forceRefresh || _currentExtensionsPath !== extensionsPath) {
		_currentExtensionsPath = extensionsPath;
		const rootPaths = ProX-Code.workspace.workspaceFolders?.length ? ProX-Code.workspace.workspaceFolders.map(f => f.uri) : undefined;
		const fileSystem = ProX-Code.workspace.fs;
 ...
 * https://github.com/microsoft/ProX-Code/issues/117517
 */
export function migrateEmmetExtensionsPath() {
	// Get the detail info of emmet.extensionsPath setting
	const config = ProX-Code.workspace.getConfiguration().inspect('emmet.extensionsPath');

	// Update Global setting if the value type is string or the value is null
	if (typeof config?.globalValue === 'string') {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', [config.globalValue], true);
	} else if (config?.globalValue === null) {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', [], true);
	}
	// Update Workspace setting if the value type is string or the value is null
	if (typeof config?.workspaceValue === 'string') {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', [config.workspaceValue], false);
	} else if (config?.workspaceValue === null) {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', [], false);
	}
	...
	if (typeof config?.workspaceFolderValue === 'string') {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', [config.workspaceFolderValue]);
	} else if (config?.workspaceFolderValue === null) {
		ProX-Code.workspace.getConfiguration().update('emmet.extensionsPath', []);
	}
}
...
export function getMappingForIncludedLanguages(): Record<string, string> {
	// Explicitly map languages that have built-in grammar in ProX Code to their parent language
	...
	const includeLanguagesConfig = ProX-Code.workspace.getConfiguration('emmet').get<Record<string, string>>('includeLanguages');
...
/**
* Get the corresponding emmet mode for given ProX-Code language mode
 ...
 * @param document ProX-Code.TextDocument
 * @param position ProX-Code.Position
 */
export function parsePartialStylesheet(document: ProX-Code.TextDocument, position: ProX-Code.Position): FlatStylesheet | undefined {
	...
	function consumeLineCommentBackwards() {
		...
		if (!isCSS && currentLine !== posLineNumber) {
			...
			if (startLineComment > -1) {
				stream.pos = document.offsetAt(new ProX-Code.Position(currentLine, startLineComment));
			}
		}
	}
	...
	function consumeCommentForwards() {
		if (stream.eat(slash)) {
			if (stream.eat(slash) && !isCSS) {
				const posLineNumber = document.positionAt(stream.pos).line;
				stream.pos = document.offsetAt(new ProX-Code.Position(posLineNumber + 1, 0));
...
export function offsetRangeToSelection(document: ProX-Code.TextDocument, start: number, end: number): ProX-Code.Selection {
	const startPos = document.positionAt(start);
	const endPos = document.positionAt(end);
	return new ProX-Code.Selection(startPos, endPos);
}

export function offsetRangeToVsRange(document: ProX-Code.TextDocument, start: number, end: number): ProX-Code.Range {
	const startPos = document.positionAt(start);
	const endPos = document.positionAt(end);
	return new ProX-Code.Range(startPos, endPos);
}
 ...
 * be found
 */
export function getCssPropertyFromDocument(editor: ProX-Code.TextEditor, position: ProX-Code.Position): FlatProperty | null {
...
export function getEmbeddedCssNodeIfAny(document: ProX-Code.TextDocument, currentNode: FlatNode | undefined, position: ProX-Code.Position): FlatNode | undefined {
	if (!currentNode) {
		return;
	}
...
export function toLSTextDocument(doc: ProX-Code.TextDocument): LSTextDocument {
