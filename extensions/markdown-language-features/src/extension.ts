...
import { LanguageClient, ServerOptions, TransportKind } from 'ProX-Code-languageclient/node';
...
function startServer(context: ProX-Code.ExtensionContext, parser: IMdParser): Promise<MdLanguageClient> {
	const isDebugBuild = context.extension.packageJSON.main.includes('/out/');

	const serverModule = context.asAbsolutePath(
		isDebugBuild
			// For local non bundled version of ProX-Code-markdown-languageserver
			// ? './node_modules/ProX-Code-markdown-languageserver/out/node/workerMain'
			? './node_modules/ProX-Code-markdown-languageserver/dist/node/workerMain'
	...
	process.env['VSCODE_L10N_BUNDLE_LOCATION'] = ProX-Code.l10n.uri?.toString() ?? '';
