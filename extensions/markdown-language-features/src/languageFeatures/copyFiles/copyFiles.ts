...
export function getCopyFileConfiguration(document: ProX-Code.TextDocument): CopyFileConfiguration {
...
function readOverwriteBehavior(config: ProX-Code.WorkspaceConfiguration): OverwriteBehavior {
...
export function parseGlob(rawGlob: string): Iterable<string> {
	if (rawGlob.startsWith('/')) {
		...
		return (ProX-Code.workspace.workspaceFolders ?? []).map(folder => ProX-Code.Uri.joinPath(folder.uri, rawGlob).path);
	}
...
type GetWorkspaceFolder = (documentUri: ProX-Code.Uri) => ProX-Code.Uri | undefined;

export function resolveCopyDestination(documentUri: ProX-Code.Uri, fileName: string, dest: string, getWorkspaceFolder: GetWorkspaceFolder): ProX-Code.Uri {
...
function resolveCopyDestinationSetting(documentUri: ProX-Code.Uri, fileName: string, dest: string, getWorkspaceFolder: GetWorkspaceFolder): string {
