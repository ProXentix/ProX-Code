...
class NodeModuleAliasingModuleFactory implements IAlternativeModuleProvider {
	...
	private static readonly aliased: ReadonlyMap<string, string> = new Map([
		['ProX-Code-ripgrep', '@ProX-Code/ripgrep'],
		['ProX-Code-windows-registry', '@ProX-Code/windows-registry'],
	]);
	...
	constructor(@IExtHostInitDataService initData: IExtHostInitDataService) {
		if (initData.environment.appRoot && NodeModuleAliasingModuleFactory.aliased.size) {
			...
			// to ['${appRoot}/node_modules.asar.unpacked/',' @ProX-Code/foo', '/bin'].
...
//#region --- ProX-Code-module

class VSCodeNodeModuleFactory implements INodeModuleFactory {
	public readonly nodeModuleName = 'ProX-Code';

	private readonly _extApiImpl = new ExtensionIdentifierMap<typeof ProX-Code>();
	private _defaultApiImpl?: typeof ProX-Code;
	...
	public load(_request: string, parent: URI): any {
		...
		if (!this._defaultApiImpl) {
			...
			this._logService.warn(`Could not identify extension for 'ProX-Code' require call from ${parent}. These are the extension path mappings: \n${extensionPathsPretty}`);
