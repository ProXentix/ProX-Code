...
export default class LanguageProvider extends Disposable {
	...
	private get documentSelector(): DocumentSelector {
		const semantic: ProX-Code.DocumentFilter[] = [];
		const syntax: ProX-Code.DocumentFilter[] = [];
	...
	private configurationChanged(): void {
		const config = ProX-Code.workspace.getConfiguration(this.id, null);
		this.updateValidate(config.get(validateSetting, true));
		this.updateSuggestionDiagnostics(config.get(suggestionSetting, true));
	}

	public handlesUri(resource: ProX-Code.Uri): boolean {
	...
	public handlesDocument(doc: ProX-Code.TextDocument): boolean {
		return this.description.languageIds.includes(doc.languageId) || this.handlesConfigFile(doc.uri);
	}

	private handlesConfigFile(resource: ProX-Code.Uri) {
	...
	public diagnosticsReceived(
		...
		file: ProX-Code.Uri,
		diagnostics: (ProX-Code.Diagnostic & { reportUnnecessary: any; reportDeprecated: any })[],
		ranges: ProX-Code.Range[] | undefined): void {
		...
		const config = ProX-Code.workspace.getConfiguration(this.id, file);
		...
		this.client.diagnosticsManager.updateDiagnostics(file, this._diagnosticLanguage, diagnosticsKind, diagnostics.filter(diag => {
			...
			if (!reportUnnecessary) {
				if (diag.reportUnnecessary && diag.severity === ProX-Code.DiagnosticSeverity.Hint) {
					return false;
				}
			}
			if (!reportDeprecated) {
				if (diag.reportDeprecated && diag.severity === ProX-Code.DiagnosticSeverity.Hint) {
					return false;
				}
			}
			return true;
	...
	public configFileDiagnosticsReceived(file: ProX-Code.Uri, diagnostics: ProX-Code.Diagnostic[]): void {
