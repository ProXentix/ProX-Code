...
import type { ActivationFunction, OutputItem, RendererContext } from 'ProX-Code-notebook-renderer';
...
function renderImage(outputInfo: OutputItem, element: HTMLElement): IDisposable {
	...
	image.setAttribute('data-ProX-Code-context', JSON.stringify({
...
function getAltText(outputInfo: OutputItem) {
	...
	if (typeof metadata === 'object' && metadata && 'ProX-Code_altText' in metadata && typeof metadata.ProX-Code_altText === 'string') {
...
function fixUpSvgElement(outputInfo: OutputItem, element: HTMLElement) {
	if (outputInfo.mime.indexOf('svg') > -1) {
		...
		if (svgElement) {
			...
			svgElement.setAttribute('data-ProX-Code-context', JSON.stringify({
				webviewSection: 'image',
				outputId: outputInfo.id,
				'preventDefaultContextMenuItems': true
			}));
...
export const activate: ActivationFunction<void> = (ctx) => {
	...
	#container div.output .scrollable.scrollbar-visible {
		border-color: var(--ProX-Code-editorWidget-border);
	}
	...
	#container div.output .scrollable.more-above {
		box-shadow: var(--ProX-Code-scrollbar-shadow) 0 6px 6px -6px inset
	}
	...
	#container .error-output-actions li {
		...
		display: inline-flex;
		cursor: pointer;
		border: solid 1px var(--ProX-Code-notebook-cellToolbarSeparator);
	}
	#container .error-output-actions li.hover {
		background-color: var(--ProX-Code-toolbar-hoverBackground);
	}
	...
	#container .error-output-actions li a {
		color: var(--ProX-Code-foreground);
