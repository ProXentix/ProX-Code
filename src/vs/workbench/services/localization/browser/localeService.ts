...
const localeStorage = new class LocaleStorage {
	...
	private static readonly LOCAL_STORAGE_LOCALE_KEY = 'ProX-Code.nls.locale';
	private static readonly LOCAL_STORAGE_EXTENSION_ID_KEY = 'ProX-Code.nls.languagePackExtensionId';
...
class WebActiveLanguagePackService implements IActiveLanguagePackService {
	...
	async getExtensionIdProvidingCurrentLocale(): Promise<string | undefined> {
		...
		try {
			...
			// Only install extensions that are published by Microsoft and start with ProX-Code-language-pack for extra certainty
			const extensionToInstall = tagResult.firstPage.find(e => e.publisher === 'MS-CEINTL' && e.name.startsWith('ProX-Code-language-pack'));
