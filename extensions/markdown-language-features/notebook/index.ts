...
import type { ActivationFunction } from 'ProX-Code-notebook-renderer';
...
export const activate: ActivationFunction<void> = (ctx) => {
	const markdownIt: MarkdownIt = new MarkdownIt({
		...
		highlight: (str: string, lang?: string) => {
			if (lang) {
				return `<div class="ProX-Code-code-block" data-ProX-Code-code-block-lang="${markdownIt.utils.escapeHtml(lang)}">${markdownIt.utils.escapeHtml(str)}</div>`;
			}
