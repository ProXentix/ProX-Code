...
export class ExtHostDebugService extends ExtHostDebugServiceBase {
	...
	protected override createDebugAdapter(adapter: ProX-Code.DebugAdapterDescriptor, session: ExtHostDebugSession): AbstractDebugAdapter | undefined {
	...
	public override async $runInTerminal(args: DebugProtocol.RunInTerminalRequestArguments, sessionId: string): Promise<number | undefined> {
		...
		if (args.kind === 'integrated') {
			...
			if (!terminal) {
				const options: ProX-Code.TerminalOptions = {
					shellPath: shell,
					shellArgs: shellArgs,
					cwd: args.cwd,
					name: terminalName,
			...
			if (giveShellTimeToInitialize) {
				...
				if (terminal.state.isInteractedWith && !terminal.shellIntegration) {
					...
					await timeout(200); // mirroring https://github.com/microsoft/ProX-Code/blob/c67ccc70ece5f472ec25464d3eeb874cfccee9f1/src/vs/workbench/contrib/terminal/browser/terminalInstance.ts#L852-L857
				}
...
class DebugTerminalCollection {
	...
	private _terminalInstances = new Map<ProX-Code.Terminal, { lastUsedAt: number; config: string }>();
	...
	public async checkout(config: string, name: string, cleanupOthersByName = false) {
		...
		return await firstParallel(promises, (t): t is ProX-Code.Terminal => !!t);
	}

	public insert(terminal: ProX-Code.Terminal, termConfig: string) {
		this._terminalInstances.set(terminal, { lastUsedAt: Date.now(), config: termConfig });
	}

	public free(terminal: ProX-Code.Terminal) {
	...
	public onTerminalClosed(terminal: ProX-Code.Terminal) {
