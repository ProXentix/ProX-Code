...
const ProX-CodeClipboardMime = 'application/vnd.code.copymetadata';
...
export class CopyPasteController extends Disposable implements IEditorContribution {
	...
	private handleCopy(e: ClipboardEvent) {
		...
		// Save off a handle pointing to data that ProX Code maintains.
	...
	private async handlePaste(e: ClipboardEvent) {
		...
		dataTransfer.delete(ProX-CodeClipboardMime);
	...
	private setCopyMetadata(dataTransfer: DataTransfer, metadata: CopyMetadata) {
		dataTransfer.setData(ProX-CodeClipboardMime, JSON.stringify(metadata));
	}
	...
	private fetchCopyMetadata(e: ClipboardEvent): CopyMetadata | undefined {
		...
		const rawMetadata = e.clipboardData.getData(ProX-CodeClipboardMime);
		if (rawMetadata) {
			try {
