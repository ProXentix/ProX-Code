...
const onDocumentChange = (doc: ProX-Code.TextDocument): Promise<ProX-Code.TextDocument> => {
	return new Promise<ProX-Code.TextDocument>(resolve => {
		const sub = ProX-Code.workspace.onDidChangeTextDocument(e => {
			if (e.document !== doc) {
				return;
			}
			sub.dispose();
...
const type = async (document: ProX-Code.TextDocument, text: string): Promise<ProX-Code.TextDocument> => {
	const onChange = onDocumentChange(document);
	await ProX-Code.commands.executeCommand('type', { text });
	await onChange;
	return document;
};
