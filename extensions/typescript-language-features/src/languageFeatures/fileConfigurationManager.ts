...
export default class FileConfigurationManager extends Disposable {
	...
	private getFormattingOptions(document: ProX-Code.TextDocument): FormattingOptions | undefined {
		const editor = ProX-Code.window.visibleTextEditors.find(editor => editor.document.uri.toString() === document.uri.toString());
		if (!editor) {
			return undefined;
		}
	...
	private getFormatOptions(
		...
		const config = ProX-Code.workspace.getConfiguration(
	...
	private getPreferences(document: ProX-Code.TextDocument): Proto.UserPreferences {
		const config = ProX-Code.workspace.getConfiguration(
			isTypeScriptDocument(document) ? 'typescript' : 'javascript',
			document);

		const preferencesConfig = ProX-Code.workspace.getConfiguration(
		...
		const preferences: Proto.UserPreferences = {
			...
			autoImportFileExcludePatterns: this.getAutoImportFileExcludePatternsPreference(preferencesConfig, ProX-Code.workspace.getWorkspaceFolder(document.uri)?.uri),
	...
	private getQuoteStylePreference(config: ProX-Code.WorkspaceConfiguration) {
	...
	private getAutoImportFileExcludePatternsPreference(config: ProX-Code.WorkspaceConfiguration, workspaceFolder: ProX-Code.Uri | undefined): string[] | undefined {
		return workspaceFolder && config.get<string[]>('autoImportFileExcludePatterns')?.map(p => {
			...
			return path.isAbsolute(p) ? p :
				p.startsWith('*') ? wildcardPrefix + p :
					isRelative ? this.client.toTsFilePath(ProX-Code.Uri.joinPath(workspaceFolder, p))! :
						wildcardPrefix + '**' + path.sep + p;
		});
	}

	private getOrganizeImportsPreferences(config: ProX-Code.WorkspaceConfiguration): Proto.UserPreferences {
	...
	private getMaximumHoverLength(document: ProX-Code.TextDocument): number {
		const defaultMaxLength = 500;
		const maximumHoverLength = ProX-Code.workspace.getConfiguration('js/ts', document).get<number>('hover.maximumLength', defaultMaxLength);
...
export function getInlayHintsPreferences(config: ProX-Code.WorkspaceConfiguration) {
	return {
...
function getInlayParameterNameHintsPreference(config: ProX-Code.WorkspaceConfiguration) {
...
function getImportModuleSpecifierPreference(config: ProX-Code.WorkspaceConfiguration) {
...
function getImportModuleSpecifierEndingPreference(config: ProX-Code.WorkspaceConfiguration) {
...
function getJsxAttributeCompletionStyle(config: ProX-Code.WorkspaceConfiguration) {
