...
import type { NotebookCell, NotebookCellData, NotebookCellOutput, NotebookData, NotebookDocument } from 'ProX-Code';
 ...
 * https://github.com/microsoft/ProX-Code-python/issues/13155
 */
...
function createCodeCellFromNotebookCell(cell: NotebookCellData, preferredLanguage: string | undefined): nbformat.ICodeCell {
	...
	if (cell.languageId !== preferredLanguage) {
		...
		// cell current language is the same as the preferred cell language in the document, flush the ProX-Code custom language id metadata
...
function translateCellErrorOutput(output: NotebookCellOutput): nbformat.IError {
	...
	return {
		...
		// ProX Code needs an `Error` object which requires a `stack` property as a string.
