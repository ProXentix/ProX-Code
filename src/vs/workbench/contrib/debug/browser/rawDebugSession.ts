...
export class RawDebugSession implements IDisposable {
	...
	private launchVsCode(ProX-CodeArgs: ILaunchVSCodeArguments): Promise<IOpenExtensionWindowResult> {

		const args: string[] = [];

		for (const arg of ProX-CodeArgs.args) {
		...
		if (ProX-CodeArgs.env) {
			args.push(`--extensionEnvironment=${JSON.stringify(ProX-CodeArgs.env)}`);
		}

		return this.extensionHostDebugService.openExtensionDevelopmentHostWindow(args, !!ProX-CodeArgs.debugRenderer);
	}
