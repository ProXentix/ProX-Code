...
class DocumentSymbolAdapter {
	...
	constructor(
		...
		private readonly _provider: ProX-Code.DocumentSymbolProvider
	) { }
...
class CodeLensAdapter {
	...
	private readonly _cache = new Cache<ProX-Code.CodeLens>('CodeLens');
	private readonly _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: ProX-Code.CodeLensProvider,
	...
	async resolveCodeLens(symbol: extHostProtocol.ICodeLensDto, token: CancellationToken): Promise<extHostProtocol.ICodeLensDto | undefined> {
		...
		let resolvedLens: ProX-Code.CodeLens | undefined | null;
...
function convertToLocationLinks(value: ProX-Code.Location | ProX-Code.Location[] | ProX-Code.LocationLink[] | undefined | null): languages.LocationLink[] {
...
class DefinitionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DefinitionProvider
	) { }
...
class DeclarationAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DeclarationProvider
	) { }
...
class ImplementationAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.ImplementationProvider
	) { }
...
class TypeDefinitionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.TypeDefinitionProvider
	) { }
...
class HoverAdapter {
	...
	private _hoverMap: Map<number, ProX-Code.Hover> = new Map<number, ProX-Code.Hover>();

	private static HOVER_MAP_MAX_SIZE = 10;

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.HoverProvider,
	) { }
	...
	async provideHover(resource: URI, position: IPosition, context: languages.HoverContext<{ id: number }> | undefined, token: CancellationToken): Promise<extHostProtocol.HoverWithId | undefined> {
		...
		let value: ProX-Code.Hover | null | undefined;
		if (context && context.verbosityRequest) {
			...
			const hoverContext: ProX-Code.HoverContext = { verbosityDelta: context.verbosityRequest.verbosityDelta, previousHover };
...
class EvaluatableExpressionAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.EvaluatableExpressionProvider,
	) { }
...
class InlineValuesAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.InlineValuesProvider,
	) { }
...
class DocumentHighlightAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentHighlightProvider
	) { }
...
class MultiDocumentHighlightAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.MultiDocumentHighlightProvider,
		private readonly _logService: ILogService,
	) { }
...
class LinkedEditingRangeAdapter {
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.LinkedEditingRangeProvider
	) { }
...
class ReferenceAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.ReferenceProvider
	) { }
...
class CodeActionAdapter {
	...
	private readonly _cache = new Cache<ProX-Code.CodeAction | ProX-Code.Command>('CodeAction');
	private readonly _disposables = new Map<number, DisposableStore>();
	...
	constructor(
		...
		private readonly _provider: ProX-Code.CodeActionProvider,
		private readonly _logService: ILogService,
		private readonly _extension: IExtensionDescription,
	...
	async provideCodeActions(resource: URI, rangeOrSelection: IRange | ISelection, context: languages.CodeActionContext, token: CancellationToken): Promise<extHostProtocol.ICodeActionListDto | undefined> {
		...
		const ran = Selection.isISelection(rangeOrSelection)
			? <ProX-Code.Selection>typeConvert.Selection.to(rangeOrSelection)
			: <ProX-Code.Range>typeConvert.Range.to(rangeOrSelection);
		const allDiagnostics: ProX-Code.Diagnostic[] = [];
		...
		const codeActionContext: ProX-Code.CodeActionContext = {
		...
		for (let i = 0; i < commandsOrActions.length; i++) {
			...
			if (CodeActionAdapter._isCommand(candidate) && !(candidate instanceof CodeAction)) {
				...
				const toConvert = candidate as ProX-Code.CodeAction;
	...
	private static _isCommand(thing: any): thing is ProX-Code.Command {
		return typeof (<ProX-Code.Command>thing).command === 'string' && typeof (<ProX-Code.Command>thing).title === 'string';
	}
}

class DocumentPasteEditProvider {

	private _cachedPrepare?: Map<string, ProX-Code.DataTransferItem>;

	private readonly _editsCache = new Cache<ProX-Code.DocumentPasteEdit>('DocumentPasteEdit.edits');

	constructor(
		private readonly _proxy: extHostProtocol.MainThreadLanguageFeaturesShape,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentPasteEditProvider,
	...
	async prepareDocumentPaste(resource: URI, ranges: IRange[], dataTransferDto: extHostProtocol.DataTransferDTO, token: CancellationToken): Promise<extHostProtocol.DataTransferDTO | undefined> {
		...
		const doc = this._documents.getDocument(resource);
		const ProX-CodeRanges = ranges.map(range => typeConvert.Range.to(range));

		const dataTransfer = typeConvert.DataTransfer.toDataTransfer(dataTransferDto, () => {
			throw new NotImplementedError();
		});
		await this._provider.prepareDocumentPaste(doc, ProX-CodeRanges, dataTransfer, token);
		if (token.isCancellationRequested) {
			return;
		}
		...
		const newCache = new Map<string, ProX-Code.DataTransferItem>();
	...
	async providePasteEdits(requestId: number, resource: URI, ranges: IRange[], dataTransferDto: extHostProtocol.DataTransferDTO, context: extHostProtocol.IDocumentPasteContextDto, token: CancellationToken): Promise<extHostProtocol.IPasteEditDto[]> {
		...
		const items = dataTransferDto.items.map(([mime, value]): [string, ProX-Code.DataTransferItem] => {
		...
		const edits = await this._provider.provideDocumentPasteEdits(doc, ProX-CodeRanges, dataTransfer, {
	...
	async resolvePasteEdit(id: extHostProtocol.ChainedCacheId, token: CancellationToken): Promise<{ insertText?: string | ProX-Code.SnippetString; additionalEdit?: extHostProtocol.IWorkspaceEditDto }> {
		const [sessionId, itemId] = id;
...
class DocumentFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentFormattingEditProvider
	) { }
...
class RangeFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentRangeFormattingEditProvider
	) { }
...
class OnTypeFormattingAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.OnTypeFormattingEditProvider
	) { }
...
class NavigateTypeAdapter {
	...
	private readonly _cache = new Cache<ProX-Code.SymbolInformation>('WorkspaceSymbols');

	constructor(
		private readonly _provider: ProX-Code.WorkspaceSymbolProvider,
		private readonly _logService: ILogService
	) { }
...
class RenameAdapter {
	...
	static supportsResolving(provider: ProX-Code.RenameProvider): boolean {
		return typeof provider.prepareRename === 'function';
	}

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.RenameProvider,
		private readonly _logService: ILogService
	) { }
...
class NewSymbolNamesAdapter {
	...
	private static languageTriggerKindToVSCodeTriggerKind: Record<languages.NewSymbolNameTriggerKind, ProX-Code.NewSymbolNameTriggerKind> = {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.NewSymbolNamesProvider,
		private readonly _logService: ILogService
	) { }
...
type ProvidedSemanticTokens = ProX-Code.SemanticTokens | RelaxedSemanticTokens;
type ProvidedSemanticTokensEdits = ProX-Code.SemanticTokensEdits | RelaxedSemanticTokensEdits;
...
class DocumentSemanticTokensAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentSemanticTokensProvider,
	) {
	...
	private static _fixProvidedSemanticTokens(v: ProvidedSemanticTokens | ProvidedSemanticTokensEdits): ProX-Code.SemanticTokens | ProX-Code.SemanticTokensEdits {
	...
	private static _isCorrectSemanticTokens(v: ProvidedSemanticTokens): v is ProX-Code.SemanticTokens {
	...
	private static _isCorrectSemanticTokensEdits(v: ProvidedSemanticTokensEdits): v is ProX-Code.SemanticTokensEdits {
	...
	private static _convertToEdits(previousResult: SemanticTokensPreviousResult | null | undefined, newResult: ProX-Code.SemanticTokens | ProX-Code.SemanticTokensEdits): ProX-Code.SemanticTokens | ProX-Code.SemanticTokensEdits {
	...
	private _send(value: ProX-Code.SemanticTokens | ProX-Code.SemanticTokensEdits, original: ProX-Code.SemanticTokens | ProX-Code.SemanticTokensEdits): VSBuffer | null {
...
class DocumentRangeSemanticTokensAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentRangeSemanticTokensProvider,
	) { }
	...
	private _send(value: ProX-Code.SemanticTokens): VSBuffer {
		return encodeSemanticTokensDto({
			id: 0,
			type: 'full',
			data: value.data
...
class CompletionsAdapter {
	...
	static supportsResolving(provider: ProX-Code.CompletionItemProvider): boolean {
		return typeof provider.resolveCompletionItem === 'function';
	}

	private _cache = new Cache<ProX-Code.CompletionItem>('CompletionItem');
	private _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: ProX-Code.CompletionItemProvider,
		private readonly _apiDeprecation: IExtHostApiDeprecationService,
		private readonly _extension: IExtensionDescription,
	) { }
	...
	async provideCompletionItems(resource: URI, position: IPosition, context: languages.CompletionContext, token: CancellationToken): Promise<extHostProtocol.ISuggestResultDto | undefined> {
		...
		// https://github.com/microsoft/ProX-Code/issues/83400#issuecomment-546851421
	...
	private _convertCompletionItem(item: ProX-Code.CompletionItem, id: extHostProtocol.ChainedCacheId, defaultInsertRange?: ProX-Code.Range, defaultReplaceRange?: ProX-Code.Range): extHostProtocol.ISuggestDataDto {
		...
		let range: ProX-Code.Range | { inserting: ProX-Code.Range; replacing: ProX-Code.Range } | undefined;
		if (item.textEdit) {
...
class InlineCompletionAdapter {
	private readonly _references = new ReferenceMap<{
		dispose(): void;
		items: readonly ProX-Code.InlineCompletionItem[];
	}>();
	...
	constructor(
		private readonly _extension: IExtensionDescription,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.InlineCompletionItemProvider,
		private readonly _commands: CommandsConverter,
	) {
...
class InlineEditAdapter {
	...
	constructor(
		...
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.InlineEditProvider,
		private readonly _commands: CommandsConverter,
	) {
	}
}
...
class SignatureHelpAdapter {
	...
	private readonly _cache = new Cache<ProX-Code.SignatureHelp>('SignatureHelp');

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.SignatureHelpProvider,
	) { }
	...
	async provideSignatureHelp(resource: URI, position: IPosition, context: extHostProtocol.ISignatureHelpContextDto, token: CancellationToken): Promise<extHostProtocol.ISignatureHelpDto | undefined> {
		const doc = this._documents.getDocument(resource);
		const pos = typeConvert.Position.to(position);
		const ProX-CodeContext = this.reviveContext(context);

		const value = await this._provider.provideSignatureHelp(doc, pos, token, ProX-CodeContext);
		if (value) {
	...
	private reviveContext(context: extHostProtocol.ISignatureHelpContextDto): ProX-Code.SignatureHelpContext {
		let activeSignatureHelp: ProX-Code.SignatureHelp | undefined = undefined;
...
class InlayHintsAdapter {
	...
	private _cache = new Cache<ProX-Code.InlayHint>('InlayHints');
	private readonly _disposables = new Map<number, DisposableStore>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _commands: CommandsConverter,
		private readonly _provider: ProX-Code.InlayHintsProvider,
		private readonly _logService: ILogService,
		private readonly _extension: IExtensionDescription
	) { }
	...
	private _isValidInlayHint(hint: ProX-Code.InlayHint, range?: ProX-Code.Range): boolean {
	...
	private _convertInlayHint(hint: ProX-Code.InlayHint, id: extHostProtocol.ChainedCacheId): extHostProtocol.IInlayHintDto {
...
class LinkProviderAdapter {
	...
	private _cache = new Cache<ProX-Code.DocumentLink>('DocumentLink');

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentLinkProvider
	) { }
	...
	private static _validateLink(link: ProX-Code.DocumentLink): boolean {
...
class ColorProviderAdapter {
	...
	constructor(
		...
		private _provider: ProX-Code.DocumentColorProvider
	) { }
...
class FoldingProviderAdapter {
	...
	constructor(
		private _documents: ExtHostDocuments,
		private _provider: ProX-Code.FoldingRangeProvider
	) { }
...
class SelectionRangeAdapter {
	...
	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.SelectionRangeProvider,
		private readonly _logService: ILogService
	) { }
	...
	async provideSelectionRanges(resource: URI, pos: IPosition[], token: CancellationToken): Promise<languages.SelectionRange[][]> {
		...
		for (let i = 0; i < positions.length; i++) {
			...
			let last: ProX-Code.Position | ProX-Code.Range = positions[i];
...
class CallHierarchyAdapter {
	...
	private readonly _cache = new Map<string, Map<string, ProX-Code.CallHierarchyItem>>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.CallHierarchyProvider
	) { }
	...
	private _cacheAndConvertItem(sessionId: string, item: ProX-Code.CallHierarchyItem): extHostProtocol.ICallHierarchyItemDto {
	...
	private _itemFromCache(sessionId: string, itemId: string): ProX-Code.CallHierarchyItem | undefined {
		const map = this._cache.get(sessionId);
...
class TypeHierarchyAdapter {
	...
	private readonly _idPool = new IdGenerator('');
	private readonly _cache = new Map<string, Map<string, ProX-Code.TypeHierarchyItem>>();

	constructor(
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.TypeHierarchyProvider
	) { }
	...
	private _cacheAndConvertItem(sessionId: string, item: ProX-Code.TypeHierarchyItem): extHostProtocol.ITypeHierarchyItemDto {
		const map = this._cache.get(sessionId)!;
	...
	private _itemFromCache(sessionId: string, itemId: string): ProX-Code.TypeHierarchyItem | undefined {
		const map = this._cache.get(sessionId);
		return map?.get(itemId);
	}
}
...
class DocumentDropEditAdapter {
	...
	private readonly _cache = new Cache<ProX-Code.DocumentDropEdit>('DocumentDropEdit');

	constructor(
		private readonly _proxy: extHostProtocol.MainThreadLanguageFeaturesShape,
		private readonly _documents: ExtHostDocuments,
		private readonly _provider: ProX-Code.DocumentDropEditProvider,
		private readonly _handle: number,
		private readonly _extension: IExtensionDescription,
	) { }
...
export class ExtHostLanguageFeatures implements extHostProtocol.ExtHostLanguageFeaturesShape {
	...
	private _transformDocumentSelector(selector: ProX-Code.DocumentSelector, extension: IExtensionDescription): Array<extHostProtocol.IDocumentFilterDto> {
	...
	registerDocumentSymbolProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentSymbolProvider, metadata?: ProX-Code.DocumentSymbolProviderMetadata): ProX-Code.Disposable {
	...
	registerCodeLensProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.CodeLensProvider): ProX-Code.Disposable {
		const handle = this._nextHandle();
	...
	registerDefinitionProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DefinitionProvider): ProX-Code.Disposable {
	...
	registerDeclarationProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DeclarationProvider): ProX-Code.Disposable {
	...
	registerImplementationProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.ImplementationProvider): ProX-Code.Disposable {
	...
	registerTypeDefinitionProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.TypeDefinitionProvider): ProX-Code.Disposable {
	...
	registerHoverProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.HoverProvider, extensionId?: ExtensionIdentifier): ProX-Code.Disposable {
	...
	registerEvaluatableExpressionProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.EvaluatableExpressionProvider, extensionId?: ExtensionIdentifier): ProX-Code.Disposable {
	...
	registerInlineValuesProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.InlineValuesProvider, extensionId?: ExtensionIdentifier): ProX-Code.Disposable {
	...
	registerDocumentHighlightProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentHighlightProvider): ProX-Code.Disposable {
	...
	registerMultiDocumentHighlightProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.MultiDocumentHighlightProvider): ProX-Code.Disposable {
	...
	registerLinkedEditingRangeProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.LinkedEditingRangeProvider): ProX-Code.Disposable {
	...
	registerReferenceProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.ReferenceProvider): ProX-Code.Disposable {
	...
	registerCodeActionProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.CodeActionProvider, metadata?: ProX-Code.CodeActionProviderMetadata): ProX-Code.Disposable {
	...
	registerDocumentFormattingEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentFormattingEditProvider): ProX-Code.Disposable {
	...
	registerDocumentRangeFormattingEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentRangeFormattingEditProvider): ProX-Code.Disposable {
	...
	registerOnTypeFormattingEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.OnTypeFormattingEditProvider, triggerCharacters: string[]): ProX-Code.Disposable {
	...
	registerWorkspaceSymbolProvider(extension: IExtensionDescription, provider: ProX-Code.WorkspaceSymbolProvider): ProX-Code.Disposable {
	...
	registerRenameProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.RenameProvider): ProX-Code.Disposable {
	...
	registerNewSymbolNamesProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.NewSymbolNamesProvider): ProX-Code.Disposable {
	...
	registerDocumentSemanticTokensProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentSemanticTokensProvider, legend: ProX-Code.SemanticTokensLegend): ProX-Code.Disposable {
	...
	registerDocumentRangeSemanticTokensProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentRangeSemanticTokensProvider, legend: ProX-Code.SemanticTokensLegend): ProX-Code.Disposable {
	...
	registerCompletionItemProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.CompletionItemProvider, triggerCharacters: string[]): ProX-Code.Disposable {
	...
	registerInlineCompletionsProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.InlineCompletionItemProvider, metadata: ProX-Code.InlineCompletionItemProviderMetadata | undefined): ProX-Code.Disposable {
	...
	registerInlineEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.InlineEditProvider): ProX-Code.Disposable {
	...
	registerSignatureHelpProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.SignatureHelpProvider, metadataOrTriggerChars: string[] | ProX-Code.SignatureHelpProviderMetadata): ProX-Code.Disposable {
	...
	registerInlayHintsProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.InlayHintsProvider): ProX-Code.Disposable {
	...
	registerDocumentLinkProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentLinkProvider): ProX-Code.Disposable {
	...
	registerColorProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentColorProvider): ProX-Code.Disposable {
	...
	registerFoldingRangeProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.FoldingRangeProvider): ProX-Code.Disposable {
		const handle = this._nextHandle();
	...
	$provideFoldingRanges(handle: number, resource: UriComponents, context: ProX-Code.FoldingContext, token: CancellationToken): Promise<languages.FoldingRange[] | undefined> {
		return this._withAdapter(
			handle,
	...
	registerSelectionRangeProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.SelectionRangeProvider): ProX-Code.Disposable {
	...
	registerCallHierarchyProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.CallHierarchyProvider): ProX-Code.Disposable {
	...
	// --- type hierarchy
	registerTypeHierarchyProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.TypeHierarchyProvider): ProX-Code.Disposable {
	...
	registerDocumentOnDropEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentDropEditProvider, metadata?: ProX-Code.DocumentDropEditProviderMetadata) {
		const handle = this._nextHandle();
	...
	registerDocumentPasteEditProvider(extension: IExtensionDescription, selector: ProX-Code.DocumentSelector, provider: ProX-Code.DocumentPasteEditProvider, metadata: ProX-Code.DocumentPasteProviderMetadata): ProX-Code.Disposable {
		const handle = this._nextHandle();
	...
	private static _serializeIndentationRule(indentationRule: ProX-Code.IndentationRule): extHostProtocol.IIndentationRuleDto {
		return {
	...
	private static _serializeOnEnterRule(onEnterRule: ProX-Code.OnEnterRule): extHostProtocol.IOnEnterRuleDto {
		return {
	...
	private static _serializeOnEnterRules(onEnterRules: ProX-Code.OnEnterRule[]): extHostProtocol.IOnEnterRuleDto[] {
		return onEnterRules.map(ExtHostLanguageFeatures._serializeOnEnterRule);
	}

	private static _serializeAutoClosingPair(autoClosingPair: ProX-Code.AutoClosingPair): IAutoClosingPairConditional {
		return {
	...
	private static _serializeAutoClosingPairs(autoClosingPairs: ProX-Code.AutoClosingPair[]): IAutoClosingPairConditional[] {
		return autoClosingPairs.map(ExtHostLanguageFeatures._serializeAutoClosingPair);
	}

	setLanguageConfiguration(extension: IExtensionDescription, languageId: string, configuration: ProX-Code.LanguageConfiguration): ProX-Code.Disposable {
