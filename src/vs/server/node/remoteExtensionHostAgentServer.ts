...
class RemoteExtensionHostAgentServer extends Disposable implements IServerAPI {
	...
	public async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (pathname === '/ProX-Code-remote-resource') {
...
export async function createServer(address: string | net.AddressInfo | null, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string): Promise<IServerAPI> {
	...
	function initUnexpectedErrorHandler(handler: (err: any) => void) {
		setUnexpectedErrorHandler(err => {
			// See https://github.com/microsoft/ProX-Code-remote-release/issues/6481
	...
	process.on('SIGPIPE', () => {
		// See https://github.com/microsoft/ProX-Code-remote-release/issues/6543
...
`;
	...
	const ProX-CodeServerStartTime: number = (<any>global).ProX-CodeServerStartTime;
	const ProX-CodeServerListenTime: number = (<any>global).ProX-CodeServerListenTime;
	const ProX-CodeServerCodeLoadedTime: number = (<any>global).ProX-CodeServerCodeLoadedTime;
	...
	instantiationService.invokeFunction(async (accessor) => {
		...
		telemetryService.publicLog2<ServerStartEvent, ServerStartClassification>('serverStart', {
			startTime: ProX-CodeServerStartTime,
			startedTime: ProX-CodeServerListenTime,
			codeLoadedTime: ProX-CodeServerCodeLoadedTime,
	...
	if (args['print-startup-performance']) {
		let output = '';
		output += `Start-up time: ${ProX-CodeServerListenTime - ProX-CodeServerStartTime}\n`;
		output += `Code loading time: ${ProX-CodeServerCodeLoadedTime - ProX-CodeServerStartTime}\n`;
		output += `Initialized time: ${currentTime - ProX-CodeServerStartTime}\n`;
		output += `\n`;
		console.log(output);
	}
