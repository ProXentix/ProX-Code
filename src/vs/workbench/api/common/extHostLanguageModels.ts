...
type LanguageModelData = {
	...
	readonly provider: ProX-Code.ChatResponseProvider;
};

class LanguageModelResponseStream {

	readonly stream = new AsyncIterableSource<ProX-Code.LanguageModelTextPart | ProX-Code.LanguageModelToolCallPart>();

	constructor(
		readonly option: number,
		stream?: AsyncIterableSource<ProX-Code.LanguageModelTextPart | ProX-Code.LanguageModelToolCallPart>
	) {
		this.stream = stream ?? new AsyncIterableSource<ProX-Code.LanguageModelTextPart | ProX-Code.LanguageModelToolCallPart>();
	}
}

class LanguageModelResponse {

	readonly apiObject: ProX-Code.LanguageModelChatResponse;

	private readonly _responseStreams = new Map<number, LanguageModelResponseStream>();
	private readonly _defaultStream = new AsyncIterableSource<ProX-Code.LanguageModelTextPart | ProX-Code.LanguageModelToolCallPart>();
	...
	handleFragment(fragment: IChatResponseFragment): void {
		...
		let out: ProX-Code.LanguageModelTextPart | ProX-Code.LanguageModelDataPart | ProX-Code.LanguageModelToolCallPart;
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	private readonly _allLanguageModelData = new Map<string, { metadata: ILanguageModelChatMetadata; apiObjects: ExtensionIdentifierMap<ProX-Code.LanguageModelChat> }>(); // these are ALL models, not just the one in this EH
	...
	private readonly _ignoredFileProviders = new Map<number, ProX-Code.LanguageModelIgnoredFileProvider>();
	...
	registerLanguageModel(extension: IExtensionDescription, identifier: string, provider: ProX-Code.ChatResponseProvider, metadata: ProX-Code.ChatResponseProviderMetadata): IDisposable {
	...
	async $startChatRequest(handle: number, requestId: number, from: ExtensionIdentifier, messages: SerializableObjectWithBuffers<IChatMessage[]>, options: ProX-Code.LanguageModelChatRequestOptions, token: CancellationToken): Promise<void> {
		const data = this._languageModels.get(handle);
		if (!data) {
			throw new Error('Provider not found');
		}
		const progress = new Progress<ProX-Code.ChatResponseFragment2>(async fragment => {
	...
	async getDefaultLanguageModel(extension: IExtensionDescription): Promise<ProX-Code.LanguageModelChat | undefined> {
	...
	async getLanguageModelByIdentifier(extension: IExtensionDescription, identifier: string): Promise<ProX-Code.LanguageModelChat | undefined> {
	...
	async selectLanguageModels(extension: IExtensionDescription, selector: ProX-Code.LanguageModelChatSelector) {
		...
		const result: ProX-Code.LanguageModelChat[] = [];
	...
	private async _sendChatRequest(extension: IExtensionDescription, languageModelId: string, messages: ProX-Code.LanguageModelChatMessage2[], options: ProX-Code.LanguageModelChatRequestOptions, token: CancellationToken) {
	...
	private _convertMessages(extension: IExtensionDescription, messages: ProX-Code.LanguageModelChatMessage2[]) {
	...
	private async _computeTokenLength(languageModelId: string, value: string | ProX-Code.LanguageModelChatMessage2, token: ProX-Code.CancellationToken): Promise<number> {
	...
	createLanguageModelAccessInformation(from: Readonly<IExtensionDescription>): ProX-Code.LanguageModelAccessInformation {
		...
		return {
			...
			canSendRequest(chat: ProX-Code.LanguageModelChat): boolean | undefined {
	...
	fileIsIgnored(extension: IExtensionDescription, uri: ProX-Code.Uri, token: ProX-Code.CancellationToken = CancellationToken.None): Promise<boolean> {
	...
	registerIgnoredFileProvider(extension: IExtensionDescription, provider: ProX-Code.LanguageModelIgnoredFileProvider): ProX-Code.Disposable {
