...
class TypeScriptInlayHintsProvider extends Disposable implements ProX-Code.InlayHintsProvider {

	public static readonly minVersion = API.v440;

	private readonly _onDidChangeInlayHints = this._register(new ProX-Code.EventEmitter<void>());
	...
	async provideInlayHints(model: ProX-Code.TextDocument, range: ProX-Code.Range, token: ProX-Code.CancellationToken): Promise<ProX-Code.InlayHint[] | undefined> {
		...
		return response.body.map(hint => {
			const result = new ProX-Code.InlayHint(
	...
	private convertInlayHintText(tsHint: Proto.InlayHintItem): string | ProX-Code.InlayHintLabelPart[] {
		if (tsHint.displayParts) {
			return tsHint.displayParts.map((part): ProX-Code.InlayHintLabelPart => {
				const out = new ProX-Code.InlayHintLabelPart(part.text);
				if (part.span) {
...
function fromProtocolInlayHintKind(kind: Proto.InlayHintKind): ProX-Code.InlayHintKind | undefined {
	switch (kind) {
		case 'Parameter': return ProX-Code.InlayHintKind.Parameter;
		case 'Type': return ProX-Code.InlayHintKind.Type;
		case 'Enum': return undefined;
		default: return undefined;
	}
}

function areInlayHintsEnabledForFile(language: LanguageDescription, document: ProX-Code.TextDocument) {
	const config = ProX-Code.workspace.getConfiguration(language.id, document);
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerInlayHintsProvider(selector.semantic, provider);
	});
}
