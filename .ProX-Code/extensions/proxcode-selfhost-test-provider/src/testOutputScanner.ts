...
export class TestOutputScanner implements ProX-Code.Disposable {
	protected mochaEventEmitter = new ProX-Code.EventEmitter<MochaEventTuple>();
	protected outputEventEmitter = new ProX-Code.EventEmitter<string>();
	protected onExitEmitter = new ProX-Code.EventEmitter<string | undefined>();
...
type QueuedOutput = string | [string, ProX-Code.Location | undefined, ProX-Code.TestItem | undefined];

export async function scanTestOutput(
	tests: Map<string, ProX-Code.TestItem>,
	task: ProX-Code.TestRun,
	scanner: TestOutputScanner,
	coverageDir: string | undefined,
	cancellation: ProX-Code.CancellationToken
): Promise<void> {
	...
	let lastTest: ProX-Code.TestItem | undefined;
	...
	try {
		...
		await new Promise<void>(resolve => {
			...
			let currentTest: ProX-Code.TestItem | undefined;
					...
					case MochaEvent.Fail:
						{
							...
							const testFirstLine =
								tcase.range &&
								new ProX-Code.Location(
									tcase.uri!,
									new ProX-Code.Range(
										tcase.range.start,
										new ProX-Code.Position(tcase.range.start.line, 100)
									)
								);
							...
							enqueueExitBlocker(
								(async () => {
									...
									let message: ProX-Code.TestMessage;

									if (hasDiff) {
										message = new ProX-Code.TestMessage(tryMakeMarkdown(err));
										...
										message = new ProX-Code.TestMessage(
		...
		if (!ranAnyTest) {
			await ProX-Code.commands.executeCommand('testing.showMostRecentOutput');
		}
	} catch (e) {
...
const tryMakeMarkdown = (message: string) => {
	...
	lines.push('```');
	return new ProX-Code.MarkdownString(lines.join('\n'));
};
...
export type SourceLocationMapper = (line: number, col: number, strategy: SearchStrategy) => ProX-Code.Location | undefined;
...
export class SourceMapStore {
	...
	async getSourceLocationMapper(fileUri: string): Promise<SourceLocationMapper> {
		...
		return (line, col, strategy) => {
			...
			if (idealPosition.line !== null && idealPosition.column !== null && idealPosition.source !== null) {
				return new ProX-Code.Location(
					this.completeSourceMapUrl(sourceMap, idealPosition.source),
					new ProX-Code.Position(idealPosition.line - 1, idealPosition.column)
				);
			}
			...
			do {
				...
				return new ProX-Code.Location(
					...
					new ProX-Code.Position(segment[MapField.SOURCE_LINE] - 1, segment[MapField.SOURCE_COLUMN])
				);
		...
		for (const bias of sourceMapBiases) {
			...
			if (position.line !== null && position.column !== null && position.source !== null) {
				return new ProX-Code.Location(
					...
					new ProX-Code.Position(position.line - 1, position.column)
				);
			}
		}
	...
	private completeSourceMapUrl(sm: TraceMap, source: string) {
		if (sm.sourceRoot) {
			try {
				return ProX-Code.Uri.parse(new URL(source, sm.sourceRoot).toString());
			} catch {
				// ignored
			}
		}

		return ProX-Code.Uri.parse(source);
	}
	...
	private loadSourceMap(fileUri: string) {
		...
		const promise = (async () => {
			try {
				const contents = await getContentFromFilesystem(ProX-Code.Uri.parse(fileUri));
...
async function deriveStackLocations(
	...
	const locationsMapped = await Promise.all(locationsRaw.map(async location => {
		...
		const stack = new ProX-Code.TestMessageStackFrame(location.label || '<anonymous>', mapped?.uri, mapped?.range.start || new ProX-Code.Position(location.lineBase1 - 1, location.columnBase1 - 1));
		return { location: mapped, stack };
	}));

	let best: undefined | { location: ProX-Code.Location; score: number };
