...
suite('ProX-Code API - configuration', () => {

	teardown(assertNoRpc);

	test('configurations, language defaults', function () {
		const defaultLanguageSettings = ProX-Code.workspace.getConfiguration().get('[abcLang]');
	...
	test('configuration, defaults', () => {
		const config = ProX-Code.workspace.getConfiguration('farboo');
