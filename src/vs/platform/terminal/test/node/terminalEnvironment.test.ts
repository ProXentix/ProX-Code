...
const productService = { applicationName: 'ProX-Code' } as IProductService;
		...
		if (process.platform !== 'win32') {
			suite('zsh', async () => {
				suite('should override args', async () => {
					...
					const expectedDir = new RegExp(`.+\/${username}-ProX-Code-zsh`);
					const customZdotdir = '/custom/zsh/dotdir';
					const expectedDests = [
						new RegExp(`.+\\/${username}-ProX-Code-zsh\\/\\.zshrc`),
						new RegExp(`.+\\/${username}-ProX-Code-zsh\\/\\.zprofile`),
						new RegExp(`.+\\/${username}-ProX-Code-zsh\\/\\.zshenv`),
						new RegExp(`.+\\/${username}-ProX-Code-zsh\\/\\.zlogin`)
					];
