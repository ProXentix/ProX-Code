...
async function shouldSmartPasteForSelection(
	...
	const line = document.getText(new ProX-Code.Range(selectedRange.start.line, 0, selectedRange.start.line, Number.MAX_SAFE_INTEGER));
...
export function findValidUriInText(text: string): string | undefined {
	...
	try {
		uri = ProX-Code.Uri.parse(trimmedUrlList);
	} catch {
		// Could not parse
		return;
	}
