...
const originalState = ProX-Code.getState() ?? {} as any;
const state = {
	...originalState,
...
// Make sure to sync ProX Code state here
ProX-Code.setState(state);

const messaging = createPosterForVsCode(ProX-Code, settings);
...
onceDocumentLoaded(() => {
	...
	if (typeof scrollProgress === 'number' && !settings.settings.fragment) {
		doAfterImagesLoaded(() => {
			...
			// Always set scroll of at least 1 to prevent ProX Code's webview code from auto scrolling us
...
function addImageContexts() {
	...
	for (const img of images) {
		...
		img.setAttribute('data-ProX-Code-context', JSON.stringify({ webviewSection, id: img.id, 'preventDefaultContextMenuItems': true, resource: documentResource, imageSource }));
	}
}
...
window.addEventListener('message', async event => {
	...
	switch (data.type) {
		...
		case 'updateContent': {
			...
			window.dispatchEvent(new CustomEvent('ProX-Code.markdown.updateContent'));
			addImageContexts();
			break;
		}
	}
...
const passThroughLinkSchemes = ['http:', 'https:', 'mailto:', 'ProX-Code:', 'ProX-Code-insiders:'];
...
document.addEventListener('click', event => {
	...
	while (node) {
		if (node.tagName && node.tagName === 'A' && node.href) {
			...
			// If original link doesn't look like a url, delegate back to ProX Code to resolve
