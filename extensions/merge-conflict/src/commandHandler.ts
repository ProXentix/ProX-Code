...
export default class CommandHandler implements ProX-Code.Disposable {
	...
	private registerTextEditorCommand(command: string, cb: (editor: ProX-Code.TextEditor, ...args: any[]) => Promise<void>, resourceCB?: (uris: ProX-Code.Uri[]) => Promise<void>) {
		return ProX-Code.commands.registerCommand(command, (...args) => {
	...
	acceptCurrent(editor: ProX-Code.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Current, editor, ...args);
	}

	acceptIncoming(editor: ProX-Code.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Incoming, editor, ...args);
	}

	acceptBoth(editor: ProX-Code.TextEditor, ...args: any[]): Promise<void> {
		return this.accept(interfaces.CommitType.Both, editor, ...args);
	}

	acceptAllCurrent(editor: ProX-Code.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Current, editor);
	}

	acceptAllIncoming(editor: ProX-Code.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Incoming, editor);
	}

	acceptAllCurrentResources(resources: ProX-Code.Uri[]): Promise<void> {
		return this.acceptAllResources(interfaces.CommitType.Current, resources);
	}

	acceptAllIncomingResources(resources: ProX-Code.Uri[]): Promise<void> {
		return this.acceptAllResources(interfaces.CommitType.Incoming, resources);
	}

	acceptAllBoth(editor: ProX-Code.TextEditor): Promise<void> {
		return this.acceptAll(interfaces.CommitType.Both, editor);
	}

	async compare(editor: ProX-Code.TextEditor, conflict: interfaces.IDocumentMergeConflict | null) {
		...
		if (!conflict) {
			...
			if (!conflict) {
				ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("Editor cursor is not within a merge conflict"));
				return;
			}
		}
		...
		const selection = new ProX-Code.Range(
		...
		const title = ProX-Code.l10n.t("{0}: Current Changes ↔ Incoming Changes", fileName);
		const mergeConflictConfig = ProX-Code.workspace.getConfiguration('merge-conflict');
		const openToTheSide = mergeConflictConfig.get<string>('diffViewPosition');
		const opts: ProX-Code.TextDocumentShowOptions = {
			viewColumn: openToTheSide === 'Beside' ? ProX-Code.ViewColumn.Beside : ProX-Code.ViewColumn.Active,
			selection
		};

		if (openToTheSide === 'Below') {
			await ProX-Code.commands.executeCommand('workbench.action.newGroupBelow');
		}

		await ProX-Code.commands.executeCommand('ProX-Code.diff', leftUri, rightUri, title, opts);
	}

	navigateNext(editor: ProX-Code.TextEditor): Promise<void> {
		return this.navigate(editor, NavigationDirection.Forwards);
	}

	navigatePrevious(editor: ProX-Code.TextEditor): Promise<void> {
		return this.navigate(editor, NavigationDirection.Backwards);
	}

	async acceptSelection(editor: ProX-Code.TextEditor): Promise<void> {
		...
		let tokenAfterCurrentBlock: ProX-Code.Range = conflict.splitter;
		...
		if (editor.selection.active.isBefore(tokenAfterCurrentBlock.start)) {
			...
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t('Editor cursor is within the common ancestors block, please move it to either the "current" or "incoming" block'));
			return;
		}
		else {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t('Editor cursor is within the merge conflict splitter, please move it to either the "current" or "incoming" block'));
			return;
		}
	...
	private async navigate(editor: ProX-Code.TextEditor, direction: NavigationDirection): Promise<void> {
		...
		if (!navigationResult) {
			...
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("No merge conflicts found in this file"));
			return;
		}
		else if (!navigationResult.canNavigate) {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("No other merge conflicts within this file"));
			return;
		}
		...
		editor.selection = new ProX-Code.Selection(navigationResult.conflict.range.start, navigationResult.conflict.range.start);
		editor.revealRange(navigationResult.conflict.range, ProX-Code.TextEditorRevealType.Default);
	}

	private async accept(type: interfaces.CommitType, editor: ProX-Code.TextEditor, ...args: any[]): Promise<void> {
	...
	private async acceptAll(type: interfaces.CommitType, editor: ProX-Code.TextEditor): Promise<void> {
		const conflicts = await this.tracker.getConflicts(editor.document);
	...
	private async acceptAllResources(type: interfaces.CommitType, resources: ProX-Code.Uri[]): Promise<void> {
		const documents = await Promise.all(resources.map(resource => ProX-Code.workspace.openTextDocument(resource)));
		...
		ProX-Code.workspace.applyEdit(edit);
	}

	private async findConflictContainingSelection(editor: ProX-Code.TextEditor, conflicts?: interfaces.IDocumentMergeConflict[]): Promise<interfaces.IDocumentMergeConflict | null> {
	...
	private async findConflictForNavigation(editor: ProX-Code.TextEditor, direction: NavigationDirection, conflicts?: interfaces.IDocumentMergeConflict[]): Promise<IDocumentMergeConflictNavigationResults | null> {
		if (!conflicts) {
			conflicts = await this.tracker.getConflicts(editor.document);
		}
