	...
	suite('~ -> $HOME', () => {
		...
		setup(() => {
			...
			validResources = [
				...
				URI.parse('file:///home'),
				URI.parse('file:///home/ProX-Code'),
				URI.parse('file:///home/ProX-Code/foo'),
				URI.parse('file:///home/ProX-Code/bar.txt'),
			];
			childResources = [
				{ resource: URI.parse('file:///home/ProX-Code'), isDirectory: true },
				{ resource: URI.parse('file:///home/ProX-Code/foo'), isDirectory: true },
				{ resource: URI.parse('file:///home/ProX-Code/bar.txt'), isFile: true },
			];
		});
		...
		test('~/| should return folder completions relative to $HOME', async () => {
			assertCompletions(await terminalCompletionService.resolveResources(resourceRequestConfig, '~/', 2, provider, capabilities), [
				{ label: '~/', detail: '/home/' },
				{ label: '~/ProX-Code/', detail: '/home/ProX-Code/' },
			], { replacementIndex: 0, replacementLength: 2 });
		});

		test('~/ProX-Code/| should return folder completions relative to $HOME/ProX-Code', async () => {
			assertCompletions(await terminalCompletionService.resolveResources(resourceRequestConfig, '~/ProX-Code/', 9, provider, capabilities), [
				{ label: '~/ProX-Code/', detail: '/home/ProX-Code/' },
				{ label: '~/ProX-Code/foo/', detail: '/home/ProX-Code/foo/' },
				{ label: '~/ProX-Code/bar.txt', detail: '/home/ProX-Code/bar.txt', kind: TerminalCompletionItemKind.File },
