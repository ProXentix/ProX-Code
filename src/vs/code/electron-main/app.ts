...
/**
 * The main ProX Code application. There will only ever be one instance,
...
export class CodeApplication extends Disposable {
	...
	private configureSession(): void {
		...
		const allowedPermissionsInWebview = new Set([
			...
			// https://github.com/microsoft/ProX-Code/issues/239228
		...
		session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.scheme === Schemas.ProX-CodeWebview) {
				if (!isAllowedWebviewRequest(uri, details)) {
					this.logService.error('Blocked ProX-Code-webview request', details.url);
			...
			if (uri.scheme === Schemas.ProX-CodeFileResource) {
				if (!isAllowedVsCodeFileRequest(details)) {
					this.logService.error('Blocked ProX-Code-file request', details.url);
					return callback({ cancel: true });
				}
			}
		...
		// https://github.com/microsoft/ProX-Code/issues/97564
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			...
			if (contentTypes && Array.isArray(contentTypes)) {
				...
				// http://127.0.0.1:<port>/ProX-Code-remote-resource?path=
		...
		// https://github.com/microsoft/ProX-Code-remote-release/issues/9246
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			if (details.url.startsWith('https://ProX-Code.download.prss.microsoft.com/')) {
		...
		if (typeof defaultSession.setCodeCachePath === 'function' && this.environmentMainService.codeCachePath) {
			...
			// (https://github.com/microsoft/ProX-Code/issues/120655)
	...
	private registerListeners(): void {
		...
		app.on('accessibility-support-changed', (event, accessibilitySupportEnabled) => {
			this.windowsMainService?.sendToAll('ProX-Code:accessibilitySupportChanged', accessibilitySupportEnabled);
		});
		...
		validatedIpcMain.handle('ProX-Code:fetchShellEnv', event => {
		...
		validatedIpcMain.on('ProX-Code:toggleDevTools', event => event.sender.toggleDevTools());
		validatedIpcMain.on('ProX-Code:openDevTools', event => event.sender.openDevTools());

		validatedIpcMain.on('ProX-Code:reloadWindow', event => event.sender.reload());

		validatedIpcMain.handle('ProX-Code:notifyZoomLevel', async (event, zoomLevel: number | undefined) => {
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting ProX Code');
		...
		// "com.microsoft.", which breaks native tabs for ProX Code when using this
		// identifier (from the official build).
		// Explicitly opt out of the patch here before creating any windows.
		// See: https://github.com/microsoft/ProX-Code/issues/35361#issuecomment-399794085
		try {
		...
		// Setup ProX-Code-remote-resource protocol handler
	...
	private async shouldBlockOpenable(openable: IWindowOpenable, windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService): Promise<boolean> {
		...
		if (openableUri.scheme !== Schemas.file && openableUri.scheme !== Schemas.ProX-CodeRemote) {
			...
			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//
			// NOTE: we currently only ask for confirmation for `file` and `ProX-Code-remote`
		...
		if (checkboxChecked) {
			// Due to https://github.com/microsoft/ProX-Code/issues/195436, we can only
			...
			const request = { channel: 'ProX-Code:disablePromptForProtocolHandling', args: openableUri.scheme === Schemas.file ? 'local' : 'remote' };
	...
	private getWindowOpenableFromProtocolUrl(uri: URI): IWindowOpenable | undefined {
		...
		// Remote path
			...
			// Example conversion:
			// From: ProX-Code://ProX-Code-remote/wsl+ubuntu/mnt/c/GitDevelopment/monaco
			//   To: ProX-Code-remote://wsl+ubuntu/mnt/c/GitDevelopment/monaco
			...
			if (params.get('windowId') === '_blank') {
				...
				// https://github.com/microsoft/ProX-Code/issues/191902
	...
	private async handleProtocolUrl(windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService, urlService: IURLService, uri: URI, options?: IOpenURLOptions): Promise<boolean> {
		...
		// Support 'workspace' URLs (https://github.com/microsoft/ProX-Code/issues/124263)
	...
	private setupSharedProcess(machineId: string, sqmId: string, devDeviceId: string): { sharedProcessReady: Promise<MessagePortClient>; sharedProcessClient: Promise<MessagePortClient> } {
		...
		this._register(sharedProcess.onDidCrash(() => this.windowsMainService?.sendToFocused('ProX-Code:reportSharedProcessCrash')));
	...
	private afterWindowOpen(): void {
		...
		protocol.registerHttpProtocol(Schemas.ProX-CodeRemoteResource, (request, callback) => {
			callback({
				url: request.url.replace(/^ProX-Code-remote-resource:/, 'http:'),
		...
		if (isMacintosh && app.runningUnderARM64Translation) {
			this.windowsMainService?.sendToFocused('ProX-Code:showTranslatedBuildWarning');
		}
	}
	...
	private async installMutex(): Promise<void> {
		...
		if (isWindows && win32MutexName) {
			try {
				const WindowsMutex = await import('@ProX-Code/windows-mutex');
	...
	private async resolveShellEnvironment(args: NativeParsedArgs, env: IProcessEnvironment, notifyOnError: boolean): Promise<typeof process.env> {
		try {
			...
			if (notifyOnError) {
				this.windowsMainService?.sendToFocused('ProX-Code:showResolveShellEnvError', errorMessage);
			} else {
	...
	private async updateCrashReporterEnablement(): Promise<void> {
		...
		try {
			...
			this.windowsMainService?.sendToFocused('ProX-Code:showArgvParseWarning');
		}
	}
	...
	private eventuallyAfterWindowOpen(): void {
		...
		// Refs: https://github.com/microsoft/ProX-Code/issues/234064
