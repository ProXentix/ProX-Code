...
const { ProX-CodeWebEntryPoints, ProX-CodeWebResourceIncludes, createVSCodeWebFileContentMapper } = require('./gulpfile.ProX-Code.web');
...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const localWorkspaceExtensions = glob.sync('extensions/*/package.json')
			...
			.filter(name => name !== 'ProX-Code-api-tests' && name !== 'ProX-Code-test-resolver'); // Do not ship the test extensions
...
['reh', 'reh-web'].forEach(type => {
	const optimizeTask = task.define(`optimize-ProX-Code-${type}`, task.series(
		util.rimraf(`out-ProX-Code-${type}`),
		optimize.optimizeTask(
			{
				out: `out-ProX-Code-${type}`,
				amd: {
					src: 'out-build',
	...
	const minifyTask = task.define(`minify-ProX-Code-${type}`, task.series(
		optimizeTask,
		util.rimraf(`out-ProX-Code-${type}-min`),
		optimize.minifyTask(`out-ProX-Code-${type}`, `https://ticino.blob.core.windows.net/sourcemaps/${commit}/core`)
	));
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			const sourceFolderName = `out-ProX-Code-${type}${dashed(minified)}`;
			const destinationFolderName = `ProX-Code-${type}${dashed(platform)}${dashed(arch)}`;

			const serverTaskCI = task.define(`ProX-Code-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}-ci`, task.series(
			...
			const serverTask = task.define(`ProX-Code-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
				compileBuildTask,
