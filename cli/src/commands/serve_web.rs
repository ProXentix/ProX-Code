...
/// Number of seconds where, if there's no connections to a ProX Code server,
...
const SECRET_KEY_MINT_PATH: &str = "_ProX-Code-cli/mint-key";
/// Cookie set to the `SECRET_KEY_MINT_PATH`
const PATH_COOKIE_NAME: &str = "ProX-Code-secret-key-path";
/// HTTP-only cookie where the client's secret half is stored.
const SECRET_KEY_COOKIE_NAME: &str = "ProX-Code-cli-secret-half";

/// Implements the ProX-Code "server of servers". Clients who go to the URI get
/// served the latest version of the ProX Code server whenever they load the
/// page. The ProX Code server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new ProX Code Server versions.
...
/// Gets the release info from the ProX Code path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the ProX Code server, triggering
