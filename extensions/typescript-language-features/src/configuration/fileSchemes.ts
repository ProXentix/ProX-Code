...
export const chatCodeBlock = 'ProX-Code-chat-code-block';

export function getSemanticSupportedSchemes() {
	const alwaysSupportedSchemes = [
		untitled,
		walkThroughSnippet,
		ProX-CodeNotebookCell,
		chatCodeBlock,
	];

	if (isWeb()) {
		return [
			...(ProX-Code.workspace.workspaceFolders ?? []).map(folder => folder.uri.scheme),
...
export function isOfScheme(uri: ProX-Code.Uri, ...schemes: string[]): boolean {
