...
export class WebClientServer {
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
		...
		const resolveWorkspaceURI = (defaultLocation?: string) => defaultLocation && URI.file(path.resolve(defaultLocation)).with({ scheme: Schemas.ProX-CodeRemote, authority: remoteAuthority });
		...
		const locale = cookies['ProX-Code.nls.locale'] || req.headers['accept-language']?.split(',')[0]?.toLowerCase() || 'en';
		...
		if (useTestResolver) {
			...
			for (const extensionPath of ['ProX-Code-test-resolver', 'github-authentication']) {
		...
		const cspDirectives = [
			...
			'child-src \'self\';',
			`frame-src 'self' https://*.ProX-Code-cdn.net data:;`,
