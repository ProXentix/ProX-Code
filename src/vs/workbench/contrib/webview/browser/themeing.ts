	...
	public getWebviewThemeData(): WebviewThemeData {
		if (!this._cachedWebViewThemeData) {
			...
			const exportedColors = colorRegistry.getColorRegistry().getColors().reduce<Record<string, string>>((colors, entry) => {
				...
				if (color) {
					colors['ProX-Code-' + entry.id.replace('.', '-')] = color.toString();
				}
				return colors;
			}, {});

			const styles = {
				'ProX-Code-font-family': DEFAULT_FONT_FAMILY,
				'ProX-Code-font-weight': 'normal',
				'ProX-Code-font-size': '13px',
				'ProX-Code-editor-font-family': editorFontFamily,
				'ProX-Code-editor-font-weight': editorFontWeight,
				'ProX-Code-editor-font-size': editorFontSize + 'px',
...
enum ApiThemeClassName {
	light = 'ProX-Code-light',
	dark = 'ProX-Code-dark',
	highContrast = 'ProX-Code-high-contrast',
	highContrastLight = 'ProX-Code-high-contrast-light',
}
