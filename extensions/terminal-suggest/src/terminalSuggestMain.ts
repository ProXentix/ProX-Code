...
export async function activate(context: ProX-Code.ExtensionContext) {
	...
	context.subscriptions.push(ProX-Code.window.registerTerminalCompletionProvider({
		id: 'terminal-suggest',
		async provideTerminalCompletions(terminal: ProX-Code.Terminal, terminalContext: ProX-Code.TerminalCompletionContext, token: ProX-Code.CancellationToken): Promise<ProX-Code.TerminalCompletionItem[] | ProX-Code.TerminalCompletionList | undefined> {
			...
			if (terminal.shellIntegration?.env) {
				...
				if (homeDirCompletion && terminal.shellIntegration.env?.value?.HOME) {
					homeDirCompletion.documentation = getFriendlyResourcePath(ProX-Code.Uri.file(terminal.shellIntegration.env.value.HOME), pathSeparator, ProX-Code.TerminalCompletionItemKind.Folder);
					homeDirCompletion.kind = ProX-Code.TerminalCompletionItemKind.Folder;
				}
			}

			if (terminal.shellIntegration?.cwd && (result.filesRequested || result.foldersRequested)) {
				return new ProX-Code.TerminalCompletionList(result.items, { filesRequested: result.filesRequested, foldersRequested: result.foldersRequested, fileExtensions: result.fileExtensions, cwd: result.cwd ?? terminal.shellIntegration.cwd, env: terminal.shellIntegration?.env?.value, });
			}
			return result.items;
		}
	}, '/', '\\'));
 ...
 */
export async function resolveCwdFromPrefix(prefix: string, currentCwd?: ProX-Code.Uri): Promise<ProX-Code.Uri | undefined> {
	if (!currentCwd) {
		return;
	}
	try {
...
export async function getCompletionItemsFromSpecs(
	specs: Fig.Spec[],
	terminalContext: ProX-Code.TerminalCompletionContext,
	availableCommands: ICompletionResource[],
	prefix: string,
	tokenType: TokenType,
	shellIntegrationCwd: ProX-Code.Uri | undefined,
	env: Record<string, string>,
	name: string,
	token?: ProX-Code.CancellationToken,
	executeExternals?: IFigExecuteExternals,
): Promise<{ items: ProX-Code.TerminalCompletionItem[]; filesRequested: boolean; foldersRequested: boolean; fileExtensions?: string[]; cwd?: ProX-Code.Uri }> {
	const items: ProX-Code.TerminalCompletionItem[] = [];
	...
	let cwd: ProX-Code.Uri | undefined;
