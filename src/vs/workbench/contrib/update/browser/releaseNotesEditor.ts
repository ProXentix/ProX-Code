			...
			<head>
				...
				<style nonce="${nonce}">
					...
					code:has(.codesetting) {
						background-color: var(--ProX-Code-textPreformat-background);
						color: var(--ProX-Code-textPreformat-foreground);
						padding-left: 1px;
						margin-right: 3px;
						padding-right: 0px;
					}
					...
					code:has(.codesetting):focus {
						border: 1px solid var(--ProX-Code-button-border, transparent);
					}
					...
					.codesetting svg {
						...
						text-align: center;
						cursor: pointer;
						border: 1px solid var(--ProX-Code-button-secondaryBorder, transparent);
					...
					.codesetting:hover {
						color: var(--ProX-Code-textPreformat-foreground) !important;
					...
					.codesetting:focus {
						outline: 0 !important;
						text-decoration: none !important;
						color: var(--ProX-Code-button-hoverForeground) !important;
					}
					.codesetting .separator {
						...
						margin-bottom: -3px;
						display: inline-block;
						background-color: var(--ProX-Code-editor-background);
						font-size: 12px;
						margin-right: 4px;
					}
				...
				<script nonce="${nonce}">
					...
					window.addEventListener('click', event => {
						...
						if (href && (href.startsWith('${Schemas.codeSetting}'))) {
							ProX-Code.postMessage({ type: 'clickSetting', value: { uri: href, x: event.clientX, y: event.clientY }});
						}
					});
					...
					window.addEventListener('keypress', event => {
						if (event.keyCode === 13) {
							if (event.target.children.length > 0 && event.target.children[0].href) {
								...
								ProX-Code.postMessage({ type: 'clickSetting', value: { uri: event.target.children[0].href, x: clientRect.right , y: clientRect.bottom }});
							}
						}
					});

					input.addEventListener('change', event => {
						ProX-Code.postMessage({ type: 'showReleaseNotes', value: input.checked }, '*');
					});
				</script>
			</body>
		</html>`;
