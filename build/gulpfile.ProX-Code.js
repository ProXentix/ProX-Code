...
const ProX-CodeResourceIncludes = [
...
const ProX-CodeResources = [

	// Includes
	...ProX-CodeResourceIncludes,
...
const bundleVSCodeTask = task.define('bundle-ProX-Code', task.series(
	util.rimraf('out-ProX-Code'),
	...
	optimize.bundleTask(
		{
			...
			esm: {
				src: 'out-build',
				entryPoints: [
					...ProX-CodeEntryPoints,
					...bootstrapEntryPoints
				],
				resources: ProX-CodeResources,
...
const sourceMappingURLBase = `https://main.ProX-Code-cdn.net/sourcemaps/${commit}`;
const minifyVSCodeTask = task.define('minify-ProX-Code', task.series(
	bundleVSCodeTask,
	util.rimraf('out-ProX-Code-min'),
	optimize.minifyTask('out-ProX-Code', `${sourceMappingURLBase}/core`)
));
...
const coreCI = task.define('core-ci', task.series(
	...
	task.parallel(
		gulp.task('minify-ProX-Code'),
		gulp.task('minify-ProX-Code-reh'),
		gulp.task('minify-ProX-Code-reh-web'),
	)
));
gulp.task(coreCI);
...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	return () => {
		const electron = require('@ProX-Code/gulp-electron');
		...
		const api = gulp.src('src/ProX-Code-dts/ProX-Code.d.ts').pipe(rename('out/ProX-Code-dts/ProX-Code.d.ts'));
		...
		let result = all
			...
			.pipe(filter(['**', '!**/.github/**'], { dot: true })) // https://github.com/microsoft/ProX-Code/issues/116523
		...
		if (platform === 'win32') {
			...
			result = es.merge(result, gulp.src('resources/win32/bin/code.sh', { base: 'resources/win32' })
				...
				.pipe(replace('@@APPNAME@@', product.applicationName))
				.pipe(replace('@@SERVERDATAFOLDER@@', product.serverDataFolderName || '.ProX-Code-remote'))
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [ProX-Code, ProX-CodeMin] = ['', 'min'].map(minified => {
		const sourceFolderName = `out-ProX-Code${dashed(minified)}`;
		...
		const ProX-CodeTaskCI = task.define(`ProX-Code${dashed(platform)}${dashed(arch)}${dashed(minified)}-ci`, task.series(...tasks));
		gulp.task(ProX-CodeTaskCI);

		const ProX-CodeTask = task.define(`ProX-Code${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
		...
		gulp.task(ProX-CodeTask);

		return ProX-CodeTask;
	});

	if (process.platform === platform && process.arch === arch) {
		gulp.task(task.define('ProX-Code', task.series(ProX-Code)));
		gulp.task(task.define('ProX-Code-min', task.series(ProX-CodeMin)));
	}
});
...
gulp.task(task.define(
	...
	task.series(
		...
		function () {
			...
			).pipe(vfs.dest('../ProX-Code-translations-export'));
		}
	)
));

gulp.task('ProX-Code-translations-import', function () {
	const options = minimist(process.argv.slice(2), {
		string: 'location',
		default: {
			location: '../ProX-Code-translations-import'
		}
	});
	return es.merge([...i18n.defaultLanguages, ...i18n.extraLanguages].map(language => {
		const id = language.id;
		return gulp.src(`${options.location}/${id}/ProX-Code-setup/messages.xlf`)
