...
const redirectUrl = 'https://ProX-Code.dev/redirect';
...
export class AzureActiveDirectoryService {
	// For details on why this is set to 2/3... see https://github.com/microsoft/ProX-Code/issues/133201#issuecomment-966668197
	private static REFRESH_TIMEOUT_MODIFIER = 1000 * 2 / 3;
	private static POLLING_CONSTANT = 1000 * 60 * 30;

	private _tokens: IToken[] = [];
	private _refreshTimeouts: Map<string, NodeJS.Timeout> = new Map<string, NodeJS.Timeout>();
	private _sessionChangeEmitter: ProX-Code.EventEmitter<ProX-Code.AuthenticationProviderAuthenticationSessionsChangeEvent> = new ProX-Code.EventEmitter<ProX-Code.AuthenticationProviderAuthenticationSessionsChangeEvent>();

	// Used to keep track of current requests when not using the local server approach.
	private _pendingNonces = new Map<string, string[]>();
	private _codeExchangePromises = new Map<string, Promise<ProX-Code.AuthenticationSession>>();
	...
	constructor(
		private readonly _logger: ProX-Code.LogOutputChannel,
		_context: ProX-Code.ExtensionContext,
		...
		_context.subscriptions.push(ProX-Code.window.onDidChangeWindowState(async (e) => e.focused && await this.storePendingTokens()));

		// In the event that a window isn't focused for a long time, we should still try to store the tokens at some point.
		const timer = new IntervalTimer();
		timer.cancelAndSet(
			() => !ProX-Code.window.state.focused && this.storePendingTokens(),
		...
		const refreshes = sessions.map(async session => {
			...
			try {
				...
				if (e.message === REFRESH_NETWORK_FAILURE) {
					...
					ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('You have been signed out because reading stored authentication information failed.'));
	...
	public get onDidChangeSessions(): ProX-Code.Event<ProX-Code.AuthenticationProviderAuthenticationSessionsChangeEvent> {
		return this._sessionChangeEmitter.event;
	}

	public getSessions(scopes: string[] | undefined, { account, issuer }: ProX-Code.AuthenticationProviderSessionOptions = {}): Promise<ProX-Code.AuthenticationSession[]> {
		if (!scopes) {
	...
	private async doGetSessions(scopeData: IScopeData, account?: ProX-Code.AuthenticationSessionAccountInformation): Promise<ProX-Code.AuthenticationSession[]> {
		...
		return results
			...
			.map(result => (result as PromiseFulfilledResult<ProX-Code.AuthenticationSession>).value);
	}

	public createSession(scopes: string[], { account, issuer }: ProX-Code.AuthenticationProviderSessionOptions = {}): Promise<ProX-Code.AuthenticationSession> {
		let modifiedScopes = [...scopes];
		if (!modifiedScopes.includes('openid')) {
			modifiedScopes.push('openid');
		}
	...
	private async doCreateSession(scopeData: IScopeData, account?: ProX-Code.AuthenticationSessionAccountInformation): Promise<ProX-Code.AuthenticationSession> {
		this._logger.info(`[${scopeData.scopeStr}] Creating session` + account ? ` for ${account?.label}` : '');

		const runsRemote = ProX-Code.env.remoteName !== undefined;
		const runsServerless = ProX-Code.env.remoteName === undefined && ProX-Code.env.uiKind === ProX-Code.UIKind.Web;
		...
		return await ProX-Code.window.withProgress({ location: ProX-Code.ProgressLocation.Notification, title: ProX-Code.l10n.t('Signing in to your account...'), cancellable: true }, async (_progress, token) => {
	...
	private async createSessionWithLocalServer(scopeData: IScopeData, loginHint: string | undefined, token: ProX-Code.CancellationToken): Promise<ProX-Code.AuthenticationSession> {
		...
		let codeToExchange;
		try {
			ProX-Code.env.openExternal(ProX-Code.Uri.parse(`http://127.0.0.1:${server.port}/signin?nonce=${encodeURIComponent(server.nonce)}`));
	...
	private async createSessionWithoutLocalServer(scopeData: IScopeData, loginHint: string | undefined, token: ProX-Code.CancellationToken): Promise<ProX-Code.AuthenticationSession> {
		this._logger.trace(`[${scopeData.scopeStr}] Starting login flow without local server`);
		let callbackUri = await ProX-Code.env.asExternalUri(ProX-Code.Uri.parse(`${ProX-Code.env.uriScheme}://ProX-Code.microsoft-authentication`));
		...
		const uri = ProX-Code.Uri.parse(signInUrl.toString());
		ProX-Code.env.openExternal(uri);
	...
	public async removeSessionById(sessionId: string, writeToDisk: boolean = true): Promise<ProX-Code.AuthenticationSession | undefined> {
	...
	private async removeSessionByIToken(token: IToken, writeToDisk: boolean = true): Promise<ProX-Code.AuthenticationSession | undefined> {
	 ...
	 */
	private convertToSessionSync(token: IToken): ProX-Code.AuthenticationSession {
		return {
			id: token.sessionId,
	...
	private async convertToSession(token: IToken, scopeData: IScopeData): Promise<ProX-Code.AuthenticationSession> {
	...
	private async handleCodeResponse(scopeData: IScopeData): Promise<ProX-Code.AuthenticationSession> {
		let uriEventListener: ProX-Code.Disposable;
		return new Promise((resolve: (value: ProX-Code.AuthenticationSession) => void, reject) => {
			uriEventListener = this._uriHandler.event(async (uri: ProX-Code.Uri) => {
				try {
	...
	private async handleCodeInputBox(inputBox: ProX-Code.InputBox, verifier: string, scopeData: IScopeData): Promise<ProX-Code.AuthenticationSession> {
		this._logger.trace(`[${scopeData.scopeStr}] Starting login flow with input box`);
		inputBox.ignoreFocusOut = true;
		inputBox.title = ProX-Code.l10n.t('Microsoft Authentication');
		inputBox.prompt = ProX-Code.l10n.t('Provide the authorization code to complete the sign in flow.');
		inputBox.placeholder = ProX-Code.l10n.t('Paste authorization code here...');
		return new Promise((resolve: (value: ProX-Code.AuthenticationSession) => void, reject) => {
			inputBox.show();
	...
	private async exchangeCodeForSession(code: string, codeVerifier: string, scopeData: IScopeData): Promise<ProX-Code.AuthenticationSession> {
	...
	private async fetchTokenResponse(postData: string, scopeData: IScopeData): Promise<ITokenResponse> {
		...
		if (this._env.activeDirectoryEndpointUrl !== defaultActiveDirectoryEndpointUrl) {
			...
			const proxyEndpoints: { [providerId: string]: string } | undefined = await ProX-Code.commands.executeCommand('workbench.getCodeExchangeProxyEndpoints');
	...
	private async storeToken(token: IToken, scopeData: IScopeData): Promise<void> {
		if (!ProX-Code.window.state.focused) {
