...
import type { IRawTheme } from 'ProX-Code-textmate';
...
export class ThreadedBackgroundTokenizerFactory implements IDisposable {
	...
	private async _createWorkerProxy(): Promise<Proxied<TextMateTokenizationWorker> | null> {
		const onigurumaModuleLocation: AppResourcePath = `${nodeModulesPath}/ProX-Code-oniguruma`;
		const onigurumaModuleLocationAsar: AppResourcePath = `${nodeModulesAsarPath}/ProX-Code-oniguruma`;
