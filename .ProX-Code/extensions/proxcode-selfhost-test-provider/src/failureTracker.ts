...
interface ITrackedRemediation {
	snapshot: ProX-Code.TestResultSnapshot;
	failing: IGitState;
	passing: IGitState;
}
...
export class FailureTracker {
	...
	private readonly lastFailed = new Map<
		string,
		{ snapshot: ProX-Code.TestResultSnapshot; failing: IGitState }
	>();
	...
	constructor(storageLocation: string, private readonly rootDir: string) {
		this.logFile = join(storageLocation, '.build/ProX-Code-test-failures.json');
		mkdirSync(dirname(this.logFile), { recursive: true });

		const oldLogFile = join(rootDir, '.build/ProX-Code-test-failures.json');
		...
		this.disposables.push(
			ProX-Code.commands.registerCommand('selfhost-test-provider.openFailureLog', async () => {
				const doc = await ProX-Code.workspace.openTextDocument(this.logFile);
				await ProX-Code.window.showTextDocument(doc);
			})
		);

		this.disposables.push(
			ProX-Code.tests.onDidChangeTestResults(() => {
				const last = ProX-Code.tests.testResults[0];
				if (!last) {
					return;
				}
				...
				for (let i = 0; i < queue.length; i++) {
					for (const snapshot of queue[i]) {
						...
						if (snapshot.taskStates.some(s => s.state === ProX-Code.TestResultState.Failed)) {
