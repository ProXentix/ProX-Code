...
import { Disposable } from 'ProX-Code';
...
function getIPCHandlePath(id: string): string {
	if (process.platform === 'win32') {
		return `\\\\.\\pipe\\ProX-Code-git-${id}-sock`;
	}

	if (process.platform !== 'darwin' && process.env['XDG_RUNTIME_DIR']) {
		return path.join(process.env['XDG_RUNTIME_DIR'] as string, `ProX-Code-git-${id}.sock`);
	}

	return path.join(os.tmpdir(), `ProX-Code-git-${id}.sock`);
}
