...
class OriginDocumentMergeConflictTracker implements interfaces.IDocumentMergeConflictTracker {
	...
	getConflicts(document: ProX-Code.TextDocument): PromiseLike<interfaces.IDocumentMergeConflict[]> {
	...
	forget(document: ProX-Code.TextDocument) {
		this.parent.forget(document);
	}
}

export default class DocumentMergeConflictTracker implements ProX-Code.Disposable, interfaces.IDocumentMergeConflictTrackerService {
	private cache: Map<string, ScanTask> = new Map();
	private delayExpireTime: number = 0;

	constructor(private readonly telemetryReporter: TelemetryReporter) { }

	getConflicts(document: ProX-Code.TextDocument, origin: string): PromiseLike<interfaces.IDocumentMergeConflict[]> {
		// Attempt from cache
	...
	isPending(document: ProX-Code.TextDocument, origin: string): boolean {
		if (!document) {
			return false;
		}
	...
	private getConflictsOrEmpty(document: ProX-Code.TextDocument, _origins: string[]): interfaces.IDocumentMergeConflict[] {
	...
	private getCacheKey(document: ProX-Code.TextDocument): string | null {
		if (document.uri) {
