...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private readonly validExtensions = new Set(['.svg', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.mp4', '.otf', '.ttf']); // https://github.com/microsoft/ProX-Code/issues/119384
	...
	constructor(
		...
		// - storage    : all files in global and workspace storage (https://github.com/microsoft/ProX-Code/issues/116735)
	...
	private handleProtocols(): void {
		...
		// Register ProX-Code-file:// handler
	...
	private handleResourceRequest(request: Electron.ProtocolRequest, callback: ProtocolCallback): void {
		...
		// Refs https://github.com/microsoft/ProX-Code/issues/148541#issuecomment-2670891511
	...
	private requestToNormalizedFilePath(request: Electron.ProtocolRequest): string {
		...
		//     `ProX-Code-file:` URI to a `file:` URI.
	...
	createIPCObjectUrl<T>(): IIPCObjectUrl<T> {
		...
		const resource = URI.from({
			scheme: 'ProX-Code', // used for all our IPC communication (ProX-Code:<channel>)
			path: generateUuid()
		});
