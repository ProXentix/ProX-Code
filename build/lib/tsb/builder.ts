							...
							if (sourcemapFile) {
								...
								if (snapshot instanceof VinylScriptSnapshot && snapshot.sourceMap) {
									...
									if (didChange) {
										...
										// const filename = '/Users/jrieken/Code/ProX-Code/src2/' + vinyl.relative + '.map';
										// fs.promises.mkdir(path.dirname(filename), { recursive: true }).then(async () => {
										// 	await fs.promises.writeFile(filename, smg.toString());
										// 	await fs.promises.writeFile('/Users/jrieken/Code/ProX-Code/src2/' + vinyl.relative, vinyl.contents);
										// });
									}
								}
