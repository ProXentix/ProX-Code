...
export class FileWatcherManager {
	...
	create(id: number, uri: ProX-Code.Uri, watchParentDirs: boolean, listeners: { create?: () => void; change?: () => void; delete?: () => void }): void {
		// Non-writable file systems do not support file watching
		if (!ProX-Code.workspace.fs.isWritableFileSystem(uri.scheme)) {
			return;
		}

		const watcher = ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(uri, '*'), !listeners.create, !listeners.change, !listeners.delete);
