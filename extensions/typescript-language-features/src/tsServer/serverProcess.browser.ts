...
import { ServiceConnection } from '@ProX-Code/sync-api-common/browser';
import { ApiService, Requests } from '@ProX-Code/sync-api-service';
...
export class WorkerServerProcessFactory implements TsServerProcessFactory {
	constructor(
		private readonly _extensionUri: ProX-Code.Uri,
	...
	public constructor(
		...
		this._watcher.onmessage = (event: MessageEvent<BrowserWatchEvent>) => {
			switch (event.data.type) {
				...
				case 'watchDirectory':
				case 'watchFile': {
					this._watches.create(event.data.id, ProX-Code.Uri.from(event.data.uri), /*watchParentDirs*/ true, !!event.data.recursive, {
		...
		new ApiService('ProX-Code-wasm-typescript', connection);
