...
export class AtaProgressReporter extends Disposable {
	...
	private _onBegin(eventId: number): void {
		...
		ProX-Code.window.withProgress({
			...
			title: ProX-Code.l10n.t("Fetching data for better TypeScript IntelliSense")
		}, () => promise);
	}
	...
	private async onTypesInstallerInitializationFailed() {
		const config = ProX-Code.workspace.getConfiguration('typescript');

		if (config.get<boolean>('check.npmIsInstalled', true)) {
			const dontShowAgain: ProX-Code.MessageItem = {
				title: ProX-Code.l10n.t("Don't Show Again"),
			};
			const selected = await ProX-Code.window.showWarningMessage(
				ProX-Code.l10n.t(
