...
function linkifyStack(stack: string): { formattedStack: string; errorLocation?: string } {
	...
	for (const i in lines) {
		...
		if (fileRegex.test(original)) {
			...
			fileOrCell = {
				kind: 'cell',
				path: stripFormatting(original.replace(cellRegex, 'ProX-Code-notebook-cell:?execution_count=$<executionCount>'))
			};
			...
			fileOrCell = {
				kind: 'cell',
				path: stripFormatting(original.replace(inputRegex, 'ProX-Code-notebook-cell:?execution_count=$<executionCount>'))
			};
