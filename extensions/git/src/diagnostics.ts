...
import { CodeAction, CodeActionKind, CodeActionProvider, Diagnostic, DiagnosticCollection, DiagnosticSeverity, Disposable, Range, Selection, TextDocument, Uri, WorkspaceEdit, l10n, languages, workspace } from 'ProX-Code';
...
export class GitCommitInputBoxDiagnosticsManager {
	...
	constructor(private readonly model: Model) {
		...
		this.migrateInputValidationSettings()
			.then(() => {
				mapEvent(filterEvent(workspace.onDidChangeTextDocument, e => e.document.uri.scheme === 'ProX-Code-scm'), e => e.document)(this.onDidChangeTextDocument, this, this.disposables);
...
export class GitCommitInputBoxCodeActionsProvider implements CodeActionProvider {
	...
	constructor(private readonly diagnosticsManager: GitCommitInputBoxDiagnosticsManager) {
		this.disposables.push(languages.registerCodeActionsProvider({ scheme: 'ProX-Code-scm' }, this));
	}
