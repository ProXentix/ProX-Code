...
export async function resolveElectronConfiguration(options: LaunchOptions): Promise<IElectronConfiguration> {
	...
	if (remote) {
		...
		args[0] = `--${workspacePath.endsWith('.code-workspace') ? 'file' : 'folder'}-uri=ProX-Code-remote://test+test/${URI.file(workspacePath).path}`;

		if (codePath) {
			// running against a build: copy the test resolver extension
			await measureAndLog(() => copyExtension(root, extensionsPath, 'ProX-Code-test-resolver'), 'copyExtension(ProX-Code-test-resolver)', logger);
		}
		args.push('--enable-proposed-api=ProX-Code.ProX-Code-test-resolver');
