...
export class PathExecutableCache implements ProX-Code.Disposable {
	...
	constructor() {
		if (isWindows) {
			this._cachedWindowsExeExtensions = ProX-Code.workspace.getConfiguration(SettingsIds.SuggestPrefix).get(SettingsIds.CachedWindowsExecutableExtensionsSuffixOnly);
			this._disposables.push(ProX-Code.workspace.onDidChangeConfiguration(e => {
	...
	private async _getFilesInPath(path: string, pathSeparator: string, labels: Set<string>): Promise<Set<ICompletionResource> | undefined> {
		try {
			...
			const fileResource = ProX-Code.Uri.file(path);
			const files = await ProX-Code.workspace.fs.readDirectory(fileResource);
			for (const [file, fileType] of files) {
				const formattedPath = getFriendlyResourcePath(ProX-Code.Uri.joinPath(fileResource, file), pathSeparator);
				if (!labels.has(file) && fileType !== ProX-Code.FileType.Unknown && fileType !== ProX-Code.FileType.Directory && await isExecutable(formattedPath, this._cachedWindowsExeExtensions)) {
					result.add({ label: file, documentation: formattedPath, kind: ProX-Code.TerminalCompletionItemKind.Method });
					labels.add(file);
				}
			}
			return result;
...
export async function watchPathDirectories(context: ProX-Code.ExtensionContext, env: ITerminalEnvironment, pathExecutableCache: PathExecutableCache | undefined): Promise<void> {
	...
	for (const dir of pathDirectories) {
		try {
			...
			context.subscriptions.push(new ProX-Code.Disposable(() => {
				try {
					watcher.close();
