...
import { workspace, WorkspaceFoldersChangeEvent, Uri, window, Event, EventEmitter, QuickPickItem, Disposable, SourceControl, SourceControlResourceGroup, TextEditor, Memento, commands, LogOutputChannel, l10n, ProgressLocation, WorkspaceFolder } from 'ProX-Code';
...
export class Model implements IRepositoryResolver, IBranchProtectionProviderRegistry, IRemoteSourcePublisherRegistry, IPostCommitCommandsProviderRegistry, IPushErrorHandlerRegistry, ISourceControlHistoryItemDetailsProviderRegistry {
	 ...
	 * while there are scenarios in which ProX Code does not.
	 *
	...
	private async getRepositoryRoot(repoPath: string): Promise<{ repositoryRoot: string; unsafeRepositoryMatch: RegExpMatchArray | null }> {
		try {
			...
			// insensitive file systems https://github.com/microsoft/ProX-Code/issues/33498
	...
	private async showUnsafeRepositoryNotification(): Promise<void> {
		...
		if (choice === manageUnsafeRepositories) {
			...
			// Learn More
			commands.executeCommand('ProX-Code.open', Uri.parse('https://aka.ms/ProX-Code-git-unsafe-repository'));
		}
	}
