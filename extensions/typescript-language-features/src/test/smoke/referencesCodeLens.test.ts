...
async function updateConfig(newConfig: VsCodeConfiguration): Promise<VsCodeConfiguration> {
	...
	const config = ProX-Code.workspace.getConfiguration(undefined);
...
suite('TypeScript References', () => {
	...
	const _disposables: ProX-Code.Disposable[] = [];
	let oldConfig: { [key: string]: any } = {};

	setup(async () => {
		// the tests assume that typescript features are registered
		await ProX-Code.extensions.getExtension('ProX-Code.typescript-language-features')!.activate();
	...
	test('Should show on basic class', async () => {
		const testDocumentUri = ProX-Code.Uri.parse('untitled:test1.ts');
	...
	test('Should show on basic class properties', async () => {
		const testDocumentUri = ProX-Code.Uri.parse('untitled:test2.ts');
		await createTestEditor(testDocumentUri,
			`class Foo {`,
			`	prop: number;`,
			`	meth(): void {}`,
	...
	test('Should not show on const property', async () => {
		const testDocumentUri = ProX-Code.Uri.parse('untitled:test3.ts');
		await createTestEditor(testDocumentUri,
			`const foo = {`,
			`	prop: 1;`,
			`	meth(): void {}`,
	...
	test.skip('Should not show duplicate references on ES5 class (https://github.com/microsoft/ProX-Code/issues/90396)', async () => {
		const testDocumentUri = ProX-Code.Uri.parse('untitled:test3.js');
		await createTestEditor(testDocumentUri,
			`function A() {`,
			`    console.log("hi");`,
			`}`,
...
function getCodeLenses(document: ProX-Code.Uri): Thenable<readonly ProX-Code.CodeLens[] | undefined> {
	return ProX-Code.commands.executeCommand<readonly ProX-Code.CodeLens[]>('ProX-Code.executeCodeLensProvider', document, 100);
}
