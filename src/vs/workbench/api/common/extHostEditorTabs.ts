...
class ExtHostEditorTab {
	private _apiObject: ProX-Code.Tab | undefined;
	...
	get apiObject(): ProX-Code.Tab {
		if (!this._apiObject) {
			...
			this._apiObject = Object.freeze<ProX-Code.Tab>(obj);
		}
...
class ExtHostEditorTabGroup {
	...
	private _apiObject: ProX-Code.TabGroup | undefined;
	...
	get apiObject(): ProX-Code.TabGroup {
		if (!this._apiObject) {
			// Don't want to lose reference to parent `this` in the getters
			const that = this;
			const obj: ProX-Code.TabGroup = {
				get isActive() {
			...
			this._apiObject = Object.freeze<ProX-Code.TabGroup>(obj);
		}
		return this._apiObject;
	}
...
export class ExtHostEditorTabs implements IExtHostEditorTabs {
	...
	private readonly _onDidChangeTabs = new Emitter<ProX-Code.TabChangeEvent>();
	private readonly _onDidChangeTabGroups = new Emitter<ProX-Code.TabGroupChangeEvent>();
	...
	private _apiObject: ProX-Code.TabGroups | undefined;
	...
	get tabGroups(): ProX-Code.TabGroups {
		if (!this._apiObject) {
			...
			const obj: ProX-Code.TabGroups = {
				...
				close: async (tabOrTabGroup: ProX-Code.Tab | readonly ProX-Code.Tab[] | ProX-Code.TabGroup | readonly ProX-Code.TabGroup[], preserveFocus?: boolean) => {
					...
					if (isTabGroup(tabsOrTabGroups[0])) {
						return this._closeGroups(tabsOrTabGroups as ProX-Code.TabGroup[], preserveFocus);
					} else {
						return this._closeTabs(tabsOrTabGroups as ProX-Code.Tab[], preserveFocus);
					}
				},
				// move: async (tab: ProX-Code.Tab, viewColumn: ViewColumn, index: number, preserveFocus?: boolean) => {
	...
	$acceptEditorTabModel(tabGroups: IEditorTabGroupDto[]): void {
		...
		const closed: ProX-Code.TabGroup[] = this._extHostTabGroups.filter(group => diff.removed.includes(group.groupId)).map(group => group.apiObject);
		const opened: ProX-Code.TabGroup[] = [];
		const changed: ProX-Code.TabGroup[] = [];
	...
	private _findExtHostTabFromApi(apiTab: ProX-Code.Tab): ExtHostEditorTab | undefined {
	...
	private _findExtHostTabGroupFromApi(apiTabGroup: ProX-Code.TabGroup): ExtHostEditorTabGroup | undefined {
		return this._extHostTabGroups.find(candidate => candidate.apiObject === apiTabGroup);
	}

	private async _closeTabs(tabs: ProX-Code.Tab[], preserveFocus?: boolean): Promise<boolean> {
	...
	private async _closeGroups(groups: ProX-Code.TabGroup[], preserverFoucs?: boolean): Promise<boolean> {
...
//#region Utils
function isTabGroup(obj: unknown): obj is ProX-Code.TabGroup {
	const tabGroup = obj as ProX-Code.TabGroup;
