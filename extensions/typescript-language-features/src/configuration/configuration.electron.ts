...
export class ElectronServiceConfigurationProvider extends BaseServiceConfigurationProvider {
	...
	protected readGlobalTsdk(configuration: ProX-Code.WorkspaceConfiguration): string | null {
	...
	protected readLocalTsdk(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		const inspect = configuration.inspect('typescript.tsdk');
	...
	protected readLocalNodePath(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		return this.validatePath(this.readLocalNodePathWorker(configuration));
	}

	private readLocalNodePathWorker(configuration: ProX-Code.WorkspaceConfiguration): string | null {
	...
	protected readGlobalNodePath(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		return this.validatePath(this.readGlobalNodePathWorker(configuration));
	}

	private readGlobalNodePathWorker(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		const inspect = configuration.inspect('typescript.tsserver.nodePath');
	...
	private findNodePath(): string | null {
		try {
			const out = child_process.execFileSync('node', ['-e', 'console.log(process.execPath)'], {
				windowsHide: true,
				timeout: 2000,
				cwd: ProX-Code.workspace.workspaceFolders?.[0].uri.fsPath,
				encoding: 'utf-8',
			});
			return out.trim();
		} catch (error) {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("Could not detect a Node installation to run TS Server."));
			return null;
		}
	}
	...
	private validatePath(nodePath: string | null): string | null {
		if (nodePath && (!fs.existsSync(nodePath) || fs.lstatSync(nodePath).isDirectory())) {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("The path {0} doesn\'t point to a valid Node installation to run TS Server. Falling back to bundled Node.", nodePath));
			return null;
		}
		return nodePath;
	}
