...
class TypeScriptHoverProvider implements ProX-Code.HoverProvider {
	private lastHoverAndLevel: [ProX-Code.Hover, number] | undefined;
	...
	public async provideHover(
		...
		context?: ProX-Code.HoverContext,
	): Promise<ProX-Code.VerboseHover | undefined> {
		const filepath = this.client.toOpenTsFilePath(document);
		if (!filepath) {
			return undefined;
		}

		const enableExpandableHover = ProX-Code.workspace.getConfiguration('typescript').get<boolean>('experimental.expandableHover', true);
	...
	private getContents(
		...
		const parts: ProX-Code.MarkdownString[] = [];
		...
		if (data.displayString) {
			...
			parts.push(new ProX-Code.MarkdownString().appendCodeblock(displayParts.join(' '), 'typescript'));
		}
	...
	private getPreviousLevel(previousHover: ProX-Code.Hover | undefined): number {
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerHoverProvider(selector.syntax,
