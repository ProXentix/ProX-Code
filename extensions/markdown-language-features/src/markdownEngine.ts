...
interface RenderEnv {
	containingImages: Set<string>;
	currentDocument: ProX-Code.Uri | undefined;
...
export class MarkdownItEngine implements IMdParser {
	...
	public async getEngine(resource: ProX-Code.Uri | undefined): Promise<MarkdownIt> {
	...
	private _getConfig(resource?: ProX-Code.Uri): MarkdownItConfig {
	...
	private _addLinkValidator(md: MarkdownIt): void {
		...
		md.validateLink = (link: string) => {
			return validateLink(link)
				...
				|| isOfScheme(Schemes['ProX-Code-insiders'], link)
	...
	private _toResourceUri(href: string, currentDocument: ProX-Code.Uri | undefined, resourceProvider: WebviewResourceProvider | undefined): string {
		try {
			// Support file:// links
			if (isOfScheme(Schemes.file, href)) {
				const uri = ProX-Code.Uri.parse(href);
				if (resourceProvider) {
			...
			if (!/^[a-z\-]+:/i.test(href)) {
				...
				let uri = ProX-Code.Uri.parse('markdown-link:' + href);
				...
				if (uri.path[0] === '/' && currentDocument) {
					const root = ProX-Code.workspace.getWorkspaceFolder(currentDocument);
					if (root) {
						uri = ProX-Code.Uri.joinPath(root.uri, uri.fsPath).with({
