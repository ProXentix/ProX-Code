...
import { extensions, NotebookCellData, NotebookCellExecutionSummary, NotebookCellKind, NotebookCellOutput, NotebookCellOutputItem, NotebookData } from 'ProX-Code';
...
export function getPreferredLanguage(metadata?: nbformat.INotebookMetadata) {
	...
	const defaultLanguage =
		extensions.getExtension('ms-python.python')
			? 'python'
			: (extensions.getExtension('ms-dotnettools.dotnet-interactive-ProX-Code') ? 'csharp' : 'python');
...
function sortOutputItemsBasedOnDisplayOrder(outputItems: NotebookCellOutputItem[]): NotebookCellOutputItem[] {
	return outputItems
		.map(item => {
			...
			// https://github.com/microsoft/ProX-Code-jupyter/issues/6109
...
function convertJupyterOutputToBuffer(mime: string, value: unknown): NotebookCellOutputItem {
	...
	try {
		if (
			...
			// ProX Code expects bytes when rendering images.
...
export function jupyterCellOutputToCellOutput(output: nbformat.IOutput): NotebookCellOutput {
	 ...
	 * outputs: [
			new ProX-Code.NotebookCellOutput([
				new ProX-Code.NotebookCellOutputItem('application/x.notebook.stream', 2),
				new ProX-Code.NotebookCellOutputItem('application/x.notebook.stream', 3),
			]),
			new ProX-Code.NotebookCellOutput([
				new ProX-Code.NotebookCellOutputItem('text/markdown', '## header 2'),
				new ProX-Code.NotebookCellOutputItem('image/svg+xml', [
...
function createNotebookCellDataFromCodeCell(cell: nbformat.ICodeCell, cellLanguage: string): NotebookCellData {
	...
	const ProX-CodeCustomMetadata = cell.metadata['ProX-Code'] as { [key: string]: any } | undefined;
	const cellLanguageId = ProX-CodeCustomMetadata && ProX-CodeCustomMetadata.languageId && typeof ProX-CodeCustomMetadata.languageId === 'string' ? ProX-CodeCustomMetadata.languageId : cellLanguage;
...
/**
 * Converts a NotebookModel into ProX Code format.
 */
