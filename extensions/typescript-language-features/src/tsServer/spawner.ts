...
export class TypeScriptServerSpawner {
	...
	@memoize
	public static get tsServerLogOutputChannel(): ProX-Code.OutputChannel {
		return ProX-Code.window.createOutputChannel(ProX-Code.l10n.t("TypeScript Server Log"));
	}
	...
	private getTsServerArgs(
	...
	): { args: string[]; tsServerLog: TsServerLog | undefined; tsServerTraceDirectory: ProX-Code.Uri | undefined } {
		const args: string[] = [];
		let tsServerLog: TsServerLog | undefined;
		let tsServerTraceDirectory: ProX-Code.Uri | undefined;
		...
		if (TypeScriptServerSpawner.isLoggingEnabled(configuration)) {
			if (isWeb()) {
				...
				if (logDir) {
					const logFilePath = ProX-Code.Uri.joinPath(logDir, `tsserver.log`);
		...
		if (
			...
			&& !isYarnPnp // Disable for yarn pnp as it currently breaks with the ProX Code watcher
		) {
