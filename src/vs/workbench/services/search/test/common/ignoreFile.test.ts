...
suite('Parsing .gitignore files', () => {
	...
	test('real world example: ProX-Code-js-debug', () => {
		const i = `.cache/
			.profile/
			.cdp-profile/
			.headless-profile/
			...
			/coverage
			/.nyc_output
			demos/web-worker/ProX-Code-pwa-dap.log
			demos/web-worker/ProX-Code-pwa-cdp.log
		...
		const included = [
			...
			'/inner/demos/web-worker/ProX-Code-pwa-dap.log',
			'/inner/demos/web-worker/ProX-Code-pwa-cdp.log',
	...
	test('real world example: ProX-Code', () => {
		const i = `.DS_Store
			.cache
			npm-debug.log
			Thumbs.db
