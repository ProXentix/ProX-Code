 ...
 *--------------------------------------------------------------------------------------------*/
import { FileSystem } from '@ProX-Code/sync-api-client';
...
export function startWorkerSession(
	...
	const worker = new class WorkerSession extends ts.server.Session<{}> {
		...
		constructor() {
			...
			const typingsInstaller = options.disableAutomaticTypingAcquisition || !fs ? ts.server.nullTypingsInstaller : new WebTypingsInstallerClient(host, '/ProX-Code-global-typings/ts-nul-authority/projects');
