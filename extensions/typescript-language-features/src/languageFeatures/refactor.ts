...
function toWorkspaceEdit(client: ITypeScriptServiceClient, edits: readonly Proto.FileCodeEdits[]): ProX-Code.WorkspaceEdit {
...
namespace DidApplyRefactoringCommand {
	export interface Args {
		...
		readonly trigger: ProX-Code.CodeActionTriggerKind;
	}
}
...
namespace SelectRefactorCommand {
	export interface Args {
		...
		readonly rangeOrSelection: ProX-Code.Range | ProX-Code.Selection;
		readonly trigger: ProX-Code.CodeActionTriggerKind;
	}
}
...
class SelectRefactorCommand implements Command {
	...
	public async execute(args: SelectRefactorCommand.Args): Promise<void> {
		...
		const selected = await ProX-Code.window.showQuickPick(args.refactor.actions.map((action): ProX-Code.QuickPickItem & { action: Proto.RefactorActionInfo } => ({
			action,
			label: action.name,
		...
		if (tsAction.edit) {
			if (!(await ProX-Code.workspace.applyEdit(tsAction.edit, { isRefactoring: true }))) {
				ProX-Code.window.showErrorMessage(ProX-Code.l10n.t("Could not apply refactoring"));
				return;
			}
		}

		if (tsAction.command) {
			await ProX-Code.commands.executeCommand(tsAction.command.command, ...(tsAction.command.arguments ?? []));
		}
	}
}
...
class MoveToFileRefactorCommand implements Command {
	...
	public async execute(args: MoveToFileRefactorCommand.Args): Promise<void> {
		...
		if (!(await ProX-Code.workspace.applyEdit(edit, { isRefactoring: true }))) {
			ProX-Code.window.showErrorMessage(ProX-Code.l10n.t("Could not apply refactoring"));
			return;
		}
	...
	private async getTargetFile(document: ProX-Code.TextDocument, file: string, range: ProX-Code.Range): Promise<string | undefined> {
		...
		type DestinationItem = ProX-Code.QuickPickItem & { readonly file?: string };
		const selectExistingFileItem: ProX-Code.QuickPickItem = { label: ProX-Code.l10n.t("Select existing file...") };
		const selectNewFileItem: ProX-Code.QuickPickItem = { label: ProX-Code.l10n.t("Enter new file path...") };

		const workspaceFolder = ProX-Code.workspace.getWorkspaceFolder(document.uri);
		const quickPick = ProX-Code.window.createQuickPick<DestinationItem>();
		...
		const updateItems = () => {
			...
			quickPick.items = [
				...
				selectNewFileItem,
				{ label: ProX-Code.l10n.t("destination files"), kind: ProX-Code.QuickPickItemKind.Separator },
				...coalesce(destinationItems)
			];
		};
		quickPick.title = ProX-Code.l10n.t("Move to File");
		quickPick.placeholder = ProX-Code.l10n.t("Enter file path");
		...
		if (picked === selectExistingFileItem) {
			const picked = await ProX-Code.window.showOpenDialog({
				title: ProX-Code.l10n.t("Select move destination"),
				openLabel: ProX-Code.l10n.t("Move to File"),
			...
			const picked = await ProX-Code.window.showSaveDialog({
				title: ProX-Code.l10n.t("Select move destination"),
				saveLabel: ProX-Code.l10n.t("Move to File"),
...
const Rewrite_Export = Object.freeze<CodeActionKind>({
	kind: ProX-Code.CodeActionKind.RefactorRewrite.append('export'),
...
const Rewrite_Arrow_Braces = Object.freeze<CodeActionKind>({
	kind: ProX-Code.CodeActionKind.RefactorRewrite.append('arrow').append('braces'),
	matches: refactor => refactor.name.startsWith('Convert default export') || refactor.name.startsWith('Convert named export')
});

const Rewrite_Parameters_ToDestructured = Object.freeze<CodeActionKind>({
	kind: ProX-Code.CodeActionKind.RefactorRewrite.append('parameters').append('toDestructured'),
...
const Rewrite_Property_GenerateAccessors = Object.freeze<CodeActionKind>({
	kind: ProX-Code.CodeActionKind.RefactorRewrite.append('property').append('generateAccessors'),
...
class InlinedCodeAction extends ProX-Code.CodeAction {
	constructor(
		...
		trigger: ProX-Code.CodeActionTriggerKind,
	) {
	...
	public async resolve(token: ProX-Code.CancellationToken): Promise<undefined> {
		...
		if (response.body.renameLocation) {
			// Disable renames in interactive playground https://github.com/microsoft/ProX-Code/issues/75137
	...
	private static getKind(refactor: Proto.RefactorActionInfo) {
		if ((refactor as Proto.RefactorActionInfo & { kind?: string }).kind) {
			return ProX-Code.CodeActionKind.Empty.append((refactor as Proto.RefactorActionInfo & { kind?: string }).kind!);
		}
		const match = allKnownCodeActionKinds.find(kind => kind.matches(refactor));
		return match ? match.kind : ProX-Code.CodeActionKind.Refactor;
	}
}

class MoveToFileCodeAction extends ProX-Code.CodeAction {
	constructor(
...
class SelectCodeAction extends ProX-Code.CodeAction {
	constructor(
		info: Proto.ApplicableRefactorInfo,
		document: ProX-Code.TextDocument,
		rangeOrSelection: ProX-Code.Range | ProX-Code.Selection,
		trigger: ProX-Code.CodeActionTriggerKind,
	) {
		super(info.description, ProX-Code.CodeActionKind.Refactor);
		this.command = {
...
class TypeScriptRefactorProvider implements ProX-Code.CodeActionProvider<TsCodeAction> {
	...
	private static isOnSignatureName(node: Proto.NavigationTree, range: ProX-Code.Range): boolean {
		if (this._declarationKinds.has(node.kind)) {
			...
			if (!node.nameSpan && /\bexport\b/.test(node.kindModifiers) && node.spans.length) {
				...
				if (range.intersection(new ProX-Code.Range(convertedSpan.start.line, 0, convertedSpan.start.line, Number.MAX_SAFE_INTEGER))) {
					return true;
				}
			}
		}
	...
	public static readonly metadata: ProX-Code.CodeActionProviderMetadata = {
		providedCodeActionKinds: [
			ProX-Code.CodeActionKind.Refactor,
			...allKnownCodeActionKinds.map(x => x.kind),
		],
		documentation: [
			{
				kind: ProX-Code.CodeActionKind.Refactor,
				command: {
					command: LearnMoreAboutRefactoringsCommand.id,
					title: ProX-Code.l10n.t("Learn more about JS/TS refactorings")
				}
			}
		]
	};
	...
	public async provideCodeActions(
		document: ProX-Code.TextDocument,
		rangeOrSelection: ProX-Code.Range | ProX-Code.Selection,
		context: ProX-Code.CodeActionContext,
		...
		const actions = coalesce(await Promise.all(Array.from(applicableRefactors, async action => {
			...
			if (this.client.apiVersion.lt(API.v540) && context.triggerKind === ProX-Code.CodeActionTriggerKind.Automatic) {
	...
	private interruptGetErrIfNeeded<R>(context: ProX-Code.CodeActionContext, f: () => R): R {
		...
		if (context.triggerKind === ProX-Code.CodeActionTriggerKind.Invoke) {
	...
	private toTsTriggerReason(context: ProX-Code.CodeActionContext): Proto.RefactorTriggerReason | undefined {
		return context.triggerKind === ProX-Code.CodeActionTriggerKind.Invoke ? 'invoked' : 'implicit';
	}
	...
	private shouldTrigger(context: ProX-Code.CodeActionContext, rangeOrSelection: ProX-Code.Range | ProX-Code.Selection) {
		if (context.only && !ProX-Code.CodeActionKind.Refactor.contains(context.only)) {
			return false;
		}
		if (context.triggerKind === ProX-Code.CodeActionTriggerKind.Invoke) {
			return true;
		}
		return rangeOrSelection instanceof ProX-Code.Selection;
	}
	...
	private appendInvalidActions(actions: ProX-Code.CodeAction[]): ProX-Code.CodeAction[] {
		...
		if (!actions.some(action => action.kind && Extract_Constant.kind.contains(action.kind))) {
			const disabledAction = new ProX-Code.CodeAction(
				ProX-Code.l10n.t("Extract to constant"),
				Extract_Constant.kind);

			disabledAction.disabled = {
				reason: ProX-Code.l10n.t("The current selection cannot be extracted"),
			};
		...
		if (!actions.some(action => action.kind && Extract_Function.kind.contains(action.kind))) {
			const disabledAction = new ProX-Code.CodeAction(
				ProX-Code.l10n.t("Extract to function"),
	...
	private pruneInvalidActions(actions: ProX-Code.CodeAction[], only?: ProX-Code.CodeActionKind, numberOfInvalid?: number): ProX-Code.CodeAction[] {
		if (this.client.apiVersion.lt(API.v400)) {
			// Older TS version don't return extra actions
			return actions;
		}

		const availableActions: ProX-Code.CodeAction[] = [];
		const invalidCommonActions: ProX-Code.CodeAction[] = [];
		const invalidUncommonActions: ProX-Code.CodeAction[] = [];
		...
		const prioritizedActions: ProX-Code.CodeAction[] = [];
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerCodeActionsProvider(selector.semantic,
