 ...
 *
 * @see https://github.com/microsoft/ProX-Code/issues/211462#issuecomment-2155471996
 */
...
// Related to https://github.com/microsoft/ProX-Code/issues/30624
...
export function createRandomIPCHandle(): string {
	...
	if (process.platform === 'win32') {
		return `\\\\.\\pipe\\ProX-Code-ipc-${randomSuffix}-sock`;
	}
	...
	const result = join(basePath, `ProX-Code-ipc-${randomSuffix}.sock`);
...
export function createStaticIPCHandle(directoryPath: string, type: string, version: string): string {
	...
	if (process.platform !== 'darwin' && XDG_RUNTIME_DIR && !process.env['VSCODE_PORTABLE']) {
		result = join(XDG_RUNTIME_DIR, `ProX-Code-${scopeForSocket}-${versionForSocket}-${typeForSocket}.sock`);
	} else {
