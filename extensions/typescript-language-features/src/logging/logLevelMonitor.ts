...
export class LogLevelMonitor extends Disposable {
	...
	constructor(private readonly context: ProX-Code.ExtensionContext) {
		super();

		this._register(ProX-Code.workspace.onDidChangeConfiguration(this.onConfigurationChange, this, this._disposables));
	...
	private onConfigurationChange(event: ProX-Code.ConfigurationChangeEvent) {
	...
	private get logLevel(): TsServerLogLevel {
		return TsServerLogLevel.fromString(ProX-Code.workspace.getConfiguration().get<string>(LogLevelMonitor.logLevelConfigKey, 'off'));
	}
	...
	private notifyExtendedLogging() {
		...
		interface Item extends ProX-Code.MessageItem {
			readonly choice: Choice;
		}

		ProX-Code.window.showInformationMessage<Item>(
			ProX-Code.l10n.t("TS Server logging is currently enabled which may impact performance."),
			{
				title: ProX-Code.l10n.t("Disable logging"),
				choice: Choice.DisableLogging
			},
			{
				title: ProX-Code.l10n.t("Don't show again"),
			...
			.then(selection => {
				...
				if (selection.choice === Choice.DisableLogging) {
					return ProX-Code.workspace.getConfiguration().update(LogLevelMonitor.logLevelConfigKey, 'off', true);
