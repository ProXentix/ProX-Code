...
export class Win32UpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected override async initialize(): Promise<void> {
		if (this.environmentMainService.isBuilt) {
			...
			try {
				...
				// Ignore if 'appUpdate' path is not supported by Electron
			}
			try {
	...
	private async getUpdatePackagePath(version: string): Promise<string> {
		const cachePath = await this.cachePath;
		return path.join(cachePath, `ProXCodeSetup-${this.productService.quality}-${version}.exe`);
	}
	...
	protected override async doApplyUpdate(): Promise<void> {
		...
		this.availableUpdate.updateFilePath = path.join(cachePath, `ProXCodeSetup-${this.productService.quality}-${update.version}.flag`);
