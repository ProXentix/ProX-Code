...
import type { IGrammar, IOnigLib, IRawTheme, Registry, StateStack } from 'ProX-Code-textmate';
...
export class TMGrammarFactory extends Disposable {
	...
	constructor(host: ITMGrammarFactoryHost, grammarDefinitions: IValidGrammarDefinition[], ProX-CodeTextmate: typeof import('ProX-Code-textmate'), onigLib: Promise<IOnigLib>) {
		super();
		this._host = host;
		this._initialState = ProX-CodeTextmate.INITIAL;
		this._scopeRegistry = new TMScopeRegistry();
		this._injections = {};
		this._injectedEmbeddedLanguages = {};
		this._languageToScope = new Map<string, string>();
		this._grammarRegistry = this._register(new ProX-CodeTextmate.Registry({
			onigLib: onigLib,
			loadGrammar: async (scopeName: string) => {
				...
				try {
					...
					return ProX-CodeTextmate.parseRawGrammar(content, location.path);
				} catch (e) {
