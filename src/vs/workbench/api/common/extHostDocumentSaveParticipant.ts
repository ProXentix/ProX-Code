...
export class ExtHostDocumentSaveParticipant implements ExtHostDocumentSaveParticipantShape {
	...
	getOnWillSaveTextDocumentEvent(extension: IExtensionDescription): Event<ProX-Code.TextDocumentWillSaveEvent> {
	...
	private _deliverEventAsyncAndBlameBadListeners([listener, thisArg, extension]: Listener, stubEvent: ProX-Code.TextDocumentWillSaveEvent): Promise<any> {
	...
	private _deliverEventAsync(extension: IExtensionDescription, listener: Function, thisArg: any, stubEvent: ProX-Code.TextDocumentWillSaveEvent): Promise<any> {

		const promises: Promise<ProX-Code.TextEdit[]>[] = [];

		const t1 = Date.now();
		const { document, reason } = stubEvent;
		const { version } = document;

		const event = Object.freeze<ProX-Code.TextDocumentWillSaveEvent>({
			document,
			reason,
			waitUntil(p: Promise<any | ProX-Code.TextEdit[]>) {
		...
		return new Promise<ProX-Code.TextEdit[][]>((resolve, reject) => {
			...
			for (const value of values) {
				if (Array.isArray(value) && (<ProX-Code.TextEdit[]>value).every(e => e instanceof TextEdit)) {
