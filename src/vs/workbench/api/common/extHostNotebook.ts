...
export class ExtHostNotebookController implements ExtHostNotebookShape {
	...
	private readonly _notebookStatusBarItemProviders = new Map<number, ProX-Code.NotebookCellStatusBarItemProvider>();
	private readonly _documents = new ResourceMap<ExtHostNotebookDocument>();
	private readonly _editors = new Map<string, ExtHostNotebookEditor>();
	private readonly _commandsConverter: CommandsConverter;

	private readonly _onDidChangeActiveNotebookEditor = new Emitter<ProX-Code.NotebookEditor | undefined>();
	...
	get visibleNotebookEditors(): ProX-Code.NotebookEditor[] {
		return this._visibleNotebookEditors.map(editor => editor.apiEditor);
	}

	private _onDidOpenNotebookDocument = new Emitter<ProX-Code.NotebookDocument>();
	onDidOpenNotebookDocument: Event<ProX-Code.NotebookDocument> = this._onDidOpenNotebookDocument.event;
	private _onDidCloseNotebookDocument = new Emitter<ProX-Code.NotebookDocument>();
	onDidCloseNotebookDocument: Event<ProX-Code.NotebookDocument> = this._onDidCloseNotebookDocument.event;

	private _onDidChangeVisibleNotebookEditors = new Emitter<ProX-Code.NotebookEditor[]>();
	...
	getIdByEditor(editor: ProX-Code.NotebookEditor): string | undefined {
	...
	private static _convertNotebookRegistrationData(extension: IExtensionDescription, registration: ProX-Code.NotebookRegistrationData | undefined): INotebookContributionData | undefined {
		if (!registration) {
			return;
		}
	...
	registerNotebookCellStatusBarItemProvider(extension: IExtensionDescription, notebookType: string, provider: ProX-Code.NotebookCellStatusBarItemProvider) {
	...
	async createNotebookDocument(options: { viewType: string; content?: ProX-Code.NotebookData }): Promise<URI> {
	...
	async openNotebookDocument(uri: URI): Promise<ProX-Code.NotebookDocument> {
	...
	async showNotebookDocument(notebook: ProX-Code.NotebookDocument, options?: ProX-Code.NotebookDocumentShowOptions): Promise<ProX-Code.NotebookEditor> {
	...
	private readonly _notebookSerializer = new Map<number, { viewType: string; serializer: ProX-Code.NotebookSerializer; options: ProX-Code.NotebookDocumentContentOptions | undefined }>();

	registerNotebookSerializer(extension: IExtensionDescription, viewType: string, serializer: ProX-Code.NotebookSerializer, options?: ProX-Code.NotebookDocumentContentOptions, registration?: ProX-Code.NotebookRegistrationData): ProX-Code.Disposable {
	...
	async $saveNotebook(handle: number, uriComponents: UriComponents, versionId: number, options: files.IWriteFileOptions, token: CancellationToken): Promise<INotebookPartialFileStatsWithMetadata> {
		...
		const data: ProX-Code.NotebookData = {
					...
					return this._extHostSearch.doInternalFileSearchWithCustomCallback(query, token, (data) => {
						data.forEach(uri => {
							...
							const hasOtherMatches = otherViewTypeFileTargets.some(target => {
								...
								// https://github.com/microsoft/ProX-Code/blob/ac1631528e67637da65ec994c6dc35d73f6e33cc/src/vs/workbench/services/editor/browser/editorResolverService.ts#L359-L366
						...
						// temporary fix for https://github.com/microsoft/ProX-Code/issues/205044: don't show notebook results for remotehub repos.
						if (err.code === 'ENOENT') {
	...
	$acceptDocumentAndEditorsDelta(delta: SerializableObjectWithBuffers<INotebookDocumentsAndEditorsDelta>): void {
		...
		if (delta.value.addedDocuments) {
			...
			for (const modelData of delta.value.addedDocuments) {
				...
				// add cell document as ProX-Code.TextDocument
	...
	private static _registerApiCommands(extHostCommands: ExtHostCommands) {
		...
		const commandDataToNotebook = new ApiCommand(
			'ProX-Code.executeDataToNotebook', '_executeDataToNotebook', 'Invoke notebook serializer',
			...
			new ApiCommandResult<SerializableObjectWithBuffers<NotebookDataDto>, ProX-Code.NotebookData>('Notebook Data', data => typeConverters.NotebookData.to(data.value))
		);

		const commandNotebookToData = new ApiCommand(
			'ProX-Code.executeNotebookToData', '_executeNotebookToData', 'Invoke notebook serializer',
			[notebookTypeArg, new ApiCommandArgument<ProX-Code.NotebookData, SerializableObjectWithBuffers<NotebookDataDto>>('NotebookData', 'Notebook data to convert to bytes', v => true, v => new SerializableObjectWithBuffers(typeConverters.NotebookData.from(v)))],
