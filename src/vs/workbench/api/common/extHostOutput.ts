...
class ExtHostOutputChannel extends AbstractMessageLogger implements ProX-Code.LogOutputChannel {
	...
	show(columnOrPreserveFocus?: ProX-Code.ViewColumn | boolean, preserveFocus?: boolean): void {
		this.logger.flush();
...
export class ExtHostOutputService implements ExtHostOutputServiceShape {
	...
	createOutputChannel(name: string, options: string | { log: true } | undefined, extension: IExtensionDescription): ProX-Code.OutputChannel | ProX-Code.LogOutputChannel {
		name = name.trim();
		if (!name) {
	...
	private createExtHostOutputChannel(name: string, channelPromise: Promise<ExtHostOutputChannel>, channelDisposables: DisposableStore): ProX-Code.OutputChannel {
		const validate = () => {
	...
	private createExtHostLogOutputChannel(name: string, logLevel: LogLevel, channelPromise: Promise<ExtHostOutputChannel>, channelDisposables: DisposableStore): ProX-Code.LogOutputChannel {
		const validate = () => {
			if (channelDisposables.isDisposed) {
				throw new Error('Channel has been closed');
			}
