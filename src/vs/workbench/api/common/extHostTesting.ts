...
interface ControllerInfo {
	...
	profiles: Map<number, ProX-Code.TestRunProfile>;
	collection: ExtHostTestItemCollection;
	extension: IExtensionDescription;
	relatedCodeProvider?: ProX-Code.TestRelatedCodeProvider;
...
const testResultInternalIDs = new WeakMap<ProX-Code.TestRunResult, string>();
...
export class ExtHostTesting extends Disposable implements ExtHostTestingShape {
	...
	private readonly followupProviders = new Set<ProX-Code.TestFollowupProvider>();
	private readonly testFollowups = new Map<number, ProX-Code.Command>();

	public onResultsChanged = this.resultsChangedEmitter.event;
	public results: ReadonlyArray<ProX-Code.TestRunResult> = [];
	...
	/**
	 * Implements ProX-Code.test.registerTestProvider
	 */
	public createTestController(extension: IExtensionDescription, controllerId: string, label: string, refreshHandler?: (token: CancellationToken) => Thenable<void> | void): ProX-Code.TestController {
		...
		const profiles = new Map<number, ProX-Code.TestRunProfile>();
		...
		const controller: ProX-Code.TestController = {
			...
			set relatedCodeProvider(value: ProX-Code.TestRelatedCodeProvider | undefined) {
				checkProposedApiEnabled(extension, 'testRelatedCode');
				info.relatedCodeProvider = value;
				proxy.$updateController(controllerId, { capabilities: getCapability() });
			},
			createRunProfile: (label, group, runHandler, isDefault, tag?: ProX-Code.TestTag | undefined, supportsContinuousRun?: boolean) => {
			...
			get resolveHandler() {
				return collection.resolveHandler as undefined | ((item?: ProX-Code.TestItem) => void);
			},
			dispose: () => {
				disposable.dispose();
			},
	...
	/**
	 * Implements ProX-Code.test.createTestObserver
	 */
	...
	/**
	 * Implements ProX-Code.test.runTests
	 */
	public async runTests(req: ProX-Code.TestRunRequest, token = CancellationToken.None) {
		const profile = tryGetProfileFromTestRunReq(req);
		if (!profile) {
			throw new Error('The request passed to `ProX-Code.test.runTests` must include a profile');
		}
	...
	/**
	 * Implements ProX-Code.test.registerTestFollowupProvider
	 */
	public registerTestFollowupProvider(provider: ProX-Code.TestFollowupProvider): ProX-Code.Disposable {
	...
	async $getTestsRelatedToCode(uri: UriComponents, _position: IPosition, token: CancellationToken): Promise<string[]> {
		...
		await Promise.all([...this.controllers.values()].map(async (c) => {
			let tests: ProX-Code.TestItem[] | undefined | null;
			try {
	...
	public async $provideTestFollowups(req: TestMessageFollowupRequest, token: CancellationToken): Promise<TestMessageFollowupResponse[]> {
		...
		let followups: ProX-Code.Command[] = [];
	...
	public getMetadataForRun(run: ProX-Code.TestRun) {
...
class TestRunTracker extends Disposable {
	...
	private running = 0;
	private readonly tasks = new Map</* task ID */string, { cts: CancellationTokenSource; run: ProX-Code.TestRun }>();
	...
	private readonly publishedCoverage = new Map<string, { report: ProX-Code.FileCoverage; extIds: string[] }>();
	...
	constructor(
		...
		private readonly profile: ProX-Code.TestRunProfile | undefined,
	...
	public getTaskIdForRun(run: ProX-Code.TestRun) {
	...
	public async getCoverageDetails(id: string, testId: string | undefined, token: CancellationToken): Promise<ProX-Code.FileCoverageDetail[]> {
		...
		let testItem: ProX-Code.TestItem | undefined;
	...
	public createRun(name: string | undefined): ProX-Code.TestRun {
		const runId = this.dto.id;
		const ctrlId = this.dto.controllerId;
		const taskId = generateUuid();

		const guardTestMutation = <Args extends unknown[]>(fn: (test: ProX-Code.TestItem, ...args: Args) => void) =>
			(test: ProX-Code.TestItem, ...args: Args) => {
				if (ended) {
		...
		const appendMessages = (test: ProX-Code.TestItem, messages: ProX-Code.TestMessage | readonly ProX-Code.TestMessage[]) => {
		...
		const run: ProX-Code.TestRun = {
			...
			//#endregion
			appendOutput: (output, location?: ProX-Code.Location, test?: ProX-Code.TestItem) => {
				if (ended) {
					return;
				}
	...
	private ensureTestIsKnown(test: ProX-Code.TestItem) {
...
export class TestRunCoordinator {
	private readonly tracked = new Map<ProX-Code.TestRunRequest, TestRunTracker>();
	 ...
	 */
	public getCoverageDetails(id: string, testId: string | undefined, token: ProX-Code.CancellationToken) {
	 ...
	 */
	public prepareForMainThreadTestRun(extension: IExtensionDescription, req: ProX-Code.TestRunRequest, dto: TestRunDto, profile: ProX-Code.TestRunProfile, token: CancellationToken) {
	 ...
	 */
	public createTestRun(extension: IExtensionDescription, controllerId: string, collection: ExtHostTestItemCollection, request: ProX-Code.TestRunRequest, name: string | undefined, persist: boolean): ProX-Code.TestRun {
	...
	private getTracker(req: ProX-Code.TestRunRequest, dto: TestRunDto, profile: ProX-Code.TestRunProfile | undefined, extension: IExtensionDescription, token?: CancellationToken) {
...
const tryGetProfileFromTestRunReq = (request: ProX-Code.TestRunRequest) => {
	if (!request.profile) {
		return undefined;
	}
...
export class TestRunDto {
	public static fromPublic(controllerId: string, collection: ExtHostTestItemCollection, request: ProX-Code.TestRunRequest, persist: boolean) {
...
class MirroredChangeCollector implements IncrementalChangeCollector<MirroredCollectionTestItem> {
	...
	constructor(private readonly emitter: Emitter<ProX-Code.TestsChangeEvent>) {
	}
	 ...
	 * @inheritdoc
	 */
	public getChangeEvent(): ProX-Code.TestsChangeEvent {
...
class MirroredTestCollection extends AbstractIncrementalTestCollection<MirroredCollectionTestItem> {
	private changeEmitter = new Emitter<ProX-Code.TestsChangeEvent>();
	 ...
	 */
	public getMirroredTestDataByReference(item: ProX-Code.TestItem) {
	...
	protected createItem(item: InternalTestItem, parent?: MirroredCollectionTestItem): MirroredCollectionTestItem {
		return {
			...
			revived: Convert.TestItem.toPlain(item.item) as ProX-Code.TestItem,
...
class TestObservers {
	...
	public checkout(): ProX-Code.TestObserver {
		if (!this.current) {
	 ...
	 */
	public getMirroredTestDataByReference(ref: ProX-Code.TestItem) {
...
export class TestRunProfileImpl extends TestRunProfileBase implements ProX-Code.TestRunProfile {
	...
	public set tag(tag: ProX-Code.TestTag | undefined) {
	...
	constructor(
		...
		public runHandler: (request: ProX-Code.TestRunRequest, token: ProX-Code.CancellationToken) => Thenable<void> | void,
		_isDefault = false,
		public _tag: ProX-Code.TestTag | undefined = undefined,
...
function findTestInResultSnapshot(extId: TestId, snapshot: readonly Readonly<ProX-Code.TestResultSnapshot>[]) {
