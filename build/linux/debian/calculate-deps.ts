...
export function generatePackageDeps(files: string[], arch: DebianArchString, chromiumSysroot: string, ProX-CodeSysroot: string): Set<string>[] {
	const dependencies: Set<string>[] = files.map(file => calculatePackageDeps(file, arch, chromiumSysroot, ProX-CodeSysroot));
...
function calculatePackageDeps(binaryPath: string, arch: DebianArchString, chromiumSysroot: string, ProX-CodeSysroot: string): Set<string> {
	try {
	...
	switch (arch) {
		case 'amd64':
			cmd.push(`-l${chromiumSysroot}/usr/lib/x86_64-linux-gnu`,
				...
				`-l${ProX-CodeSysroot}/usr/lib/x86_64-linux-gnu`,
				`-l${ProX-CodeSysroot}/lib/x86_64-linux-gnu`);
			break;
		case 'armhf':
			cmd.push(`-l${chromiumSysroot}/usr/lib/arm-linux-gnueabihf`,
				`-l${chromiumSysroot}/lib/arm-linux-gnueabihf`,
				`-l${ProX-CodeSysroot}/usr/lib/arm-linux-gnueabihf`,
				`-l${ProX-CodeSysroot}/lib/arm-linux-gnueabihf`);
			break;
		case 'arm64':
			cmd.push(`-l${chromiumSysroot}/usr/lib/aarch64-linux-gnu`,
				`-l${chromiumSysroot}/lib/aarch64-linux-gnu`,
				`-l${ProX-CodeSysroot}/usr/lib/aarch64-linux-gnu`,
				`-l${ProX-CodeSysroot}/lib/aarch64-linux-gnu`);
			break;
	}
