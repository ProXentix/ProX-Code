...
import { commands, Position, Range, Uri, ViewColumn, window, workspace } from 'ProX-Code';
import { assertNoRpc, closeAllEditors } from '../utils';

suite('ProX-Code API - commands', () => {
	...
	test('api-command: ProX-Code.diff', function () {
		...
		const a = commands.executeCommand('ProX-Code.diff', Uri.parse('sc:a'), Uri.parse('sc:b'), 'DIFF').then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const b = commands.executeCommand('ProX-Code.diff', Uri.parse('sc:a'), Uri.parse('sc:b')).then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const c = commands.executeCommand('ProX-Code.diff', Uri.parse('sc:a'), Uri.parse('sc:b'), 'Title', { selection: new Range(new Position(1, 1), new Position(1, 2)) }).then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const d = commands.executeCommand('ProX-Code.diff').then(() => assert.ok(false), () => assert.ok(true));
		const e = commands.executeCommand('ProX-Code.diff', 1, 2, 3).then(() => assert.ok(false), () => assert.ok(true));

		return Promise.all([a, b, c, d, e]);
	});

	test('api-command: ProX-Code.open', async function () {
		...
		await commands.executeCommand('ProX-Code.open', uri);
		...
		await commands.executeCommand('ProX-Code.open', uri, ViewColumn.Two);
		...
		await commands.executeCommand('ProX-Code.open', uri, ViewColumn.One);
		assert.strictEqual(window.tabGroups.all.length, 2);
		assert.strictEqual(window.tabGroups.all[0].activeTab?.group.viewColumn, ViewColumn.One);

		let e1: Error | undefined = undefined;
		try {
			await commands.executeCommand('ProX-Code.open');
		} catch (error) {
			e1 = error;
		}
		assert.ok(e1);
		...
		try {
			await commands.executeCommand('ProX-Code.open', uri, true);
		} catch (error) {
			e2 = error;
		}
		assert.ok(e2);
		...
		try {
			await commands.executeCommand('ProX-Code.open', 'file:///some/path/not/http');
	...
	test('api-command: ProX-Code.open with untitled supports associated resource (#138925)', async function () {
		const uri = Uri.parse(workspace.workspaceFolders![0].uri.toString() + '/untitled-file.txt').with({ scheme: 'untitled' });
		await commands.executeCommand('ProX-Code.open', uri).then(() => assert.ok(true), () => assert.ok(false));
