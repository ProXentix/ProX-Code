...
export interface TsDiagnostics {
	...
	readonly resource: ProX-Code.Uri;
...
export default class TypeScriptServiceClient extends Disposable implements ITypeScriptServiceClient {
	...
	constructor(
		private readonly context: ProX-Code.ExtensionContext,
		...
		ProX-Code.workspace.onDidChangeConfiguration(() => {
	...
	private readonly _onDidChangeCapabilities = this._register(new ProX-Code.EventEmitter<void>());
	...
	private cancelInflightRequestsForResource(resource: ProX-Code.Uri): void {
		if (this.serverState.type !== ServerState.Type.Running) {
			return;
		}
	...
	private readonly _onTsServerStarted = this._register(new ProX-Code.EventEmitter<{ version: TypeScriptVersion; usedApiVersion: API }>());
	public readonly onTsServerStarted = this._onTsServerStarted.event;

	private readonly _onDiagnosticsReceived = this._register(new ProX-Code.EventEmitter<TsDiagnostics>());
	public readonly onDiagnosticsReceived = this._onDiagnosticsReceived.event;

	private readonly _onConfigDiagnosticsReceived = this._register(new ProX-Code.EventEmitter<Proto.ConfigFileDiagnosticEvent>());
	public readonly onConfigDiagnosticsReceived = this._onConfigDiagnosticsReceived.event;

	private readonly _onResendModelsRequested = this._register(new ProX-Code.EventEmitter<void>());
	public readonly onResendModelsRequested = this._onResendModelsRequested.event;

	private readonly _onProjectLanguageServiceStateChanged = this._register(new ProX-Code.EventEmitter<Proto.ProjectLanguageServiceStateEventBody>());
	public readonly onProjectLanguageServiceStateChanged = this._onProjectLanguageServiceStateChanged.event;

	private readonly _onDidBeginInstallTypings = this._register(new ProX-Code.EventEmitter<Proto.BeginInstallTypesEventBody>());
	public readonly onDidBeginInstallTypings = this._onDidBeginInstallTypings.event;

	private readonly _onDidEndInstallTypings = this._register(new ProX-Code.EventEmitter<Proto.EndInstallTypesEventBody>());
	public readonly onDidEndInstallTypings = this._onDidEndInstallTypings.event;

	private readonly _onTypesInstallerInitializationFailed = this._register(new ProX-Code.EventEmitter<Proto.TypesInstallerInitializationFailedEventBody>());
	public readonly onTypesInstallerInitializationFailed = this._onTypesInstallerInitializationFailed.event;

	private readonly _onSurveyReady = this._register(new ProX-Code.EventEmitter<Proto.SurveyReadyEventBody>());
	...
	private startService(resendModels: boolean = false): ServerState.State {
		...
		if (!version.isValid) {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("The path {0} doesn't point to a valid tsserver install. Falling back to bundled TypeScript version.", version.path));
		...
		*/
		...
		handle.onError((err: Error) => {
			...
			if (err) {
				ProX-Code.window.showErrorMessage(ProX-Code.l10n.t("TypeScript language server exited with error. Error message is: {0}", err.message || err.name));
			}
	...
	public async openTsServerLogFile(): Promise<boolean> {
		if (this._configuration.tsServerLogLevel === TsServerLogLevel.Off) {
			ProX-Code.window.showErrorMessage<ProX-Code.MessageItem>(
				ProX-Code.l10n.t("TS Server logging is off. Please set 'typescript.tsserver.log' and restart the TS server to enable logging"),
				{
					title: ProX-Code.l10n.t("Enable logging and restart TS server"),
				})
				.then(selection => {
					if (selection) {
						return ProX-Code.workspace.getConfiguration().update('typescript.tsserver.log', 'verbose', true).then(() => {
							this.restartTsServer();
						});
					}
					return undefined;
		...
		if (this.serverState.type !== ServerState.Type.Running || !this.serverState.server.tsServerLog) {
			ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("TS Server has not started logging."));
			return false;
		}
		...
		switch (this.serverState.server.tsServerLog.type) {
			...
			case 'file': {
				try {
					const doc = await ProX-Code.workspace.openTextDocument(this.serverState.server.tsServerLog.uri);
				...
				try {
					await ProX-Code.commands.executeCommand('revealFileInOS', this.serverState.server.tsServerLog.uri);
					return true;
				} catch {
					ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("Could not open TS Server log file"));
					return false;
				}
			}
		}
	...
	private serviceExited(restart: boolean, tsVersion: API): void {
		...
		if (restart) {
			...
			const reportIssueItem: ProX-Code.MessageItem = {
				title: ProX-Code.l10n.t("Report Issue"),
			};
			let prompt: Thenable<undefined | ProX-Code.MessageItem> | undefined = undefined;
			...
			if (this.numberRestarts > 5) {
				...
				if (diff < 10 * 1000 /* 10 seconds */) {
					...
					if (this.pluginManager.plugins.length) {
						prompt = ProX-Code.window.showErrorMessage<ProX-Code.MessageItem>(
							ProX-Code.l10n.t("The JS/TS language service immediately crashed 5 times. The service will not be restarted.\nThis may be caused by a plugin contributed by one of these extensions: {0}.\nPlease try disabling these extensions before filing an issue against ProX Code.", pluginExtensionList));
					} else {
						prompt = ProX-Code.window.showErrorMessage(
							ProX-Code.l10n.t("The JS/TS language service immediately crashed 5 times. The service will not be restarted."),
							reportIssueItem);
					}
					...
					*/
					...
					if (!this._isPromptingAfterCrash) {
						if (this.pluginManager.plugins.length) {
							prompt = ProX-Code.window.showWarningMessage<ProX-Code.MessageItem>(
								ProX-Code.l10n.t("The JS/TS language service crashed 5 times in the last 5 Minutes.\nThis may be caused by a plugin contributed by one of these extensions: {0}\nPlease try disabling these extensions before filing an issue against ProX Code.", pluginExtensionList));
						} else {
							prompt = ProX-Code.window.showWarningMessage(
								ProX-Code.l10n.t("The JS/TS language service crashed 5 times in the last 5 Minutes."),
								reportIssueItem);
						}
					}
				}
			} else if (['ProX-Code-insiders', 'ProX-Code'].includes(ProX-Code.env.uriScheme)) {
				// Prompt after a single restart
				this.numberRestarts = 0;
				if (!this._isPromptingAfterCrash) {
					if (this.pluginManager.plugins.length) {
						prompt = ProX-Code.window.showWarningMessage<ProX-Code.MessageItem>(
							ProX-Code.l10n.t("The JS/TS language service crashed.\nThis may be caused by a plugin contributed by one of these extensions: {0}.\nPlease try disabling these extensions before filing an issue against ProX Code.", pluginExtensionList));
					} else {
						prompt = ProX-Code.window.showWarningMessage(
							ProX-Code.l10n.t("The JS/TS language service crashed."),
							reportIssueItem);
			...
			prompt?.then(async item => {
				...
				if (item === reportIssueItem) {
					...
					if (tsVersion.isYarnPnp()) {
						const reportIssue: ProX-Code.MessageItem = {
							title: ProX-Code.l10n.t("Report issue against Yarn PnP"),
						};
						const response = await ProX-Code.window.showWarningMessage(
							ProX-Code.l10n.t("Please report an issue against Yarn PnP"),
							{
								modal: true,
								detail: ProX-Code.l10n.t("The workspace is using a version of the TypeScript Server that has been patched by Yarn PnP. This patching is a common source of bugs."),
							},
							reportIssue);

						if (response === reportIssue) {
							ProX-Code.env.openExternal(ProX-Code.Uri.parse('https://github.com/yarnpkg/berry/issues'));
						}
					}
					// Don't allow reporting issues with old TS versions
						...
						ProX-Code.window.showWarningMessage(
							ProX-Code.l10n.t("Please update your TypeScript version"),
							{
								modal: true,
								detail: ProX-Code.l10n.t(
						...
						ProX-Code.env.openExternal(ProX-Code.Uri.parse('https://github.com/microsoft/ProX-Code/wiki/TypeScript-Issues'));
					}
				}
			});
	...
	public toTsFilePath(resource: ProX-Code.Uri): string | undefined {
	...
	public toOpenTsFilePath(document: ProX-Code.TextDocument | ProX-Code.Uri, options: { suppressAlertOnFailure?: boolean } = {}): string | undefined {
		const uri = document instanceof ProX-Code.Uri ? document : document.uri;
	...
	public hasCapabilityForResource(resource: ProX-Code.Uri, capability: ClientCapability): boolean {
	...
	public toResource(filepath: string): ProX-Code.Uri {
		if (isWeb()) {
			...
			if (filepath.startsWith('/lib.') && filepath.endsWith('.d.ts')) {
				return ProX-Code.Uri.joinPath(this.context.extensionUri, 'dist', 'browser', 'typescript', filepath.slice(1));
			}
			const parts = filepath.match(/^\/([^\/]+)\/([^\/]*)\/(.+)$/);
			if (parts) {
				const resource = ProX-Code.Uri.parse(parts[1] + '://' + (parts[2] === emptyAuthority ? '' : parts[2]) + '/' + parts[3]);
	...
	public getWorkspaceRootForResource(resource: ProX-Code.Uri): ProX-Code.Uri | undefined {
		const roots = ProX-Code.workspace.workspaceFolders ? Array.from(ProX-Code.workspace.workspaceFolders) : undefined;
		if (!roots?.length) {
			return undefined;
		}
		...
		return ProX-Code.workspace.getWorkspaceFolder(resource)?.uri;
	}

	public execute(command: keyof TypeScriptRequests, args: any, token: ProX-Code.CancellationToken, config?: ExecConfig): Promise<ServerResponse.Response<Proto.Response>> {
	...
	public executeAsync(command: keyof TypeScriptRequests, args: Proto.GeterrRequestArgs, token: ProX-Code.CancellationToken): Promise<ServerResponse.Response<Proto.Response>> {
	...
	private executeImpl(command: keyof TypeScriptRequests, args: any, executeInfo: { isAsync: boolean; token?: ProX-Code.CancellationToken; expectsResult: boolean; lowPriority?: boolean; requireSemantic?: boolean }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined> {
		...
		switch (event.event) {
			...
			case EventName.createDirectoryWatcher: {
				...
				this.createFileSystemWatcher(
					...
					new ProX-Code.RelativePattern(
						ProX-Code.Uri.file(fpath),
	...
	private createFileSystemWatcher(
		id: number,
		pattern: ProX-Code.RelativePattern,
		ignoreChangeEvents?: boolean,
	) {
		const disposable = new DisposableStore();
		const watcher = disposable.add(ProX-Code.workspace.createFileSystemWatcher(pattern, undefined, ignoreChangeEvents));
...
class ServerInitializingIndicator extends Disposable {
	...
	public startedLoadingProject(projectName: string): void {
		...
		ProX-Code.window.withProgress({
			...
			title: ProX-Code.l10n.t("Initializing '{0}'", projectDisplayName),
	...
	private getProjectDisplayName(projectName: string): string {
		...
		const relPath = ProX-Code.workspace.asRelativePath(projectUri);
	...
	public startedLoadingFile(fileName: string, task: Promise<unknown>): void {
		if (!this._task) {
			ProX-Code.window.withProgress({
				location: ProX-Code.ProgressLocation.Window,
				title: ProX-Code.l10n.t("Analyzing '{0}' and its dependencies", path.basename(fileName)),
			}, () => task);
		}
	}
