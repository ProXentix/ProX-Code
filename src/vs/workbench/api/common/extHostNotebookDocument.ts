...
class RawContentChangeEvent {
	...
	constructor(
		...
		readonly deletedItems: ProX-Code.NotebookCell[],
		readonly items: ExtHostCell[]
	) { }

	asApiEvent(): ProX-Code.NotebookDocumentContentChange {
		return {
...
export class ExtHostCell {
	...
	private _outputs: ProX-Code.NotebookCellOutput[];
	private _metadata: Readonly<notebookCommon.NotebookCellMetadata>;
	private _previousResult: Readonly<ProX-Code.NotebookCellExecutionSummary | undefined>;
	...
	private _apiCell: ProX-Code.NotebookCell | undefined;
	...
	get apiCell(): ProX-Code.NotebookCell {
		if (!this._apiCell) {
			const that = this;
			...
			const apiCell: ProX-Code.NotebookCell = {
...
export class ExtHostNotebookDocument {
	...
	private _notebook: ProX-Code.NotebookDocument | undefined;
	...
	get apiNotebook(): ProX-Code.NotebookDocument {
		if (!this._notebook) {
			const that = this;
			const apiObject: ProX-Code.NotebookDocument = {
	...
	acceptModelChanged(event: extHostProtocol.NotebookCellsChangedEventDto, isDirty: boolean, newMetadata: notebookCommon.NotebookDocumentMetadata | undefined): ProX-Code.NotebookDocumentChangeEvent {
		...
		const result = {
			...
			cellChanges: <ProX-Code.NotebookDocumentCellChange[]>[],
			contentChanges: <ProX-Code.NotebookDocumentContentChange[]>[],
		};

		type RelaxedCellChange = Partial<ProX-Code.NotebookDocumentCellChange> & { cell: ProX-Code.NotebookCell };
		...
		const map = new Map<ProX-Code.NotebookCell, number>();
	...
	private _validateRange(range: ProX-Code.NotebookRange): ProX-Code.NotebookRange {
	...
	private _getCells(range: ProX-Code.NotebookRange): ExtHostCell[] {
	...
	private _spliceNotebookCells(splices: notebookCommon.NotebookCellTextModelSplice<extHostProtocol.NotebookCellDto>[], initialization: boolean, bucket: ProX-Code.NotebookDocumentContentChange[] | undefined): void {
		if (this._disposed) {
			return;
		}
	...
	private _moveCells(index: number, length: number, newIdx: number, bucket: ProX-Code.NotebookDocumentContentChange[]): void {
	...
	getCellFromApiCell(apiCell: ProX-Code.NotebookCell): ExtHostCell | undefined {
