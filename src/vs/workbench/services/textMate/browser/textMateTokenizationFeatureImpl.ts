...
import type { IGrammar, IOnigLib, IRawTheme } from 'ProX-Code-textmate';
...
export class TextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
	...
	constructor(
		...
		this._ProX-CodeOniguruma = null;

		this._styleElement = domStylesheets.createStyleSheet();
		this._styleElement.className = 'ProX-Code-tokens-styles';
		...
		if (this._debugMode) {
			this._progressService.withProgress(
				...
				(progress) => {
					...
					return this._getVSCodeOniguruma().then((ProX-CodeOniguruma) => {
						ProX-CodeOniguruma.setDefaultDebugCall(true);
						progress.report({
				...
				(choice) => {
					this._getVSCodeOniguruma().then((ProX-CodeOniguruma) => {
						this._debugModePrintFunc = () => { };
						this._debugMode = false;
						ProX-CodeOniguruma.setDefaultDebugCall(false);
						onStop();
					});
				}
			);
	...
	private async _getOrCreateGrammarFactory(): Promise<TMGrammarFactory> {
		...
		const [ProX-CodeTextmate, ProX-CodeOniguruma] = await Promise.all([importAMDNodeModule<typeof import('ProX-Code-textmate')>('ProX-Code-textmate', 'release/main.js'), this._getVSCodeOniguruma()]);
		const onigLib: Promise<IOnigLib> = Promise.resolve({
			createOnigScanner: (sources: string[]) => ProX-CodeOniguruma.createOnigScanner(sources),
			createOnigString: (str: string) => ProX-CodeOniguruma.createOnigString(str)
		});
		...
		}, this._grammarDefinitions || [], ProX-CodeTextmate, onigLib);
	...
	private _ProX-CodeOniguruma: Promise<typeof import('ProX-Code-oniguruma')> | null;
	private _getVSCodeOniguruma(): Promise<typeof import('ProX-Code-oniguruma')> {
		if (!this._ProX-CodeOniguruma) {
			this._ProX-CodeOniguruma = (async () => {
				const [ProX-CodeOniguruma, wasm] = await Promise.all([importAMDNodeModule<typeof import('ProX-Code-oniguruma')>('ProX-Code-oniguruma', 'release/main.js'), this._loadVSCodeOnigurumaWASM()]);
				await ProX-CodeOniguruma.loadWASM({
					data: wasm,
					print: (str: string) => {
						this._debugModePrintFunc(str);
					}
				});
				return ProX-CodeOniguruma;
			})();
		}
		return this._ProX-CodeOniguruma;
	}

	private async _loadVSCodeOnigurumaWASM(): Promise<Response | ArrayBuffer> {
		if (isWeb) {
			const response = await fetch(resolveAmdNodeModulePath('ProX-Code-oniguruma', 'release/onig.wasm'));
			...
			const response = await fetch(canASAR && this._environmentService.isBuilt
				? FileAccess.asBrowserUri(`${nodeModulesAsarUnpackedPath}/ProX-Code-oniguruma/release/onig.wasm`).toString(true)
				: FileAccess.asBrowserUri(`${nodeModulesPath}/ProX-Code-oniguruma/release/onig.wasm`).toString(true));
			return response;
		}
	}
