...
interface AttachmentCleanRequest {
	notebook: ProX-Code.NotebookDocument;
...
export class AttachmentCleaner implements ProX-Code.CodeActionProvider {
	...
	private _imageDiagnosticCollection: ProX-Code.DiagnosticCollection;
	private readonly _delayer = new Delayer(750);

	constructor() {
		this._disposables = [];
		this._imageDiagnosticCollection = ProX-Code.languages.createDiagnosticCollection('Notebook Image Attachment');
		this._disposables.push(this._imageDiagnosticCollection);

		this._disposables.push(ProX-Code.commands.registerCommand(ATTACHMENT_CLEANUP_COMMANDID, async (document: ProX-Code.Uri, range: ProX-Code.Range) => {
		...
		this._disposables.push(ProX-Code.languages.registerCodeActionsProvider(JUPYTER_NOTEBOOK_MARKDOWN_SELECTOR, this, {
			providedCodeActionKinds: [
				ProX-Code.CodeActionKind.QuickFix
			],
		}));

		this._disposables.push(ProX-Code.workspace.onDidChangeNotebookDocument(e => {
			this._delayer.trigger(() => {
				...
				e.cellChanges.forEach(change => {
					...
					if (change.cell.kind !== ProX-Code.NotebookCellKind.Markup) {
						return;
					}
					...
					if (metadataEdit) {
						...
						ProX-Code.workspace.applyEdit(workspaceEdit);
					}
				});
			});
		}));
		...
		this._disposables.push(ProX-Code.workspace.onWillSaveNotebookDocument(e => {
			if (e.reason === ProX-Code.TextDocumentSaveReason.Manual) {
				this._delayer.dispose();
				if (e.notebook.getCells().length === 0) {
					return;
				}
				const notebookEdits: ProX-Code.NotebookEdit[] = [];
				for (const cell of e.notebook.getCells()) {
					if (cell.kind !== ProX-Code.NotebookCellKind.Markup) {
						continue;
					}
		...
		this._disposables.push(ProX-Code.workspace.onDidCloseNotebookDocument(e => {
			this._attachmentCache.delete(e.uri.toString());
		}));

		this._disposables.push(ProX-Code.workspace.onWillRenameFiles(e => {
			const re = /\.ipynb$/;
		...
		this._disposables.push(ProX-Code.workspace.onDidOpenTextDocument(e => {
			this.analyzeMissingAttachments(e);
		}));

		this._disposables.push(ProX-Code.workspace.onDidCloseTextDocument(e => {
			this.analyzeMissingAttachments(e);
		}));

		ProX-Code.workspace.textDocuments.forEach(document => {
	 ...
	 * @returns ProX-Code.NotebookEdit, the metadata alteration performed on the json behind the ipynb
	 */
	private cleanNotebookAttachments(e: AttachmentCleanRequest): ProX-Code.NotebookEdit | undefined {
		...
		if (cell.index > -1 && !objectEquals(markdownAttachmentsInUse || {}, cell.metadata.attachments || {})) {
			...
			const metadataEdit = ProX-Code.NotebookEdit.updateCellMetadata(cell.index, updateMetadata);
			return metadataEdit;
		}
		return;
	}

	private analyzeMissingAttachments(document: ProX-Code.TextDocument): void {
		if (document.uri.scheme !== 'ProX-Code-notebook-cell') {
			// not notebook
			return;
		}
		...
		let activeCell: ProX-Code.NotebookCell | undefined;
		for (const notebookDocument of ProX-Code.workspace.notebookDocuments) {
	...
	private updateDiagnostics(cellUri: ProX-Code.Uri, diagnostics: IAttachmentDiagnostic[]) {
		const ProX-CodeDiagnostics: ProX-Code.Diagnostic[] = [];
		for (const currDiagnostic of diagnostics) {
			currDiagnostic.ranges.forEach(range => {
				const diagnostic = new ProX-Code.Diagnostic(range, `The image named: '${currDiagnostic.name}' is not present in cell metadata.`, ProX-Code.DiagnosticSeverity.Warning);
				diagnostic.code = DiagnosticCode.missing_attachment;
				ProX-CodeDiagnostics.push(diagnostic);
			});
		}

		this._imageDiagnosticCollection.set(cellUri, ProX-CodeDiagnostics);
	}
	 ...
	 */
	private getAttachmentNames(document: ProX-Code.TextDocument) {
		const source = document.getText();
		const filenames: Map<string, { valid: boolean; ranges: ProX-Code.Range[] }> = new Map();
		...
		while ((match = re.exec(source))) {
			if (match.groups?.filename) {
				...
				const range = new ProX-Code.Range(startPosition, endPosition);
