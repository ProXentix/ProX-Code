...
suite('IndexTreeModel', () => {
	...
	test('recursive filter', () => {
		...
		query = /build/;
		model.refilter();
		assert.deepStrictEqual(toArray(list), ['ProX-Code', '.build', 'github', 'build.js', 'build']);

		model.setCollapsed([0], true);
		assert.deepStrictEqual(toArray(list), ['ProX-Code']);
	...
	test('recursive filter with collapse', () => {
		...
		query = /gulp/;
		model.refilter();
		assert.deepStrictEqual(toArray(list), ['ProX-Code', 'build', 'gulpfile.js']);

		model.setCollapsed([0, 3], true);
		assert.deepStrictEqual(toArray(list), ['ProX-Code', 'build']);
	...
	test('recursive filter while collapsed', () => {
		...
		model.splice([0], 0, [
			{
				element: 'ProX-Code', collapsed: true, children: [
					{ element: '.build' },
					{ element: 'git' },
					{
						element: 'github', children: [
