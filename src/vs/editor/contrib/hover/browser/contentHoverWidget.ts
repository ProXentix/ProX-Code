...
export class ContentHoverWidget extends ResizableContentWidget {
	...
	private _setHoverWidgetMaxDimensions(width: number | string, height: number | string): void {
		...
		this._hover.containerDomNode.style.setProperty('--ProX-Code-hover-maxWidth', typeof width === 'number' ? `${width}px` : width);
		this._layoutContentWidget();
	}
	...
	private _isHoverTextOverflowing(): boolean {
		...
		this._hover.containerDomNode.style.setProperty('--ProX-Code-hover-whiteSpace', 'nowrap');
		this._hover.containerDomNode.style.setProperty('--ProX-Code-hover-sourceWhiteSpace', 'nowrap');

		const overflowing = Array.from(this._hover.contentsDomNode.children).some((hoverElement) => {
			return hoverElement.scrollWidth > hoverElement.clientWidth;
		});

		this._hover.containerDomNode.style.removeProperty('--ProX-Code-hover-whiteSpace');
		this._hover.containerDomNode.style.removeProperty('--ProX-Code-hover-sourceWhiteSpace');
	...
	public show(renderedHover: RenderedContentHover): void {
		...
		// See https://github.com/microsoft/ProX-Code/issues/140339
