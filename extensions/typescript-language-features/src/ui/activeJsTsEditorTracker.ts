 ...
 * instead of using `ProX-Code.window.activeTextEditor`
 */
export class ActiveJsTsEditorTracker extends Disposable {

	private _activeJsTsEditor: ProX-Code.TextEditor | undefined;

	private readonly _onDidChangeActiveJsTsEditor = this._register(new ProX-Code.EventEmitter<ProX-Code.TextEditor | undefined>());
	public readonly onDidChangeActiveJsTsEditor = this._onDidChangeActiveJsTsEditor.event;

	public constructor() {
		super();

		this._register(ProX-Code.window.onDidChangeActiveTextEditor(_ => this.update()));
		this._register(ProX-Code.window.onDidChangeVisibleTextEditors(_ => this.update()));
		this._register(ProX-Code.window.tabGroups.onDidChangeTabGroups(_ => this.update()));

		this.update();
	}

	public get activeJsTsEditor(): ProX-Code.TextEditor | undefined {
	...
	private getEditorCandidatesForActiveTab(): ProX-Code.TextEditor[] {
		const tab = ProX-Code.window.tabGroups.activeTabGroup.activeTab;
		if (!tab) {
			return [];
		}
		...
		if (tab.input instanceof ProX-Code.TabInputText) {
			...
			const editor = ProX-Code.window.visibleTextEditors.find(editor => {
		...
		if (tab.input instanceof ProX-Code.TabInputTextDiff) {
			...
			// view column of its parent diff editor. See https://github.com/microsoft/ProX-Code/issues/201845
			return coalesce([ProX-Code.window.activeTextEditor, ...ProX-Code.window.visibleTextEditors]).filter(editor => {
		...
		if (tab.input instanceof ProX-Code.TabInputNotebook) {
			...
			const notebook = ProX-Code.window.visibleNotebookEditors.find(editor =>
				editor.notebook.uri.toString() === (tab.input as ProX-Code.TabInputNotebook).uri.toString()
				&& editor.viewColumn === tab.group.viewColumn);
	...
	private isManagedFile(editor: ProX-Code.TextEditor): boolean {
		return this.isManagedScriptFile(editor) || this.isManagedConfigFile(editor);
	}

	private isManagedScriptFile(editor: ProX-Code.TextEditor): boolean {
		return isSupportedLanguageMode(editor.document);
	}

	private isManagedConfigFile(editor: ProX-Code.TextEditor): boolean {
