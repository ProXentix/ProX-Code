/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt in the project root for license information.
 *--------------------------------------------------------------------------------------------*/

/**
 * Helpers for converting FROM ProX-Code types TO ts types
 */

import * as ProX-Code from 'ProX-Code';
import type * as Proto from './tsServer/protocol/protocol';
import * as PConst from './tsServer/protocol/protocol.const';
import { ITypeScriptServiceClient } from './typescriptService';

export namespace Range {
	export const fromTextSpan = (span: Proto.TextSpan): ProX-Code.Range =>
		fromLocations(span.start, span.end);

	export const toTextSpan = (range: ProX-Code.Range): Proto.TextSpan => ({
		start: Position.toLocation(range.start),
		end: Position.toLocation(range.end)
	});

	export const fromLocations = (start: Proto.Location, end: Proto.Location): ProX-Code.Range =>
		new ProX-Code.Range(
			Math.max(0, start.line - 1), Math.max(start.offset - 1, 0),
			Math.max(0, end.line - 1), Math.max(0, end.offset - 1));

	export const toFileRange = (range: ProX-Code.Range): Proto.FileRange => ({
		startLine: range.start.line + 1,
		startOffset: range.start.character + 1,
		endLine: range.end.line + 1,
		endOffset: range.end.character + 1
	});

	export const toFileRangeRequestArgs = (file: string, range: ProX-Code.Range): Proto.FileRangeRequestArgs => ({
		file,
		...toFileRange(range)
	});

	export const toFileRangesRequestArgs = (file: string, ranges: ProX-Code.Range[]): Proto.FileRangesRequestArgs => ({
		file,
		ranges: ranges.map(toFileRange)
	});

	export const toFormattingRequestArgs = (file: string, range: ProX-Code.Range): Proto.FormatRequestArgs => ({
		file,
		line: range.start.line + 1,
		offset: range.start.character + 1,
		endLine: range.end.line + 1,
		endOffset: range.end.character + 1
	});
}

export namespace Position {
	export const fromLocation = (tslocation: Proto.Location): ProX-Code.Position =>
		new ProX-Code.Position(tslocation.line - 1, tslocation.offset - 1);

	export const toLocation = (vsPosition: ProX-Code.Position): Proto.Location => ({
		line: vsPosition.line + 1,
		offset: vsPosition.character + 1,
	});

	export const toFileLocationRequestArgs = (file: string, position: ProX-Code.Position): Proto.FileLocationRequestArgs => ({
		file,
		line: position.line + 1,
		offset: position.character + 1,
	});
}

export namespace Location {
	export const fromTextSpan = (resource: ProX-Code.Uri, tsTextSpan: Proto.TextSpan): ProX-Code.Location =>
		new ProX-Code.Location(resource, Range.fromTextSpan(tsTextSpan));
}

export namespace TextEdit {
	export const fromCodeEdit = (edit: Proto.CodeEdit): ProX-Code.TextEdit =>
		new ProX-Code.TextEdit(
			Range.fromTextSpan(edit),
			edit.newText);
}

export namespace WorkspaceEdit {
	export function fromFileCodeEdits(
		client: ITypeScriptServiceClient,
		edits: Iterable<Proto.FileCodeEdits>
	): ProX-Code.WorkspaceEdit {
		return withFileCodeEdits(new ProX-Code.WorkspaceEdit(), client, edits);
	}

	export function withFileCodeEdits(
		workspaceEdit: ProX-Code.WorkspaceEdit,
		client: ITypeScriptServiceClient,
		edits: Iterable<Proto.FileCodeEdits>
	): ProX-Code.WorkspaceEdit {
		for (const edit of edits) {
			const resource = client.toResource(edit.fileName);
			for (const textChange of edit.textChanges) {
				workspaceEdit.replace(resource,
					Range.fromTextSpan(textChange),
					textChange.newText);
			}
		}

		return workspaceEdit;
	}
}

export namespace SymbolKind {
	export function fromProtocolScriptElementKind(kind: Proto.ScriptElementKind) {
		switch (kind) {
			case PConst.Kind.module: return ProX-Code.SymbolKind.Module;
			case PConst.Kind.class: return ProX-Code.SymbolKind.Class;
			case PConst.Kind.enum: return ProX-Code.SymbolKind.Enum;
			case PConst.Kind.enumMember: return ProX-Code.SymbolKind.EnumMember;
			case PConst.Kind.interface: return ProX-Code.SymbolKind.Interface;
			case PConst.Kind.indexSignature: return ProX-Code.SymbolKind.Method;
			case PConst.Kind.callSignature: return ProX-Code.SymbolKind.Method;
			case PConst.Kind.method: return ProX-Code.SymbolKind.Method;
			case PConst.Kind.memberVariable: return ProX-Code.SymbolKind.Property;
			case PConst.Kind.memberGetAccessor: return ProX-Code.SymbolKind.Property;
			case PConst.Kind.memberSetAccessor: return ProX-Code.SymbolKind.Property;
			case PConst.Kind.variable: return ProX-Code.SymbolKind.Variable;
			case PConst.Kind.let: return ProX-Code.SymbolKind.Variable;
			case PConst.Kind.const: return ProX-Code.SymbolKind.Variable;
			case PConst.Kind.localVariable: return ProX-Code.SymbolKind.Variable;
			case PConst.Kind.alias: return ProX-Code.SymbolKind.Variable;
			case PConst.Kind.function: return ProX-Code.SymbolKind.Function;
			case PConst.Kind.localFunction: return ProX-Code.SymbolKind.Function;
			case PConst.Kind.constructSignature: return ProX-Code.SymbolKind.Constructor;
			case PConst.Kind.constructorImplementation: return ProX-Code.SymbolKind.Constructor;
			case PConst.Kind.typeParameter: return ProX-Code.SymbolKind.TypeParameter;
			case PConst.Kind.string: return ProX-Code.SymbolKind.String;
			default: return ProX-Code.SymbolKind.Variable;
		}
	}
}

export namespace CompletionTriggerKind {
	export function toProtocolCompletionTriggerKind(kind: ProX-Code.CompletionTriggerKind): Proto.CompletionTriggerKind {
		switch (kind) {
			case ProX-Code.CompletionTriggerKind.Invoke: return 1;
			case ProX-Code.CompletionTriggerKind.TriggerCharacter: return 2;
			case ProX-Code.CompletionTriggerKind.TriggerForIncompleteCompletions: return 3;
		}
	}
}

export namespace OrganizeImportsMode {
	export function toProtocolOrganizeImportsMode(mode: PConst.OrganizeImportsMode): Proto.OrganizeImportsMode {
		switch (mode) {
			case PConst.OrganizeImportsMode.All: return 'All' as Proto.OrganizeImportsMode.All;
			case PConst.OrganizeImportsMode.SortAndCombine: return 'SortAndCombine' as Proto.OrganizeImportsMode.SortAndCombine;
			case PConst.OrganizeImportsMode.RemoveUnused: return 'RemoveUnused' as Proto.OrganizeImportsMode.RemoveUnused;
		}
	}
}
