...
export class Condition extends Disposable {
	...
	private readonly _onDidChange = this._register(new ProX-Code.EventEmitter<void>());
	public readonly onDidChange = this._onDidChange.event;
}

class ConditionalRegistration {
	private registration: ProX-Code.Disposable | undefined = undefined;

	public constructor(
		private readonly conditions: readonly Condition[],
		private readonly doRegister: () => ProX-Code.Disposable
	) {
...
export function conditionalRegistration(
	...
	doRegister: () => ProX-Code.Disposable,
...
export function requireGlobalConfiguration(
	...
	return new Condition(
		() => {
			const config = ProX-Code.workspace.getConfiguration(section, null);
