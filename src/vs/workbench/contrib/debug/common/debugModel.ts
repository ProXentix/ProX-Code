...
interface IDebugProtocolVariableWithContext extends DebugProtocol.Variable {
	__ProX-CodeVariableMenuContext?: string;
}
		...
		try {
			...
			const vars = response.body.variables.filter(v => !!v).map((v: IDebugProtocolVariableWithContext) => {
				if (isString(v.value) && isString(v.name) && typeof v.variablesReference === 'number') {
					...
					return new Variable(this.session, this.threadId, this, v.variablesReference, v.name, v.evaluateName, v.value, v.namedVariables, v.indexedVariables, v.memoryReference, v.presentationHint, v.type, v.__ProX-CodeVariableMenuContext, true, 0, idDuplicationIndex, v.declarationLocationReference, v.valueLocationReference);
				}
...
export class Variable extends ExpressionContainer implements IExpression {
	...
	async setVariable(value: string, stackFrame: IStackFrame): Promise<any> {
		...
		try {
			// Send out a setExpression for debug extensions that do not support set variables https://github.com/microsoft/ProX-Code/issues/124679#issuecomment-869844437
