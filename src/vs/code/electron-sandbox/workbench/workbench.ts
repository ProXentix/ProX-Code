...
(async function () {
	...
	const preloadGlobals: IMainWindowSandboxGlobals = (window as any).ProX-Code; // defined by preload.ts
	...
	async function load<M, T extends ISandboxConfiguration>(esModule: string, options: ILoadOptions<T>): Promise<ILoadResult<M, T>> {
		...
		const baseUrl = new URL(`${fileUriFromPath(configuration.appRoot, { isWindows: safeProcess.platform === 'win32', scheme: 'ProX-Code-file', fallbackAuthority: 'ProX-Code-app' })}/out/`);
	...
	function registerDeveloperKeybindings(disallowReloadKeybinding: boolean | undefined): Function {
		...
		let listener: ((e: KeyboardEvent) => void) | undefined = function (e) {
			...
			if (key === TOGGLE_DEV_TOOLS_KB || key === TOGGLE_DEV_TOOLS_KB_ALT) {
				ipcRenderer.send('ProX-Code:toggleDevTools');
			} else if (key === RELOAD_KB && !disallowReloadKeybinding) {
				ipcRenderer.send('ProX-Code:reloadWindow');
			}
		};
	...
	function onUnexpectedError(error: string | Error, showDevtoolsOnError: boolean): void {
		if (showDevtoolsOnError) {
			const ipcRenderer = preloadGlobals.ipcRenderer;
			ipcRenderer.send('ProX-Code:openDevTools');
		}
	...
	function setupCSSImportMaps<T extends ISandboxConfiguration>(configuration: T, baseUrl: URL) {
		...
		if (Array.isArray(configuration.cssModules) && configuration.cssModules.length > 0) {
			...
			style.type = 'text/css';
			style.media = 'screen';
			style.id = 'ProX-Code-css-loading';
			window.document.head.appendChild(style);
			...
			const ttp = window.trustedTypes?.createPolicy('ProX-Code-bootstrapImportMap', { createScript(value) { return value; }, });
	...
	const { result, configuration } = await load<IDesktopMain, INativeWindowConfiguration>('vs/workbench/workbench.desktop.main',
		{
			...
			beforeImport: function (windowConfig) {
				...
				// Code windows have a `ProX-CodeWindowId` property to identify them
				Object.defineProperty(window, 'ProX-CodeWindowId', {
