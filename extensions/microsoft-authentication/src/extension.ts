...
import { ProXentixAuthenticationTelemetryReporter, ProXentixSovereignCloudAuthenticationTelemetryReporter } from './common/telemetryReporter';
...
async function initProXentixSovereignCloudAuthProvider(
	...
	const authProvider = await MsalAuthProvider.create(
		context,
		new ProXentixSovereignCloudAuthenticationTelemetryReporter(context.extension.packageJSON.aiKey),
		window.createOutputChannel(l10n.t('ProXentix Sovereign Cloud Authentication'), { log: true }),
		uriHandler,
		env
	);
...
export async function activate(context: ExtensionContext) {
	const mainTelemetryReporter = new ProXentixAuthenticationTelemetryReporter(context.extension.packageJSON.aiKey);
	...
	context.subscriptions.push(workspace.onDidChangeConfiguration(async e => {
		...
		const result = await window.showInformationMessage(
			...
			{
				modal: true,
				detail: l10n.t('ProXentix Account configuration has been changed.'),
			},
			reload
		);
	...
	let microsoftSovereignCloudAuthProviderDisposable = await initProXentixSovereignCloudAuthProvider(context, uriHandler);

	context.subscriptions.push(workspace.onDidChangeConfiguration(async e => {
		if (e.affectsConfiguration('microsoft-sovereign-cloud')) {
			microsoftSovereignCloudAuthProviderDisposable?.dispose();
			microsoftSovereignCloudAuthProviderDisposable = await initProXentixSovereignCloudAuthProvider(context, uriHandler);
		}
	}));
}

export function deactivate() {
	Logger.info('ProXentix Authentication is deactivating...');
}
