...
export class DiskTypeScriptVersionProvider implements ITypeScriptVersionProvider {
	...
	public get bundledVersion(): TypeScriptVersion {
		const version = this.getContributedVersion(TypeScriptVersionSource.Bundled, 'ProX-Code.typescript-language-features', ['..', 'node_modules']);
		if (version) {
			return version;
		}
	...
	private get contributedTsNextVersion(): TypeScriptVersion | undefined {
		return this.getContributedVersion(TypeScriptVersionSource.TsNightlyExtension, 'ms-ProX-Code.ProX-Code-typescript-next', ['node_modules']);
	}
	...
	private getContributedVersion(source: TypeScriptVersionSource, extensionId: string, pathToTs: readonly string[]): TypeScriptVersion | undefined {
		try {
			const extension = ProX-Code.extensions.getExtension(extensionId);
			if (extension) {
	...
	private loadTypeScriptVersionsFromPath(source: TypeScriptVersionSource, relativePath: string): TypeScriptVersion[] {
		...
		for (const root of ProX-Code.workspace.workspaceFolders) {
			let label: string = relativePath;
			if (ProX-Code.workspace.workspaceFolders.length > 1) {
	...
	private static getApiVersion(serverPath: string): API | undefined {
		...
		const tsdkVersion = ProX-Code.workspace.getConfiguration().get<string | undefined>('typescript.tsdk_version', undefined);
		if (tsdkVersion) {
