...
import VsCodeTelemetryReporter from '@ProX-Code/extension-telemetry';
...
export async function activate(context: ProX-Code.ExtensionContext): Promise<Api> {
	...
	const onCompletionAccepted = new ProX-Code.EventEmitter<ProX-Code.CompletionItem>();
	...
	const versionProvider = new StaticVersionProvider(
		new TypeScriptVersion(
			...
			ProX-Code.Uri.joinPath(context.extensionUri, 'dist/browser/typescript/tsserver.web.js').toString(),
...
async function startPreloadWorkspaceContentsIfNeeded(context: ProX-Code.ExtensionContext, logger: Logger): Promise<void> {
	...
	await Promise.all(ProX-Code.workspace.workspaceFolders.map(async folder => {
		const workspaceUri = folder.uri;
		if (workspaceUri.scheme !== 'ProX-Code-vfs' || !workspaceUri.authority.startsWith('github')) {
...
class RemoteWorkspaceContentsPreloader extends Disposable {
	...
	constructor(
		private readonly workspaceUri: ProX-Code.Uri,
		private readonly logger: Logger,
	) {
		super();

		const fsWatcher = this._register(ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(workspaceUri, '*')));
