...
async function main() {
    ...
    if (process.env['BUILT_SERVER']) {
        ...
        const serverArchivePath = `.build/win32-${arch}/ProX-Code-server-win32-${arch}.zip`;
        await (0, zx_1.$) `7z.exe a -tzip ${serverArchivePath} ../ProX-Code-server-win32-${arch}`.pipe(process.stdout);
    ...
    if (process.env['BUILT_WEB']) {
        ...
        const webArchivePath = `.build/win32-${arch}/ProX-Code-server-win32-${arch}-web.zip`;
        await (0, zx_1.$) `7z.exe a -tzip ${webArchivePath} ../ProX-Code-server-win32-${arch}-web`.pipe(process.stdout);
        await (0, zx_1.$) `7z.exe l ${webArchivePath}`.pipe(process.stdout);
    }
    // Sign setup
    if (process.env['BUILT_CLIENT']) {
        (0, codesign_1.printBanner)('Sign setup packages (system, user)');
        const task = (0, zx_1.$) `npm exec -- npm-run-all -lp "gulp ProX-Code-win32-${arch}-system-setup -- --sign" "gulp ProX-Code-win32-${arch}-user-setup -- --sign"`;
