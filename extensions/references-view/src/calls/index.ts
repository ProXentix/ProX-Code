...
export function register(tree: SymbolsTree, context: ProX-Code.ExtensionContext): void {
	...
	function showCallHierarchy() {
		if (ProX-Code.window.activeTextEditor) {
			const input = new CallsTreeInput(new ProX-Code.Location(ProX-Code.window.activeTextEditor.document.uri, ProX-Code.window.activeTextEditor.selection.active), direction.value);
			tree.setInput(input);
		}
	}
	...
	function setCallsDirection(value: CallsDirection, anchor: CallItem | unknown) {
		...
		if (anchor instanceof CallItem) {
			newInput = new CallsTreeInput(new ProX-Code.Location(anchor.item.uri, anchor.item.selectionRange.start), direction.value);
	...
	context.subscriptions.push(
		ProX-Code.commands.registerCommand('references-view.showCallHierarchy', showCallHierarchy),
		ProX-Code.commands.registerCommand('references-view.showOutgoingCalls', (item: CallItem | unknown) => setCallsDirection(CallsDirection.Outgoing, item)),
		ProX-Code.commands.registerCommand('references-view.showIncomingCalls', (item: CallItem | unknown) => setCallsDirection(CallsDirection.Incoming, item)),
		ProX-Code.commands.registerCommand('references-view.removeCallItem', removeCallItem)
	);
}
