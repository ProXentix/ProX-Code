...
import { LanguageClient, LanguageClientOptions } from 'ProX-Code-languageclient/browser';
...
function startServer(context: ProX-Code.ExtensionContext, parser: IMdParser): Promise<MdLanguageClient> {
	const serverMain = ProX-Code.Uri.joinPath(context.extensionUri, 'dist', 'browser', 'serverWorkerMain.js');

	const worker = new Worker(serverMain.toString());
	worker.postMessage({ i10lLocation: ProX-Code.l10n.uri?.toString() ?? '' });
