...
(function () {
	...
	const initialState = ProX-Code.getState() || { scale: 'fit', offsetX: 0, offsetY: 0 };
	...
	function updateScale(newScale) {
		...
		if (newScale === 'fit') {
			...
			ProX-Code.setState({ scale: scale, offsetX: newScrollX, offsetY: newScrollY });
		}
	...
	window.addEventListener('scroll', e => {
		...
		const entry = ProX-Code.getState();
		if (entry) {
			ProX-Code.setState({ scale: entry.scale, offsetX: window.scrollX, offsetY: window.scrollY });
		}
	}, { passive: true });
