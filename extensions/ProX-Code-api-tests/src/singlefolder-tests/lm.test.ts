...
suite('lm', function () {
	...
	const testProviderOptions: ProX-Code.ChatResponseProviderMetadata = {
		name: 'test-lm',
		version: '1.0.0',
		family: 'test',
	...
	test('lm request and stream', async function () {
		...
		let p: ProX-Code.Progress<ProX-Code.ChatResponseFragment2> | undefined;
		const defer = new DeferredPromise<void>();

		disposables.push(ProX-Code.lm.registerChatModelProvider('test-lm', {
		...
		const models = await ProX-Code.lm.selectChatModels({ id: 'test-lm' });
		assert.strictEqual(models.length, 1);

		const request = await models[0].sendRequest([ProX-Code.LanguageModelChatMessage.User('Hello')]);
		...
		p.report({ index: 0, part: new ProX-Code.LanguageModelTextPart('Hello') });
		defer.complete();
	...
	test('lm request fail', async function () {
		...
		try {
			await models[0].sendRequest([ProX-Code.LanguageModelChatMessage.User('Hello')]);
	...
	test('lm stream fail', async function () {
		...
		const res = await models[0].sendRequest([ProX-Code.LanguageModelChatMessage.User('Hello')]);
		assert.ok(res);
	...
	test('LanguageModelError instance is not thrown to extensions#235322 (SYNC)', async function () {
		...
		disposables.push(ProX-Code.lm.registerChatModelProvider('test-lm', {
			provideLanguageModelResponse(_messages, _options, _extensionId, _progress, _token) {
				throw ProX-Code.LanguageModelError.Blocked('You have been blocked SYNC');
			},
			async provideTokenCount(_text, _token) {
				return 1;
			}
		...
		try {
			...
			assert.ok(error instanceof ProX-Code.LanguageModelError);
	...
	test('LanguageModelError instance is not thrown to extensions#235322 (ASYNC)', async function () {
		...
		disposables.push(ProX-Code.lm.registerChatModelProvider('test-lm', {
			async provideLanguageModelResponse(_messages, _options, _extensionId, _progress, _token) {
				throw ProX-Code.LanguageModelError.Blocked('You have been blocked ASYNC');
			},
			async provideTokenCount(_text, _token) {
				return 1;
			}
		...
		const response = await models[0].sendRequest([ProX-Code.LanguageModelChatMessage.User('Hello')]);
		assert.ok(response);
