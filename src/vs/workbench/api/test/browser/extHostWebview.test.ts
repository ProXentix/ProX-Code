...
suite('ExtHostWebview', () => {
	...
	test('asWebviewUri for remote file paths', () => {
		...
		assert.strictEqual(
			(webview.webview.asWebviewUri(URI.parse('file:///Users/codey/file.html')).toString()),
			`https://vscode-remote%2Bremote.prox-code-resource.${webviewResourceBaseHost}/Users/codey/file.html`,
			'Unix basic'
		);
	});
	...
	test('asWebviewUri for remote with / and + in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vscode-remote%2Bssh-002dremote-002blocalhost-003dfoo-002fbar.prox-code-resource.prox-code-cdn.net/Users/cody/x.png`,
			'Check transform');

		assert.strictEqual(
			decodeAuthority(webviewUri.authority),
			`vscode-remote+${authority}.prox-code-resource.prox-code-cdn.net`,
	...
	test('asWebviewUri for remote with port in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vscode-remote%2Blocalhost-003a8080.prox-code-resource.prox-code-cdn.net/Users/cody/x.png`,
			'Check transform');
