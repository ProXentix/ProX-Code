			...
			expand.kind === 'navtree-function'
				...
				: expand.kind === 'refactor-info'
					...
					: expand.kind === 'code-action'
						...
						: expand.range;
		const initialSelection = initialRange ? new ProX-Code.Selection(initialRange.start, initialRange.end) : undefined;
		await ProX-Code.commands.executeCommand('ProX-Code.editorChat.start', {
			initialRange,
			initialSelection,
			message,
			autoSend: true,
...
export class CompositeCommand implements Command {
	...
	public async execute(...commands: ProX-Code.Command[]): Promise<void> {
...
export type Expand =
	| { kind: 'none'; readonly range: ProX-Code.Range }
	| { kind: 'navtree-function'; readonly pos: ProX-Code.Position }
...
async function findEditScope(
	...
	return new ProX-Code.Range(start, expandEnd?.end ?? end);
}
