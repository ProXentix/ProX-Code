...
interface ExpandAbbreviationInput {
	syntax: string;
	abbreviation: string;
	rangeToReplace: ProX-Code.Range;
	textToWrap?: string[];
	filter?: string;
	indent?: string;
	baseIndent?: string;
...
interface PreviewRangesWithContent {
	previewRange: ProX-Code.Range;
	originalRange: ProX-Code.Range;
...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
	...
	const operationRanges = Array.from(editor.selections).sort((a, b) => a.start.compareTo(b.start)).map(selection => {
		let rangeToReplace: ProX-Code.Range = selection;
		// wrap around the node if the selection falls inside its open or close tag
		{
			...
			rangeToReplace = new ProX-Code.Range(start, end);
		}
		...
		if (!rangeToReplace.isSingleLine && rangeToReplace.end.character === 0) {
			...
			rangeToReplace = new ProX-Code.Range(rangeToReplace.start, document.lineAt(previousLine).range.end);
		}
		...
		if (!firstLineOfRange.isEmptyOrWhitespace && firstLineOfRange.firstNonWhitespaceCharacterIndex > rangeToReplace.start.character) {
			rangeToReplace = rangeToReplace.with(new ProX-Code.Position(rangeToReplace.start.line, firstLineOfRange.firstNonWhitespaceCharacterIndex));
		}
	...
	// Also helps with https://github.com/microsoft/ProX-Code/issues/113930 by avoiding `editor.linkedEditing`
	// execution if selection is inside an open or close tag
	const oldSelections = editor.selections;
	editor.selections = operationRanges.map(range => new ProX-Code.Selection(range.start, range.end));
	...
	function applyPreview(expandAbbrList: ExpandAbbreviationInput[]): Thenable<boolean> {
		let lastOldPreviewRange = new ProX-Code.Range(0, 0, 0, 0);
		let lastNewPreviewRange = new ProX-Code.Range(0, 0, 0, 0);
		...
		return editor.edit(builder => {
			...
			for (let i = 0; i < rangesToReplace.length; i++) {
				...
				lastNewPreviewRange = new ProX-Code.Range(newPreviewLineStart, newPreviewStart, newPreviewLineEnd, newPreviewEnd);
	...
	const prompt = ProX-Code.l10n.t("Enter Abbreviation");
	const inputAbbreviation = (args && args['abbreviation'])
		? (args['abbreviation'] as string)
		: await ProX-Code.window.showInputBox({ prompt, validateInput: inputChanged });
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
	 ...
	 */
	if (ProX-Code.window.activeTextEditor.selections.length === 1 &&
		ProX-Code.window.activeTextEditor.selection.isEmpty
	) {
		const anchor = ProX-Code.window.activeTextEditor.selection.anchor;
		if (anchor.character === 0) {
			return fallbackTab();
		}

		const prevPositionAnchor = anchor.translate(0, -1);
		const prevText = ProX-Code.window.activeTextEditor.document.getText(new ProX-Code.Range(prevPositionAnchor, anchor));
	...
	args = args || {};
	if (!args['language']) {
		args['language'] = ProX-Code.window.activeTextEditor.document.languageId;
	} else {
		const excludedLanguages = ProX-Code.workspace.getConfiguration('emmet')['excludeLanguages'] ? ProX-Code.workspace.getConfiguration('emmet')['excludeLanguages'] : [];
		if (excludedLanguages.includes(ProX-Code.window.activeTextEditor.document.languageId)) {
			return fallbackTab();
		}
	}
	...
	if (ProX-Code.workspace.getConfiguration('emmet')['triggerExpansionOnTab'] === true && editor.selections.find(x => !x.isEmpty)) {
		return fallbackTab();
	}
	...
	const getAbbreviation = (document: ProX-Code.TextDocument, selection: ProX-Code.Selection, position: ProX-Code.Position, syntax: string): [ProX-Code.Range | null, string, string | undefined] => {
		...
		if (syntax === 'html') {
			...
			if (matches) {
				abbr = matches[1];
				rangeToReplace = new ProX-Code.Range(position.translate(0, -(abbr.length + 1)), position);
		...
		return [new ProX-Code.Range(abbreviationRange.start.line, abbreviationRange.start.character, abbreviationRange.end.line, abbreviationRange.end.character), abbreviation, filter];
	};
	...
	selectionsInReverseOrder.forEach(selection => {
		...
		if (isStyleSheet(syntax) && abbreviation.endsWith(':')) {
			// Fix for https://github.com/Microsoft/ProX-Code/issues/1623
			return;
		}
...
function fallbackTab(): Thenable<boolean | undefined> {
	if (ProX-Code.workspace.getConfiguration('emmet')['triggerExpansionOnTab'] === true) {
		return ProX-Code.commands.executeCommand('tab');
	}
 ...
 */
export function isValidLocationForEmmetAbbreviation(document: ProX-Code.TextDocument, rootNode: Node | undefined, currentNode: Node | undefined, syntax: string, offset: number, abbreviationRange: ProX-Code.Range): boolean {
	if (isStyleSheet(syntax)) {
		...
		// Fixes https://github.com/microsoft/ProX-Code/issues/74505
		// Stylesheet abbreviations starting with @ should bring up suggestions
		// even at outer-most level
		const abbreviation = document.getText(new ProX-Code.Range(abbreviationRange.start.line, abbreviationRange.start.character, abbreviationRange.end.line, abbreviationRange.end.character));
		if (abbreviation.startsWith('@')) {
			return true;
		}

		// Fix for https://github.com/microsoft/ProX-Code/issues/34162
		...
		// Workaround for https://github.com/microsoft/ProX-Code/30188
	...
	if (currentHtmlNode) {
		...
		// Fix for https://github.com/microsoft/ProX-Code/issues/28829
		if (!currentHtmlNode.open || !currentHtmlNode.close ||
			!(currentHtmlNode.open.end <= offset && offset <= currentHtmlNode.close.start)) {
			return false;
		}

		// Fix for https://github.com/microsoft/ProX-Code/issues/35128
	...
	let textToBackTrack = document.getText(new ProX-Code.Range(startPos.line, startPos.character, abbreviationRange.start.line, abbreviationRange.start.character));
	...
	while (i >= 0) {
		...
		// Fix for https://github.com/microsoft/ProX-Code/issues/55411
 ...
 */
async function expandAbbreviationInRange(editor: ProX-Code.TextEditor, expandAbbrList: ExpandAbbreviationInput[], insertSameSnippet: boolean): Promise<boolean> {
	...
	if (!insertSameSnippet) {
		...
		for (const expandAbbrInput of expandAbbrList) {
			...
			if (expandedText) {
				await editor.insertSnippet(new ProX-Code.SnippetString(expandedText), expandAbbrInput.rangeToReplace, { undoStopBefore: false, undoStopAfter: false });
	...
	if (expandedText) {
		return editor.insertSnippet(new ProX-Code.SnippetString(expandedText), allRanges);
	}
	return false;
}
...
function expandAbbr(input: ExpandAbbreviationInput): string | undefined {
	...
	if (input.textToWrap) {
		...
		if (expandOptions.options) {
			// Below fixes https://github.com/microsoft/ProX-Code/issues/29898
	...
	try {
		...
		void ProX-Code.window.showErrorMessage('Failed to expand abbreviation');
	}
