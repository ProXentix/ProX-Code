...
interface ProviderWithMetadata {
	label: string;
	provider: ProX-Code.AuthenticationProvider;
	options: ProX-Code.AuthenticationProviderOptions;
}
...
export class ExtHostAuthentication implements ExtHostAuthenticationShape {
	...
	private _onDidChangeSessions = new Emitter<ProX-Code.AuthenticationSessionsChangeEvent & { extensionIdFilter?: string[] }>();
	private _getSessionTaskSingler = new TaskSingler<ProX-Code.AuthenticationSession | undefined>();
	 ...
	 */
	getExtensionScopedSessionsEvent(extensionId: string): Event<ProX-Code.AuthenticationSessionsChangeEvent> {
	...
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopes: readonly string[], options: ProX-Code.AuthenticationGetSessionOptions & ({ createIfNone: true } | { forceNewSession: true } | { forceNewSession: ProX-Code.AuthenticationForceNewSessionOptions })): Promise<ProX-Code.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopes: readonly string[], options: ProX-Code.AuthenticationGetSessionOptions & { forceNewSession: true }): Promise<ProX-Code.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopes: readonly string[], options: ProX-Code.AuthenticationGetSessionOptions & { forceNewSession: ProX-Code.AuthenticationForceNewSessionOptions }): Promise<ProX-Code.AuthenticationSession>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopes: readonly string[], options: ProX-Code.AuthenticationGetSessionOptions): Promise<ProX-Code.AuthenticationSession | undefined>;
	async getSession(requestingExtension: IExtensionDescription, providerId: string, scopes: readonly string[], options: ProX-Code.AuthenticationGetSessionOptions = {}): Promise<ProX-Code.AuthenticationSession | undefined> {
		const extensionId = ExtensionIdentifier.toKey(requestingExtension.identifier);
		const sortedScopes = [...scopes].sort().join(' ');
		const keys: (keyof ProX-Code.AuthenticationGetSessionOptions)[] = Object.keys(options) as (keyof ProX-Code.AuthenticationGetSessionOptions)[];
	...
	async $createSession(providerId: string, scopes: string[], options: ProX-Code.AuthenticationProviderSessionOptions): Promise<ProX-Code.AuthenticationSession> {
		const providerData = this._authenticationProviders.get(providerId);
		if (providerData) {
	...
	async $getSessions(providerId: string, scopes: ReadonlyArray<string> | undefined, options: ProX-Code.AuthenticationProviderSessionOptions): Promise<ReadonlyArray<ProX-Code.AuthenticationSession>> {
		const providerData = this._authenticationProviders.get(providerId);
		if (providerData) {
			options.issuer = URI.revive(options.issuer);
