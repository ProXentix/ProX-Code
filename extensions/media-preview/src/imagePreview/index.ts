...
export class ImagePreviewManager implements ProX-Code.CustomReadonlyEditorProvider {
	...
	constructor(
		private readonly extensionRoot: ProX-Code.Uri,
		private readonly sizeStatusBarEntry: SizeStatusBarEntry,
		private readonly binarySizeStatusBarEntry: BinarySizeStatusBarEntry,
		private readonly zoomStatusBarEntry: ZoomStatusBarEntry,
	) { }

	public async openCustomDocument(uri: ProX-Code.Uri) {
		return { uri, dispose: () => { } };
	}

	public async resolveCustomEditor(
		document: ProX-Code.CustomDocument,
		webviewEditor: ProX-Code.WebviewPanel,
	): Promise<void> {
	...
	public getPreviewFor(resource: ProX-Code.Uri, viewColumn?: ProX-Code.ViewColumn): ImagePreview | undefined {
...
<body class="container image scale-to-fit loading" data-ProX-Code-context='{ "preventDefaultContextMenuItems": true }'>
	...
	private async getResourcePath(webviewEditor: ProX-Code.WebviewPanel, resource: ProX-Code.Uri, version: string): Promise<string> {
	...
	private extensionResource(...parts: string[]) {
		return this._webviewEditor.webview.asWebviewUri(ProX-Code.Uri.joinPath(this.extensionRoot, ...parts));
	}

	public async reopenAsText() {
		await ProX-Code.commands.executeCommand('reopenActiveEditorWith', 'default');
		this._webviewEditor.dispose();
	}
}


export function registerImagePreviewSupport(context: ProX-Code.ExtensionContext, binarySizeStatusBarEntry: BinarySizeStatusBarEntry): ProX-Code.Disposable {
	...
	disposables.push(ProX-Code.window.registerCustomEditorProvider(ImagePreviewManager.viewType, previewManager, {
		supportsMultipleEditorsPerDocument: true,
	}));

	disposables.push(ProX-Code.commands.registerCommand('imagePreview.zoomIn', () => {
		previewManager.activePreview?.zoomIn();
	}));

	disposables.push(ProX-Code.commands.registerCommand('imagePreview.zoomOut', () => {
		previewManager.activePreview?.zoomOut();
	}));

	disposables.push(ProX-Code.commands.registerCommand('imagePreview.copyImage', () => {
		previewManager.activePreview?.copyImage();
	}));

	disposables.push(ProX-Code.commands.registerCommand('imagePreview.reopenAsText', async () => {
		return previewManager.activePreview?.reopenAsText();
	}));

	disposables.push(ProX-Code.commands.registerCommand('imagePreview.reopenAsPreview', async () => {

		await ProX-Code.commands.executeCommand('reopenActiveEditorWith', ImagePreviewManager.viewType);
	}));

	return ProX-Code.Disposable.from(...disposables);
}
