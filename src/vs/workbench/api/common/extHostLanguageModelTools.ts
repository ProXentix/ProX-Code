...
class Tool {
	...
	private _apiObject: ProX-Code.LanguageModelToolInformation | undefined;
	...
	get apiObject(): ProX-Code.LanguageModelToolInformation {
		if (!this._apiObject) {
			const that = this;
...
export class ExtHostLanguageModelTools implements ExtHostLanguageModelToolsShape {
	...
	private readonly _registeredTools = new Map<string, { extension: IExtensionDescription; tool: ProX-Code.LanguageModelTool<Object> }>();
	private readonly _proxy: MainThreadLanguageModelToolsShape;
	private readonly _tokenCountFuncs = new Map</* call ID */string, (text: string, token?: ProX-Code.CancellationToken) => Thenable<number>>();

	/** A map of all known tools, from other EHs or registered in ProX-Code core */
	...
	async invokeTool(extension: IExtensionDescription, toolId: string, options: ProX-Code.LanguageModelToolInvocationOptions<any>, token?: CancellationToken): Promise<ProX-Code.LanguageModelToolResult> {
	...
	getTools(extension: IExtensionDescription): ProX-Code.LanguageModelToolInformation[] {
	...
	async $invokeTool(dto: IToolInvocation, token: CancellationToken): Promise<Dto<IToolResult> | SerializableObjectWithBuffers<Dto<IToolResult>>> {
		...
		const options: ProX-Code.LanguageModelToolInvocationOptions<Object> = {
			input: dto.parameters,
			toolInvocationToken: dto.context as ProX-Code.ChatParticipantToolToken | undefined,
		};
		...
		let progress: ProX-Code.Progress<{ message?: string | ProX-Code.MarkdownString; increment?: number }> | undefined;
	...
	private async getModel(modelId: string, extension: IExtensionDescription): Promise<ProX-Code.LanguageModelChat> {
	...
	async $prepareToolInvocation(toolId: string, input: any, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		const options: ProX-Code.LanguageModelToolInvocationPrepareOptions<any> = { input };
	...
	registerTool(extension: IExtensionDescription, id: string, tool: ProX-Code.LanguageModelTool<any>): IDisposable {
