...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'ProX Code', separator: { label: ' - ' } }), 'ProX Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'ProX Code', separator: { label: ' - ' } }), 'Untitled-1 - ProX Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'ProX Code', separator: { label: ' - ' } }), 'monaco - ProX Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'ProX Code', separator: { label: ' - ' } }), 'somefile.txt - monaco - ProX Code');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'ProX Code', separator: { label: ' - ' } }), '* somefile.txt - monaco - ProX Code');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'ProX Code', separator: { label: ' - ' } }), 'monaco - ProX Code');
	});
	...
	test('getPathLabel', () => {
		...
		const nixBadFileUri = URI.revive({ scheme: 'ProX-Code', authority: 'file', path: '//some/folder/file.txt' });
		const uncFileUri = URI.file('c:/some/folder/file.txt').with({ authority: 'auth' });
		const remoteFileUri = URI.file('/some/folder/file.txt').with({ scheme: 'ProX-Code-test', authority: 'auth' });
		...
		const remoteUserHome = URI.file('/some').with({ scheme: 'ProX-Code-test', authority: 'auth' });
