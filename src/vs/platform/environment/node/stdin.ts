...
async function createStdInFile(targetPath: string) {
	...
	await fs.promises.chmod(targetPath, 0o600); // Ensure the file is only read/writable by the user: https://github.com/microsoft/ProX-Code-remote-release/issues/9048
}
...
export async function readFromStdin(targetPath: string, verbose: boolean, onEnd?: Function): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		import('@ProX-Code/iconv-lite-umd'),		// lazy load encoding module for usage
		createStdInFile(targetPath) 			// make sure file exists right away (https://github.com/microsoft/ProX-Code/issues/155341)
	]);
	...
	// file descriptor.
	// (https://github.com/microsoft/ProX-Code/issues/148952)
