...
type ApplyCodeActionCommand_args = {
	...
	readonly diagnostic: ProX-Code.Diagnostic;
...
class DiagnosticsSet {
	public static from(diagnostics: ProX-Code.Diagnostic[]) {
		const values = new Map<string, ProX-Code.Diagnostic>();
	...
	private static key(diagnostic: ProX-Code.Diagnostic) {
		const { start, end } = diagnostic.range;
		return `${diagnostic.code}-${start.line},${start.character}-${end.line},${end.character}`;
	}

	private constructor(
		private readonly _values: Map<string, ProX-Code.Diagnostic>
	) { }

	public get values(): Iterable<ProX-Code.Diagnostic> {
...
class VsCodeCodeAction extends ProX-Code.CodeAction {
	constructor(
		public readonly tsAction: Proto.CodeFixAction,
		title: string,
		kind: ProX-Code.CodeActionKind
	) {
		super(title, kind);
	}
}
...
class SupportedCodeActionProvider {
	...
	public async getFixableDiagnosticsForContext(diagnostics: readonly ProX-Code.Diagnostic[]): Promise<DiagnosticsSet> {
...
class TypeScriptQuickFixProvider implements ProX-Code.CodeActionProvider<VsCodeCodeAction> {
	...
	public static readonly metadata: ProX-Code.CodeActionProviderMetadata = {
		providedCodeActionKinds: [ProX-Code.CodeActionKind.QuickFix]
	};
	...
	public async resolveCodeAction(codeAction: VsCodeCodeAction, token: ProX-Code.CancellationToken): Promise<VsCodeCodeAction> {
	...
	private async getFixesForDiagnostic(
		document: ProX-Code.TextDocument,
		file: string,
		diagnostic: ProX-Code.Diagnostic,
	...
	private getFixesForTsCodeAction(
		...
		const codeAction = new VsCodeCodeAction(action, action.description, ProX-Code.CodeActionKind.QuickFix);
		...
		const copilot = ProX-Code.extensions.getExtension('github.copilot-chat');
		if (copilot?.isActive) {
			...
			if (action.fixName === fixNames.classIncorrectlyImplementsInterface) {
				...
				const inferFromBody = new VsCodeCodeAction(action, 'Infer types using Copilot', ProX-Code.CodeActionKind.QuickFix);
				inferFromBody.edit = new ProX-Code.WorkspaceEdit();
			...
			if (expand && message !== undefined) {
				const aiCodeAction = new VsCodeCodeAction(action, title, ProX-Code.CodeActionKind.QuickFix);
	...
	private addFixAllForTsCodeAction(
		...
		const action = new VsCodeFixAllCodeAction(
			tsAction,
			file,
			tsAction.fixAllDescription || ProX-Code.l10n.t("{0} (Fix all in file)", tsAction.description),
