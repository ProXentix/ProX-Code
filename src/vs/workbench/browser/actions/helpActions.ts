...
class OpenNewsletterSignupUrlAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('newsletterSignup', 'Signup for the ProX Code Newsletter'),
			category: Categories.Help,
			f1: true
		});
	}
...
class AskVSCodeCopilot extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('askVScode', 'Ask @ProX-Code'),
			category: Categories.Help,
			f1: true,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('workbench.action.chat.open', { mode: 'ask', query: '@ProX-Code ', isPartialQuery: true });
	}
}
