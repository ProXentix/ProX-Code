...
export class QuickInputController extends Disposable {
	...
	constructor(
		...
		this._register(dom.onWillUnregisterWindow(window => {
			if (this.ui && dom.getWindow(this.ui.container) === window) {
				...
				// (https://github.com/microsoft/ProX-Code/issues/195870)
	...
	private getUI(showInActiveContainer?: boolean): QuickInputUI {
		...
		this._register(list.onLeave(() => {
			...
			// TODO@TylerLeonhardt https://github.com/microsoft/ProX-Code/issues/203675
			setTimeout(() => {
				if (!this.controller) {
					return;
				}
	...
	private updateStyles() {
		if (this.ui) {
			...
			if (this.styles.pickerGroup.pickerGroupForeground) {
				content.push(`.quick-input-list .quick-input-list-separator-as-item { color: var(--ProX-Code-descriptionForeground); }`);
			}
