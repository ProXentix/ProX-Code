...
export function activate(context: ProX-Code.ExtensionContext): any {
	...
	const legend = new ProX-Code.SemanticTokensLegend(tokenTypes, tokenModifiers);

	const outputChannel = ProX-Code.window.createOutputChannel('Semantic Tokens Test');

	const documentSemanticHighlightProvider: ProX-Code.DocumentSemanticTokensProvider = {
		provideDocumentSemanticTokens(document: ProX-Code.TextDocument): ProX-Code.ProviderResult<ProX-Code.SemanticTokens> {
	...
	context.subscriptions.push(ProX-Code.languages.registerDocumentSemanticTokensProvider({ pattern: '**/*semantic-test.json' }, documentSemanticHighlightProvider, legend));
