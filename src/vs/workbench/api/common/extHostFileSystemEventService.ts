...
class FileSystemWatcher implements ProX-Code.FileSystemWatcher {
	...
	private readonly _onDidCreate = new Emitter<ProX-Code.Uri>();
	private readonly _onDidChange = new Emitter<ProX-Code.Uri>();
	private readonly _onDidDelete = new Emitter<ProX-Code.Uri>();
	...
	constructor(
		...
		// unexpected.
		// https://github.com/microsoft/ProX-Code/issues/3025
	...
	private ensureWatching(mainContext: IMainContext, workspace: IExtHostWorkspace, configuration: ExtHostConfigProvider, extension: IExtensionDescription, globPattern: string | IRelativePatternDto, options: FileSystemWatcherCreateOptions, correlate: boolean | undefined): Disposable {
		...
		// Uncorrelated: adjust includes and excludes based on settings
			...
			// (https://github.com/microsoft/ProX-Code/issues/148245)
			else if (!recursive) {
				const workspaceFolder = workspace.getWorkspaceFolder(URI.revive(globPattern.baseUri));
				if (workspaceFolder) {
					const watcherExcludes = configuration.getConfiguration('files', workspaceFolder).get<IGlobPatterns>('watcherExclude');
	...
	get onDidCreate(): Event<ProX-Code.Uri> {
		return this._onDidCreate.event;
	}

	get onDidChange(): Event<ProX-Code.Uri> {
		return this._onDidChange.event;
	}

	get onDidDelete(): Event<ProX-Code.Uri> {
...
export class ExtHostFileSystemEventService implements ExtHostFileSystemEventServiceShape {
	...
	private readonly _onDidRenameFile = new Emitter<ProX-Code.FileRenameEvent>();
	private readonly _onDidCreateFile = new Emitter<ProX-Code.FileCreateEvent>();
	private readonly _onDidDeleteFile = new Emitter<ProX-Code.FileDeleteEvent>();
	private readonly _onWillRenameFile = new AsyncEmitter<ProX-Code.FileWillRenameEvent>();
	private readonly _onWillCreateFile = new AsyncEmitter<ProX-Code.FileWillCreateEvent>();
	private readonly _onWillDeleteFile = new AsyncEmitter<ProX-Code.FileWillDeleteEvent>();

	readonly onDidRenameFile: Event<ProX-Code.FileRenameEvent> = this._onDidRenameFile.event;
	readonly onDidCreateFile: Event<ProX-Code.FileCreateEvent> = this._onDidCreateFile.event;
	readonly onDidDeleteFile: Event<ProX-Code.FileDeleteEvent> = this._onDidDeleteFile.event;
	...
	createFileSystemWatcher(workspace: IExtHostWorkspace, configProvider: ExtHostConfigProvider, extension: IExtensionDescription, globPattern: ProX-Code.GlobPattern, options: FileSystemWatcherCreateOptions): ProX-Code.FileSystemWatcher {
	...
	getOnWillRenameFileEvent(extension: IExtensionDescription): Event<ProX-Code.FileWillRenameEvent> {
		return this._createWillExecuteEvent(extension, this._onWillRenameFile);
	}

	getOnWillCreateFileEvent(extension: IExtensionDescription): Event<ProX-Code.FileWillCreateEvent> {
		return this._createWillExecuteEvent(extension, this._onWillCreateFile);
	}

	getOnWillDeleteFileEvent(extension: IExtensionDescription): Event<ProX-Code.FileWillDeleteEvent> {
