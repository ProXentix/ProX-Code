...
export class ZoomStatusBarEntry extends OwnedStatusBarEntry {
	...
	private readonly _onDidChangeScale = this._register(new ProX-Code.EventEmitter<{ scale: Scale }>());
	public readonly onDidChangeScale = this._onDidChangeScale.event;

	constructor() {
		super('status.imagePreview.zoom', ProX-Code.l10n.t("Image Zoom"), ProX-Code.StatusBarAlignment.Right, 102 /* to the left of editor size entry (101) */);

		this._register(ProX-Code.commands.registerCommand(selectZoomLevelCommandId, async () => {
			type MyPickItem = ProX-Code.QuickPickItem & { scale: Scale };
			...
			const pick = await ProX-Code.window.showQuickPick(options, {
				placeHolder: ProX-Code.l10n.t("Select zoom level")
			});
			if (pick) {
