...
export class FileWatcherManager implements IDisposable {
	...
	private readonly _fileWatchers = new Map<number, {
		readonly uri: ProX-Code.Uri;
		readonly watcher: ProX-Code.FileSystemWatcher;
	...
	create(id: number, uri: ProX-Code.Uri, watchParentDirs: boolean, isRecursive: boolean, listeners: { create?: (uri: ProX-Code.Uri) => void; change?: (uri: ProX-Code.Uri) => void; delete?: (uri: ProX-Code.Uri) => void }): void {
		this.logger.trace(`Creating file watcher for ${uri.toString()}`);

		// Non-writable file systems do not support file watching
		if (!ProX-Code.workspace.fs.isWritableFileSystem(uri.scheme)) {
			return;
		}

		const watcher = ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(uri, isRecursive ? '**' : '*'), !listeners.create, !listeners.change, !listeners.delete);
		...
		if (watchParentDirs && uri.scheme !== Schemes.untitled) {
			...
			for (let dirUri = Utils.dirname(uri); dirUri.path.length > 1; dirUri = Utils.dirname(dirUri)) {
				...
				if (!parentDirWatcher) {
					...
					const glob = new ProX-Code.RelativePattern(Utils.dirname(dirUri), Utils.basename(dirUri));
					const parentWatcher = ProX-Code.workspace.createFileSystemWatcher(glob, !listeners.create, true, !listeners.delete);
				...
				if (listeners.create) {
					disposables.push(parentDirWatcher.watcher.onDidCreate(async () => {
						...
						try {
							...
							if (stat.type === ProX-Code.FileType.File) {
