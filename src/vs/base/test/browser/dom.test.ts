...
suite('dom', () => {
	...
	test('multibyteAwareBtoa', () => {
		...
		assert.ok(multibyteAwareBtoa(new Array(100000).fill('vs').join('')).length > 0); // https://github.com/microsoft/ProX-Code/issues/112013
	});
	...
	test('window utilities', () => {
		...
		assert.ok(typeof mainWindow.ProX-CodeWindowId === 'number');
