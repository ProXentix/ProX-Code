...
interface QuickPickItem extends ProX-Code.QuickPickItem {
	run(): void;
}
...
export class TypeScriptVersionManager extends Disposable {
	...
	public async promptUserForVersion(): Promise<void> {
		const selected = await ProX-Code.window.showQuickPick<QuickPickItem>([
			this.getBundledPickItem(),
			...this.getLocalPickItems(),
			{
				kind: ProX-Code.QuickPickItemKind.Separator,
				label: '',
				run: () => { /* noop */ },
			},
			LearnMorePickItem,
		], {
			placeHolder: ProX-Code.l10n.t("Select the TypeScript version used for JavaScript and TypeScript language features"),
		});
	...
	private getLocalPickItems(): QuickPickItem[] {
		return this.versionProvider.localVersions.map(version => {
			return {
				label: (this.useWorkspaceTsdkSetting && ProX-Code.workspace.isTrusted && this.currentVersion.eq(version)
					? '• '
					: '') + ProX-Code.l10n.t("Use Workspace Version"),
				description: version.displayName,
				detail: version.pathLabel,
				run: async () => {
					const trusted = await ProX-Code.workspace.requestWorkspaceTrust();
					if (trusted) {
	...
	private async promptUseWorkspaceTsdk(): Promise<void> {
		...
		const allowIt = ProX-Code.l10n.t("Allow");
		const dismissPrompt = ProX-Code.l10n.t("Dismiss");
		const suppressPrompt = ProX-Code.l10n.t("Never in this Workspace");

		const result = await ProX-Code.window.showInformationMessage(ProX-Code.l10n.t("This workspace contains a TypeScript version. Would you like to use the workspace TypeScript version for TypeScript and JavaScript language features?"),
			allowIt,
			dismissPrompt,
			suppressPrompt
		);
...
const LearnMorePickItem: QuickPickItem = {
	label: ProX-Code.l10n.t("Learn more about managing TypeScript versions"),
	description: '',
	run: () => {
		ProX-Code.env.openExternal(ProX-Code.Uri.parse('https://go.microsoft.com/fwlink/?linkid=839919'));
	}
};
