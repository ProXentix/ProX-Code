...
export class TerminalService extends Disposable implements ITerminalService {
	...
	constructor(
		...
		// Hide the panel if there are no more instances, provided that ProX Code is not shutting
	...
	async createTerminal(options?: ICreateTerminalOptions): Promise<ITerminalInstance> {
		...
		if (this._terminalProfileService.availableProfiles.length === 0) {
			...
			const isLocalInRemoteTerminal = this._remoteAgentService.getConnection() && URI.isUri(options?.cwd) && options?.cwd.scheme === Schemas.ProX-CodeFileResource;
