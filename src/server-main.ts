...
(globalThis as any).ProX-CodeServerStartTime = performance.now();
...
if (shouldSpawnCli) {
	...
	server.listen(nodeListenOptions, async () => {
		...
		// Do not change this line. ProX Code looks for this in the output.
		output += `Extension host agent listening on ${typeof address === 'string' ? address : address.port}\n`;
		console.log(output);

		perf.mark('code/server/started');
		(globalThis as any).ProX-CodeServerListenTime = performance.now();
