...
export class SymbolsTree {
	...
	private readonly _tree: ProX-Code.TreeView<unknown>;
	private readonly _navigation: Navigation;

	private _input?: SymbolTreeInput<unknown>;
	private _sessionDisposable?: ProX-Code.Disposable;

	constructor() {
		this._tree = ProX-Code.window.createTreeView<unknown>(this.viewId, {
	...
	async setInput(input: SymbolTreeInput<unknown>) {
		...
		ProX-Code.commands.executeCommand(`${this.viewId}.focus`);
		...
		if (typeof model.dispose === 'function') {
			disposables.push(new ProX-Code.Disposable(() => model.dispose!()));
		}
		this._sessionDisposable = ProX-Code.Disposable.from(...disposables);
	}
	...
	clearInput(): void {
		...
		this._tree.title = ProX-Code.l10n.t('References');
		this._tree.message = this._history.size === 0
			? ProX-Code.l10n.t('No results.')
			: ProX-Code.l10n.t('No results. Try running a previous search again:');
...
interface ActiveTreeDataProviderWrapper {
	provider: Promise<ProX-Code.TreeDataProvider<any>>;
}

class TreeDataProviderDelegate implements ProX-Code.TreeDataProvider<undefined> {

	provider?: Promise<ProX-Code.TreeDataProvider<any>>;

	private _sessionDispoables?: ProX-Code.Disposable;
	private _onDidChange = new ProX-Code.EventEmitter<any>();

	readonly onDidChangeTreeData = this._onDidChange.event;

	update(provider: Promise<ProX-Code.TreeDataProvider<any>>) {
...
class TreeDndDelegate implements ProX-Code.TreeDragAndDropController<undefined> {
	...
	handleDrag(source: undefined[], data: ProX-Code.DataTransfer) {
		if (this._delegate) {
			const urls: string[] = [];
			...
			if (urls.length > 0) {
				data.set('text/uri-list', new ProX-Code.DataTransferItem(urls.join('\r\n')));
			}
		}
	}
...
class HistoryItem {
	...
	constructor(
		...
		this.description = `${ProX-Code.workspace.asRelativePath(input.location.uri)} • ${input.title.toLocaleLowerCase()}`;
	}
}

class TreeInputHistory implements ProX-Code.TreeDataProvider<HistoryItem> {

	private readonly _onDidChangeTreeData = new ProX-Code.EventEmitter<HistoryItem | undefined>();
	...
	constructor(private readonly _tree: SymbolsTree) {
		...
		this._disposables.push(
			ProX-Code.commands.registerCommand('references-view.clear', () => _tree.clearInput()),
			ProX-Code.commands.registerCommand('references-view.clearHistory', () => {
				this.clear();
				_tree.clearInput();
			}),
			ProX-Code.commands.registerCommand('references-view.refind', (item) => {
				if (item instanceof HistoryItem) {
					this._reRunHistoryItem(item);
				}
			}),
			ProX-Code.commands.registerCommand('references-view.refresh', () => {
				const item = Array.from(this._inputs.values()).pop();
				if (item) {
					this._reRunHistoryItem(item);
				}
			}),
			ProX-Code.commands.registerCommand('_references-view.showHistoryItem', async (item) => {
				if (item instanceof HistoryItem) {
					const position = item.anchor.guessedTrackedPosition() ?? item.input.location.range.start;
					await ProX-Code.commands.executeCommand('ProX-Code.open', item.input.location.uri, { selection: new ProX-Code.Range(position, position) });
				}
			}),
			ProX-Code.commands.registerCommand('references-view.pickFromHistory', async () => {
				interface HistoryPick extends ProX-Code.QuickPickItem {
					item: HistoryItem;
				}
				...
				const pick = await ProX-Code.window.showQuickPick(picks, { placeHolder: ProX-Code.l10n.t('Select previous reference search') });
				if (pick) {
	...
	private _reRunHistoryItem(item: HistoryItem): void {
		...
		if (newPosition && !item.input.location.range.start.isEqual(newPosition)) {
			newInput = item.input.with(new ProX-Code.Location(item.input.location.uri, newPosition));
		}
	...
	async add(input: SymbolTreeInput<unknown>) {
		...
		const doc = await ProX-Code.workspace.openTextDocument(input.location.uri);
	...
	getTreeItem(item: HistoryItem): ProX-Code.TreeItem {
		const result = new ProX-Code.TreeItem(item.word);
		result.description = item.description;
		result.command = { command: '_references-view.showHistoryItem', arguments: [item], title: ProX-Code.l10n.t('Rerun') };
