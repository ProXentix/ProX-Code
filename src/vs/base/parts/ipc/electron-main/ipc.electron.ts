...
export class Server extends IPCServer {
	...
	private static getOnDidClientConnect(): Event<ClientConnectionEvent> {
		const onHello = Event.fromNodeEventEmitter<WebContents>(validatedIpcMain, 'ProX-Code:hello', ({ sender }) => sender);
		...
		return Event.map(onHello, webContents => {
			...
			const onMessage = createScopedOnMessageEvent(id, 'ProX-Code:message') as Event<VSBuffer>;
			const onDidClientDisconnect = Event.any(Event.signal(createScopedOnMessageEvent(id, 'ProX-Code:disconnect')), onDidClientReconnect.event);
