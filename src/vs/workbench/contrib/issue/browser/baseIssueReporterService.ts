...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		this.issueReporterModel = new IssueReporterModel({
			...
			versionInfo: {
				ProX-CodeVersion: `${product.nameShort} ${!!product.darwinUniversalAssetId ? `${product.version} (Universal)` : product.version} (${product.commit || 'Commit unknown'}, ${product.date || 'Date unknown'})`,
	...
	public setEventHandlers(): void {
		...
		this.addEventListener('issue-source', 'change', (e: Event) => {
			...
			if (value === IssueSource.VSCode) {
				descriptionTextArea.placeholder = localize('ProX-CodePlaceholder', "E.g Workbench is missing problems panel");
	...
	private searchDuplicates(title: string, body?: string): void {
		const url = 'https://ProX-Code-probot.westus.cloudapp.azure.com:7890/duplicate_candidates';
		const init = {
			method: 'POST',
			body: JSON.stringify({
				title,
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('ProX-Code', "ProX Code"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A ProX Code extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of ProX Code"));
	...
	public addTemplateToUrl(owner?: string, repositoryName?: string): string {
		...
		const isCopilot = owner?.toLowerCase() === 'microsoft' && repositoryName === 'ProX-Code-copilot-release';
		const isPython = owner?.toLowerCase() === 'microsoft' && repositoryName === 'ProX-Code-python';
