...
const BUNDLED_FILE_HEADER = [
	...
	' * https://github.com/microsoft/ProX-Code/blob/main/LICENSE.txt',
	' *-----------------------------------------------------------*/',
	''
].join('\n');
...
const extractEditorSrcTask = task.define('extract-editor-src', () => {
	...
	standalone.extractEditor({
		...
		redirects: {
			'@ProX-Code/tree-sitter-wasm': '../node_modules/@ProX-Code/tree-sitter-wasm/wasm/web-tree-sitter',
		}
	});
});
...
const finalEditorResourcesTask = task.define('final-editor-resources', () => {
	return es.merge(
		...
		gulp.src('build/monaco/version.txt')
			.pipe(es.through(function (data) {
				data.contents = Buffer.from(`monaco-editor-core: https://github.com/microsoft/ProX-Code/tree/${sha1}`);
				this.emit('data', data);
			}))
			.pipe(gulp.dest('out-monaco-editor-core')),
