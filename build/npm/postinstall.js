...
function npmInstall(dir, opts) {
	...
	if (process.env['VSCODE_REMOTE_DEPENDENCIES_CONTAINER_NAME'] && /^(.build\/distro\/npm\/)?remote$/.test(dir)) {
		...
		run('sudo', ['docker', 'run', '-e', 'GITHUB_TOKEN', '-v', `${process.env['VSCODE_HOST_MOUNT']}:/root/ProX-Code`, '-v', `${process.env['VSCODE_HOST_MOUNT']}/.build/.netrc:/root/.netrc`, '-w', path.resolve('/root/ProX-Code', dir), process.env['VSCODE_REMOTE_DEPENDENCIES_CONTAINER_NAME'], 'sh', '-c', `\"chown -R root:root ${path.resolve('/root/ProX-Code', dir)} && npm i -g node-gyp-build && npm ci\"`], opts);
...
for (let dir of dirs) {
	...
	if (/^(.build\/distro\/npm\/)?remote$/.test(dir)) {
		// node modules used by ProX-Code server
		opts = {
			env: {
				...process.env
			},
