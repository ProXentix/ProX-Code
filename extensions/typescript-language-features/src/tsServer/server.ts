...
import { Cancellation } from '@ProX-Code/sync-api-common/lib/common/messageCancellation';
...
export type TsServerLog =
	{ readonly type: 'file'; readonly uri: ProX-Code.Uri } |
	{ readonly type: 'output'; readonly output: ProX-Code.OutputChannel };

export interface ITypeScriptServer {
	readonly onEvent: ProX-Code.Event<Proto.Event>;
	readonly onExit: ProX-Code.Event<TypeScriptServerExitEvent>;
	readonly onError: ProX-Code.Event<any>;
	 ...
	 */
	executeImpl(command: keyof TypeScriptRequests, args: any, executeInfo: { isAsync: boolean; token?: ProX-Code.CancellationToken; expectsResult: boolean; lowPriority?: boolean; executionTarget?: ExecutionTarget }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined>;
...
export class SingleTsServer extends Disposable implements ITypeScriptServer {
	...
	private readonly _onEvent = this._register(new ProX-Code.EventEmitter<Proto.Event>());
	public readonly onEvent = this._onEvent.event;

	private readonly _onExit = this._register(new ProX-Code.EventEmitter<TypeScriptServerExitEvent>());
	public readonly onExit = this._onExit.event;

	private readonly _onError = this._register(new ProX-Code.EventEmitter<any>());
	...
	public executeImpl(command: keyof TypeScriptRequests, args: unknown, executeInfo: { isAsync: boolean; token?: ProX-Code.CancellationToken; expectsResult: boolean; lowPriority?: boolean; executionTarget?: ExecutionTarget }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined> {
...
interface ExecuteInfo {
	...
	readonly token?: ProX-Code.CancellationToken;
...
class RequestRouter {
	...
	public execute(
		...
		if (RequestRouter.sharedCommands.has(command) && typeof executeInfo.executionTarget === 'undefined') {
			...
			let token: ProX-Code.CancellationToken | undefined = undefined;
			if (executeInfo.token) {
...
export class GetErrRoutingTsServer extends Disposable implements ITypeScriptServer {
	...
	public executeImpl(command: keyof TypeScriptRequests, args: any, executeInfo: { isAsync: boolean; token?: ProX-Code.CancellationToken; expectsResult: boolean; lowPriority?: boolean; executionTarget?: ExecutionTarget }): Array<Promise<ServerResponse.Response<Proto.Response>> | undefined> {
...
export class SyntaxRoutingTsServer extends Disposable implements ITypeScriptServer {
	...
	private readonly _onExit = this._register(new ProX-Code.EventEmitter<any>());
	public readonly onExit = this._onExit.event;
