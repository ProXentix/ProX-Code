...
const previewStrings = {
	cspAlertMessageText: ProX-Code.l10n.t("Some content has been disabled in this document"),

	cspAlertMessageTitle: ProX-Code.l10n.t("Potentially unsafe or insecure content has been disabled in the Markdown preview. Change the Markdown preview security setting to allow insecure content or enable scripts"),

	cspAlertMessageLabel: ProX-Code.l10n.t("Content Disabled Security Warning")
};
...
export class MdDocumentRenderer {
	constructor(
		...
		private readonly _context: ProX-Code.ExtensionContext,
		private readonly _cspArbiter: ContentSecurityPolicyArbiter,
		private readonly _contributionProvider: MarkdownContributionProvider,
		private readonly _logger: ILogger
	) {
		this.iconPath = {
			dark: ProX-Code.Uri.joinPath(this._context.extensionUri, 'media', 'preview-dark.svg'),
			light: ProX-Code.Uri.joinPath(this._context.extensionUri, 'media', 'preview-light.svg'),
		};
	}

	public readonly iconPath: { light: ProX-Code.Uri; dark: ProX-Code.Uri };

	public async renderDocument(
		markdownDocument: ProX-Code.TextDocument,
		...
		const html = `<!DOCTYPE html>
			...
			<head>
				...
				${csp}
				<meta id="ProX-Code-markdown-preview-data"
			...
			<body class="ProX-Code-body ${config.scrollBeyondLastLine ? 'scrollBeyondLastLine' : ''} ${config.wordWrap ? 'wordWrap' : ''} ${config.markEditorSelection ? 'showEditorSelection' : ''}">
	...
	public renderFileNotFoundDocument(resource: ProX-Code.Uri): string {
		const resourcePath = uri.Utils.basename(resource);
		const body = ProX-Code.l10n.t('{0} cannot be found', resourcePath);
		return `<!DOCTYPE html>
			<html>
			<body class="ProX-Code-body">
				${body}
			</body>
			</html>`;
	}
	...
	private _extensionResourcePath(resourceProvider: WebviewResourceProvider, mediaFile: string): string {
		const webviewResource = resourceProvider.asWebviewUri(
			ProX-Code.Uri.joinPath(this._context.extensionUri, 'media', mediaFile));
		return webviewResource.toString();
	}

	private _fixHref(resourceProvider: WebviewResourceProvider, resource: ProX-Code.Uri, href: string): string {
		if (!href) {
			return href;
		}
		...
		if (href.startsWith('/') || /^[a-z]:\\/i.test(href)) {
			return resourceProvider.asWebviewUri(ProX-Code.Uri.file(href)).toString();
		}

		// Use a workspace relative path if there is a workspace
		const root = ProX-Code.workspace.getWorkspaceFolder(resource);
		if (root) {
			return resourceProvider.asWebviewUri(ProX-Code.Uri.joinPath(root.uri, href)).toString();
		}

		// Otherwise look relative to the markdown file
		return resourceProvider.asWebviewUri(ProX-Code.Uri.joinPath(uri.Utils.dirname(resource), href)).toString();
	}

	private _computeCustomStyleSheetIncludes(resourceProvider: WebviewResourceProvider, resource: ProX-Code.Uri, config: MarkdownPreviewConfiguration): string {
	...
	private _getStyles(resourceProvider: WebviewResourceProvider, resource: ProX-Code.Uri, config: MarkdownPreviewConfiguration, imageInfo: readonly ImageInfo[]): string {
