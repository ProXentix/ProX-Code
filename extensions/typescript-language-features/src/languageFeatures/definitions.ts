...
export default class TypeScriptDefinitionProvider extends DefinitionProviderBase implements ProX-Code.DefinitionProvider {

	public async provideDefinition(
		document: ProX-Code.TextDocument,
		position: ProX-Code.Position,
		token: ProX-Code.CancellationToken
	): Promise<ProX-Code.DefinitionLink[] | ProX-Code.Definition | undefined> {
		...
		if (ProX-Code.workspace.getConfiguration(document.languageId).get('preferGoToSourceDefinition', false) && this.client.apiVersion.gte(API.v470)) {
		...
		return definitions
			.map((location): ProX-Code.DefinitionLink => {
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerDefinitionProvider(selector.syntax,
