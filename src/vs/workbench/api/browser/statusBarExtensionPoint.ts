	...
	setOrUpdateEntry(entryId: string,
		...
		if (!existingEntry) {
			...
			if (typeof extensionId === 'string') {
				...
				// See https://github.com/microsoft/ProX-Code/issues/177835
				// See https://github.com/microsoft/ProX-Code/issues/123827
...
const statusBarItemSchema: IJSONSchema = {
	...
	properties: {
		id: {
			type: 'string',
			markdownDescription: localize('id', 'The identifier of the status bar entry. Must be unique within the extension. The same value must be used when calling the `ProX-Code.window.createStatusBarItem(id, ...)`-API')
		},
		name: {
			type: 'string',
...
const statusBarItemsSchema: IJSONSchema = {
	description: localize('ProX-Code.extension.contributes.statusBarItems', "Contributes items to the status bar."),
	oneOf: [
		statusBarItemSchema,
		{
			type: 'array',
