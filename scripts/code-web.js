...
const testWebLocation = require.resolve('@ProX-Code/test-web');
...
async function main() {
	...
	if (args.help) {
		console.log(
			...
			'  --playground             Include the ProX-Code-web-playground extension\n'
		);
		startServer(['--help']);
		return;
	}
	...
	// only use `./scripts/code-web.sh --playground` to add ProX-Code-web-playground extension by default.
...
function startServer(runnerArguments) {
	...
	console.log(`Starting @ProX-Code/test-web: ${testWebLocation} ${runnerArguments.join(' ')}`);
...
async function ensureWebDevExtensions(verbose) {
	...
	// Playground (https://github.com/microsoft/ProX-Code-web-playground)
	const webDevPlaygroundRoot = path.join(WEB_DEV_EXTENSIONS_ROOT, 'ProX-Code-web-playground');
	...
	if (downloadPlayground) {
		if (verbose) {
			fancyLog(`${ansiColors.magenta('Web Development extensions')}: Downloading ProX-Code-web-playground to ${webDevPlaygroundRoot}`);
		}
		const playgroundRepo = `https://raw.githubusercontent.com/microsoft/ProX-Code-web-playground/main/`;
		...
		if (verbose) {
			fancyLog(`${ansiColors.magenta('Web Development extensions')}: Using existing ProX-Code-web-playground in ${webDevPlaygroundRoot}`);
		}
	}
}
