...
suite('TreeSitterCommandParser', () => {
	...
	suite('extractSubCommands', () => {
		suite('bash', () => {
			...
			// TODO: These should be common but the pwsh grammar doesn't handle && yet https://github.com/ProXentix/ProX-Code/issues/272704
	...
	suite('getFileWrites', () => {
		...
		suite('pwsh', () => {
			...
			suite('real-world scenarios', () => {
				...
				test('registry export', () => t('Get-ItemProperty -Path "HKLM:\\Software\\ProXentix\\Windows\\CurrentVersion" > registry_info.txt', ['registry_info.txt']));
