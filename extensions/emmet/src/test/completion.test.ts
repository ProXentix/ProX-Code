...
import 'mocha';
import { CancellationTokenSource, CompletionTriggerKind, Selection } from 'ProX-Code';
...
suite('Tests for completion in CSS embedded in HTML', () => {
	...
	// https://github.com/microsoft/ProX-Code/issues/79766
	test('microsoft/ProX-Code#79766, correct region determination', async () => {
		await testCompletionProvider('html', `<div style="color: #000">di|</div>`, [
			{ label: 'div', documentation: `<div>|</div>` }
		]);
	});

	// https://github.com/microsoft/ProX-Code/issues/86941
	test('microsoft/ProX-Code#86941, widows should be completed after width', async () => {
	...
	// https://github.com/microsoft/ProX-Code/issues/117020
	test('microsoft/ProX-Code#117020, ! at end of abbreviation should have completion', async () => {
		await testCompletionProvider('css', `.foo { bdbn!| }`, [
			{ label: 'border-bottom: none !important;', documentation: `border-bottom: none !important;` }
		]);
	});

	// https://github.com/microsoft/ProX-Code/issues/138461
	test('microsoft/ProX-Code#138461, JSX array noise', async () => {
	...
	// https://github.com/microsoft/ProX-Code-emmet-helper/pull/90
	test('microsoft/ProX-Code-emmet-helper#90', async () => {
