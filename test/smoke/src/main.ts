...
import * as ProX-Codetest from '@ProX-Code/test-electron';
...
const testRepoUrl = 'https://github.com/microsoft/ProX-Code-smoketest-express';
const workspacePath = path.join(testDataPath, 'ProX-Code-smoketest-express');
...
logger.log(`ProX Code product quality: ${quality}.`);
...
async function ensureStableCode(): Promise<void> {
	...
	if (!stableCodePath) {
		...
		logger.log(`Found ProX Code v${version}, downloading previous ProX Code version ${stableVersion}...`);

		let lastProgressMessage: string | undefined = undefined;
		let lastProgressReportedAt = 0;
		const stableCodeDestination = path.join(testDataPath, 's');
		const stableCodeExecutable = await retry(() => measureAndLog(() => ProX-Codetest.download({
		...
		if (process.platform === 'darwin') {
			// ProX Code.app/Contents/MacOS/Electron
