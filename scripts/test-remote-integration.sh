...
if [[ "$1" == "" ]]; then
	AUTHORITY=ProX-Code-remote://test+test
...
then
	...
	# Running from a build, we need to enable the ProX-Code-test-resolver extension
	EXTRA_INTEGRATION_TEST_ARGUMENTS="--extensions-dir=$EXT_PATH  --enable-proposed-api=ProX-Code.ProX-Code-test-resolver --enable-proposed-api=ProX-Code.ProX-Code-api-tests"
...
then
	...
	export TESTRESOLVER_INSTALL_BUILTIN_EXTENSION='ms-ProX-Code.ProX-Code-smoketest-check'
fi
...
echo
"$INTEGRATION_TEST_ELECTRON_PATH" --folder-uri=$REMOTE_VSCODE/ProX-Code-api-tests/testWorkspace --extensionDevelopmentPath=$REMOTE_VSCODE/ProX-Code-api-tests --extensionTestsPath=$REMOTE_VSCODE/ProX-Code-api-tests/out/singlefolder-tests $API_TESTS_EXTRA_ARGS $EXTRA_INTEGRATION_TEST_ARGUMENTS
kill_app

echo
echo "### API tests (workspace)"
echo
"$INTEGRATION_TEST_ELECTRON_PATH" --file-uri=$REMOTE_VSCODE/ProX-Code-api-tests/testworkspace.code-workspace --extensionDevelopmentPath=$REMOTE_VSCODE/ProX-Code-api-tests --extensionTestsPath=$REMOTE_VSCODE/ProX-Code-api-tests/out/workspace-tests $API_TESTS_EXTRA_ARGS $EXTRA_INTEGRATION_TEST_ARGUMENTS
kill_app
