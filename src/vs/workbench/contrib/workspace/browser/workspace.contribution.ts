	...
	private registerListeners(): void {
		...
		this._register(this.workspaceTrustRequestService.onDidInitiateOpenFilesTrustRequest(async () => {
			...
			const markdownDetails = [
				...
				localize('openLooseFileLearnMore', "If you don't want to open untrusted files, we recommend to open them in Restricted Mode in a new window as the files may be malicious. See [our docs](https://aka.ms/ProX-Code-workspace-trust) to learn more.")
			];
			...
			const { result } = await this.dialogService.prompt({
				...
				custom: {
					...
					markdownDetails: [
						...
						{ markdown: new MarkdownString(localize('immediateTrustRequestLearnMore', "If you don't trust the authors of these files, we do not recommend continuing as the files may be malicious. See [our docs](https://aka.ms/ProX-Code-workspace-trust) to learn more.")) }
					]
				},
		...
		this._register(this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequestOnStartup(async () => {
			...
			this.doShowModal(
				...
				[
					...
					learnMoreString ?? localize('startupTrustRequestLearnMore', "If you don't trust the authors of these files, we recommend to continue in restricted mode as the files may be malicious. See [our docs](https://aka.ms/ProX-Code-workspace-trust) to learn more."),
					!isEmptyWindow ?
		...
		const actions =
			[
				...
				},
				{
					...
					href: 'https://aka.ms/ProX-Code-workspace-trust'
				}
			];
	...
	.registerConfiguration({
		...
		properties: {
			[WORKSPACE_TRUST_ENABLED]: {
				type: 'boolean',
				default: true,
				description: localize('workspace.trust.description', "Controls whether or not Workspace Trust is enabled within ProX Code."),
			...
			[WORKSPACE_TRUST_EMPTY_WINDOW]: {
				type: 'boolean',
				default: true,
				markdownDescription: localize('workspace.trust.emptyWindow.description', "Controls whether or not the empty window is trusted by default within ProX Code. When used with `#{0}#`, you can enable the full functionality of ProX Code without prompting in an empty window.", WORKSPACE_TRUST_UNTRUSTED_FILES),
				tags: [WORKSPACE_TRUST_SETTING_TAG],
				scope: ConfigurationScope.APPLICATION
			}
		}
