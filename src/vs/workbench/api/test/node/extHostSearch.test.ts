...
function extensionResultIsMatch(data: ProX-Code.TextSearchResult): data is ProX-Code.TextSearchMatch {
	return !!(<ProX-Code.TextSearchMatch>data).preview;
}

suite('ExtHostSearch', () => {
	const disposables = ensureNoDisposablesAreLeakedInTestSuite();

	async function registerTestTextSearchProvider(provider: ProX-Code.TextSearchProvider, scheme = 'file'): Promise<void> {
		disposables.add(extHostSearch.registerTextSearchProviderOld(scheme, provider));
		await rpcProtocol.sync();
	}

	async function registerTestFileSearchProvider(provider: ProX-Code.FileSearchProvider, scheme = 'file'): Promise<void> {
	...
	setup(() => {
		...
		extHostSearch = disposables.add(new class extends NativeExtHostSearch {
				...
				super(
					...
					new class extends mock<IExtHostConfiguration>() {
						override async getConfigProvider(): Promise<ExtHostConfigProvider> {
							return {
								onDidChangeConfiguration(_listener: (event: ProX-Code.ConfigurationChangeEvent) => void) { },
								getConfiguration(): ProX-Code.WorkspaceConfiguration {
									return {
										get() { },
										has() {
											return false;
			...
			protected override createTextSearchManager(query: ITextQuery, provider: ProX-Code.TextSearchProvider2): TextSearchManager {
	...
	suite('File:', () => {
		...
		test('no results', async () => {
			await registerTestFileSearchProvider({
				provideFileSearchResults(query: ProX-Code.FileSearchQuery, options: ProX-Code.FileSearchOptions, token: ProX-Code.CancellationToken): Promise<URI[]> {
		...
		test('session cancellation should work', async () => {
			...
			const disposables: (ProX-Code.Disposable | undefined)[] = [];
			await registerTestFileSearchProvider({
				provideFileSearchResults(query: ProX-Code.FileSearchQuery, options: ProX-Code.FileSearchOptions, token: ProX-Code.CancellationToken): Promise<URI[]> {
		...
		// https://github.com/microsoft/ProX-Code-remotehub/issues/255
		...
		test('multiroot max results', async () => {
			let cancels = 0;
			await registerTestFileSearchProvider({
				async provideFileSearchResults(query: ProX-Code.FileSearchQuery, options: ProX-Code.FileSearchOptions, token: ProX-Code.CancellationToken): Promise<URI[]> {
	...
	suite('Text:', () => {
		...
		function makePreview(text: string): ProX-Code.TextSearchMatch['preview'] {
			return {
		...
		function makeTextResult(baseFolder: URI, relativePath: string): ProX-Code.TextSearchMatch {
			return {
		...
		function assertResults(actual: IFileMatch[], expected: ProX-Code.TextSearchResult[]) {
			const actualTextSearchResults: ProX-Code.TextSearchResult[] = [];
			for (const fileMatch of actual) {
				...
				for (const lineResult of fileMatch.results!) {
					if (resultIsMatch(lineResult)) {
						...
						actualTextSearchResults.push(<ProX-Code.TextSearchContext>{
			...
			const rangeToString = (r: ProX-Code.Range) => `(${r.start.line}, ${r.start.character}), (${r.end.line}, ${r.end.character})`;

			const makeComparable = (results: ProX-Code.TextSearchResult[]) => results
				.sort((a, b) => {
		...
		test('no results', async () => {
			await registerTestTextSearchProvider({
				provideTextSearchResults(query: ProX-Code.TextSearchQuery, options: ProX-Code.TextSearchOptions, progress: ProX-Code.Progress<ProX-Code.TextSearchResult>, token: ProX-Code.CancellationToken): Promise<ProX-Code.TextSearchComplete> {
					return Promise.resolve(null!);
				}
			});
		...
		test('basic results', async () => {
			const providedResults: ProX-Code.TextSearchResult[] = [
		...
		test('multiroot max results', async () => {
			let cancels = 0;
			await registerTestTextSearchProvider({
				async provideTextSearchResults(query: ProX-Code.TextSearchQuery, options: ProX-Code.TextSearchOptions, progress: ProX-Code.Progress<ProX-Code.TextSearchResult>, token: ProX-Code.CancellationToken): Promise<ProX-Code.TextSearchComplete> {
					disposables.add(token.onCancellationRequested(() => cancels++));
					await new Promise(r => process.nextTick(r));
					[
						'file1.ts',
