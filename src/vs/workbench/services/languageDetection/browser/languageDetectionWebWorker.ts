...
import type { ModelOperations, ModelResult } from '@ProX-Code/ProX-Code-languagedetection';
...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private async getModelOperations(): Promise<ModelOperations> {
		...
		const { ModelOperations } = await importAMDNodeModule(uri, '') as typeof import('@ProX-Code/ProX-Code-languagedetection');
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in ProX Code and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in ProX Code
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in ProX Code
			case 'csv':
			case 'toml':
