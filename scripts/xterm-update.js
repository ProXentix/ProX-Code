...
const ProX-CodeDir = process.argv.length >= 3 ? process.argv[2] : process.cwd();
if (path.basename(ProX-CodeDir) !== 'ProX-Code') {
	console.error('The cwd is not named "ProX-Code"');
	return;
}

function getLatestModuleVersion(moduleName) {
	return new Promise((resolve, reject) => {
		cp.exec(`npm view ${moduleName} versions --json`, { cwd: ProX-CodeDir }, (err, stdout, stderr) => {
			if (err) {
				reject(err);
			}
...
async function update() {
	...
	const pkg = require(path.join(ProX-CodeDir, 'package.json'));
	...
	if (modulesWithVersion.length > 0) {
		for (const cwd of [ProX-CodeDir, path.join(ProX-CodeDir, 'remote'), path.join(ProX-CodeDir, 'remote/web')]) {
	...
	if (backendOnlyModulesWithVersion.length > 0) {
		for (const cwd of [ProX-CodeDir, path.join(ProX-CodeDir, 'remote')]) {
