	...
	store.add(addDompurifyHook('uponSanitizeAttribute', (element, e) => {
		if (e.attrName === 'style' || e.attrName === 'class') {
			if (element.tagName === 'SPAN') {
				if (e.attrName === 'style') {
					e.keepAttr = /^(color\:(#[0-9a-fA-F]+|var\(--ProX-Code(-[a-zA-Z0-9]+)+\));)?(background-color\:(#[0-9a-fA-F]+|var\(--ProX-Code(-[a-zA-Z0-9]+)+\));)?(border-radius:[0-9]+px;)?$/.test(e.attrValue);
					return;
	...
	for (let i = token.tokens.length - 1; i >= 0; i--) {
		...
		if (subtoken.type === 'text') {
			...
			if (lastLine.includes('`')) {
				...
				// Where "more text" is a title for the link or an argument to a ProX-Code command link
				if (
