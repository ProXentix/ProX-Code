...
const cliPath = process.env.VSCODE_FORWARDING_IS_DEV
	...
	: path.join(
		ProX-Code.env.appRoot,
		process.platform === 'darwin' ? 'bin' : '../../bin',
		ProX-Code.env.appQuality === 'stable' ? 'code-tunnel' : 'code-tunnel-insiders',
	) + (process.platform === 'win32' ? '.exe' : '');

class Tunnel implements ProX-Code.Tunnel {
	private readonly disposeEmitter = new ProX-Code.EventEmitter<void>();
...
export async function activate(context: ProX-Code.ExtensionContext) {
	if (ProX-Code.env.remoteAuthority) {
		return; // forwarding is local-only at the moment
	}

	const logger = new Logger(ProX-Code.l10n.t('Port Forwarding'));
	const provider = new TunnelProvider(logger, context);

	context.subscriptions.push(
		ProX-Code.commands.registerCommand('tunnel-forwarding.showLog', () => logger.show()),
		ProX-Code.commands.registerCommand('tunnel-forwarding.restart', () => provider.restart()),

		provider.onDidStateChange(s => {
			ProX-Code.commands.executeCommand('setContext', 'tunnelForwardingIsRunning', s.state !== State.Inactive);
		}),

		await ProX-Code.workspace.registerTunnelProvider(
			provider,
			{
				tunnelFeatures: {
					elevation: false,
					protocol: true,
					privacyOptions: [
						{ themeIcon: 'globe', id: TunnelPrivacyId.Public, label: ProX-Code.l10n.t('Public') },
						{ themeIcon: 'lock', id: TunnelPrivacyId.Private, label: ProX-Code.l10n.t('Private') },
					],
				},
			},
		),
...
class Logger {
	private outputChannel?: ProX-Code.LogOutputChannel;
	...
	public log(
		...
		if (!this.outputChannel) {
			this.outputChannel = ProX-Code.window.createOutputChannel(this.label, { log: true });
			ProX-Code.commands.executeCommand('setContext', 'tunnelForwardingHasLog', true);
		}
...
class TunnelProvider implements ProX-Code.TunnelProvider {
	private readonly tunnels = new Set<Tunnel>();
	private readonly stateChange = new ProX-Code.EventEmitter<StateT>();
	...
	constructor(private readonly logger: Logger, private readonly context: ProX-Code.ExtensionContext) { }

	/** @inheritdoc */
	public async provideTunnel(tunnelOptions: ProX-Code.TunnelOptions): Promise<ProX-Code.Tunnel | undefined> {
	...
	private async consentPublicPort(portNumber: number) {
		...
		const continueOpt = ProX-Code.l10n.t('Continue');
		const dontShowAgain = ProX-Code.l10n.t("Don't show again");
		const r = await ProX-Code.window.showWarningMessage(
			ProX-Code.l10n.t("You're about to create a publicly forwarded port. Anyone on the internet will be able to connect to the service listening on port {0}. You should only proceed if this service is secure and non-sensitive.", portNumber),
			{ modal: true },
			continueOpt,
			dontShowAgain,
		);
	...
	private async setupPortForwardingProcess() {
		const session = await ProX-Code.authentication.getSession('github', ['user:email', 'read:org'], {
			createIfNone: true,
		});
