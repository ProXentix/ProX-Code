...
class NodeModuleESMInterceptor extends RequireInterceptor {
	...
	export const resolve = async (specifier, context, nextResolve) => {
		if (specifier !== 'ProX-Code' || !context.parentURL) {
	...
	private static _ProX-CodeImportFnName = `_VSCODE_IMPORT_VSCODE_API`;
	...
	protected override _installInterceptor(): void {
		...
		const apiInstances = new BidirectionalMap<typeof ProX-Code, string>();
		...
		Object.defineProperty(globalThis, NodeModuleESMInterceptor._ProX-CodeImportFnName, {
			enumerable: false,
			configurable: false,
			writable: false,
			value: (key: string) => {
		...
		port1LayerCheckerWorkaround.onmessage = (e: { data: Message }) => {
			...
			// Get the ProX-Code-module factory - which is the same logic that's also used by
			// the CommonJS require interceptor
			if (!apiModuleFactory) {
				apiModuleFactory = this._factories.get('ProX-Code');
			...
			if (!scriptDataUrlSrc) {
				const jsCode = `const _ProX-CodeInstance = globalThis.${NodeModuleESMInterceptor._ProX-CodeImportFnName}('${key}');\n\n${Object.keys(apiInstance).map((name => `export const ${name} = _ProX-CodeInstance['${name}'];`)).join('\n')}`;
