...
impl CodeVersionManager {
	...
	/// Tries to find the binary entrypoint for ProX Code installed in the path.
...
// Based on https://github.dev/microsoft/ProX-Code-js-debug/blob/7594d05518df6700df51771895fcad0ddc7f92f9/src/common/pathUtils.ts#L15
#[cfg(target_os = "linux")]
fn detect_installed_program(log: &log::Logger) -> io::Result<Vec<PathBuf>> {
...
mod tests {
	...
	fn make_fake_ProX-Code_install(path: &Path) {
	...
	fn make_multiple_ProX-Code_install() -> tempfile::TempDir {
		let dir = tempfile::tempdir().expect("expected to make temp dir");
		make_fake_ProX-Code_install(&dir.path().join("desktop/stable"));
		make_fake_ProX-Code_install(&dir.path().join("desktop/1.68.2"));
		dir
	}

	#[test]
	fn test_detect_installed_program() {
		// developers can run this test and debug output manually; ProX Code will not
	...
	async fn test_set_preferred_version() {
		let dir = make_multiple_ProX-Code_install();
