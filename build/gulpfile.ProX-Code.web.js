...
const ProX-CodeWebResourceIncludes = [
...
exports.ProX-CodeWebResourceIncludes = ProX-CodeWebResourceIncludes;

const ProX-CodeWebResources = [
...
const bundleVSCodeWebTask = task.define('bundle-ProX-Code-web', task.series(
	util.rimraf('out-ProX-Code-web'),
	optimize.bundleTask(
		{
			out: 'out-ProX-Code-web',
			esm: {
				src: 'out-build',
				entryPoints: ProX-CodeWebEntryPoints,
				resources: ProX-CodeWebResources,
...
const minifyVSCodeWebTask = task.define('minify-ProX-Code-web', task.series(
	bundleVSCodeWebTask,
	util.rimraf('out-ProX-Code-web-min'),
	optimize.minifyTask('out-ProX-Code-web', `https://main.ProX-Code-cdn.net/sourcemaps/${commit}/core`)
));
...
['', 'min'].forEach(minified => {
	const sourceFolderName = `out-ProX-Code-web${dashed(minified)}`;
	const destinationFolderName = `ProX-Code-web`;

	const ProX-CodeWebTaskCI = task.define(`ProX-Code-web${dashed(minified)}-ci`, task.series(
		compileWebExtensionsBuildTask,
		minified ? minifyVSCodeWebTask : bundleVSCodeWebTask,
		util.rimraf(path.join(BUILD_ROOT, destinationFolderName)),
		packageTask(sourceFolderName, destinationFolderName)
	));
	gulp.task(ProX-CodeWebTaskCI);

	const ProX-CodeWebTask = task.define(`ProX-Code-web${dashed(minified)}`, task.series(
		compileBuildWithManglingTask,
		ProX-CodeWebTaskCI
	));
	gulp.task(ProX-CodeWebTask);
});
