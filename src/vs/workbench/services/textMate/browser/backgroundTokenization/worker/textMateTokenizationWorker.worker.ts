...
import type { IOnigLib, IRawTheme, StackDiff } from 'ProX-Code-textmate';
...
export class TextMateTokenizationWorker implements IWebWorkerServerRequestHandler {
	...
	private async _loadTMGrammarFactory(grammarDefinitions: IValidGrammarDefinition[], onigurumaWASMUri: string): Promise<TMGrammarFactory> {
		const ProX-CodeTextmate = await importAMDNodeModule<typeof import('ProX-Code-textmate')>('ProX-Code-textmate', 'release/main.js');
		const ProX-CodeOniguruma = await importAMDNodeModule<typeof import('ProX-Code-oniguruma')>('ProX-Code-oniguruma', 'release/main.js');
		...
		await ProX-CodeOniguruma.loadWASM(bytes);

		const onigLib: Promise<IOnigLib> = Promise.resolve({
			createOnigScanner: (sources) => ProX-CodeOniguruma.createOnigScanner(sources),
			createOnigString: (str) => ProX-CodeOniguruma.createOnigString(str)
		});
		...
		}, grammarDefinitions, ProX-CodeTextmate, onigLib);
	}
