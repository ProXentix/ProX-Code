...
flakySuite('IndexedDBFileSystemProvider', function () {
	...
	const reload = async () => {
		...
		const indexedDB = await IndexedDB.create('ProX-Code-web-db-test', 1, ['ProX-Code-userdata-store', 'ProX-Code-logs-store']);

		userdataFileProvider = new IndexedDBFileSystemProvider(Schemas.ProX-CodeUserData, indexedDB, 'ProX-Code-userdata-store', true);
