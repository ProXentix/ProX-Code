...
export default class TypeScriptServiceClientHost extends Disposable {
	...
	constructor(
		...
		context: ProX-Code.ExtensionContext,
		...
		ProX-Code.workspace.onDidChangeConfiguration(this.configurationChanged, this, this._disposables);
		this.configurationChanged();
		this._register(new LogLevelMonitor(context));
	}

	private registerExtensionLanguageProvider(description: LanguageDescription, onCompletionAccepted: (item: ProX-Code.CompletionItem) => void) {
		const manager = new LanguageProvider(this.client, description, this.commandManager, this.client.telemetryReporter, this.typingsStatus, this.fileConfigurationManager, onCompletionAccepted);
		this.languages.push(manager);
		this._register(manager);
		this.languagePerId.set(description.id, manager);
	...
	public async handles(resource: ProX-Code.Uri): Promise<boolean> {
	...
	private configurationChanged(): void {
		const typescriptConfig = ProX-Code.workspace.getConfiguration('typescript');

		this.reportStyleCheckAsWarnings = typescriptConfig.get('reportStyleChecksAsWarnings', true);
	}

	private async findLanguage(resource: ProX-Code.Uri): Promise<LanguageProvider | undefined> {
		try {
			...
			// be correct
			const doc = await ProX-Code.workspace.openTextDocument(resource);
	...
	private configFileDiagnosticsReceived(event: Proto.ConfigFileDiagnosticEvent): void {
		...
		this.findLanguage(this.client.toResource(body.configFile)).then(language => {
			language?.configFileDiagnosticsReceived(this.client.toResource(body.configFile), body.diagnostics.map(tsDiag => {
				const range = tsDiag.start && tsDiag.end ? typeConverters.Range.fromTextSpan(tsDiag) : new ProX-Code.Range(0, 0, 0, 1);
				const diagnostic = new ProX-Code.Diagnostic(range, tsDiag.text, this.getDiagnosticSeverity(tsDiag));
	...
	private createMarkerDatas(
		diagnostics: Proto.Diagnostic[],
		source: string
	): (ProX-Code.Diagnostic & { reportUnnecessary: any; reportDeprecated: any })[] {
		return diagnostics.map(tsDiag => this.tsDiagnosticToVsDiagnostic(tsDiag, source));
	}

	private tsDiagnosticToVsDiagnostic(diagnostic: Proto.Diagnostic, source: string): ProX-Code.Diagnostic & { reportUnnecessary: any; reportDeprecated: any } {
		const { start, end, text } = diagnostic;
		const range = new ProX-Code.Range(typeConverters.Position.fromLocation(start), typeConverters.Position.fromLocation(end));
		const converted = new ProX-Code.Diagnostic(range, text, this.getDiagnosticSeverity(diagnostic));
		...
		if (relatedInformation) {
			converted.relatedInformation = coalesce(relatedInformation.map((info: any) => {
				...
				return new ProX-Code.DiagnosticRelatedInformation(typeConverters.Location.fromTextSpan(this.client.toResource(span.file), span), info.message);
			}));
		}
		const tags: ProX-Code.DiagnosticTag[] = [];
		if (diagnostic.reportsUnnecessary) {
			tags.push(ProX-Code.DiagnosticTag.Unnecessary);
		}
		if (diagnostic.reportsDeprecated) {
			tags.push(ProX-Code.DiagnosticTag.Deprecated);
		}
		converted.tags = tags.length ? tags : undefined;

		const resultConverted = converted as ProX-Code.Diagnostic & { reportUnnecessary: any; reportDeprecated: any };
	...
	private getDiagnosticSeverity(diagnostic: Proto.Diagnostic): ProX-Code.DiagnosticSeverity {
		if (this.reportStyleCheckAsWarnings
			&& this.isStyleCheckDiagnostic(diagnostic.code)
			&& diagnostic.category === PConst.DiagnosticCategory.error
		) {
			return ProX-Code.DiagnosticSeverity.Warning;
		}

		switch (diagnostic.category) {
			case PConst.DiagnosticCategory.error:
				return ProX-Code.DiagnosticSeverity.Error;

			case PConst.DiagnosticCategory.warning:
				return ProX-Code.DiagnosticSeverity.Warning;

			case PConst.DiagnosticCategory.suggestion:
				return ProX-Code.DiagnosticSeverity.Hint;
