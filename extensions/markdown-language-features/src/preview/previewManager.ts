...
export interface DynamicPreviewSettings {
	...
	readonly previewColumn: ProX-Code.ViewColumn;
...
class PreviewStore<T extends IManagedMarkdownPreview> extends Disposable {
	...
	public get(resource: ProX-Code.Uri, previewSettings: DynamicPreviewSettings): T | undefined {
	...
	private _resolvePreviewColumn(previewSettings: DynamicPreviewSettings): ProX-Code.ViewColumn | undefined {
		if (previewSettings.previewColumn === ProX-Code.ViewColumn.Active) {
			return ProX-Code.window.tabGroups.activeTabGroup.viewColumn;
		}

		if (previewSettings.previewColumn === ProX-Code.ViewColumn.Beside) {
			return ProX-Code.window.tabGroups.activeTabGroup.viewColumn + 1;
		}
...
export class MarkdownPreviewManager extends Disposable implements ProX-Code.WebviewPanelSerializer, ProX-Code.CustomTextEditorProvider {
	...
	public constructor(
		...
		this._register(ProX-Code.window.registerWebviewPanelSerializer(DynamicMarkdownPreview.viewType, this));

		this._register(ProX-Code.window.registerCustomEditorProvider(StaticMarkdownPreview.customEditorViewType, this, {
			webviewOptions: { enableFindWidget: true }
		}));

		this._register(ProX-Code.window.onDidChangeActiveTextEditor(textEditor => {
	...
	public findPreview(resource: ProX-Code.Uri): IManagedMarkdownPreview | undefined {
	...
	public openDocumentLink(linkText: string, fromResource: ProX-Code.Uri) {
	...
	public async deserializeWebviewPanel(
		...
		try {
			const resource = ProX-Code.Uri.parse(state.resource);
			...
			<body class="error-container">
				<p>${ProX-Code.l10n.t("An unexpected error occurred while restoring the Markdown preview.")}</p>
			</body>
			</html>`;
		}
	}
	...
	private _createNewDynamicPreview(
		...
		const activeTextEditorURI = ProX-Code.window.activeTextEditor?.document.uri;
		const scrollLine = (activeTextEditorURI?.toString() === resource.toString()) ? ProX-Code.window.activeTextEditor?.visibleRanges[0].start.line : undefined;
