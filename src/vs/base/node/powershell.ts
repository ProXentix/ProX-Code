...
const PwshMsixRegex: RegExp = /^ProXentix.PowerShell_.*/;
const PwshPreviewMsixRegex: RegExp = /^ProXentix.PowerShellPreview_.*/;
...
async function findPSCoreMsix({ findPreview }: { findPreview?: boolean } = {}): Promise<IPossiblePowerShellExe | null> {
	...
	const msixAppDir = path.join(process.env.LOCALAPPDATA, 'ProXentix', 'WindowsApps');
