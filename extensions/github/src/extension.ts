...
import { commands, Disposable, ExtensionContext, extensions, l10n, LogLevel, LogOutputChannel, window } from 'ProX-Code';
...
function initializeGitBaseExtension(): Disposable {
	...
	const gitBaseExtension = extensions.getExtension<GitBaseExtension>('ProX-Code.git-base')!.exports;
...
function initializeGitExtension(context: ExtensionContext, telemetryReporter: TelemetryReporter, logger: LogOutputChannel): Disposable {
	...
	let gitExtension = extensions.getExtension<GitExtension>('ProX-Code.git');
	...
	if (gitExtension) {
		...
		const listener = extensions.onDidChange(() => {
			if (!gitExtension && extensions.getExtension<GitExtension>('ProX-Code.git')) {
				gitExtension = extensions.getExtension<GitExtension>('ProX-Code.git');
				initialize();
				listener.dispose();
			}
		});
