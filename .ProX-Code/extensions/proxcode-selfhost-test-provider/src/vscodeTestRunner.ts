...
const ATTACH_CONFIG_NAME = 'Attach to ProX Code';
const DEBUG_TYPE = 'pwa-chrome';

export abstract class VSCodeTestRunner {
	constructor(protected readonly repoLocation: ProX-Code.WorkspaceFolder) { }

	public async run(baseArgs: ReadonlyArray<string>, filter?: ReadonlyArray<ProX-Code.TestItem>) {
	...
	public async debug(testRun: ProX-Code.TestRun, baseArgs: ReadonlyArray<string>, filter?: ReadonlyArray<ProX-Code.TestItem>) {
		const port = await this.findOpenPort();
		const baseConfiguration = ProX-Code.workspace
			.getConfiguration('launch', this.repoLocation)
			.get<ProX-Code.DebugConfiguration[]>('configurations', [])
		...
		const args = [
			...
			// for breakpoint freeze: https://github.com/microsoft/ProX-Code/issues/122225#issuecomment-885377304
			'--js-flags="--regexp_interpret_all"',
			// for general runtime freezes: https://github.com/microsoft/ProX-Code/issues/127861#issuecomment-904144910
		...
		const factory = ProX-Code.debug.registerDebugAdapterTrackerFactory(DEBUG_TYPE, {
		...
		ProX-Code.debug.startDebugging(this.repoLocation, { ...baseConfiguration, port }, { testRun });

		let exited = false;
		let rootSession: ProX-Code.DebugSession | undefined;
		cp.once('exit', () => {
			exited = true;
			server.dispose();
			listener.dispose();
			...
			if (rootSession) {
				ProX-Code.debug.stopDebugging(rootSession);
			}
		});

		const listener = ProX-Code.debug.onDidStartDebugSession(s => {
	...
	private prepareArguments(
		...
		filter?: ReadonlyArray<ProX-Code.TestItem>
	) {
		...
		function addTestCaseOrSuite(data: TestCase | TestSuite, test: ProX-Code.TestItem): void {
	...
	private createWaitServer() {
		const onReady = new ProX-Code.EventEmitter<void>();
		let ready = false;
