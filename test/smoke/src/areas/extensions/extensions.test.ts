...
export function setup(logger: Logger) {
	describe('Extensions', () => {
		...
		it('install and enable ProX-Code-smoketest-check extension', async function () {
			const app = this.app as Application;

			await app.workbench.extensions.installExtension('ms-ProX-Code.ProX-Code-smoketest-check', true);

			// Close extension editor because keybindings dispatch is not working when web views are opened and focused
			// https://github.com/microsoft/ProX-Code/issues/110276
			await app.workbench.extensions.closeExtension('ProX-Code-smoketest-check');
