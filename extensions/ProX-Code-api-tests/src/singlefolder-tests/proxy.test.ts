...
import { resetCaches } from '@ProX-Code/proxy-agent';
import * as ProX-Code from 'ProX-Code';
import { Straightforward, Middleware, RequestContext, ConnectContext, isRequest, isConnect } from 'straightforward';
import assert from 'assert';

(ProX-Code.env.uiKind === ProX-Code.UIKind.Web ? suite.skip : suite)('ProX-Code API - network proxy support', () => {
	...
	test('basic auth', async () => {
		...
		try {
			...
			await ProX-Code.workspace.getConfiguration().update('http.proxy', `http://127.0.0.1:${proxyPort}`, ProX-Code.ConfigurationTarget.Global);
			await change;
			await new Promise<void>((resolve, reject) => {
				https.get(url, res => {
			...
			sf.close();
			const change = waitForConfigChange('http.proxy');
			await ProX-Code.workspace.getConfiguration().update('http.proxy', undefined, ProX-Code.ConfigurationTarget.Global);
			await change;
			await ProX-Code.workspace.getConfiguration().update('integration-test.http.proxyAuth', undefined, ProX-Code.ConfigurationTarget.Global);
		}
	});

	(ProX-Code.env.remoteName ? test : test.skip)('separate local / remote proxy settings', async () => {
		// Assumes test resolver runs with `--use-host-proxy`.
		const localProxy = 'http://localhost:1234';
		const remoteProxy = 'http://localhost:4321';

		const actualLocalProxy1 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p1 = waitForConfigChange('http.proxy');
		await ProX-Code.workspace.getConfiguration().update('http.proxy', localProxy, ProX-Code.ConfigurationTarget.Global);
		await p1;
		const actualLocalProxy2 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p2 = waitForConfigChange('http.useLocalProxyConfiguration');
		await ProX-Code.workspace.getConfiguration().update('http.useLocalProxyConfiguration', false, ProX-Code.ConfigurationTarget.Global);
		await p2;
		const actualRemoteProxy1 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p3 = waitForConfigChange('http.proxy');
		await ProX-Code.workspace.getConfiguration().update('http.proxy', remoteProxy, ProX-Code.ConfigurationTarget.Global);
		await p3;
		const actualRemoteProxy2 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p4 = waitForConfigChange('http.proxy');
		await ProX-Code.workspace.getConfiguration().update('http.proxy', undefined, ProX-Code.ConfigurationTarget.Global);
		await p4;
		const actualRemoteProxy3 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p5 = waitForConfigChange('http.proxy');
		await ProX-Code.workspace.getConfiguration().update('http.useLocalProxyConfiguration', true, ProX-Code.ConfigurationTarget.Global);
		await p5;
		const actualLocalProxy3 = ProX-Code.workspace.getConfiguration().get('http.proxy');

		const p6 = waitForConfigChange('http.proxy');
		await ProX-Code.workspace.getConfiguration().update('http.proxy', undefined, ProX-Code.ConfigurationTarget.Global);
		await p6;
		const actualLocalProxy4 = ProX-Code.workspace.getConfiguration().get('http.proxy');
	...
	function waitForConfigChange(key: string) {
		return new Promise<void>(resolve => {
			const s = ProX-Code.workspace.onDidChangeConfiguration(e => {
