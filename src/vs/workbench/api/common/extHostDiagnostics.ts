...
export class DiagnosticCollection implements ProX-Code.DiagnosticCollection {

	readonly #proxy: MainThreadDiagnosticsShape | undefined;
	readonly #onDidChangeDiagnostics: Emitter<readonly ProX-Code.Uri[]>;
	readonly #data: ResourceMap<ProX-Code.Diagnostic[]>;
	...
	constructor(
		...
		onDidChangeDiagnostics: Emitter<readonly ProX-Code.Uri[]>
	) {
	...
	set(uri: ProX-Code.Uri, diagnostics: ReadonlyArray<ProX-Code.Diagnostic>): void;
	set(entries: ReadonlyArray<[ProX-Code.Uri, ReadonlyArray<ProX-Code.Diagnostic>]>): void;
	set(first: ProX-Code.Uri | ReadonlyArray<[ProX-Code.Uri, ReadonlyArray<ProX-Code.Diagnostic>]>, diagnostics?: ReadonlyArray<ProX-Code.Diagnostic>) {
		...
		if (URI.isUri(first)) {
			...
			// update many rows
			toSync = [];
			let lastUri: ProX-Code.Uri | undefined;
	...
	forEach(callback: (uri: URI, diagnostics: ReadonlyArray<ProX-Code.Diagnostic>, collection: DiagnosticCollection) => any, thisArg?: any): void {
		this._checkDisposed();
	...
	*[Symbol.iterator](): IterableIterator<[uri: ProX-Code.Uri, diagnostics: readonly ProX-Code.Diagnostic[]]> {
		this._checkDisposed();
	...
	get(uri: URI): ReadonlyArray<ProX-Code.Diagnostic> {
		this._checkDisposed();
	...
	private static _compareIndexedTuplesByUri(a: [ProX-Code.Uri, readonly ProX-Code.Diagnostic[]], b: [ProX-Code.Uri, readonly ProX-Code.Diagnostic[]]): number {
...
export class ExtHostDiagnostics implements ExtHostDiagnosticsShape {
	...
	private readonly _onDidChangeDiagnostics = new DebounceEmitter<readonly ProX-Code.Uri[]>({ merge: all => all.flat(), delay: 50 });

	static _mapper(last: readonly ProX-Code.Uri[]): { uris: readonly ProX-Code.Uri[] } {
		const map = new ResourceMap<ProX-Code.Uri>();
	...
	readonly onDidChangeDiagnostics: Event<ProX-Code.DiagnosticChangeEvent> = Event.map(this._onDidChangeDiagnostics.event, ExtHostDiagnostics._mapper);
	...
	createDiagnosticCollection(extensionId: ExtensionIdentifier, name?: string): ProX-Code.DiagnosticCollection {
	...
	getDiagnostics(resource: ProX-Code.Uri): ReadonlyArray<ProX-Code.Diagnostic>;
	getDiagnostics(): ReadonlyArray<[ProX-Code.Uri, ReadonlyArray<ProX-Code.Diagnostic>]>;
	getDiagnostics(resource?: ProX-Code.Uri): ReadonlyArray<ProX-Code.Diagnostic> | ReadonlyArray<[ProX-Code.Uri, ReadonlyArray<ProX-Code.Diagnostic>]>;
	getDiagnostics(resource?: ProX-Code.Uri): ReadonlyArray<ProX-Code.Diagnostic> | ReadonlyArray<[ProX-Code.Uri, ReadonlyArray<ProX-Code.Diagnostic>]> {
		if (resource) {
			return this._getDiagnostics(resource);
		} else {
			const index = new Map<string, number>();
			const res: [ProX-Code.Uri, ProX-Code.Diagnostic[]][] = [];
	...
	private _getDiagnostics(resource: ProX-Code.Uri): ReadonlyArray<ProX-Code.Diagnostic> {
		let res: ProX-Code.Diagnostic[] = [];
		for (const collection of this._collections.values()) {
	...
	private _mirrorCollection: ProX-Code.DiagnosticCollection | undefined;
