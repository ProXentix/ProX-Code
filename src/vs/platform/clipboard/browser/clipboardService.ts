 ...
 */
const ProX-CodeResourcesMime = 'application/vnd.code.resources';
	...
	async writeResources(resources: URI[]): Promise<void> {
		...
		try {
			await getActiveWindow().navigator.clipboard.write([
				new ClipboardItem({
					[`web ${ProX-CodeResourcesMime}`]: new Blob([
						JSON.stringify(resources.map(x => x.toJSON()))
					], {
						type: ProX-CodeResourcesMime
					})
				})
			]);
	...
	async readResources(): Promise<URI[]> {
		...
		try {
			...
			for (const item of items) {
				if (item.types.includes(`web ${ProX-CodeResourcesMime}`)) {
					const blob = await item.getType(`web ${ProX-CodeResourcesMime}`);
