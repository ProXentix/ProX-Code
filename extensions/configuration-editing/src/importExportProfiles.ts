...
class GitHubGistProfileContentHandler implements ProX-Code.ProfileContentHandler {

	readonly name = ProX-Code.l10n.t('GitHub');
	readonly description = ProX-Code.l10n.t('gist');

	private _octokit: Promise<Octokit> | undefined;
	private getOctokit(): Promise<Octokit> {
		if (!this._octokit) {
			this._octokit = (async () => {
				const session = await ProX-Code.authentication.getSession('github', ['gist', 'user:email'], { createIfNone: true });
	...
	async saveProfile(name: string, content: string): Promise<{ readonly id: string; readonly link: ProX-Code.Uri } | null> {
		...
		if (result.data.id && result.data.html_url) {
			const link = ProX-Code.Uri.parse(result.data.html_url);
	...
	async readProfile(uri: ProX-Code.Uri): Promise<string | null>;
	async readProfile(arg: string | ProX-Code.Uri): Promise<string | null> {
...
ProX-Code.window.registerProfileContentHandler('github', new GitHubGistProfileContentHandler());
