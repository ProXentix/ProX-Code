...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	constructor(
		...
		this.raw = this._register(new xtermCtor({
			...
			logLevel: ProX-CodeToXtermLogLevel(this._logService.getLevel()),
			logger: this._logService,
			minimumContrastRatio: config.minimumContrastRatio,
			tabStopWidth: config.tabStopWidth,
			cursorBlink: config.cursorBlinking,
			cursorStyle: ProX-CodeToXtermCursorStyle<'cursorStyle'>(config.cursorStyle),
			cursorInactiveStyle: ProX-CodeToXtermCursorStyle(config.cursorStyleInactive),
		...
		this._register(this._logService.onDidChangeLogLevel(e => this.raw.options.logLevel = ProX-CodeToXtermLogLevel(e)));
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	private _updateFindColors(searchOptions: ISearchOptions): void {
		...
		// Theme color names align with monaco/ProX-Code whereas xterm.js has some different naming.
	...
	private _setCursorStyle(style: ITerminalConfiguration['cursorStyle']): void {
		const mapped = ProX-CodeToXtermCursorStyle<'cursorStyle'>(style);
	...
	private _setCursorStyleInactive(style: ITerminalConfiguration['cursorStyleInactive']): void {
		const mapped = ProX-CodeToXtermCursorStyle(style);
...
function ProX-CodeToXtermLogLevel(logLevel: LogLevel): XtermLogLevel {
	switch (logLevel) {
...
function ProX-CodeToXtermCursorStyle<T extends 'cursorStyle' | 'cursorStyleInactive'>(style: ITerminalConfiguration[T]): ICursorStyleVscodeToXtermMap[T] {
	// 'line' is used instead of bar in ProX Code to be consistent with editor.cursorStyle
	if (style === 'line') {
		return 'bar';
	}
