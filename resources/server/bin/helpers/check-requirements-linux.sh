...
if [ -f "/tmp/ProX-Code-skip-server-requirements-check" ] || [ -n "$VSCODE_SERVER_CUSTOM_GLIBC_LINKER" ]; then
...
if [ "$OS_ID" = "alpine" ]; then
    ...
    if [ "$(printf '%s\n' "1.2.3" "$musl_version" | sort -V | head -n1)" != "1.2.3" ]; then
        echo "Error: Unsupported alpine distribution. Please refer to our supported distro section https://aka.ms/ProX-Code-remote/linux for additional information."
        exit 99
    fi
...
if [ "$found_required_glibc" = "0" ] || [ "$found_required_glibcxx" = "0" ]; then
	echo "Error: Missing required dependencies. Please refer to our FAQ https://aka.ms/ProX-Code-remote/faq/old-linux for additional information."
