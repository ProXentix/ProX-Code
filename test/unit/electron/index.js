...
if (!args.dev) {
	app.setPath('userData', path.join(tmpdir(), `ProX-Code-tests-${Date.now()}`));
}
...
app.on('ready', () => {
	...
	session.defaultSession.protocol.registerFileProtocol('ProX-Code-file', (request, callback) => {
	...
	ipcMain.handle('ProX-Code:test-ProX-Code-window-config', async () => {
		return {
			product: {
				version: '1.x.y',
	...
	ipcMain.handle('ProX-Code:fetchShellEnv', event => { });
	...
	const win = new BrowserWindow({
		...
		webPreferences: {
			...
			additionalArguments: [`--ProX-Code-window-config=ProX-Code:test-ProX-Code-window-config`],
