...
(function () {
	...
	const resolveConfiguration: Promise<ISandboxConfiguration> = (async () => {
		const windowConfigIpcChannel = parseArgv('ProX-Code-window-config');
		if (!windowConfigIpcChannel) {
			throw new Error('Preload: did not find expected ProX-Code-window-config in renderer process arguments list.');
		}
		...
		try {
			...
			// settings per origin (if ProX-Code-file:// is used) and
			// we want to ensure that the user configuration wins.
			webFrame.setZoomLevel(resolvedConfiguration.zoomLevel ?? 0);

			return resolvedConfiguration;
		} catch (error) {
			throw new Error(`Preload: unable to fetch ProX-Code-window-config: ${error}`);
		}
	})();
	...
	const resolveShellEnv: Promise<typeof process.env> = (async () => {
		...
		const [userEnv, shellEnv] = await Promise.all([
			...
			ipcRenderer.invoke('ProX-Code:fetchShellEnv')
		]);
	...
	if (process.contextIsolated) {
		...
		(window as any).ProX-Code = globals;
	}
}());
