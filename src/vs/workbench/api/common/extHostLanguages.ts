...
export class ExtHostLanguages implements ExtHostLanguagesShape {
	...
	async changeLanguage(uri: ProX-Code.Uri, languageId: string): Promise<ProX-Code.TextDocument> {
	...
	async tokenAtPosition(document: ProX-Code.TextDocument, position: ProX-Code.Position): Promise<ProX-Code.TokenInformation> {
	...
	createLanguageStatusItem(extension: IExtensionDescription, id: string, selector: ProX-Code.DocumentSelector): ProX-Code.LanguageStatusItem {
		...
		const data: Omit<ProX-Code.LanguageStatusItem, 'dispose' | 'text2'> = {
			selector,
			id,
		...
		const result: ProX-Code.LanguageStatusItem = {
			dispose() {
