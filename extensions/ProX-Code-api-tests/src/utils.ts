...
ProX-Code.workspace.registerFileSystemProvider(testFs.scheme, testFs, { isCaseSensitive: testFs.isCaseSensitive });

export async function createRandomFile(contents: string | Uint8Array = '', dir: ProX-Code.Uri | undefined = undefined, ext = ''): Promise<ProX-Code.Uri> {
	let fakeFile: ProX-Code.Uri;
	if (dir) {
		assert.strictEqual(dir.scheme, testFs.scheme);
		fakeFile = dir.with({ path: dir.path + '/' + rndName() + ext });
	} else {
		fakeFile = ProX-Code.Uri.parse(`${testFs.scheme}:/${rndName() + ext}`);
	}
	testFs.writeFile(fakeFile, typeof contents === 'string' ? Buffer.from(contents) : Buffer.from(contents), { create: true, overwrite: true });
	return fakeFile;
}

export async function deleteFile(file: ProX-Code.Uri): Promise<boolean> {
	try {
		testFs.delete(file);
		return true;
	} catch {
...
export function saveAllEditors(): Thenable<any> {
	return ProX-Code.commands.executeCommand('workbench.action.files.saveAll');
}

export async function revertAllDirty(): Promise<void> {
	return ProX-Code.commands.executeCommand('_workbench.revertAllDirty');
}

export function disposeAll(disposables: ProX-Code.Disposable[]) {
	ProX-Code.Disposable.from(...disposables).dispose();
}
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<void> {
	return async (): Promise<void> => {
		const logLevel = await ProX-Code.commands.executeCommand('_extensionTests.getLogLevel');
		await ProX-Code.commands.executeCommand('_extensionTests.setLogLevel', level);

		try {
			await runnable();
		} finally {
			await ProX-Code.commands.executeCommand('_extensionTests.setLogLevel', logLevel);
		}
	};
}
...
export function assertNoRpc() {
	assertNoRpcFromEntry([ProX-Code, 'ProX-Code']);
}
...
export async function asPromise<T>(event: ProX-Code.Event<T>, timeout = ProX-Code.env.uiKind === ProX-Code.UIKind.Desktop ? 5000 : 15000): Promise<T> {
