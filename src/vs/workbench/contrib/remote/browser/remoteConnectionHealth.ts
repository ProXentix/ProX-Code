		...
		const { result, checkboxChecked } = await this.dialogService.prompt<ConnectionChoice>({
			...
			buttons: [
				...
				},
				{
					...
					run: async () => { await this.openerService.open('https://aka.ms/ProX-Code-remote/faq/old-linux'); return ConnectionChoice.LearnMore; }
				}
			],
			cancelButton: {
						...
						const actions = [
							{
								...
								href: 'https://aka.ms/ProX-Code-remote/faq/old-linux'
							}
						];
