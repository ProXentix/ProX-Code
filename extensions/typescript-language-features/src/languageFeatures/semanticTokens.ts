...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerDocumentRangeSemanticTokensProvider(selector.semantic, provider, provider.getLegend());
	});
}

class DocumentSemanticTokensProvider implements ProX-Code.DocumentSemanticTokensProvider, ProX-Code.DocumentRangeSemanticTokensProvider {

	constructor(
		private readonly client: ITypeScriptServiceClient
	) { }

	public getLegend(): ProX-Code.SemanticTokensLegend {
		return new ProX-Code.SemanticTokensLegend(tokenTypes, tokenModifiers);
	}

	public async provideDocumentSemanticTokens(document: ProX-Code.TextDocument, token: ProX-Code.CancellationToken): Promise<ProX-Code.SemanticTokens | null> {
	...
	public async provideDocumentRangeSemanticTokens(document: ProX-Code.TextDocument, range: ProX-Code.Range, token: ProX-Code.CancellationToken): Promise<ProX-Code.SemanticTokens | null> {
		const file = this.client.toOpenTsFilePath(document);
	...
	private async provideSemanticTokens(document: ProX-Code.TextDocument, requestArg: Proto.EncodedSemanticClassificationsRequestArgs, token: ProX-Code.CancellationToken): Promise<ProX-Code.SemanticTokens | null> {
		const file = this.client.toOpenTsFilePath(document);
		if (!file) {
			return null;
		}
		...
		const builder = new ProX-Code.SemanticTokensBuilder();
...
function waitForDocumentChangesToEnd(document: ProX-Code.TextDocument) {
