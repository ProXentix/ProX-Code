		...
		const { result, checkboxChecked } = await this.dialogService.prompt<ConnectionChoice>({
			...
			buttons: [
				...
				},
				{
					...
					run: async () => { await this.openerService.open('https://proxentix.in/proxpl-remote/faq/old-linux'); return ConnectionChoice.LearnMore; }
				}
			],
			cancelButton: {
						...
						const actions = [
							{
								...
								href: 'https://proxentix.in/proxpl-remote/faq/old-linux'
							}
						];
