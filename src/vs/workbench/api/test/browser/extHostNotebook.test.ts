...
suite('NotebookCell#Document', function () {
	...
	setup(async function () {
		...
		const reg = extHostNotebooks.registerNotebookSerializer(nullExtensionDescription, 'test', new class extends mock<ProX-Code.NotebookSerializer>() { });
	...
	test('cell document is ProX-Code.TextDocument', async function () {
	...
	test('cell document is ProX-Code.TextDocument after changing it', async function () {
	...
	test('cell document stays open when notebook is still open', async function () {
		...
		const docs: ProX-Code.TextDocument[] = [];
	...
	test('Opening a notebook results in ProX Code firing the event onDidChangeActiveNotebookEditor twice #118470', function () {
		let count = 0;
