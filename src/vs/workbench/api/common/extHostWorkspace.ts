...
export interface IExtHostWorkspaceProvider {
	getWorkspaceFolder2(uri: ProX-Code.Uri, resolveParent?: boolean): Promise<ProX-Code.WorkspaceFolder | undefined>;
	resolveWorkspaceFolder(uri: ProX-Code.Uri): Promise<ProX-Code.WorkspaceFolder | undefined>;
	getWorkspaceFolders2(): Promise<ProX-Code.WorkspaceFolder[] | undefined>;
...
function compareWorkspaceFolderByUri(a: ProX-Code.WorkspaceFolder, b: ProX-Code.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo): number {
	return isFolderEqual(a.uri, b.uri, extHostFileSystemInfo) ? 0 : compare(a.uri.toString(), b.uri.toString());
}

function compareWorkspaceFolderByUriAndNameAndIndex(a: ProX-Code.WorkspaceFolder, b: ProX-Code.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo): number {
	if (a.index !== b.index) {
...
function delta(oldFolders: ProX-Code.WorkspaceFolder[], newFolders: ProX-Code.WorkspaceFolder[], compare: (a: ProX-Code.WorkspaceFolder, b: ProX-Code.WorkspaceFolder, extHostFileSystemInfo: IExtHostFileSystemInfo) => number, extHostFileSystemInfo: IExtHostFileSystemInfo): { removed: ProX-Code.WorkspaceFolder[]; added: ProX-Code.WorkspaceFolder[] } {
...
interface MutableWorkspaceFolder extends ProX-Code.WorkspaceFolder {
	name: string;
	index: number;
}
...
class ExtHostWorkspaceImpl extends Workspace {
	...
	private readonly _workspaceFolders: ProX-Code.WorkspaceFolder[] = [];
	private readonly _structure: TernarySearchTree<URI, ProX-Code.WorkspaceFolder>;

	constructor(id: string, private _name: string, folders: ProX-Code.WorkspaceFolder[], transient: boolean, configuration: URI | null, private _isUntitled: boolean, ignorePathCasing: (key: URI) => boolean) {
		super(id, folders.map(f => new WorkspaceFolder(f)), transient, configuration, ignorePathCasing);
		this._structure = TernarySearchTree.forUris<ProX-Code.WorkspaceFolder>(ignorePathCasing, () => true);
	...
	get workspaceFolders(): ProX-Code.WorkspaceFolder[] {
		return this._workspaceFolders.slice(0);
	}

	getWorkspaceFolder(uri: URI, resolveParent?: boolean): ProX-Code.WorkspaceFolder | undefined {
	...
	resolveWorkspaceFolder(uri: URI): ProX-Code.WorkspaceFolder | undefined {
...
export class ExtHostWorkspace implements ExtHostWorkspaceShape, IExtHostWorkspaceProvider {
	...
	private readonly _onDidChangeWorkspace = new Emitter<ProX-Code.WorkspaceFoldersChangeEvent>();
	readonly onDidChangeWorkspace: Event<ProX-Code.WorkspaceFoldersChangeEvent> = this._onDidChangeWorkspace.event;
	...
	private readonly _editSessionIdentityProviders = new Map<string, ProX-Code.EditSessionIdentityProvider>();
	...
	get workspaceFile(): ProX-Code.Uri | undefined {
	...
	getWorkspaceFolders(): ProX-Code.WorkspaceFolder[] | undefined {
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async getWorkspaceFolders2(): Promise<ProX-Code.WorkspaceFolder[] | undefined> {
	...
	updateWorkspaceFolders(extension: IExtensionDescription, index: number, deleteCount: number, ...workspaceFoldersToAdd: { uri: ProX-Code.Uri; name?: string }[]): boolean {
		const validatedDistinctWorkspaceFoldersToAdd: { uri: ProX-Code.Uri; name?: string }[] = [];
	...
	getWorkspaceFolder(uri: ProX-Code.Uri, resolveParent?: boolean): ProX-Code.WorkspaceFolder | undefined {
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async getWorkspaceFolder2(uri: ProX-Code.Uri, resolveParent?: boolean): Promise<ProX-Code.WorkspaceFolder | undefined> {
		await this._barrier.wait();
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	async resolveWorkspaceFolder(uri: ProX-Code.Uri): Promise<ProX-Code.WorkspaceFolder | undefined> {
		await this._barrier.wait();
		if (!this._actualWorkspace) {
			return undefined;
		}
	...
	getRelativePath(pathOrUri: string | ProX-Code.Uri, includeWorkspace?: boolean): string {
	...
	private trySetWorkspaceFolders(folders: ProX-Code.WorkspaceFolder[]): void {
	 ...
	 */
	findFiles(include: ProX-Code.GlobPattern | undefined, exclude: ProX-Code.GlobPattern | null | undefined, maxResults: number | undefined, extensionId: ExtensionIdentifier, token: ProX-Code.CancellationToken = CancellationToken.None): Promise<ProX-Code.Uri[]> {
	...
	findFiles2(filePatterns: readonly ProX-Code.GlobPattern[],
		options: ProX-Code.FindFiles2Options = {},
		extensionId: ExtensionIdentifier,
		token: ProX-Code.CancellationToken = CancellationToken.None): Promise<ProX-Code.Uri[]> {
	...
	private async _findFilesImpl(
		...
		query: { readonly type: 'include'; readonly value: ProX-Code.GlobPattern | undefined } | { readonly type: 'filePatterns'; readonly value: readonly ProX-Code.GlobPattern[] },
		options: ProX-Code.FindFiles2Options,
		token: ProX-Code.CancellationToken
	): Promise<ProX-Code.Uri[]> {
	...
	findTextInFiles2(query: ProX-Code.TextSearchQuery2, options: ProX-Code.FindTextInFilesOptions2 | undefined, extensionId: ExtensionIdentifier, token: ProX-Code.CancellationToken = CancellationToken.None): ProX-Code.FindTextInFilesResponse {
		...
		const getOptions = (include: ProX-Code.GlobPattern | undefined): QueryOptions<ITextQueryBuilderOptions> => {
			if (!options) {
				return {
					folder: undefined,
					options: {}
		...
		const asyncIterable = new AsyncIterableObject<ProX-Code.TextSearchResult2>(async emitter => {
	...
	async findTextInFilesBase(query: ProX-Code.TextSearchQuery, queryOptions: QueryOptions<ITextQueryBuilderOptions>[] | undefined, callback: (result: ITextSearchResult<URI>, uri: URI) => void, token: ProX-Code.CancellationToken = CancellationToken.None): Promise<ProX-Code.TextSearchComplete> {
	...
	async findTextInFiles(query: ProX-Code.TextSearchQuery, options: ProX-Code.FindTextInFilesOptions & { useSearchExclude?: boolean }, callback: (result: ProX-Code.TextSearchResult) => void, extensionId: ExtensionIdentifier, token: ProX-Code.CancellationToken = CancellationToken.None): Promise<ProX-Code.TextSearchComplete> {
		this._logService.trace(`extHostWorkspace#findTextInFiles: textSearch, extension: ${extensionId.value}, entryPoint: findTextInFiles`);

		const previewOptions: ProX-Code.TextSearchPreviewOptions = typeof options.previewOptions === 'undefined' ?
			{
				matchLines: 100,
				charsPerLine: 10000
			} :
		...
		const progress = (result: ITextSearchResult<URI>, uri: URI) => {
			if (resultIsMatch(result)) {
				...
				} satisfies ProX-Code.TextSearchMatch);
			} else {
				callback({
					uri,
					text: result.text,
					lineNumber: result.lineNumber
				} satisfies ProX-Code.TextSearchContext);
			}
		};
	...
	requestWorkspaceTrust(options?: ProX-Code.WorkspaceTrustRequestOptions): Promise<boolean | undefined> {
	...
	private readonly _onWillCreateEditSessionIdentityEvent = new AsyncEmitter<ProX-Code.EditSessionIdentityWillCreateEvent>();

	getOnWillCreateEditSessionIdentityEvent(extension: IExtensionDescription): Event<ProX-Code.EditSessionIdentityWillCreateEvent> {
		return (listener, thisArg, disposables) => {
			const wrappedListener: IExtensionListener<ProX-Code.EditSessionIdentityWillCreateEvent> = function wrapped(e) { listener.call(thisArg, e); };
	...
	async $onWillCreateEditSessionIdentity(workspaceFolder: UriComponents, token: CancellationToken, timeout: number): Promise<void> {
		...
		await this._onWillCreateEditSessionIdentityEvent.fireAsync({ workspaceFolder: folder }, token, async (thenable: Promise<unknown>, listener) => {
			...
			if (Date.now() - now > timeout) {
				this._logService.warn('SLOW edit session create-participant', (<IExtensionListener<ProX-Code.EditSessionIdentityWillCreateEvent>>listener).extension.identifier);
			}
		});
	...
	private readonly _canonicalUriProviders = new Map<string, ProX-Code.CanonicalUriProvider>();
	...
	async provideCanonicalUri(uri: URI, options: ProX-Code.CanonicalUriRequestOptions, cancellationToken: CancellationToken): Promise<URI | undefined> {
	...
	async decode(content: Uint8Array, args?: { uri?: ProX-Code.Uri; encoding?: string }): Promise<string> {
	...
	async encode(content: string, args?: { uri?: ProX-Code.Uri; encoding?: string }): Promise<Uint8Array> {
	...
	private toEncodeDecodeParameters(opts?: { uri?: ProX-Code.Uri; encoding?: string }): [UriComponents | undefined, { encoding: string } | undefined] {
...
function globsToISearchPatternBuilder(excludes: ProX-Code.GlobPattern[] | undefined): ISearchPatternBuilder<URI>[] {
	return (
