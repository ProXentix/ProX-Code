...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private getWelcomeViewContent(): IChatWelcomeMessageContent {
		...
		if (this.input.currentMode === ChatMode.Ask) {
			...
			return {
				title: localize('editsTitle', "Edit with Copilot"),
				message: new MarkdownString(localize('agentMessage', "Ask Copilot to edit your files in [agent mode]({0}). Copilot will automatically use multiple requests to pick files to edit, run terminal commands, and iterate on errors.", 'https://aka.ms/ProX-Code-copilot-agent') + `\n\n${baseMessage}`),
				icon: Codicon.copilotLarge
			};
		}
	}
	...
	private onDidStyleChange(): void {
		this.container.style.setProperty('--ProX-Code-interactive-result-editor-background-color', this.editorOptions.configuration.resultEditor.backgroundColor?.toString() ?? '');
		this.container.style.setProperty('--ProX-Code-interactive-session-foreground', this.editorOptions.configuration.foreground?.toString() ?? '');
		this.container.style.setProperty('--ProX-Code-chat-list-background', this.themeService.getColorTheme().getColor(this.styles.listBackground)?.toString() ?? '');
	}
