...
#
#         string match -q "$TERM_PROGRAM" "ProX-Code"
...
and string match --quiet "$TERM_PROGRAM" "ProX-Code"
and ! set --query VSCODE_SHELL_INTEGRATION
or exit

set --global VSCODE_SHELL_INTEGRATION 1
set --global __ProX-Code_shell_env_reporting $VSCODE_SHELL_ENV_REPORTING
set -e VSCODE_SHELL_ENV_REPORTING
set -g envVarsToReport
if test -n "$__ProX-Code_shell_env_reporting"
	set envVarsToReport (string split "," "$__ProX-Code_shell_env_reporting")
end
...
function __preserve_fish_prompt --on-event fish_prompt
	if functions --query fish_prompt
		...
		__init_ProX-Code_shell_integration
	else
		if functions --query __vsc_fish_prompt
			functions --erase __preserve_fish_prompt
			__init_ProX-Code_shell_integration
...
function __init_ProX-Code_shell_integration
