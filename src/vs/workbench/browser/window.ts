...
export abstract class BaseWindow extends Disposable {
	...
	protected enableMultiWindowAwareTimeout(targetWindow: Window, dom = { getWindowsCount, getWindows }): void {
		...
		Object.defineProperty(targetWindow, 'ProX-CodeOriginalSetTimeout', { get: () => originalSetTimeout });

		const originalClearTimeout = targetWindow.clearTimeout;
		Object.defineProperty(targetWindow, 'ProX-CodeOriginalClearTimeout', { get: () => originalClearTimeout });
		...
		targetWindow.setTimeout = function (this: unknown, handler: TimerHandler, timeout = 0, ...args: unknown[]): number {
			...
			for (const { window, disposables } of dom.getWindows()) {
				...
				const handle = (window as any).ProX-CodeOriginalSetTimeout.apply(this, [(...args: unknown[]) => {
					if (didClear) {
						return;
					}
					handlerFn(...args);
				...
				const timeoutDisposable = toDisposable(() => {
					didClear = true;
					(window as any).ProX-CodeOriginalClearTimeout(handle);
...
export class BrowserWindow extends BaseWindow {
	...
	private setupOpenHandlers(): void {
		...
		// ProX-Code itself. We do not want to open these links
							...
							await this.dialogService.prompt({
								...
								buttons: [
									...
									},
									{
										...
										run: () => this.openerService.open(URI.parse('https://aka.ms/allow-ProX-Code-popup'))
									}
								],
								cancelButton: true
							});
