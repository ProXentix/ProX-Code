...
declare module 'ProX-Code' {
	...
	export class RelativePattern {
		 ...
		 * Example:
		 * ```ts
		 * const folder = ProX-Code.workspace.workspaceFolders?.[0];
		 * if (folder) {
		 *
		 *   // Match any TypeScript file in the root of this workspace folder
		 *   const pattern1 = new ProX-Code.RelativePattern(folder, '*.ts');
		 *
		 *   // Match any TypeScript file in `someFolder` inside this workspace folder
		 *   const pattern2 = new ProX-Code.RelativePattern(folder, 'someFolder/*.ts');
		 * }
		 * ```
		 *
	...
	export class MarkdownString {
		 ...
		 * and attributes to be rendered. See https://github.com/microsoft/ProX-Code/blob/6d2920473c6f13759c978dd89104c4270a83422d/src/vs/base/browser/markdownRenderer.ts#L296
		 ...
		 *
		 * ```ts
		 * const md = new ProX-Code.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = ProX-Code.Uri.file('/path/to/dir/');
		 ...
		 * ```ts
		 * const md = new ProX-Code.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = ProX-Code.Uri.file('/path/to/otherFile.js');
	 ...
	 * const config = workspace.getConfiguration('launch', ProX-Code.workspace.workspaceFolders[0].uri);
	 *
	 * // retrieve values
	...
	export interface Terminal {
		 ...
		 *   	ProX-Code.window.showInformationMessage(`Exit code: ${t.exitStatus.code}`);
		 *   }
		 * });
		 * ```
		 */
	...
	export interface ExtensionContext {
		 ...
		 * {@linkcode ExtensionContext.extensionUri extensionUri}, e.g. `ProX-Code.Uri.joinPath(context.extensionUri, relativePath);`
		 *
	 ...
	 * a file or folder doesn't exist, use them like so: `throw ProX-Code.FileSystemError.FileNotFound(someUri);`
	 */
	...
	export interface Webview {
		 ...
		 * ```html
		 * <script>
		 *     const ProX-Code = acquireVsCodeApi(); // acquireVsCodeApi can only be invoked once
		 *     ProX-Code.postMessage({ message: 'hello!' });
		 * </script>
		 * ```
		 *
		 ...
		 * setting a [content security policy](https://aka.ms/ProX-Code-api-webview-csp).
		 */
		html: string;
		 ...
		 *
		 *   For older versions of ProX-Code, if an `ArrayBuffer` is included in `message`,
		 ...
		 *
		 *   However if your extension targets ProX-Code 1.57+ in the `engines` field of its
		 ...
		 *
		 * ```ts
		 * webview.html = `<img src="${webview.asWebviewUri(ProX-Code.Uri.file('/Users/codey/workspace/cat.gif'))}">`
		 * ```
		 */
	...
	/**
	 * Restore webview panels that have been persisted when ProX-Code shuts down.
	 *
	 ...
	 * ```js
	 * // Within the webview
	 * const ProX-Code = acquireVsCodeApi();
	 *
	 * // Get existing state
	 * const oldState = ProX-Code.getState() || { value: 0 };
	 *
	 * // Update state
	...
	export namespace env {
		...
		/**
		 * The application name of the editor, like 'ProX Code'.
		 */
		 ...
		 * * VSCode itself (`ProX-Code:` from `ProX-Code.env.uriScheme`)
		 *
		 ...
		 * ```typescript
		 * ProX-Code.window.registerUriHandler({
		 *   handleUri(uri: ProX-Code.Uri): ProX-Code.ProviderResult<void> {
		 ...
		 * });
		 *
		 * const callableUri = await ProX-Code.env.asExternalUri(ProX-Code.Uri.parse(ProX-Code.env.uriScheme + '://my.extension/did-authenticate'));
		 * await ProX-Code.env.openExternal(callableUri);
		 * ```
		 *
	...
	export namespace window {
		 ...
		 * the following rules:
		 *
		 * - The uri-scheme must be `ProX-Code.env.uriScheme`;
	...
	export class TreeItem {
		 ...
		 *
		 * Please use `ProX-Code.open` or `ProX-Code.diff` as command IDs when the tree item is opening
	...
	export interface Pseudoterminal {
		 ...
		 * ```typescript
		 * const writeEmitter = new ProX-Code.EventEmitter<string>();
		 * const pty: ProX-Code.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 *   open: () => writeEmitter.fire('\x1b[31mHello world\x1b[0m'),
		 *   close: () => {}
		 * };
		 * ProX-Code.window.createTerminal({ name: 'My terminal', pty });
		 * ```
		 *
		 ...
		 * ```typescript
		 * const dimensionsEmitter = new ProX-Code.EventEmitter<ProX-Code.TerminalDimensions>();
		 * const pty: ProX-Code.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * ```typescript
		 * const writeEmitter = new ProX-Code.EventEmitter<string>();
		 * const closeEmitter = new ProX-Code.EventEmitter<void>();
		 * const pty: ProX-Code.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 *   handleInput: data => {
		 *     if (data !== 'y') {
		 *       ProX-Code.window.showInformationMessage('Something went wrong');
		 *     }
		 *     closeEmitter.fire();
		 *   }
		 * };
		 * const terminal = ProX-Code.window.createTerminal({ name: 'Exit example', pty });
		 * terminal.show(true);
		 * ```
		 */
		 ...
		 * ```typescript
		 * const writeEmitter = new ProX-Code.EventEmitter<string>();
		 * const changeNameEmitter = new ProX-Code.EventEmitter<string>();
		 * const pty: ProX-Code.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * };
		 * ProX-Code.window.createTerminal({ name: 'Local echo', pty });
		 * ```
		 */
	...
	export namespace workspace {
		 ...
		 * files, e.g. `ProX-Code.workspace.fs.readDirectory(someUri)` allows to retrieve all entries
		 * of a directory or `ProX-Code.workspace.fs.stat(anotherUri)` returns the meta data for a
		 * file.
		 */
		 ...
		 *
		 * The location can e.g. be used with the `ProX-Code.openFolder` command to
		 * open the workspace again after it has been closed.
		 *
		 * **Example:**
		 * ```typescript
		 * ProX-Code.commands.executeCommand('ProX-Code.openFolder', uriOfWorkspace);
		 * ```
		 *
		 * Refer to https://code.visualstudio.com/docs/editor/workspaces for more information on
		 * the concept of workspaces.
		 ...
		 * by an optional set of `workspaceFoldersToAdd` on the `ProX-Code.workspace.workspaceFolders` array. This "splice"
		 ...
		 *
		 * ```ts
		 * const watcher = ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(<folder>, <pattern>));
		 *
		 ...
		 *
		 * ```ts
		 * ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(ProX-Code.workspace.workspaceFolders[0], '**​/*.js'));
		 * ```
		 *
		 * If you want to monitor file events across all opened workspace folders:
		 *
		 * ```ts
		 * ProX-Code.workspace.createFileSystemWatcher('**​/*.js');
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(ProX-Code.Uri.file(<path to folder outside workspace>), '*.js'));
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(ProX-Code.window.activeTextEditor.document.uri, '*'));
		 * ```
		 *
	...
	export namespace languages {
		 ...
		 * doc.uri; // `ProX-Code-notebook-cell:///my/notebook.ipynb#gl65s2pmha`;
	...
	export class NotebookCellOutput {
		 ...
		 * 	ProX-Code.NotebookCellOutputItem.text('Hello', 'text/plain'),
		 * 	ProX-Code.NotebookCellOutputItem.text('<i>Hello</i>', 'text/html'),
		 * 	ProX-Code.NotebookCellOutputItem.text('_Hello_', 'text/markdown'),
		 * 	ProX-Code.NotebookCellOutputItem.text('Hey', 'text/plain'), // INVALID: repeated type, editor will pick just one
		 * ])
		 * ```
		 */
	 ...
	 * [ProX-Code-l10n repo](https://github.com/microsoft/ProX-Code-l10n).
	 *
	 ...
	 *
	 * ```js
	 * const sender: ProX-Code.TelemetrySender = {...};
	 * const logger = ProX-Code.env.createTelemetryLogger(sender);
	 *
	 ...
	 * failure causes, like `if(someError.code === ProX-Code.LanguageModelError.NotFound.name) {...}`
	...
	export namespace lm {
		 ...
		 *
		 * ```ts
		 * const models = await ProX-Code.lm.selectChatModels({ family: 'gpt-3.5-turbo' });
		 * if (models.length > 0) {
		 * 	const [first] = models;
		 ...
		 * {@link LanguageModelPromptTsxPart prompt-tsx}-parts. If the tool caller is using `@ProX-Code/prompt-tsx`, it can
	...
	/**
	 * A language model response part containing a PromptElementJSON from `@ProX-Code/prompt-tsx`.
	...
	export class LanguageModelPromptTsxPart {
		 ...
		 * @param value The value of the part, the result of `renderPromptElementJSON` from `@ProX-Code/prompt-tsx`.
		 */
		constructor(value: unknown);
	}

	/**
	 * A result returned from a tool invocation. If using `@ProX-Code/prompt-tsx`, this result may be rendered using a `ToolResult`.
	 */
