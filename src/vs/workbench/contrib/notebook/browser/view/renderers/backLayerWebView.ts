...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
		...
		if (rendererMessaging) {
			...
			rendererMessaging.receiveMessageHandler = (rendererId, message) => {
				...
				this._sendMessageToWebview({
					__ProX-Code_notebook_message: true,
			...
			<head>
				...
				<style nonce="${this.nonce}">
					::highlight(find-highlight) {
						background-color: var(--ProX-Code-editor-findMatchBackground, ${findMatchHighlight});
					}

					::highlight(current-find-highlight) {
						background-color: var(--ProX-Code-editor-findMatchHighlightBackground, ${currentHighlight});
					}
					...
					#container .cell_container.nb-insertHighlight div.output_container div.output {
						background-color: var(--ProX-Code-diffEditor-insertedLineBackground, var(--ProX-Code-diffEditor-insertedTextBackground));
					}
					...
					#container .nb-chatGenerationHighlight .output_container .output {
						background-color: var(--ProX-Code-notebook-selectedCellBackground);
					}
					...
					#container .no-renderer-error {
						color: var(--ProX-Code-editorError-foreground);
					}
					...
					.find-match {
						background-color: var(--ProX-Code-editor-findMatchHighlightBackground);
					}

					.current-find-match {
						background-color: var(--ProX-Code-editor-findMatchBackground);
					}

					#_defaultColorPalatte {
						color: var(--ProX-Code-editor-findMatchHighlightBackground);
						background-color: var(--ProX-Code-editor-findMatchBackground);
					}
				</style>
			</head>
	...
	private _initialize(content: string, targetWindow: CodeWindow): Promise<void> {
		...
		this._register(this.webview.onMessage(async (message) => {
			const data: FromWebviewMessage | { readonly __ProX-Code_notebook_message: undefined } = message.message;
			if (this._disposed) {
				return;
			}

			if (!data.__ProX-Code_notebook_message) {
				return;
			}
			...
			switch (data.type) {
				...
				case 'notebookPerformanceMessage': {
					...
					if (data.outputSize && data.rendererId === 'ProX-Code.builtin-renderer') {
	...
	private createMetadata(output: ICellOutput, mimeType: string) {
		if (mimeType.startsWith('image')) {
			...
			if (buffer?.length && buffer?.length > 0) {
				...
				return { ...output.metadata, ProX-Code_altText: altText };
			}
		}
