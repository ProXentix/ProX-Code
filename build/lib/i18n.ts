...
import iconv from '@ProX-Code/iconv-lite-umd';
import { l10nJsonFormat, getL10nXlf, l10nJsonDetails, getL10nFilesFromXlf, getL10nJson } from '@ProX-Code/l10n-dev';
...
function processCoreBundleFormat(base: string, fileHeader: string, languages: Language[], json: NLSKeysFormat, emitter: ThroughStream) {
	const languageDirectory = path.join(REPO_ROOT_PATH, '..', 'ProX-Code-loc', 'i18n');
	...
	sortedLanguages.forEach((language) => {
		...
		const i18nFile = path.join(languageDirectory, `ProX-Code-language-pack-${languageFolderName}`, 'translations', 'main.i18n.json');
...
const editorProject: string = 'ProX-Code-editor',
	workbenchProject: string = 'ProX-Code-workbench',
	extensionsProject: string = 'ProX-Code-extensions',
	setupProject: string = 'ProX-Code-setup',
	serverProject: string = 'ProX-Code-server';
...
function createL10nBundleForExtension(extensionFolderName: string, prefixWithBuildFolder: boolean): NodeJS.ReadWriteStream {
	...
	return gulp
		.src([
			...
			// // For any dependencies pulled in (think ProX-Code-css-languageservice or @ProX-Code/emmet-helper)
			`${prefix}extensions/${extensionFolderName}/**/node_modules/{@ProX-Code,ProX-Code-*}/**/*.{js,jsx}`,
			// // For any dependencies pulled in that bundle @ProX-Code/l10n. They needed to export the bundle
...
export const EXTERNAL_EXTENSIONS = [
	'ms-ProX-Code.js-debug',
	'ms-ProX-Code.js-debug-companion',
	'ms-ProX-Code.ProX-Code-js-profile-table',
];
...
export function prepareI18nPackFiles(resultingTranslationPaths: TranslationPath[]): NodeJS.ReadWriteStream {
	...
	return through(function (this: ThroughStream, xlf: File) {
		...
		// strip `-new` since ProX-Code-extensions-loc uses the `-new` suffix to indicate that it's from the new loc pipeline
		...
		Promise.all(parsePromises)
			.then(() => {
				...
				resultingTranslationPaths.push({ id: 'ProX-Code', resourceName: 'main.i18n.json' });
