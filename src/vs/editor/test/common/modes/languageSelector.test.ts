...
suite('LanguageSelector', function () {
	...
	test('score, notebookType', function () {
		const obj = {
			uri: URI.parse('ProX-Code-notebook-cell:///my/file.js#blabla'),
			langId: 'javascript',
	...
	test('Snippet choices lost #149363', function () {
		const selector: LanguageSelector = {
			scheme: 'ProX-Code-notebook-cell',
			pattern: '/some/path/file.py',
			language: 'python'
		};

		const modelUri = URI.parse('ProX-Code-notebook-cell:///some/path/file.py');
