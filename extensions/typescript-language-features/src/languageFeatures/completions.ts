...
interface CompletionContext {
	...
	readonly wordRange: ProX-Code.Range | undefined;
	readonly line: string;
	readonly optionalReplacementRange: ProX-Code.Range | undefined;
}

type ResolvedCompletionItem = {
	readonly edits?: readonly ProX-Code.TextEdit[];
	readonly commands: readonly ProX-Code.Command[];
};

class MyCompletionItem extends ProX-Code.CompletionItem {
	...
	constructor(
		...
		if (tsEntry.source && tsEntry.hasAction && client.apiVersion.lt(API.v490)) {
			...
			// https://github.com/microsoft/ProX-Code/issues/40311
		...
		if (tsEntry.source && tsEntry.hasAction) {
			...
			const qualifierCandidate = ProX-Code.workspace.asRelativePath(tsEntry.source);
		...
		this.useCodeSnippet = completionContext.completeFunctionCalls && (this.kind === ProX-Code.CompletionItemKind.Function || this.kind === ProX-Code.CompletionItemKind.Method);
		...
		this.insertText = isSnippet && tsEntry.insertText ? new ProX-Code.SnippetString(tsEntry.insertText) : tsEntry.insertText;
		this.filterText = tsEntry.filterText || this.getFilterText(completionContext.line, tsEntry.insertText);

		if (completionContext.isMemberCompletion && completionContext.dotAccessorContext && !(this.insertText instanceof ProX-Code.SnippetString)) {
		...
		if (tsEntry.kindModifiers) {
			...
			if (kindModifiers.has(PConst.KindModifiers.deprecated)) {
				this.tags = [ProX-Code.CompletionItemTag.Deprecated];
			}

			if (kindModifiers.has(PConst.KindModifiers.color)) {
				this.kind = ProX-Code.CompletionItemKind.Color;
			}
	...
	private _resolvedPromise?: {
		readonly requestToken: ProX-Code.CancellationTokenSource;
	...
	public async resolveCompletionItem(
		...
		const requestToken = new ProX-Code.CancellationTokenSource();
		...
		const promise = (async (): Promise<ResolvedCompletionItem | undefined> => {
			...
			const commands: ProX-Code.Command[] = [{
			...
			if (this.useCodeSnippet) {
				...
				if (shouldCompleteFunction) {
					...
					if (parameterCount > 0) {
						//Fix for https://github.com/microsoft/ProX-Code/issues/104059
						//Don't show parameter hints if "editor.parameterHints.enabled": false
						if (ProX-Code.workspace.getConfiguration('editor.parameterHints').get('enabled')) {
	...
	): ProX-Code.MarkdownString | undefined {
		const documentation = new ProX-Code.MarkdownString();
	...
	private async isValidFunctionCompletionContext(
		...
		// https://github.com/microsoft/ProX-Code/issues/18131
	...
	private getCodeActions(
		detail: Proto.CompletionEntryDetails,
		filepath: string
	): { command?: ProX-Code.Command; additionalTextEdits?: ProX-Code.TextEdit[] } {
		...
		// using a ProX-Code command
		const additionalTextEdits: ProX-Code.TextEdit[] = [];
		...
		let command: ProX-Code.Command | undefined = undefined;
	...
	private getRangeFromReplacementSpan(tsEntry: Proto.CompletionEntry, completionContext: CompletionContext) {
		if (!tsEntry.replacementSpan) {
			if (completionContext.optionalReplacementRange) {
				return {
					inserting: new ProX-Code.Range(completionContext.optionalReplacementRange.start, this.position),
		...
		if (!replaceRange.isSingleLine) {
			replaceRange = new ProX-Code.Range(replaceRange.start.line, replaceRange.start.character, replaceRange.start.line, completionContext.line.length);
		}
		return {
	...
	private resolveRange(): void {
		...
		if (replaceRange) {
			this.range = {
				inserting: new ProX-Code.Range(replaceRange.start, this.position),
				replacing: replaceRange
			};
		}
	}

	private static convertKind(kind: string): ProX-Code.CompletionItemKind {
		switch (kind) {
			case PConst.Kind.primitiveType:
			case PConst.Kind.keyword:
				return ProX-Code.CompletionItemKind.Keyword;
			...
			case PConst.Kind.parameter:
				return ProX-Code.CompletionItemKind.Variable;

			case PConst.Kind.memberVariable:
			case PConst.Kind.memberGetAccessor:
			case PConst.Kind.memberSetAccessor:
				return ProX-Code.CompletionItemKind.Field;

			case PConst.Kind.function:
			case PConst.Kind.localFunction:
				return ProX-Code.CompletionItemKind.Function;

			case PConst.Kind.method:
			case PConst.Kind.constructSignature:
			case PConst.Kind.callSignature:
			case PConst.Kind.indexSignature:
				return ProX-Code.CompletionItemKind.Method;

			case PConst.Kind.enum:
				return ProX-Code.CompletionItemKind.Enum;

			case PConst.Kind.enumMember:
				return ProX-Code.CompletionItemKind.EnumMember;

			case PConst.Kind.module:
			case PConst.Kind.externalModuleName:
				return ProX-Code.CompletionItemKind.Module;

			case PConst.Kind.class:
			case PConst.Kind.type:
				return ProX-Code.CompletionItemKind.Class;

			case PConst.Kind.interface:
				return ProX-Code.CompletionItemKind.Interface;

			case PConst.Kind.warning:
				return ProX-Code.CompletionItemKind.Text;

			case PConst.Kind.script:
				return ProX-Code.CompletionItemKind.File;

			case PConst.Kind.directory:
				return ProX-Code.CompletionItemKind.Folder;

			case PConst.Kind.string:
				return ProX-Code.CompletionItemKind.Constant;

			default:
				return ProX-Code.CompletionItemKind.Property;
		}
	}
...
class CompletionAcceptedCommand implements Command {
	...
	public constructor(
		private readonly onCompletionAccepted: (item: ProX-Code.CompletionItem) => void,
		private readonly telemetryReporter: TelemetryReporter,
	) { }

	public execute(item: ProX-Code.CompletionItem) {
...
class ApplyCompletionCommand implements Command {
	...
	public async execute(item: MyCompletionItem) {
		...
		for (const command of commands) {
			await ProX-Code.commands.executeCommand(command.command, ...(command.arguments ?? []));
		}
	}
}
...
class ApplyCompletionCodeActionCommand implements Command {
	...
	public async execute(_file: string, codeActions: Proto.CodeAction[]): Promise<boolean> {
		...
		const selection = await ProX-Code.window.showQuickPick(
			codeActions.map(action => ({
				label: action.description,
				description: '',
				action,
			})), {
			placeHolder: ProX-Code.l10n.t("Select code action to apply")
		});
...
namespace CompletionConfiguration {
	...
	export function getConfigurationForResource(
		...
		const config = ProX-Code.workspace.getConfiguration(modeId, resource);
		return {
...
class TypeScriptCompletionItemProvider implements ProX-Code.CompletionItemProvider<MyCompletionItem> {
	...
	constructor(
		...
		private readonly telemetryReporter: TelemetryReporter,
		onCompletionAccepted: (item: ProX-Code.CompletionItem) => void
	) {
	...
	public async provideCompletionItems(
		...
		position: ProX-Code.Position,
		token: ProX-Code.CancellationToken,
		context: ProX-Code.CompletionContext
	): Promise<ProX-Code.CompletionList<MyCompletionItem> | undefined> {
		if (!ProX-Code.workspace.getConfiguration(this.language.id, document).get('suggest.enabled')) {
			return undefined;
		}

		if (this.typingsStatus.isAcquiringTypings) {
			return Promise.reject<ProX-Code.CompletionList<MyCompletionItem>>({
				label: ProX-Code.l10n.t({
		...
		if (wordRange && !wordRange.isEmpty) {
			...
			const firstChar = document.getText(new ProX-Code.Range(wordRange.start, secondCharPosition));
			if (firstChar === '@') {
		...
		let optionalReplacementRange: ProX-Code.Range | undefined;
		...
		if (isMemberCompletion) {
			...
			if (dotMatch) {
				const range = new ProX-Code.Range(position.translate({ characterDelta: -dotMatch[0].length }), position);
		...
		return new ProX-Code.CompletionList(items, isIncomplete);
	}
	...
	private getTsTriggerCharacter(context: ProX-Code.CompletionContext): Proto.CompletionsTriggerCharacter | undefined {
