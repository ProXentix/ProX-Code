...
class TypeScriptRenameProvider implements ProX-Code.RenameProvider {
	...
	): Promise<ProX-Code.Range | undefined> {
		...
		switch (response.type) {
			case 'rename': {
				...
				if (!renameInfo.canRename) {
					return Promise.reject<ProX-Code.Range>(renameInfo.localizedErrorMessage);
				}
	...
	public async provideRenameEdits(
		...
		newName: string,
		token: ProX-Code.CancellationToken
	): Promise<ProX-Code.WorkspaceEdit | undefined> {
		...
		switch (response.type) {
			case 'rename': {
				const renameInfo = response.body.info;
				if (!renameInfo.canRename) {
					return Promise.reject<ProX-Code.WorkspaceEdit>(renameInfo.localizedErrorMessage);
				}
				...
				if (renameInfo.fileToRename) {
					...
					if (edits) {
						...
						return Promise.reject<ProX-Code.WorkspaceEdit>(ProX-Code.l10n.t("An error occurred while renaming file"));
					}
				}
	...
	public async execRename(
		...
		if (this.client.apiVersion.gte(API.v510) &&
			ProX-Code.workspace.getConfiguration(this.language.id).get('preferences.renameMatchingJsxTags', true) &&
	...
	private looksLikePotentialJsxTagContext(document: ProX-Code.TextDocument, position: ProX-Code.Position): boolean {
		if (![languageIds.typescriptreact, languageIds.javascript, languageIds.javascriptreact].includes(document.languageId)) {
			return false;
		}

		const prefix = document.getText(new ProX-Code.Range(position.line, 0, position.line, position.character));
	...
	private async renameFile(
		...
		edits.renameFile(ProX-Code.Uri.file(fileToRename), ProX-Code.Uri.file(newFilePath));
		return edits;
	}
}
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerRenameProvider(selector.semantic,
