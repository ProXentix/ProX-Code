...
function getVSCodeSysrootChecksum(expectedName: string) {
	const checksums = fs.readFileSync(path.join(REPO_ROOT, 'build', 'checksums', 'ProX-Code-sysroot.txt'), 'utf8');
...
export async function getVSCodeSysroot(arch: DebianArchString, isMusl: boolean = false): Promise<string> {
	...
	const sysroot = process.env['VSCODE_SYSROOT_DIR'] ?? path.join(tmpdir(), `ProX-Code-${arch}-sysroot`);
