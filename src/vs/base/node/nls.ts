...
export async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath }: IResolveNLSConfigurationContext): Promise<INLSConfiguration> {
	...
	try {
		...
		const mainLanguagePackPath = languagePack?.translations?.['ProX-Code'];
		if (
			!languagePack ||
		...
		const result: INLSConfiguration = {
			...
			// NLS: below properties are a relic from old times only used by ProX-Code-nls and deprecated
			locale: userLocale,
 ...
 * core (`ProX-Code`) and all extensions it supports, it points to the related
 * translation files.
 *
