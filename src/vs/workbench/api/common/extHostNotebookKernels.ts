...
interface IKernelData {
	...
	controller: ProX-Code.NotebookController;
	onDidChangeSelection: Emitter<{ selected: boolean; notebook: ProX-Code.NotebookDocument }>;
	onDidReceiveMessage: Emitter<{ editor: ProX-Code.NotebookEditor; message: any }>;
	associatedNotebooks: ResourceMap<boolean>;
}

type ExtHostSelectKernelArgs = ControllerInfo | { notebookEditor: ProX-Code.NotebookEditor } | ControllerInfo & { notebookEditor: ProX-Code.NotebookEditor } | undefined;
...
export class ExtHostNotebookKernels implements ExtHostNotebookKernelsShape {
	...
	private _kernelDetectionTask = new Map<number, ProX-Code.NotebookControllerDetectionTask>();
	private _kernelDetectionTaskHandlePool: number = 0;

	private _kernelSourceActionProviders = new Map<number, ProX-Code.NotebookKernelSourceActionProvider>();
	...
	constructor(
		...
		const requestKernelVariablesApiCommand = new ApiCommand(
			'ProX-Code.executeNotebookVariableProvider',
			'_executeNotebookVariableProvider',
			'Execute notebook variable provider',
			[ApiCommandArgument.Uri],
			new ApiCommandResult<VariablesResult[], ProX-Code.VariablesResult[]>('A promise that resolves to an array of variables', (value, apiArgs) => {
	...
	createNotebookController(extension: IExtensionDescription, id: string, viewType: string, label: string, handler?: (cells: ProX-Code.NotebookCell[], notebook: ProX-Code.NotebookDocument, controller: ProX-Code.NotebookController) => void | Thenable<void>, preloads?: ProX-Code.NotebookRendererScript[]): ProX-Code.NotebookController {
		...
		const onDidChangeSelection = new Emitter<{ selected: boolean; notebook: ProX-Code.NotebookDocument }>();
		const onDidReceiveMessage = new Emitter<{ editor: ProX-Code.NotebookEditor; message: any }>();
		...
		let _interruptHandler: ((this: ProX-Code.NotebookController, notebook: ProX-Code.NotebookDocument) => void | Thenable<void>) | undefined;
		let _variableProvider: ProX-Code.NotebookVariableProvider | undefined;
		...
		const controller: ProX-Code.NotebookController = {
			get id() { return id; },
	...
	getIdByController(controller: ProX-Code.NotebookController) {
	...
	createNotebookControllerDetectionTask(extension: IExtensionDescription, viewType: string): ProX-Code.NotebookControllerDetectionTask {
		...
		const detectionTask: ProX-Code.NotebookControllerDetectionTask = {
			dispose: () => {
	...
	registerKernelSourceActionProvider(extension: IExtensionDescription, viewType: string, provider: ProX-Code.NotebookKernelSourceActionProvider) {
		...
		let subscription: ProX-Code.Disposable | undefined;
	...
	async $executeCells(handle: number, uri: UriComponents, handles: number[]): Promise<void> {
		...
		const cells: ProX-Code.NotebookCell[] = [];
	...
	private id = 0;
	private variableStore: Record<string, ProX-Code.Variable> = {};
	...
	async $provideVariables(handle: number, requestId: string, notebookUri: UriComponents, parentId: number | undefined, kind: 'named' | 'indexed', start: number, token: CancellationToken): Promise<void> {
		...
		let parent: ProX-Code.Variable | undefined = undefined;
	...
	_createNotebookCellExecution(cell: ProX-Code.NotebookCell, controllerId: string): ProX-Code.NotebookCellExecution {
		if (cell.index < 0) {
	...
	_createNotebookExecution(nb: ProX-Code.NotebookDocument, controllerId: string): ProX-Code.NotebookExecution {
...
class NotebookCellExecutionTask extends Disposable {
	...
	private cellIndexToHandle(cellOrCellIndex: ProX-Code.NotebookCell | undefined): number {
	...
	private validateAndConvertOutputs(items: ProX-Code.NotebookCellOutput[]): NotebookOutputDto[] {
	...
	private async updateOutputs(outputs: ProX-Code.NotebookCellOutput | ProX-Code.NotebookCellOutput[], cell: ProX-Code.NotebookCell | undefined, append: boolean): Promise<void> {
	...
	private async updateOutputItems(items: ProX-Code.NotebookCellOutputItem | ProX-Code.NotebookCellOutputItem[], output: ProX-Code.NotebookCellOutput, append: boolean): Promise<void> {
	...
	asApiObject(): ProX-Code.NotebookCellExecution {
		const that = this;
		const result: ProX-Code.NotebookCellExecution = {
			...
			end(success: boolean | undefined, endTime?: number, executionError?: ProX-Code.CellExecutionError): void {
			...
			clearOutput(cell?: ProX-Code.NotebookCell): Thenable<void> {
				that.verifyStateForOutput();
				return that.updateOutputs([], cell, false);
			},

			appendOutput(outputs: ProX-Code.NotebookCellOutput | ProX-Code.NotebookCellOutput[], cell?: ProX-Code.NotebookCell): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputs(outputs, cell, true);
			},

			replaceOutput(outputs: ProX-Code.NotebookCellOutput | ProX-Code.NotebookCellOutput[], cell?: ProX-Code.NotebookCell): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputs(outputs, cell, false);
			},

			appendOutputItems(items: ProX-Code.NotebookCellOutputItem | ProX-Code.NotebookCellOutputItem[], output: ProX-Code.NotebookCellOutput): Promise<void> {
				that.verifyStateForOutput();
				return that.updateOutputItems(items, output, true);
			},

			replaceOutputItems(items: ProX-Code.NotebookCellOutputItem | ProX-Code.NotebookCellOutputItem[], output: ProX-Code.NotebookCellOutput): Promise<void> {
				that.verifyStateForOutput();
...
function createSerializeableError(executionError: ProX-Code.CellExecutionError | undefined) {
	const convertRange = (range: ProX-Code.Range | undefined) => (range ? {
	...
	const convertStackFrame = (frame: ProX-Code.CellErrorStackFrame) => ({
		uri: frame.uri,
...
class NotebookExecutionTask extends Disposable {
	...
	asApiObject(): ProX-Code.NotebookExecution {
		const result: ProX-Code.NotebookExecution = {
			start: () => {
