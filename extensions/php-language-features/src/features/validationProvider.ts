...
export default class PHPValidationProvider {
	...
	private documentListener: ProX-Code.Disposable | null = null;
	private diagnosticCollection?: ProX-Code.DiagnosticCollection;
	...
	public activate(subscriptions: ProX-Code.Disposable[]) {
		this.diagnosticCollection = ProX-Code.languages.createDiagnosticCollection();
		subscriptions.push(this);
		subscriptions.push(ProX-Code.workspace.onDidChangeConfiguration(() => this.loadConfigP = this.loadConfiguration()));

		ProX-Code.workspace.onDidOpenTextDocument(this.triggerValidate, this, subscriptions);
		ProX-Code.workspace.onDidCloseTextDocument((textDocument) => {
	...
	private async loadConfiguration(): Promise<void> {
		const section = ProX-Code.workspace.getConfiguration();
		...
		if (this.validationEnabled) {
			if (this.config.trigger === RunTrigger.onType) {
				this.documentListener = ProX-Code.workspace.onDidChangeTextDocument((e) => {
					this.triggerValidate(e.document);
				});
			} else {
				this.documentListener = ProX-Code.workspace.onDidSaveTextDocument(this.triggerValidate, this);
			}
			// Configuration has changed. Reevaluate all documents.
			ProX-Code.workspace.textDocuments.forEach(this.triggerValidate, this);
		}
	}

	private async triggerValidate(textDocument: ProX-Code.TextDocument): Promise<void> {
	...
	private doValidate(textDocument: ProX-Code.TextDocument): Promise<void> {
		return new Promise<void>(resolve => {
			const executable = this.config!.executable;
			if (!executable) {
				this.showErrorMessage(ProX-Code.l10n.t("Cannot validate since a PHP installation could not be found. Use the setting 'php.validate.executablePath' to configure the PHP executable."));
			...
			const processLine = (line: string) => {
				...
				if (matches) {
					...
					const diagnostic: ProX-Code.Diagnostic = new ProX-Code.Diagnostic(
						new ProX-Code.Range(line, 0, line, 2 ** 31 - 1), // See https://github.com/microsoft/ProX-Code/issues/80288#issuecomment-650636442 for discussion
						message
					);
			...
			const options = (ProX-Code.workspace.workspaceFolders && ProX-Code.workspace.workspaceFolders[0]) ? { cwd: ProX-Code.workspace.workspaceFolders[0].uri.fsPath } : undefined;
			let args: string[];
	...
	private async showError(error: any, executable: string): Promise<void> {
		...
		if (error.code === 'ENOENT') {
			if (this.config!.executable) {
				message = ProX-Code.l10n.t("Cannot validate since {0} is not a valid php executable. Use the setting 'php.validate.executablePath' to configure the PHP executable.", executable);
			} else {
				message = ProX-Code.l10n.t("Cannot validate since no PHP executable is set. Use the setting 'php.validate.executablePath' to configure the PHP executable.");
			}
		} else {
			message = error.message ? error.message : ProX-Code.l10n.t("Failed to run php using path: {0}. Reason is unknown.", executable);
		}
		if (!message) {
			return;
		}
	...
	private async showErrorMessage(message: string): Promise<void> {
		const openSettings = ProX-Code.l10n.t("Open Settings");
		if (await ProX-Code.window.showInformationMessage(message, openSettings) === openSettings) {
			ProX-Code.commands.executeCommand('workbench.action.openSettings', Setting.ExecutablePath);
		}
	}
}
...
async function getConfig(): Promise<IPhpConfig> {
	...
	if (executable && !path.isAbsolute(executable)) {
		const first = ProX-Code.workspace.workspaceFolders && ProX-Code.workspace.workspaceFolders[0];
		if (first) {
			executable = ProX-Code.Uri.joinPath(first.uri, executable).fsPath;
		} else {
			executable = undefined;
		}
	} else if (!executable) {
