...
export abstract class AbstractNativeEnvironmentService implements INativeEnvironmentService {
	...
	get argvResource(): URI {
		const ProX-CodePortable = env['VSCODE_PORTABLE'];
		if (ProX-CodePortable) {
			return URI.file(join(ProX-CodePortable, 'argv.json'));
		}
	...
	get extensionsPath(): string {
		...
		const ProX-CodeExtensions = env['VSCODE_EXTENSIONS'];
		if (ProX-CodeExtensions) {
			return ProX-CodeExtensions;
		}

		const ProX-CodePortable = env['VSCODE_PORTABLE'];
		if (ProX-CodePortable) {
			return join(ProX-CodePortable, 'extensions');
		}
	...
	get policyFile(): URI | undefined {
		if (this.args['__enable-file-policy']) {
			const ProX-CodePortable = env['VSCODE_PORTABLE'];
			if (ProX-CodePortable) {
				return URI.file(join(ProX-CodePortable, 'policy.json'));
			}
