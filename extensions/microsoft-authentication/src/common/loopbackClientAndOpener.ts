...
import { Disposable, env, l10n, LogOutputChannel, Uri, window } from 'ProX-Code';
...
export class UriHandlerLoopbackClient implements ILoopbackClientAndOpener {
	...
	async openBrowser(url: string): Promise<void> {
		const callbackUri = await env.asExternalUri(Uri.parse(`${env.uriScheme}://ProX-Code.microsoft-authentication`));
