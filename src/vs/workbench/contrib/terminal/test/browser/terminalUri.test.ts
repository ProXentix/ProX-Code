...
suite('terminalUri', () => {
	...
	suite('getTerminalResourcesFromDragEvent', () => {
		...
		test('should return terminal resource when event contains one', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["ProX-Code-terminal:/1626874386474/3"]'))?.map(e => e.toString()),
				['ProX-Code-terminal:/1626874386474/3']
			);
		});
		test('should return multiple terminal resources when event contains multiple', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["ProX-Code-terminal:/foo/1","ProX-Code-terminal:/bar/2"]'))?.map(e => e.toString()),
				['ProX-Code-terminal:/foo/1', 'ProX-Code-terminal:/bar/2']
			);
		});
	});
