...
function isCancellationToken(value: any): value is ProX-Code.CancellationToken {
...
export class TSServerRequestCommand implements Command {
	...
	public async execute(command: keyof TypeScriptRequests, args?: any, config?: any, token?: ProX-Code.CancellationToken): Promise<unknown> {
		if (!isCancellationToken(token)) {
			token = nulToken;
		}
		if (args && typeof args === 'object' && !Array.isArray(args)) {
			const requestArgs = args as RequestArgs;
			const hasFile = requestArgs.file instanceof ProX-Code.Uri;
