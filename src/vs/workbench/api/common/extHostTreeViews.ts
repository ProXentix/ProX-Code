...
export class ExtHostTreeViews extends Disposable implements ExtHostTreeViewsShape {
	...
	private treeDragAndDropService: ITreeViewsDnDService<ProX-Code.DataTransfer> = new TreeViewsDnDService<ProX-Code.DataTransfer>();
	...
	registerTreeDataProvider<T>(id: string, treeDataProvider: ProX-Code.TreeDataProvider<T>, extension: IExtensionDescription): ProX-Code.Disposable {
		const treeView = this.createTreeView(id, { treeDataProvider }, extension);
		return { dispose: () => treeView.dispose() };
	}

	createTreeView<T>(viewId: string, options: ProX-Code.TreeViewOptions<T>, extension: IExtensionDescription): ProX-Code.TreeView<T> {
		...
		const view = {
			...
			set message(message: string | ProX-Code.MarkdownString) {
			...
			set badge(badge: ProX-Code.ViewBadge | undefined) {
		...
		this._register(view);
		return view as ProX-Code.TreeView<T>;
	}
	...
	private async addAdditionalTransferItems(treeDataTransfer: ProX-Code.DataTransfer, treeView: ExtHostTreeView<any>,
		sourceTreeItemHandles: string[], token: CancellationToken, operationUuid?: string): Promise<ProX-Code.DataTransfer | undefined> {
	...
	$resolve(treeViewId: string, treeItemHandle: string, token: ProX-Code.CancellationToken): Promise<ITreeItem | undefined> {
		const treeView = this.treeViews.get(treeViewId);
		if (!treeView) {
			throw new NoTreeViewError(treeViewId);
		}
	...
	private createExtHostTreeView<T>(id: string, options: ProX-Code.TreeViewOptions<T>, extension: IExtensionDescription): ExtHostTreeView<T> {
...
interface TreeNode extends IDisposable {
	item: ITreeItem;
	extensionItem: ProX-Code.TreeItem;
...
class ExtHostTreeView<T> extends Disposable {
	...
	private readonly dataProvider: ProX-Code.TreeDataProvider<T>;
	private readonly dndController: ProX-Code.TreeDragAndDropController<T> | undefined;
	...
	private _onDidExpandElement: Emitter<ProX-Code.TreeViewExpansionEvent<T>> = this._register(new Emitter<ProX-Code.TreeViewExpansionEvent<T>>());
	readonly onDidExpandElement: Event<ProX-Code.TreeViewExpansionEvent<T>> = this._onDidExpandElement.event;

	private _onDidCollapseElement: Emitter<ProX-Code.TreeViewExpansionEvent<T>> = this._register(new Emitter<ProX-Code.TreeViewExpansionEvent<T>>());
	readonly onDidCollapseElement: Event<ProX-Code.TreeViewExpansionEvent<T>> = this._onDidCollapseElement.event;

	private _onDidChangeSelection: Emitter<ProX-Code.TreeViewSelectionChangeEvent<T>> = this._register(new Emitter<ProX-Code.TreeViewSelectionChangeEvent<T>>());
	readonly onDidChangeSelection: Event<ProX-Code.TreeViewSelectionChangeEvent<T>> = this._onDidChangeSelection.event;

	private _onDidChangeActiveItem: Emitter<ProX-Code.TreeViewActiveItemChangeEvent<T>> = this._register(new Emitter<ProX-Code.TreeViewActiveItemChangeEvent<T>>());
	readonly onDidChangeActiveItem: Event<ProX-Code.TreeViewActiveItemChangeEvent<T>> = this._onDidChangeActiveItem.event;

	private _onDidChangeVisibility: Emitter<ProX-Code.TreeViewVisibilityChangeEvent> = this._register(new Emitter<ProX-Code.TreeViewVisibilityChangeEvent>());
	readonly onDidChangeVisibility: Event<ProX-Code.TreeViewVisibilityChangeEvent> = this._onDidChangeVisibility.event;

	private _onDidChangeCheckboxState = this._register(new Emitter<ProX-Code.TreeCheckboxChangeEvent<T>>());
	readonly onDidChangeCheckboxState: Event<ProX-Code.TreeCheckboxChangeEvent<T>> = this._onDidChangeCheckboxState.event;
	...
	constructor(
		private viewId: string, options: ProX-Code.TreeViewOptions<T>,
	...
	private _message: string | ProX-Code.MarkdownString = '';
	get message(): string | ProX-Code.MarkdownString {
	...
	private _badge: ProX-Code.ViewBadge | undefined;
	get badge(): ProX-Code.ViewBadge | undefined {
		return this._badge;
	}
	...
	async setCheckboxState(checkboxUpdates: CheckboxUpdate[]) {
		type CheckboxUpdateWithItem = { extensionItem: NonNullable<T>; treeItem: ProX-Code.TreeItem; newState: extHostTypes.TreeItemCheckboxState };
	...
	async handleDrag(sourceTreeItemHandles: TreeItemHandle[], treeDataTransfer: ProX-Code.DataTransfer, token: CancellationToken): Promise<ProX-Code.DataTransfer | undefined> {
	...
	async onDrop(treeDataTransfer: ProX-Code.DataTransfer, targetHandleOrNode: TreeItemHandle | undefined, token: CancellationToken): Promise<void> {
	...
	async resolveTreeItem(treeItemHandle: string, token: ProX-Code.CancellationToken): Promise<ITreeItem | undefined> {
	...
	private createAndRegisterTreeNode(element: T, extTreeItem: ProX-Code.TreeItem, parentNode: TreeNode | Root): TreeNode {
	...
	private getTooltip(tooltip?: string | ProX-Code.MarkdownString): string | IMarkdownString | undefined {
	...
	private getCommand(disposable: DisposableStore, command?: ProX-Code.Command): TreeCommand | undefined {
		return command ? { ...this.commands.toInternal(command, disposable), originalId: command.command } : undefined;
	}

	private getCheckbox(extensionTreeItem: ProX-Code.TreeItem): ITreeItemCheckboxState | undefined {
	...
	private validateTreeItem(extensionTreeItem: ProX-Code.TreeItem) {
	...
	private createTreeNode(element: T, extensionTreeItem: ProX-Code.TreeItem, parent: TreeNode | Root): TreeNode {
	...
	private getThemeIcon(extensionTreeItem: ProX-Code.TreeItem): extHostTypes.ThemeIcon | undefined {
		return extensionTreeItem.iconPath instanceof extHostTypes.ThemeIcon ? extensionTreeItem.iconPath : undefined;
	}

	private createHandle(element: T, { id, label, resourceUri }: ProX-Code.TreeItem, parent: TreeNode | Root, returnFirst?: boolean): TreeItemHandle {
		if (id) {
	...
	private getLightIconPath(extensionTreeItem: ProX-Code.TreeItem): URI | undefined {
	...
	private getDarkIconPath(extensionTreeItem: ProX-Code.TreeItem): URI | undefined {
