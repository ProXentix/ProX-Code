...
export class ImplicitProjectConfiguration {
	...
	constructor(configuration: ProX-Code.WorkspaceConfiguration) {
	...
	private static readTarget(configuration: ProX-Code.WorkspaceConfiguration): string | undefined {
		return configuration.get<string>('js/ts.implicitProjectConfig.target');
	}

	private static readModule(configuration: ProX-Code.WorkspaceConfiguration): string | undefined {
		return configuration.get<string>('js/ts.implicitProjectConfig.module');
	}

	private static readCheckJs(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.checkJs', false);
	}

	private static readExperimentalDecorators(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.experimentalDecorators', false);
	}

	private static readImplicitStrictNullChecks(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('js/ts.implicitProjectConfig.strictNullChecks', true);
	}

	private static readImplicitStrictFunctionTypes(configuration: ProX-Code.WorkspaceConfiguration): boolean {
...
const ProX-CodeWatcherName = 'ProX-Code';
type ProX-CodeWatcherName = typeof ProX-CodeWatcherName;
...
export abstract class BaseServiceConfigurationProvider implements ServiceConfigurationProvider {
	...
	public loadFromWorkspace(): TypeScriptServiceConfiguration {
		const configuration = ProX-Code.workspace.getConfiguration();
		return {
	...
	protected abstract readGlobalTsdk(configuration: ProX-Code.WorkspaceConfiguration): string | null;
	protected abstract readLocalTsdk(configuration: ProX-Code.WorkspaceConfiguration): string | null;
	protected abstract readLocalNodePath(configuration: ProX-Code.WorkspaceConfiguration): string | null;
	protected abstract readGlobalNodePath(configuration: ProX-Code.WorkspaceConfiguration): string | null;

	protected readTsServerLogLevel(configuration: ProX-Code.WorkspaceConfiguration): TsServerLogLevel {
		const setting = configuration.get<string>('typescript.tsserver.log', 'off');
		return TsServerLogLevel.fromString(setting);
	}

	protected readTsServerPluginPaths(configuration: ProX-Code.WorkspaceConfiguration): string[] {
		return configuration.get<string[]>('typescript.tsserver.pluginPaths', []);
	}

	protected readNpmLocation(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		return configuration.get<string | null>('typescript.npm', null);
	}

	protected readDisableAutomaticTypeAcquisition(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.disableAutomaticTypeAcquisition', false);
	}

	protected readLocale(configuration: ProX-Code.WorkspaceConfiguration): string | null {
		const value = configuration.get<string>('typescript.locale', 'auto');
		return !value || value === 'auto' ? null : value;
	}

	protected readUseSyntaxServer(configuration: ProX-Code.WorkspaceConfiguration): SyntaxServerConfiguration {
	...
	protected readEnableDiagnosticsTelemetry(configuration: ProX-Code.WorkspaceConfiguration): boolean {
	...
	protected readEnableProjectDiagnostics(configuration: ProX-Code.WorkspaceConfiguration): boolean {
	...
	private readUseVsCodeWatcher(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		...
		return configuration.get<Proto.WatchOptions | ProX-CodeWatcherName>('typescript.tsserver.watchOptions', ProX-CodeWatcherName) === ProX-CodeWatcherName;
	}

	private readWatchOptions(configuration: ProX-Code.WorkspaceConfiguration): Proto.WatchOptions | undefined {
		const watchOptions = configuration.get<Proto.WatchOptions | ProX-CodeWatcherName>('typescript.tsserver.watchOptions');
		if (watchOptions === ProX-CodeWatcherName) {
			return undefined;
		}
	...
	protected readIncludePackageJsonAutoImports(configuration: ProX-Code.WorkspaceConfiguration): 'auto' | 'on' | 'off' | undefined {
		return configuration.get<'auto' | 'on' | 'off'>('typescript.preferences.includePackageJsonAutoImports');
	}

	protected readMaxTsServerMemory(configuration: ProX-Code.WorkspaceConfiguration): number {
	...
	protected readEnablePromptUseWorkspaceTsdk(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.enablePromptUseWorkspaceTsdk', false);
	}

	protected readEnableTsServerTracing(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.enableTracing', false);
	}

	private readWorkspaceSymbolsExcludeLibrarySymbols(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.workspaceSymbols.excludeLibrarySymbols', true);
	}

	private readWebProjectWideIntellisenseEnable(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.web.projectWideIntellisense.enabled', true);
	}

	private readWebProjectWideIntellisenseSuppressSemanticErrors(configuration: ProX-Code.WorkspaceConfiguration): boolean {
	...
	private readWebTypeAcquisition(configuration: ProX-Code.WorkspaceConfiguration): boolean {
		return configuration.get<boolean>('typescript.tsserver.web.typeAcquisition.enabled', true);
	}

	private readEnableRegionDiagnostics(configuration: ProX-Code.WorkspaceConfiguration): boolean {
