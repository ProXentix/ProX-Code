...
async function getPipelineArtifacts(): Promise<Artifact[]> {
	...
	return result.value.filter(a => /^ProX-Code_/.test(a.name) && !/sbom$/.test(a.name));
}
...
async function processArtifact(
	...
	const match = /^ProX-Code_(?<product>[^_]+)_(?<os>[^_]+)(?:_legacy)?_(?<arch>[^_]+)_(?<unprocessedType>[^_]+)$/.exec(artifact.name);
