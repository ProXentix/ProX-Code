...
export class Menu extends ActionBar {
	...
	constructor(container: HTMLElement, actions: ReadonlyArray<IAction>, options: IMenuOptions, private readonly menuStyles: IMenuStyles) {
		...
		this._register(addDisposableListener(scrollElement, EventType.MOUSE_UP, e => {
			// Absorb clicks in menu dead space https://github.com/microsoft/ProX-Code/issues/63575
...
.monaco-menu .monaco-action-bar .action-item.disabled .action-label:hover {
	color: var(--ProX-Code-disabledForeground);
}
...
.monaco-menu .monaco-action-bar.vertical .action-label.separator {
	display: block;
	border-bottom: 1px solid var(--ProX-Code-menu-separatorBackground);
	padding-top: 1px;
	padding: 30px;
}
...
.monaco-menu .action-item {
	...
	if (isForShadowDom) {
		...
		result += `
			...
			/* Fix for https://github.com/microsoft/ProX-Code/issues/103170 */
