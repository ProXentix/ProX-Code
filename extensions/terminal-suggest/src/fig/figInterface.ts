...
export interface IFigSpecSuggestionsResult {
	...
	items: ProX-Code.TerminalCompletionItem[];
}
	...
	for (const spec of specs) {
		...
		for (const specLabel of specLabels) {
			...
			if (tokenType === TokenType.Command) {
				if (availableCommand.kind !== ProX-Code.TerminalCompletionItemKind.Alias) {
					...
					result.items.push(createCompletionItem(
						...
						prefix,
						{
							...
							kind: ProX-Code.TerminalCompletionItemKind.Method
						},
						description,
...
export async function collectCompletionItemResult(
	...
	const addSuggestions = async (specArgs: SpecArg[] | Record<string, SpecArg> | undefined, kind: ProX-Code.TerminalCompletionItemKind, parsedArguments?: ArgumentParserResult) => {
		if (kind === ProX-Code.TerminalCompletionItemKind.Argument && parsedArguments?.currentArg?.generators) {
		...
		const flagsToExclude = kind === ProX-Code.TerminalCompletionItemKind.Flag ? parsedArguments?.passedOptions.map(option => option.name).flat() : undefined;
		...
		function addItem(label: string, item: SpecArg) {
			...
			if (lastArgType === 'subcommand_arg') {
				if (typeof item === 'object' && 'args' in item && (asArray(item.args ?? [])).length > 0) {
					itemKind = ProX-Code.TerminalCompletionItemKind.Option;
				}
			}
			else if (lastArgType === 'option_arg') {
				itemKind = ProX-Code.TerminalCompletionItemKind.OptionValue;
			}
	...
	if (parsedArguments.suggestionFlags & SuggestionFlag.Args) {
		await addSuggestions(parsedArguments.currentArg?.suggestions, ProX-Code.TerminalCompletionItemKind.Argument, parsedArguments);
	}
	if (parsedArguments.suggestionFlags & SuggestionFlag.Subcommands) {
		await addSuggestions(parsedArguments.completionObj.subcommands, ProX-Code.TerminalCompletionItemKind.Method);
	}
	if (parsedArguments.suggestionFlags & SuggestionFlag.Options) {
		await addSuggestions(parsedArguments.completionObj.options, ProX-Code.TerminalCompletionItemKind.Flag, parsedArguments);
	}
