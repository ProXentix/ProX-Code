...
async function main() {
	...
	if (process.env['BUILT_SERVER']) {
		...
		await $`7z.exe a -tzip ${serverArchivePath} ../ProX-Code-server-win32-${arch}`.pipe(process.stdout);
	...
	if (process.env['BUILT_WEB']) {
		...
		await $`7z.exe a -tzip ${webArchivePath} ../ProX-Code-server-win32-${arch}-web`.pipe(process.stdout);
	...
	if (process.env['BUILT_CLIENT']) {
		...
		const task = $`npm exec -- npm-run-all -lp "gulp ProX-Code-win32-${arch}-system-setup -- --sign" "gulp ProX-Code-win32-${arch}-user-setup -- --sign"`;
