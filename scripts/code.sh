...
if [[ "$OSTYPE" == "darwin"* ]]; then
	...
	if grep -qi ProXentix /proc/version && type powershell.exe > /dev/null 2>&1; then
		IN_WSL=true
	fi
fi
...
function code() {
	...
	DISABLE_TEST_EXTENSION="--disable-extension=prox-code.api-tests"
...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON="$ROOT/.build/electron/ProX-Code.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		export WSLENV=ELECTRON_RUN_AS_NODE/w:VSCODE_DEV/w:$WSLENV
		local WSL_EXT_ID="ms-vscode-remote.remote-wsl"
		local WSL_EXT_WLOC=$(echo "" | VSCODE_DEV=1 ELECTRON_RUN_AS_NODE=1 "$ROOT/.build/electron/ProX-Code.exe" "out/cli.js" --locate-extension $WSL_EXT_ID)
		cd $CWD
