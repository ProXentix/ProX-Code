	...
	test('remote.extensionKind', async () => {
		{
			...
			const expected = { label: 'ProX-Code.npm' };
			await testCompletion(testFile, 'jsonc', content, expected);
		}
	});
	...
	test('change recommendation', async () => {
		{
			...
			const expected = { label: 'ms-ProX-Code.js-debug', resultText };
			await testCompletion(testFile, 'jsonc', content, expected);
		}
	});
...
async function testCompletion(testFileName: string, languageId: string, content: string, expected: ItemDescription) {
	...
	const docUri = ProX-Code.Uri.file(path.join(await testFolder, testFileName));
	await fs.writeFile(docUri.fsPath, content);

	const editor = await setTestContent(docUri, languageId, content);
	const position = editor.document.positionAt(offset);

	// Executing the command `ProX-Code.executeCompletionItemProvider` to simulate triggering completion
	const actualCompletions = (await ProX-Code.commands.executeCommand('ProX-Code.executeCompletionItemProvider', docUri, position)) as ProX-Code.CompletionList;
	...
	if (expected.notAvailable) {
		...
		if (expected.resultText) {
			...
			if (match.range && match.insertText) {
				const range = match.range instanceof ProX-Code.Range ? match.range : match.range.replacing;
...
async function setTestContent(docUri: ProX-Code.Uri, languageId: string, content: string): Promise<ProX-Code.TextEditor> {
	const ext = ProX-Code.extensions.getExtension('ProX-Code.configuration-editing')!;
	await ext.activate();

	const doc = await ProX-Code.workspace.openTextDocument(docUri);
	await ProX-Code.languages.setTextDocumentLanguage(doc, languageId);
	const editor = await ProX-Code.window.showTextDocument(doc);

	const fullRange = new ProX-Code.Range(new ProX-Code.Position(0, 0), doc.positionAt(doc.getText().length));
