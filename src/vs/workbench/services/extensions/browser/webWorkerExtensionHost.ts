...
export class WebWorkerExtensionHost extends Disposable implements IExtensionHost {
	...
	private async _startInsideIframe(): Promise<IMessagePassingProtocol> {
		...
		const ProX-CodeWebWorkerExtHostId = generateUuid();
		iframe.setAttribute('src', `${webWorkerExtensionHostIframeSrc}&ProX-CodeWebWorkerExtHostId=${ProX-CodeWebWorkerExtHostId}`);
		...
		this._register(dom.addDisposableListener(mainWindow, 'message', (event) => {
			...
			if (event.data.ProX-CodeWebWorkerExtHostId !== ProX-CodeWebWorkerExtHostId) {
				return;
			}
			if (event.data.error) {
			...
			if (event.data.type === 'ProX-Code.bootstrap.nls') {
		...
		iframe.contentWindow!.postMessage({ type: 'ProX-Code.init', data: messagePorts }, '*', [...messagePorts.values()]);
