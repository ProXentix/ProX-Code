...
try {
    tag = child_process_1.default
        ...
        .toString()
        .trim();
    const dtsUri = `https://raw.githubusercontent.com/microsoft/ProX-Code/${tag}/src/ProX-Code-dts/ProX-Code.d.ts`;
    const outPath = path_1.default.resolve(process.cwd(), 'DefinitelyTyped/types/ProX-Code/index.d.ts');
    child_process_1.default.execSync(`curl ${dtsUri} --output ${outPath}`);
    updateDTSFile(outPath, tag);
    console.log(`Done updating ProX-Code.d.ts at ${outPath}`);
}
catch (err) {
    console.error(err);
...
function getNewFileHeader(tag) {
    ...
    const header = [
        ...
        ` *  See https://github.com/microsoft/ProX-Code/blob/main/LICENSE.txt for license information.`,
        ` *--------------------------------------------------------------------------------------------*/`,
        ``,
        `/**`,
