...
const resourceCacheName = `ProX-Code-resource-cache-${VERSION}`;
 ...
 */
const resourceBaseAuthority = searchParams.get('ProX-Code-resource-base-authority');
...
sw.addEventListener('fetch', (event: FetchEvent) => {
	...
	// through ProX Code itself so that we are authenticated properly.  If the
...
async function processResourceRequest(
	...
	const resolveResourceEntry = (
		...
		const coiRequest = new URL(event.request.url).searchParams.get('ProX-Code-coi');
		if (coiRequest === '3') {
...
function getWebviewIdForClient(client: Client): string | null {
	// Refs https://github.com/microsoft/ProX-Code/issues/244143
