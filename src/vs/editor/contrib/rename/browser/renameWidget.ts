...
export class RenameWidget implements IRenameWidget, IContentWidget, IDisposable {
	...
	afterRender(position: ContentWidgetPositionPreference | null): void {
		// FIXME@ulugbekna: commenting trace log out until we start unmounting the widget from editor properly - https://github.com/microsoft/ProX-Code/issues/226975
	...
	get domNode() {
		if (!this._domNode) {
			...
			this._disposables.add(dom.addDisposableListener(this.input, dom.EventType.FOCUS, () => {
				...
				this.domNode.style.outlineColor = 'var(--ProX-Code-focusBorder)';
			}));
