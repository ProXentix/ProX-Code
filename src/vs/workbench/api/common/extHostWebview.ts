...
export class ExtHostWebview implements ProX-Code.Webview {
	...
	#html: string = '';
	#options: ProX-Code.WebviewOptions;
	...
	public asWebviewUri(resource: ProX-Code.Uri): ProX-Code.Uri {
	...
	public set html(value: string) {
		...
		if (this.#html !== value) {
			this.#html = value;
			if (this.#shouldRewriteOldResourceUris && !this.#hasCalledAsWebviewUri && /(["'])ProX-Code-resource:([^\s'"]+?)(["'])/i.test(value)) {
				this.#hasCalledAsWebviewUri = true;
				this.#deprecationService.report('Webview ProX-Code-resource: uris', this.#extension,
					`Please migrate to use the 'webview.asWebviewUri' api instead: https://aka.ms/ProX-Code-webview-use-aswebviewuri`);
			}
			this.#proxy.$setHtml(this.#handle, this.rewriteOldResourceUrlsIfNeeded(value));
		}
	}

	public get options(): ProX-Code.WebviewOptions {
		this.assertNotDisposed();
		return this.#options;
	}

	public set options(newOptions: ProX-Code.WebviewOptions) {
		this.assertNotDisposed();
	...
	private rewriteOldResourceUrlsIfNeeded(value: string): string {
		...
		return value
			.replace(/(["'])(?:ProX-Code-resource):(\/\/([^\s\/'"]+?)(?=\/))?([^\s'"]+?)(["'])/gi, (_match, startQuote, _1, scheme, path, endQuote) => {
				const uri = URI.from({
					scheme: scheme || 'file',
			...
			.replace(/(["'])(?:ProX-Code-webview-resource):(\/\/[^\s\/'"]+\/([^\s\/'"]+?)(?=\/))?([^\s'"]+?)(["'])/gi, (_match, startQuote, _1, scheme, path, endQuote) => {
				const uri = URI.from({
					scheme: scheme || 'file',
					path: decodeURIComponent(path),
				});
...
export class ExtHostWebviews extends Disposable implements extHostProtocol.ExtHostWebviewsShape {
	...
	public $onMissingCsp(
		...
		this._logService.warn(`${extensionId} created a webview without a content security policy: https://aka.ms/ProX-Code-webview-missing-csp`);
	}
...
function reviveOptions(options: extHostProtocol.IWebviewContentOptions): ProX-Code.WebviewOptions {
	return {
		enableCommandUris: options.enableCommandUris,
		enableScripts: options.enableScripts,
		enableForms: options.enableForms,
