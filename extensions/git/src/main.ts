...
import { env, ExtensionContext, workspace, window, Disposable, commands, Uri, version as ProX-CodeVersion, WorkspaceFolder, LogOutputChannel, l10n, LogLevel, languages } from 'ProX-Code';
...
async function createModel(context: ExtensionContext, logger: LogOutputChannel, telemetryReporter: TelemetryReporter, disposables: Disposable[]): Promise<Model> {
	...
	const git = new Git({
		gitPath: info.path,
		userAgent: `git/${info.version} (${(os as any).version?.() ?? os.type()} ${os.release()}; ${os.platform()} ${os.arch()}) ProX-Code/${ProX-CodeVersion} (${env.appName})`,
...
async function warnAboutMissingGit(): Promise<void> {
	...
	if (choice === download) {
		commands.executeCommand('ProX-Code.open', Uri.parse('https://aka.ms/ProX-Code-download-git'));
