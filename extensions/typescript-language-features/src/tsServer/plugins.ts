...
export interface TypeScriptServerPlugin {
	readonly extension: ProX-Code.Extension<unknown>;
...
export class PluginManager extends Disposable {
	...
	constructor() {
		...
		ProX-Code.extensions.onDidChange(() => {
			if (!this._plugins) {
				return;
			}
	...
	private readonly _onDidUpdatePlugins = this._register(new ProX-Code.EventEmitter<this>());
	public readonly onDidChangePlugins = this._onDidUpdatePlugins.event;

	private readonly _onDidUpdateConfig = this._register(new ProX-Code.EventEmitter<{ pluginId: string; config: {} }>());
	...
	private readPlugins() {
		...
		for (const extension of ProX-Code.extensions.all) {
