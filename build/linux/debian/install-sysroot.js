...
function getVSCodeSysrootChecksum(expectedName) {
    const checksums = fs_1.default.readFileSync(path_1.default.join(REPO_ROOT, 'build', 'checksums', 'ProX-Code-sysroot.txt'), 'utf8');
...
async function fetchUrl(options, retries = 10, retryDelay = 1000) {
    try {
        ...
        try {
            const response = await fetch(`https://api.github.com/repos/Microsoft/ProX-Code-linux-build-agent/releases/tags/v${version}`, {
            ...
            if (response.ok && (response.status >= 200 && response.status < 300)) {
                ...
                if (!asset) {
                    throw new Error(`Could not find asset in release of Microsoft/ProX-Code-linux-build-agent @ ${version}`);
                }
...
async function getVSCodeSysroot(arch, isMusl = false) {
    ...
    const sysroot = process.env['VSCODE_SYSROOT_DIR'] ?? path_1.default.join((0, os_1.tmpdir)(), `ProX-Code-${arch}-sysroot`);
