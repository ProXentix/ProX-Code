...
function getOutputChannel(): ProX-Code.OutputChannel {
	if (!_channel) {
		_channel = ProX-Code.window.createOutputChannel('Gulp Auto Detection');
	}
	return _channel;
}

function showError() {
	ProX-Code.window.showWarningMessage(ProX-Code.l10n.t("Problem finding gulp tasks. See the output for more information."),
		ProX-Code.l10n.t("Go to output")).then((choice) => {
			if (choice !== undefined) {
				_channel.show(true);
			}
		});
...
interface GulpTaskDefinition extends ProX-Code.TaskDefinition {
	task: string;
	file?: string;
}
...
class FolderDetector {
	...
	public isEnabled(): boolean {
		return ProX-Code.workspace.getConfiguration('gulp', this._workspaceFolder.uri).get<AutoDetect>('autoDetect') === 'on';
	}
	...
	public async getTask(_task: ProX-Code.Task): Promise<ProX-Code.Task | undefined> {
		...
		if (gulpTask) {
			...
			const task = new ProX-Code.Task(kind, this.workspaceFolder, gulpTask, 'gulp', new ProX-Code.ShellExecution(await this._gulpCommand, [gulpTask], options));
			return task;
		}
		return undefined;
	}
		...
		try {
			...
			if (stdout) {
				...
				for (const line of lines) {
					...
					const options: ProX-Code.ShellExecutionOptions = { cwd: this.workspaceFolder.uri.fsPath };
					const task = new ProX-Code.Task(kind, this.workspaceFolder, line, 'gulp', new ProX-Code.ShellExecution(await this._gulpCommand, [line], options));
					result.push(task);
			...
			channel.appendLine(ProX-Code.l10n.t("Auto detecting gulp for folder {0} failed with error: {1}', this.workspaceFolder.name, err.error ? err.error.toString() : 'unknown"));
			showError();
			return emptyTasks;
		}
	}
...
class TaskDetector {
	...
	private updateProvider(): void {
		if (!this.taskProvider && this.detectors.size > 0) {
			...
			this.taskProvider = ProX-Code.tasks.registerTaskProvider('gulp', {
