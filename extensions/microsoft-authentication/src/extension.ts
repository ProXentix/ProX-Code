...
import { commands, env, ExtensionContext, l10n, window, workspace } from 'ProX-Code';
...
import { IExperimentationService } from 'ProX-Code-tas-client';
...
function shouldUseMsal(expService: IExperimentationService): boolean {
	...
	const expValue = expService.getTreatmentVariable<boolean>('ProX-Code', 'microsoft.useMsal');
...
export async function activate(context: ExtensionContext) {
	...
	const expService = await createExperimentationService(
		...
		env.uriScheme !== 'ProX-Code', // isPreRelease
	);
