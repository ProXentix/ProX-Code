...
export class TerminalNativeContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		ipcRenderer.on('ProX-Code:openFiles', (_: unknown, request: INativeOpenFileRequest) => { this._onOpenFileRequest(request); });
	...
	private async _onOpenFileRequest(request: INativeOpenFileRequest): Promise<void> {
		...
		if (request.termProgram === 'ProX-Code' && request.filesToWait) {
