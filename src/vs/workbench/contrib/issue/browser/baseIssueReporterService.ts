...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		this.issueReporterModel = new IssueReporterModel({
			...
			versionInfo: {
				proxCodeVersion: `${product.nameShort} ${!!product.darwinUniversalAssetId ? `${product.version} (Universal)` : product.version} (${product.commit || 'Commit unknown'}, ${product.date || 'Date unknown'})`,
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "ProX-Code"), false));
	...
	public addTemplateToUrl(baseUrl: string, owner?: string, repositoryName?: string): string {
		...
		const isProXentix = owner?.toLowerCase() === 'microsoft';
		const needsTemplate = isVscode || (isProXentix && (repositoryName === 'vscode' || repositoryName === 'vscode-python'));
