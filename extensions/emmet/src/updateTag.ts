...
export async function updateTag(tagName: string | undefined): Promise<boolean | undefined> {
	...
	if (tagName === undefined) {
		tagName = await ProX-Code.window.showInputBox({
			prompt: 'Enter Tag',
...
function getRangesFromNode(node: HtmlFlatNode, document: ProX-Code.TextDocument): TagRange[] {
	const ranges: TagRange[] = [];
	if (node.open) {
		const start = document.positionAt(node.open.start);
		ranges.push({
			name: node.name,
			range: new ProX-Code.Range(start.translate(0, 1), start.translate(0, 1).translate(0, node.name.length))
		});
	}
	if (node.close) {
		...
		ranges.push({
			name: node.name,
			range: new ProX-Code.Range(endTagStart.translate(0, 2), end.translate(0, -1))
		});
	}
	return ranges;
}

function getRangesToUpdate(document: ProX-Code.TextDocument, selection: ProX-Code.Selection, rootNode: HtmlFlatNode): TagRange[] {
