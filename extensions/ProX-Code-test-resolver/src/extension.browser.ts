...
export function activate(_context: ProX-Code.ExtensionContext) {
	ProX-Code.workspace.registerRemoteAuthorityResolver('test', {
		async resolve(_authority: string): Promise<ProX-Code.ResolverResult> {
			console.log(`Resolving ${_authority}`);
			console.log(`Activating ProX-Code.github-authentication to simulate auth`);
			await ProX-Code.extensions.getExtension('ProX-Code.github-authentication')?.activate();
			return new ProX-Code.ManagedResolvedAuthority(async () => {
 ...
 * actual WebSocket.
 */
class InitialManagedMessagePassing implements ProX-Code.ManagedMessagePassing {
	private readonly dataEmitter = new ProX-Code.EventEmitter<Uint8Array>();
	private readonly closeEmitter = new ProX-Code.EventEmitter<Error | undefined>();
	private readonly endEmitter = new ProX-Code.EventEmitter<void>();
...
class OpeningManagedMessagePassing {
	...
	constructor(
		url: URL,
		dataEmitter: ProX-Code.EventEmitter<Uint8Array>,
		closeEmitter: ProX-Code.EventEmitter<Error | undefined>,
		_endEmitter: ProX-Code.EventEmitter<void>
	) {
