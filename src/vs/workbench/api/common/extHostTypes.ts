...
export class Position {
	...
	static of(obj: ProX-Code.Position): Position {
...
export class Range {
	...
	static isRange(thing: any): thing is ProX-Code.Range {
	...
	static of(obj: ProX-Code.Range): Range {
	...
	constructor(start: ProX-Code.Position, end: ProX-Code.Position);
	constructor(start: Position, end: Position);
	constructor(startLine: number, startColumn: number, endLine: number, endColumn: number);
	constructor(startLineOrStart: number | Position | ProX-Code.Position, startColumnOrEnd: number | Position | ProX-Code.Position, endLine?: number, endColumn?: number) {
...
export function getDebugDescriptionOfRange(range: ProX-Code.Range): string {
	return range.isEmpty
		? `[${range.start.line}:${range.start.character})`
		: `[${range.start.line}:${range.start.character} -> ${range.end.line}:${range.end.character})`;
}

export function getDebugDescriptionOfSelection(selection: ProX-Code.Selection): string {
...
export class ManagedResolvedAuthority {
	...
	constructor(public readonly makeConnection: () => Thenable<ProX-Code.ManagedMessagePassing>, public readonly connectionToken?: string) {
		if (typeof connectionToken !== 'undefined') {
			validateConnectionToken(connectionToken);
		}
	}
...
@es5ClassCompat
export class NotebookEdit implements ProX-Code.NotebookEdit {
	...
	static insertCells(index: number, newCells: ProX-Code.NotebookCellData[]): ProX-Code.NotebookEdit {
...
export class SnippetTextEdit implements ProX-Code.SnippetTextEdit {
...
export interface IFileOperationOptions {
	...
	readonly contents?: Uint8Array | ProX-Code.DataTransferFile;
}
...
export interface IFileOperation {
	...
	readonly metadata?: ProX-Code.WorkspaceEditEntryMetadata;
}
...
export interface IFileSnippetTextEdit {
	...
	readonly uri: URI;
	readonly range: ProX-Code.Range;
	readonly edit: ProX-Code.SnippetString;
	readonly metadata?: ProX-Code.WorkspaceEditEntryMetadata;
...
export interface ICellEdit {
	...
	readonly cells: ProX-Code.NotebookCellData[];
}
...
@es5ClassCompat
export class WorkspaceEdit implements ProX-Code.WorkspaceEdit {
	...
	renameFile(from: ProX-Code.Uri, to: ProX-Code.Uri, options?: { readonly overwrite?: boolean; readonly ignoreIfExists?: boolean }, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from, to, options, metadata });
	}

	createFile(uri: ProX-Code.Uri, options?: { readonly overwrite?: boolean; readonly ignoreIfExists?: boolean; readonly contents?: Uint8Array | ProX-Code.DataTransferFile }, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from: undefined, to: uri, options, metadata });
	}

	deleteFile(uri: ProX-Code.Uri, options?: { readonly recursive?: boolean; readonly ignoreIfNotExists?: boolean }, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.File, from: uri, to: undefined, options, metadata });
	}

	// --- notebook

	private replaceNotebookMetadata(uri: URI, value: Record<string, any>, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Cell, metadata, uri, edit: { editType: CellEditType.DocumentMetadata, metadata: value } });
	}

	private replaceNotebookCells(uri: URI, startOrRange: ProX-Code.NotebookRange, cellData: ProX-Code.NotebookCellData[], metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
	...
	private replaceNotebookCellMetadata(uri: URI, index: number, cellMetadata: Record<string, any>, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Cell, metadata, uri, edit: { editType: CellEditType.Metadata, index, metadata: cellMetadata } });
	}

	// --- text

	replace(uri: URI, range: Range, newText: string, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this._edits.push({ _type: FileEditType.Text, uri, edit: new TextEdit(range, newText), metadata });
	}

	insert(resource: URI, position: Position, newText: string, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
		this.replace(resource, new Range(position, position), newText, metadata);
	}

	delete(resource: URI, range: Range, metadata?: ProX-Code.WorkspaceEditEntryMetadata): void {
	...
	set(uri: URI, edits: ReadonlyArray<[TextEdit | SnippetTextEdit, ProX-Code.WorkspaceEditEntryMetadata | undefined]>): void;
	set(uri: URI, edits: readonly NotebookEdit[]): void;
	set(uri: URI, edits: ReadonlyArray<[NotebookEdit, ProX-Code.WorkspaceEditEntryMetadata | undefined]>): void;

	set(uri: URI, edits: null | undefined | ReadonlyArray<TextEdit | SnippetTextEdit | NotebookEdit | [NotebookEdit, ProX-Code.WorkspaceEditEntryMetadata | undefined] | [TextEdit | SnippetTextEdit, ProX-Code.WorkspaceEditEntryMetadata | undefined]>): void {
		if (!edits) {
			...
			for (const editOrTuple of edits) {
				...
				let metadata: ProX-Code.WorkspaceEditEntryMetadata | undefined;
...
export class Location {
	...
	static isLocation(thing: any): thing is ProX-Code.Location {
...
export class Hover {
	...
	public contents: (ProX-Code.MarkdownString | ProX-Code.MarkedString)[];
	public range: Range | undefined;

	constructor(
		contents: ProX-Code.MarkdownString | ProX-Code.MarkedString | (ProX-Code.MarkdownString | ProX-Code.MarkedString)[],
		range?: Range
	) {
		if (!contents) {
...
export class CallHierarchyIncomingCall {
	...
	from: ProX-Code.CallHierarchyItem;
	fromRanges: ProX-Code.Range[];

	constructor(item: ProX-Code.CallHierarchyItem, fromRanges: ProX-Code.Range[]) {
...
export class CallHierarchyOutgoingCall {
	...
	to: ProX-Code.CallHierarchyItem;
	fromRanges: ProX-Code.Range[];
...
export class CodeLens {
	...
	command: ProX-Code.Command | undefined;

	constructor(range: Range, command?: ProX-Code.Command) {
		this.range = range;
...
@es5ClassCompat
export class MarkdownString implements ProX-Code.MarkdownString {

	readonly #delegate: BaseMarkdownString;

	static isMarkdownString(thing: any): thing is ProX-Code.MarkdownString {
	...
	get baseUri(): ProX-Code.Uri | undefined {
		return this.#delegate.baseUri;
	}

	set baseUri(value: ProX-Code.Uri | undefined) {
		this.#delegate.baseUri = value;
	}

	appendText(value: string): ProX-Code.MarkdownString {
		this.#delegate.appendText(value);
		return this;
	}

	appendMarkdown(value: string): ProX-Code.MarkdownString {
		this.#delegate.appendMarkdown(value);
		return this;
	}

	appendCodeblock(value: string, language?: string): ProX-Code.MarkdownString {
...
export class ParameterInformation {
	...
	documentation?: string | ProX-Code.MarkdownString;

	constructor(label: string | [number, number], documentation?: string | ProX-Code.MarkdownString) {
		this.label = label;
...
export class SignatureInformation {
	...
	constructor(label: string, documentation?: string | ProX-Code.MarkdownString) {
		this.label = label;
		this.documentation = documentation;
		this.parameters = [];
	}
...
export class InlayHintLabelPart {
	...
	value: string;
	tooltip?: string | ProX-Code.MarkdownString;
	location?: Location;
	command?: ProX-Code.Command;
...
@es5ClassCompat
export class InlayHint implements ProX-Code.InlayHint {

	label: string | InlayHintLabelPart[];
	tooltip?: string | ProX-Code.MarkdownString;
	position: Position;
	textEdits?: TextEdit[];
	kind?: ProX-Code.InlayHintKind;
	paddingLeft?: boolean;
	paddingRight?: boolean;

	constructor(position: Position, label: string | InlayHintLabelPart[], kind?: ProX-Code.InlayHintKind) {
...
@es5ClassCompat
export class CompletionItem implements ProX-Code.CompletionItem {
...
export class CompletionList {
	...
	items: ProX-Code.CompletionItem[];

	constructor(items: ProX-Code.CompletionItem[] = [], isIncomplete: boolean = false) {
		this.items = items;
...
@es5ClassCompat
export class InlineSuggestion implements ProX-Code.InlineCompletionItem {
	...
	constructor(insertText: string, range?: Range, command?: ProX-Code.Command) {
...
@es5ClassCompat
export class InlineSuggestionList implements ProX-Code.InlineCompletionList {
	items: ProX-Code.InlineCompletionItem[];

	commands: ProX-Code.Command[] | undefined = undefined;

	suppressSuggestions: boolean | undefined = undefined;

	constructor(items: ProX-Code.InlineCompletionItem[]) {
		this.items = items;
	}
}
...
export class TerminalLink implements ProX-Code.TerminalLink {
	constructor(
...
export class TerminalProfile implements ProX-Code.TerminalProfile {
	constructor(
		public options: ProX-Code.TerminalOptions | ProX-Code.ExtensionTerminalOptions
	) {
...
export class TerminalCompletionItem implements ProX-Code.TerminalCompletionItem {
	label: string | CompletionItemLabel;
	icon?: ThemeIcon | undefined;
	detail?: string | undefined;
	documentation?: string | ProX-Code.MarkdownString | undefined;
	...
	constructor(label: string | CompletionItemLabel, icon?: ThemeIcon, detail?: string, documentation?: string | ProX-Code.MarkdownString, isFile?: boolean, isDirectory?: boolean, isKeyword?: boolean, replacementIndex?: number, replacementLength?: number) {
		this.label = label;
		this.icon = icon;
		this.detail = detail;
		this.documentation = documentation;
...
@es5ClassCompat
export class TaskGroup implements ProX-Code.TaskGroup {
...
@es5ClassCompat
export class ProcessExecution implements ProX-Code.ProcessExecution {

	private _process: string;
	private _args: string[];
	private _options: ProX-Code.ProcessExecutionOptions | undefined;

	constructor(process: string, options?: ProX-Code.ProcessExecutionOptions);
	constructor(process: string, args: string[], options?: ProX-Code.ProcessExecutionOptions);
	constructor(process: string, varg1?: string[] | ProX-Code.ProcessExecutionOptions, varg2?: ProX-Code.ProcessExecutionOptions) {
	...
	get options(): ProX-Code.ProcessExecutionOptions | undefined {
		return this._options;
	}

	set options(value: ProX-Code.ProcessExecutionOptions | undefined) {
		this._options = value;
	}
...
@es5ClassCompat
export class ShellExecution implements ProX-Code.ShellExecution {

	private _commandLine: string | undefined;
	private _command: string | ProX-Code.ShellQuotedString | undefined;
	private _args: (string | ProX-Code.ShellQuotedString)[] = [];
	private _options: ProX-Code.ShellExecutionOptions | undefined;

	constructor(commandLine: string, options?: ProX-Code.ShellExecutionOptions);
	constructor(command: string | ProX-Code.ShellQuotedString, args: (string | ProX-Code.ShellQuotedString)[], options?: ProX-Code.ShellExecutionOptions);
	constructor(arg0: string | ProX-Code.ShellQuotedString, arg1?: ProX-Code.ShellExecutionOptions | (string | ProX-Code.ShellQuotedString)[], arg2?: ProX-Code.ShellExecutionOptions) {
	...
	get command(): string | ProX-Code.ShellQuotedString {
		return this._command ? this._command : '';
	}

	set command(value: string | ProX-Code.ShellQuotedString) {
	...
	get args(): (string | ProX-Code.ShellQuotedString)[] {
		return this._args;
	}

	set args(value: (string | ProX-Code.ShellQuotedString)[] | undefined) {
		this._args = value || [];
	}

	get options(): ProX-Code.ShellExecutionOptions | undefined {
		return this._options;
	}

	set options(value: ProX-Code.ShellExecutionOptions | undefined) {
		this._options = value;
	}
...
export class CustomExecution implements ProX-Code.CustomExecution {
	private _callback: (resolvedDefinition: ProX-Code.TaskDefinition) => Thenable<ProX-Code.Pseudoterminal>;
	constructor(callback: (resolvedDefinition: ProX-Code.TaskDefinition) => Thenable<ProX-Code.Pseudoterminal>) {
	...
	public set callback(value: (resolvedDefinition: ProX-Code.TaskDefinition) => Thenable<ProX-Code.Pseudoterminal>) {
		this._callback = value;
	}

	public get callback(): ((resolvedDefinition: ProX-Code.TaskDefinition) => Thenable<ProX-Code.Pseudoterminal>) {
		return this._callback;
	}
}

@es5ClassCompat
export class Task implements ProX-Code.Task {
	...
	private _definition: ProX-Code.TaskDefinition;
	private _scope: ProX-Code.TaskScope.Global | ProX-Code.TaskScope.Workspace | ProX-Code.WorkspaceFolder | undefined;
	...
	private _presentationOptions: ProX-Code.TaskPresentationOptions;
	private _runOptions: ProX-Code.RunOptions;
	private _detail: string | undefined;

	constructor(definition: ProX-Code.TaskDefinition, name: string, source: string, execution?: ProcessExecution | ShellExecution | CustomExecution, problemMatchers?: string | string[]);
	constructor(definition: ProX-Code.TaskDefinition, scope: ProX-Code.TaskScope.Global | ProX-Code.TaskScope.Workspace | ProX-Code.WorkspaceFolder, name: string, source: string, execution?: ProcessExecution | ShellExecution | CustomExecution, problemMatchers?: string | string[]);
	constructor(definition: ProX-Code.TaskDefinition, arg2: string | (ProX-Code.TaskScope.Global | ProX-Code.TaskScope.Workspace) | ProX-Code.WorkspaceFolder, arg3: any, arg4?: any, arg5?: any, arg6?: any) {
	...
	get definition(): ProX-Code.TaskDefinition {
		return this._definition;
	}

	set definition(value: ProX-Code.TaskDefinition) {
	...
	get scope(): ProX-Code.TaskScope.Global | ProX-Code.TaskScope.Workspace | ProX-Code.WorkspaceFolder | undefined {
		return this._scope;
	}

	set target(value: ProX-Code.TaskScope.Global | ProX-Code.TaskScope.Workspace | ProX-Code.WorkspaceFolder) {
		this.clear();
		this._scope = value;
	}
	...
	get presentationOptions(): ProX-Code.TaskPresentationOptions {
		return this._presentationOptions;
	}

	set presentationOptions(value: ProX-Code.TaskPresentationOptions) {
	...
	get runOptions(): ProX-Code.RunOptions {
		return this._runOptions;
	}

	set runOptions(value: ProX-Code.RunOptions) {
		if (value === null || value === undefined) {
			value = Object.create(null);
		}
		this.clear();
...
export namespace ViewBadge {
	export function isViewBadge(thing: any): thing is ProX-Code.ViewBadge {
		const viewBadgeThing = thing as ProX-Code.ViewBadge;
...
export class TreeItem {
	...
	label?: string | ProX-Code.TreeItemLabel;
	resourceUri?: URI;
	iconPath?: string | URI | { light: string | URI; dark: string | URI } | ThemeIcon;
	command?: ProX-Code.Command;
	contextValue?: string;
	tooltip?: string | ProX-Code.MarkdownString;
	checkboxState?: ProX-Code.TreeItemCheckboxState;

	static isTreeItem(thing: any, extension: IExtensionDescription): thing is TreeItem {
		const treeItemThing = thing as ProX-Code.TreeItem;
		...
		if ((treeItemThing.iconPath !== undefined) && !isString(treeItemThing.iconPath) && !URI.isUri(treeItemThing.iconPath) && (!treeItemThing.iconPath || !isString((treeItemThing.iconPath as ProX-Code.ThemeIcon).id))) {
	...
	constructor(label: string | ProX-Code.TreeItemLabel, collapsibleState?: ProX-Code.TreeItemCollapsibleState);
	constructor(resourceUri: URI, collapsibleState?: ProX-Code.TreeItemCollapsibleState);
	constructor(arg1: string | ProX-Code.TreeItemLabel | URI, public collapsibleState: ProX-Code.TreeItemCollapsibleState = TreeItemCollapsibleState.None) {
		if (URI.isUri(arg1)) {
...
@es5ClassCompat
export class DataTransferItem implements ProX-Code.DataTransferItem {

	async asString(): Promise<string> {
		return typeof this.value === 'string' ? this.value : JSON.stringify(this.value);
	}

	asFile(): undefined | ProX-Code.DataTransferFile {
		return undefined;
	}
...
export class InternalFileDataTransferItem extends InternalDataTransferItem {
	...
	readonly #file: ProX-Code.DataTransferFile;

	constructor(file: ProX-Code.DataTransferFile) {
		super('');
		this.#file = file;
	}
 ...
 * Intentionally not exported to extensions
 */
export class DataTransferFile implements ProX-Code.DataTransferFile {

	public readonly name: string;
	public readonly uri: ProX-Code.Uri | undefined;

	public readonly _itemId: string;
	private readonly _getData: () => Promise<Uint8Array>;

	constructor(name: string, uri: ProX-Code.Uri | undefined, itemId: string, getData: () => Promise<Uint8Array>) {
		this.name = name;
		this.uri = uri;
		this._itemId = itemId;
...
@es5ClassCompat
export class DataTransfer implements ProX-Code.DataTransfer {
	#items = new Map<string, ProX-Code.DataTransferItem[]>();

	constructor(init?: Iterable<readonly [string, ProX-Code.DataTransferItem]>) {
	...
	get(mimeType: string): ProX-Code.DataTransferItem | undefined {
		return this.#items.get(this.#normalizeMime(mimeType))?.[0];
	}

	set(mimeType: string, value: ProX-Code.DataTransferItem): void {
	...
	forEach(callbackfn: (value: ProX-Code.DataTransferItem, key: string, dataTransfer: DataTransfer) => void, thisArg?: unknown): void {
	...
	*[Symbol.iterator](): IterableIterator<[mimeType: string, item: ProX-Code.DataTransferItem]> {
		for (const [mime, items] of this.#items) {
			for (const item of items) {
				yield [mime, item];
			}
...
export class RelativePattern implements IRelativePattern {
	...
	constructor(base: ProX-Code.WorkspaceFolder | URI | string, pattern: string) {
...
@es5ClassCompat
export class DebugAdapterExecutable implements ProX-Code.DebugAdapterExecutable {
	readonly command: string;
	readonly args: string[];
	readonly options?: ProX-Code.DebugAdapterExecutableOptions;

	constructor(command: string, args: string[], options?: ProX-Code.DebugAdapterExecutableOptions) {
		this.command = command;
		this.args = args || [];
...
@es5ClassCompat
export class DebugAdapterServer implements ProX-Code.DebugAdapterServer {
	readonly port: number;
	readonly host?: string;
...
@es5ClassCompat
export class DebugAdapterNamedPipeServer implements ProX-Code.DebugAdapterNamedPipeServer {
	constructor(public readonly path: string) {
	}
}

@es5ClassCompat
export class DebugAdapterInlineImplementation implements ProX-Code.DebugAdapterInlineImplementation {
	readonly implementation: ProX-Code.DebugAdapter;

	constructor(impl: ProX-Code.DebugAdapter) {
		this.implementation = impl;
	}
}


export class DebugStackFrame implements ProX-Code.DebugStackFrame {
	constructor(
		public readonly session: ProX-Code.DebugSession,
		readonly threadId: number,
		readonly frameId: number) { }
}

export class DebugThread implements ProX-Code.DebugThread {
	constructor(
		public readonly session: ProX-Code.DebugSession,
		readonly threadId: number) { }
}
...
@es5ClassCompat
export class EvaluatableExpression implements ProX-Code.EvaluatableExpression {
	readonly range: ProX-Code.Range;
	readonly expression?: string;

	constructor(range: ProX-Code.Range, expression?: string) {
		this.range = range;
...
@es5ClassCompat
export class InlineValueText implements ProX-Code.InlineValueText {
...
@es5ClassCompat
export class InlineValueVariableLookup implements ProX-Code.InlineValueVariableLookup {
	readonly range: Range;
...
@es5ClassCompat
export class InlineValueEvaluatableExpression implements ProX-Code.InlineValueEvaluatableExpression {
	readonly range: Range;
	readonly expression?: string;
...
@es5ClassCompat
export class InlineValueContext implements ProX-Code.InlineValueContext {

	readonly frameId: number;
	readonly stoppedLocation: ProX-Code.Range;

	constructor(frameId: number, range: ProX-Code.Range) {
...
export class NewSymbolName implements ProX-Code.NewSymbolName {
	readonly newSymbolName: string;
	readonly tags?: readonly ProX-Code.NewSymbolNameTag[] | undefined;
...
export class SemanticTokensBuilder {
	...
	constructor(legend?: ProX-Code.SemanticTokensLegend) {
		this._prevLine = 0;
		this._prevChar = 0;
	...
	private _push(range: ProX-Code.Range, tokenType: string, tokenModifiers?: string[]): void {
		if (!this._hasLegend) {
...
export class DebugVisualization {
	...
	visualization?: ProX-Code.Command | ProX-Code.TreeDataProvider<unknown>;
...
export class QuickInputButtons {
	...
	static readonly Back: ProX-Code.QuickInputButton = { iconPath: new ThemeIcon('arrow-left') };
...
export class FileDecoration {
	...
	badge?: string | ProX-Code.ThemeIcon;
	tooltip?: string;
	color?: ProX-Code.ThemeColor;
	propagate?: boolean;
...
@es5ClassCompat
export class ColorTheme implements ProX-Code.ColorTheme {
...
export class NotebookRange {
	static isNotebookRange(thing: any): thing is ProX-Code.NotebookRange {
...
export class NotebookCellData {
	...
	static isNotebookCellDataArray(value: unknown): value is ProX-Code.NotebookCellData[] {
		return Array.isArray(value) && (<unknown[]>value).every(elem => NotebookCellData.isNotebookCellData(elem));
	}

	static isNotebookCellData(value: unknown): value is ProX-Code.NotebookCellData {
	...
	languageId: string;
	mime?: string;
	outputs?: ProX-Code.NotebookCellOutput[];
	metadata?: Record<string, any>;
	executionSummary?: ProX-Code.NotebookCellExecutionSummary;

	constructor(kind: NotebookCellKind, value: string, languageId: string, mime?: string, outputs?: ProX-Code.NotebookCellOutput[], metadata?: Record<string, any>, executionSummary?: ProX-Code.NotebookCellExecutionSummary) {
		this.kind = kind;
		this.value = value;
...
export class NotebookCellOutputItem {
	...
	static isNotebookCellOutputItem(obj: unknown): obj is ProX-Code.NotebookCellOutputItem {
		...
		return typeof (<ProX-Code.NotebookCellOutputItem>obj).mime === 'string'
			&& (<ProX-Code.NotebookCellOutputItem>obj).data instanceof Uint8Array;
	}
...
export class NotebookCellOutput {
	...
	static isNotebookCellOutput(candidate: any): candidate is ProX-Code.NotebookCellOutput {
...
@es5ClassCompat
export class TimelineItem implements ProX-Code.TimelineItem {
...
export class TestRunProfileBase {
	constructor(
		...
		public readonly kind: ProX-Code.TestRunProfileKind,
	) { }
}

@es5ClassCompat
export class TestRunRequest implements ProX-Code.TestRunRequest {
	constructor(
		public readonly include: ProX-Code.TestItem[] | undefined = undefined,
		public readonly exclude: ProX-Code.TestItem[] | undefined = undefined,
		public readonly profile: ProX-Code.TestRunProfile | undefined = undefined,
...
@es5ClassCompat
export class TestMessage implements ProX-Code.TestMessage {
	public expectedOutput?: string;
	public actualOutput?: string;
	public location?: ProX-Code.Location;
	public contextValue?: string;

	/** proposed: */
	public stackTrace?: TestMessageStackFrame[];

	public static diff(message: string | ProX-Code.MarkdownString, expected: string, actual: string) {
	...
	constructor(public message: string | ProX-Code.MarkdownString) { }
}

@es5ClassCompat
export class TestTag implements ProX-Code.TestTag {
...
export class TestCoverageCount implements ProX-Code.TestCoverageCount {
	constructor(public covered: number, public total: number) {
		validateTestCoverageCount(this);
	}
}

export function validateTestCoverageCount(cc?: ProX-Code.TestCoverageCount) {
	if (!cc) {
		return;
	}
...
export class FileCoverage implements ProX-Code.FileCoverage {
	public static fromDetails(uri: ProX-Code.Uri, details: ProX-Code.FileCoverageDetail[]): ProX-Code.FileCoverage {
	...
	detailedCoverage?: ProX-Code.FileCoverageDetail[];

	constructor(
		public readonly uri: ProX-Code.Uri,
		public statementCoverage: ProX-Code.TestCoverageCount,
		public branchCoverage?: ProX-Code.TestCoverageCount,
		public declarationCoverage?: ProX-Code.TestCoverageCount,
		public includesTests: ProX-Code.TestItem[] = [],
	) {
	}
}

export class StatementCoverage implements ProX-Code.StatementCoverage {
	...
	constructor(
		...
		public branches: ProX-Code.BranchCoverage[] = [],
	) { }
}

export class BranchCoverage implements ProX-Code.BranchCoverage {
	// back compat until finalization:
	get executionCount() { return +this.executed; }
	set executionCount(n: number) { this.executed = n; }
...
export class DeclarationCoverage implements ProX-Code.DeclarationCoverage {
	// back compat until finalization:
	get executionCount() { return +this.executed; }
	set executionCount(n: number) { this.executed = n; }
...
export class ChatCompletionItem implements ProX-Code.ChatCompletionItem {
	id: string;
	label: string | CompletionItemLabel;
	fullName?: string | undefined;
	icon?: ProX-Code.ThemeIcon;
	insertText?: string;
	values: ProX-Code.ChatVariableValue[];
	detail?: string;
	documentation?: string | MarkdownString;
	command?: ProX-Code.Command;

	constructor(id: string, label: string | CompletionItemLabel, values: ProX-Code.ChatVariableValue[]) {
		this.id = id;
		this.label = label;
		this.values = values;
	}
...
export class ChatResponseMarkdownPart {
	value: ProX-Code.MarkdownString;
	constructor(value: string | ProX-Code.MarkdownString) {
...
export class ChatResponseMarkdownWithVulnerabilitiesPart {
	value: ProX-Code.MarkdownString;
	vulnerabilities: ProX-Code.ChatVulnerability[];
	constructor(value: string | ProX-Code.MarkdownString, vulnerabilities: ProX-Code.ChatVulnerability[]) {
		if (typeof value !== 'string' && value.isTrusted === true) {
			throw new Error('The boolean form of MarkdownString.isTrusted is NOT supported for chat participants.');
		}
...
export class ChatResponseFileTreePart {
	value: ProX-Code.ChatResponseFileTree[];
	baseUri: ProX-Code.Uri;
	constructor(value: ProX-Code.ChatResponseFileTree[], baseUri: ProX-Code.Uri) {
		this.value = value;
		this.baseUri = baseUri;
	}
}

export class ChatResponseAnchorPart implements ProX-Code.ChatResponseAnchorPart {
	value: ProX-Code.Uri | ProX-Code.Location;
	title?: string;

	value2: ProX-Code.Uri | ProX-Code.Location | ProX-Code.SymbolInformation;
	resolve?(token: ProX-Code.CancellationToken): Thenable<void>;

	constructor(value: ProX-Code.Uri | ProX-Code.Location | ProX-Code.SymbolInformation, title?: string) {
...
export class ChatResponseProgressPart2 {
	value: string;
	task?: (progress: ProX-Code.Progress<ProX-Code.ChatResponseWarningPart>) => Thenable<string | void>;
	constructor(value: string, task?: (progress: ProX-Code.Progress<ProX-Code.ChatResponseWarningPart>) => Thenable<string | void>) {
		this.value = value;
		this.task = task;
	}
}
...
export class ChatResponseCommandButtonPart {
	value: ProX-Code.Command;
	constructor(value: ProX-Code.Command) {
		this.value = value;
	}
}

export class ChatResponseReferencePart {
	value: ProX-Code.Uri | ProX-Code.Location | { variableName: string; value?: ProX-Code.Uri | ProX-Code.Location } | string;
	iconPath?: ProX-Code.Uri | ProX-Code.ThemeIcon | { light: ProX-Code.Uri; dark: ProX-Code.Uri };
	options?: { status?: { description: string; kind: ProX-Code.ChatResponseReferencePartStatusKind } };
	constructor(value: ProX-Code.Uri | ProX-Code.Location | { variableName: string; value?: ProX-Code.Uri | ProX-Code.Location } | string, iconPath?: ProX-Code.Uri | ProX-Code.ThemeIcon | { light: ProX-Code.Uri; dark: ProX-Code.Uri }, options?: { status?: { description: string; kind: ProX-Code.ChatResponseReferencePartStatusKind } }) {
		this.value = value;
...
export class ChatResponseCodeblockUriPart {
	isEdit?: boolean;
	value: ProX-Code.Uri;
	constructor(value: ProX-Code.Uri, isEdit?: boolean) {
		this.value = value;
		this.isEdit = isEdit;
	}
}
...
export class ChatResponseCodeCitationPart {
	...
	license: string;
	snippet: string;
	constructor(value: ProX-Code.Uri, license: string, snippet: string) {
		this.value = value;
...
export class ChatResponseTextEditPart implements ProX-Code.ChatResponseTextEditPart {
	uri: ProX-Code.Uri;
	edits: ProX-Code.TextEdit[];
	isDone?: boolean;
	constructor(uri: ProX-Code.Uri, editsOrDone: ProX-Code.TextEdit | ProX-Code.TextEdit[] | true) {
		this.uri = uri;
		if (editsOrDone === true) {
			this.isDone = true;
			this.edits = [];
...
export class ChatResponseNotebookEditPart implements ProX-Code.ChatResponseNotebookEditPart {
	uri: ProX-Code.Uri;
	edits: ProX-Code.NotebookEdit[];
	isDone?: boolean;
	constructor(uri: ProX-Code.Uri, editsOrDone: ProX-Code.NotebookEdit | ProX-Code.NotebookEdit[] | true) {
		this.uri = uri;
		if (editsOrDone === true) {
			this.isDone = true;
			this.edits = [];
...
export class ChatRequestTurn implements ProX-Code.ChatRequestTurn2 {
	constructor(
		readonly prompt: string,
		readonly command: string | undefined,
		readonly references: ProX-Code.ChatPromptReference[],
		readonly participant: string,
		readonly toolReferences: ProX-Code.ChatLanguageModelToolReference[],
		readonly editedFileEvents?: ProX-Code.ChatRequestEditedFileEvent[]
	) { }
}

export class ChatResponseTurn implements ProX-Code.ChatResponseTurn {

	constructor(
		readonly response: ReadonlyArray<ChatResponseMarkdownPart | ChatResponseFileTreePart | ChatResponseAnchorPart | ChatResponseCommandButtonPart>,
		readonly result: ProX-Code.ChatResult,
		readonly participant: string,
		readonly command?: string
	) { }
}
...
export class ChatRequestEditorData implements ProX-Code.ChatRequestEditorData {
	constructor(
		readonly document: ProX-Code.TextDocument,
		readonly selection: ProX-Code.Selection,
		readonly wholeRange: ProX-Code.Range,
	) { }
}

export class ChatRequestNotebookData implements ProX-Code.ChatRequestNotebookData {
	constructor(
		readonly cell: ProX-Code.TextDocument
	) { }
}

export class ChatReferenceBinaryData implements ProX-Code.ChatReferenceBinaryData {
	mimeType: string;
	data: () => Thenable<Uint8Array>;
	reference?: ProX-Code.Uri;
	constructor(mimeType: string, data: () => Thenable<Uint8Array>, reference?: ProX-Code.Uri) {
...
export class ChatReferenceDiagnostic implements ProX-Code.ChatReferenceDiagnostic {
	constructor(public readonly diagnostics: [ProX-Code.Uri, ProX-Code.Diagnostic[]][]) { }
}
...
export class LanguageModelToolResultPart implements ProX-Code.LanguageModelToolResultPart {
...
export class LanguageModelToolResultPart2 implements ProX-Code.LanguageModelToolResultPart2 {
...
export class PreparedTerminalToolInvocation {
	constructor(
		...
		public readonly confirmationMessages?: ProX-Code.LanguageModelToolConfirmationMessages,
	) { }
}
...
export class LanguageModelChatMessage implements ProX-Code.LanguageModelChatMessage {
	...
	role: ProX-Code.LanguageModelChatMessageRole;
	...
	constructor(role: ProX-Code.LanguageModelChatMessageRole, content: string | (LanguageModelTextPart | LanguageModelToolResultPart | LanguageModelToolCallPart)[], name?: string) {
		this.role = role;
		this.content = content;
		this.name = name;
	}
...
export class LanguageModelChatMessage2 implements ProX-Code.LanguageModelChatMessage2 {
	...
	constructor(role: ProX-Code.LanguageModelChatMessageRole, content: string | (LanguageModelTextPart | LanguageModelToolResultPart2 | LanguageModelToolCallPart | LanguageModelDataPart)[], name?: string) {
		this.role = role;
		this.content = content;
		this.name = name;
	}
...
export class LanguageModelToolCallPart implements ProX-Code.LanguageModelToolCallPart {
	callId: string;
	name: string;
	input: any;
...
export class LanguageModelTextPart implements ProX-Code.LanguageModelTextPart {
	value: string;
...
export class LanguageModelDataPart implements ProX-Code.LanguageModelDataPart {
	mimeType: string;
	...
	static image(data: Uint8Array<ArrayBufferLike>, mimeType: ChatImageMimeType): ProX-Code.LanguageModelDataPart {
		return new LanguageModelDataPart(data, mimeType as string);
	}

	static json(value: object, mime: string = 'text/x-json'): ProX-Code.LanguageModelDataPart {
		const rawStr = JSON.stringify(value, undefined, '\t');
		return new LanguageModelDataPart(VSBuffer.fromString(rawStr).buffer, mime);
	}

	static text(value: string, mime: string = Mimes.text): ProX-Code.LanguageModelDataPart {
...
export class InlineEdit implements ProX-Code.InlineEdit {
	constructor(
...
//#region MC
export class McpStdioServerDefinition implements ProX-Code.McpStdioServerDefinition {
	cwd?: URI;
...
export class McpHttpServerDefinition implements ProX-Code.McpHttpServerDefinition {
	constructor(
		public label: string,
		public uri: URI,
