...
export class UriEventHandler extends ProX-Code.EventEmitter<ProX-Code.Uri> implements ProX-Code.UriHandler {
	...
	private readonly _codeExchangePromises = new Map<string, { promise: Promise<string>; cancel: ProX-Code.EventEmitter<void> }>();

	public handleUri(uri: ProX-Code.Uri) {
		this.fire(uri);
	}

	public async waitForCode(logger: Log, scopes: string, nonce: string, token: ProX-Code.CancellationToken) {
	...
	private handleEvent: (logger: Log, scopes: string) => PromiseAdapter<ProX-Code.Uri, string> =
...
export class GitHubAuthenticationProvider implements ProX-Code.AuthenticationProvider, ProX-Code.Disposable {
	private readonly _sessionChangeEmitter = new ProX-Code.EventEmitter<ProX-Code.AuthenticationProviderAuthenticationSessionsChangeEvent>();
	...
	private readonly _disposable: ProX-Code.Disposable | undefined;

	private _sessionsPromise: Promise<ProX-Code.AuthenticationSession[]>;

	constructor(
		private readonly context: ProX-Code.ExtensionContext,
		uriHandler: UriEventHandler,
		ghesUri?: ProX-Code.Uri
	) {
		...
		this._disposable = ProX-Code.Disposable.from(
			this._telemetryReporter,
			ProX-Code.authentication.registerAuthenticationProvider(
				type,
				...
				this,
				{
					...
					supportedIssuers: [
						ghesUri ?? ProX-Code.Uri.parse('https://github.com/login/oauth')
					]
				}
			),
	...
	async getSessions(scopes: string[] | undefined, options?: ProX-Code.AuthenticationProviderSessionOptions): Promise<ProX-Code.AuthenticationSession[]> {
	...
	private async afterSessionLoad(session: ProX-Code.AuthenticationSession): Promise<void> {
	...
	private async checkForUpdates() {
		...
		const added: ProX-Code.AuthenticationSession[] = [];
		const removed: ProX-Code.AuthenticationSession[] = [];
	...
	private async readSessions(): Promise<ProX-Code.AuthenticationSession[]> {
		...
		const sessionPromises = sessionData.map(async (session: SessionData): Promise<ProX-Code.AuthenticationSession | undefined> => {
			// For GitHub scope list, order doesn't matter so we immediately sort the scopes
		...
		const verifiedSessions = (await Promise.allSettled(sessionPromises))
			...
			.map(p => (p as PromiseFulfilledResult<ProX-Code.AuthenticationSession | undefined>).value)
	...
	private async storeSessions(sessions: ProX-Code.AuthenticationSession[]): Promise<void> {
	...
	public async createSession(scopes: string[], options?: ProX-Code.AuthenticationProviderSessionOptions): Promise<ProX-Code.AuthenticationSession> {
		try {
			...
			*/
			...
			const removed = new Array<ProX-Code.AuthenticationSession>();
			if (sessionIndex > -1) {
			...
			*/
			...
			ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('Sign in failed: {0}', `${e}`));
			this._logger.error(e);
			throw e;
		}
	}

	private async tokenToSession(token: string, scopes: string[]): Promise<ProX-Code.AuthenticationSession> {
	...
	public async removeSession(id: string) {
		try {
			...
			*/
			...
			ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('Sign out failed: {0}', `${e}`));
			this._logger.error(e);
			throw e;
		}
	}
