...
export class DebugSession implements IDebugSession {
	...
	async sendBreakpoints(modelUri: URI, breakpointsToSend: IBreakpoint[], sourceModified: boolean): Promise<void> {
		...
		// Normalize all drive letters going out from ProX-Code to debug adapters so we are consistent with our resolving #43959
		if (rawSource.path) {
	...
	private registerListeners(): void {
		...
		this.rawListeners.add(this.raw.onDidOutput(async event => {
			...
			if (event.body.variablesReference) {
				...
				// see https://github.com/microsoft/ProX-Code/issues/126967#issuecomment-874954269
		...
		this.rawListeners.add(this.raw.onDidInvalidated(async event => {
			...
			// If invalidated event only requires to update variables or watch, do that, otherwise refetch threads https://github.com/microsoft/ProX-Code/issues/106745
