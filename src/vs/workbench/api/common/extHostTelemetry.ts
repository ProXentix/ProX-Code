...
export class ExtHostTelemetry extends Disposable implements ExtHostTelemetryShape {
	...
	private readonly _onDidChangeTelemetryConfiguration = this._register(new Emitter<ProX-Code.TelemetryConfiguration>());
	readonly onDidChangeTelemetryConfiguration: Event<ProX-Code.TelemetryConfiguration> = this._onDidChangeTelemetryConfiguration.event;
	...
	getTelemetryDetails(): ProX-Code.TelemetryConfiguration {
		return {
	...
	instantiateLogger(extension: IExtensionDescription, sender: ProX-Code.TelemetrySender, options?: ProX-Code.TelemetryLoggerOptions) {
...
export class ExtHostTelemetryLogger {
	...
	static validateSender(sender: ProX-Code.TelemetrySender): void {
	...
	private readonly _onDidChangeEnableStates = new Emitter<ProX-Code.TelemetryLogger>();
	...
	private _apiObject: ProX-Code.TelemetryLogger | undefined;
	private _sender: ProX-Code.TelemetrySender | undefined;

	constructor(
		sender: ProX-Code.TelemetrySender,
		options: ProX-Code.TelemetryLoggerOptions | undefined,
	...
	private logEvent(eventName: string, data?: Record<string, any>): void {
		...
		// If it's a built-in extension (ProX-Code publisher) we don't prefix the publisher and only the ext name
		if (this._extension.publisher === 'ProX-Code') {
	...
	get apiTelemetryLogger(): ProX-Code.TelemetryLogger {
		if (!this._apiObject) {
			const that = this;
			const obj: ProX-Code.TelemetryLogger = {
	...
	dispose(): void {
		if (this._sender?.flush) {
			let tempSender: ProX-Code.TelemetrySender | undefined = this._sender;
