...
export class PreviewDocumentVersion {
	...
	public readonly resource: ProX-Code.Uri;
	private readonly _version: number;

	public constructor(document: ProX-Code.TextDocument) {
...
interface MarkdownPreviewDelegate {
	getTitle?(resource: ProX-Code.Uri): string;
	getAdditionalState(): {};
	openPreviewLinkToMarkdownFile(markdownLink: ProX-Code.Uri, fragment: string | undefined): void;
}
...
class MarkdownPreview extends Disposable implements WebviewResourceProvider {
	...
	private readonly _resource: ProX-Code.Uri;
	...
	private readonly _fileWatchersBySrc = new Map</* src: */ string, ProX-Code.FileSystemWatcher>();

	private readonly _onScrollEmitter = this._register(new ProX-Code.EventEmitter<LastScrollLocation>());
	public readonly onScroll = this._onScrollEmitter.event;

	private readonly _disposeCts = this._register(new ProX-Code.CancellationTokenSource());

	constructor(
		webview: ProX-Code.WebviewPanel,
		...
		this._register(ProX-Code.workspace.onDidChangeTextDocument(event => {
			if (this.isPreviewOf(event.document.uri)) {
				this.refresh();
			}
		}));

		this._register(ProX-Code.workspace.onDidOpenTextDocument(document => {
		...
		this._register(this._webviewPanel.webview.onDidReceiveMessage((e: FromWebviewMessage.Type) => {
			...
			switch (e.type) {
				...
				case 'showPreviewSecuritySelector':
					ProX-Code.commands.executeCommand('markdown.showPreviewSecuritySelector', e.source);
					break;

				case 'previewStyleLoadError':
					ProX-Code.window.showWarningMessage(
						ProX-Code.l10n.t("Could not load 'markdown.styles': {0}", e.unloadedStyles.join(', ')));
					break;
			}
		}));
	...
	public get resource(): ProX-Code.Uri {
	...
	*/
	...
	public isPreviewOf(resource: ProX-Code.Uri): boolean {
	...
	private async _updatePreview(forceUpdate?: boolean): Promise<void> {
		...
		let document: ProX-Code.TextDocument;
		try {
			document = await ProX-Code.workspace.openTextDocument(this._resource);
		} catch {
			if (!this._disposed) {
	...
	private async _onDidClickPreview(line: number): Promise<void> {
		...
		await ProX-Code.commands.executeCommand('markdown.showSource');

		const revealLineInEditor = (editor: ProX-Code.TextEditor) => {
			const position = new ProX-Code.Position(line, 0);
			const newSelection = new ProX-Code.Selection(position, position);
			editor.selection = newSelection;
			editor.revealRange(newSelection, ProX-Code.TextEditorRevealType.InCenterIfOutsideViewport);
		};

		for (const visibleEditor of ProX-Code.window.visibleTextEditors) {
			if (this.isPreviewOf(visibleEditor.document.uri)) {
				const editor = await ProX-Code.window.showTextDocument(visibleEditor.document, visibleEditor.viewColumn);
				revealLineInEditor(editor);
				return;
			}
		}

		await ProX-Code.workspace.openTextDocument(this._resource)
			.then(ProX-Code.window.showTextDocument)
			.then((editor) => {
				revealLineInEditor(editor);
			}, () => {
				ProX-Code.window.showErrorMessage(ProX-Code.l10n.t('Could not open {0}', this._resource.toString()));
			});
	}
	...
	private _updateImageWatchers(srcs: Set<string>) {
		...
		const root = ProX-Code.Uri.joinPath(this._resource, '../');
		for (const src of srcs) {
			...
			if (uri && !MarkdownPreview._unwatchedImageSchemes.has(uri.scheme) && !this._fileWatchersBySrc.has(src)) {
				const watcher = ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(uri, '*'));
				watcher.onDidChange(() => {
					this.refresh(true);
				});
	...
	private _getWebviewOptions(): ProX-Code.WebviewOptions {
		return {
			enableScripts: true,
			enableForms: false,
	...
	private _getLocalResourceRoots(): ReadonlyArray<ProX-Code.Uri> {
		const baseRoots = Array.from(this._contributionProvider.contributions.previewResourceRoots);

		const folder = ProX-Code.workspace.getWorkspaceFolder(this._resource);
		if (folder) {
			const workspaceRoots = ProX-Code.workspace.workspaceFolders?.map(folder => folder.uri);
			if (workspaceRoots) {
	...
	private async _onDidClickPreviewLink(href: string) {
		const config = ProX-Code.workspace.getConfiguration('markdown', this.resource);
		const openLinks = config.get<string>('preview.openMarkdownLinks', 'inPreview');
		if (openLinks === 'inPreview') {
			const resolved = await this._opener.resolveDocumentLink(href, this.resource);
			if (resolved.kind === 'file') {
				try {
					const doc = await ProX-Code.workspace.openTextDocument(ProX-Code.Uri.from(resolved.uri));
	...
	asWebviewUri(resource: ProX-Code.Uri) {
...
export interface IManagedMarkdownPreview {
	...
	readonly resourceColumn: ProX-Code.ViewColumn;

	readonly onDispose: ProX-Code.Event<void>;
	readonly onDidChangeViewState: ProX-Code.Event<ProX-Code.WebviewPanelOnDidChangeViewStateEvent>;
	...
	matchesResource(
		otherResource: ProX-Code.Uri,
		otherPosition: ProX-Code.ViewColumn | undefined,
		otherLocked: boolean
	): boolean;
}
...
export class StaticMarkdownPreview extends Disposable implements IManagedMarkdownPreview {
	...
	public static readonly customEditorViewType = 'ProX-Code.markdown.preview.editor';
	...
	private constructor(
		...
		this._preview = this._register(new MarkdownPreview(this._webviewPanel, resource, topScrollLocation, {
			...
			openPreviewLinkToMarkdownFile: (markdownLink, fragment) => {
				return ProX-Code.commands.executeCommand('ProX-Code.openWith', markdownLink.with({
					fragment
	...
	private readonly _onDispose = this._register(new ProX-Code.EventEmitter<void>());
	public readonly onDispose = this._onDispose.event;

	private readonly _onDidChangeViewState = this._register(new ProX-Code.EventEmitter<ProX-Code.WebviewPanelOnDidChangeViewStateEvent>());
	...
	public matchesResource(
		_otherResource: ProX-Code.Uri,
		_otherPosition: ProX-Code.ViewColumn | undefined,
	...
	public get resourceColumn() {
		return this._webviewPanel.viewColumn || ProX-Code.ViewColumn.One;
	}
}
...
export class DynamicMarkdownPreview extends Disposable implements IManagedMarkdownPreview {
	...
	private readonly _resourceColumn: ProX-Code.ViewColumn;
	...
	public static create(
		input: DynamicPreviewInput,
		previewColumn: ProX-Code.ViewColumn,
		contentProvider: MdDocumentRenderer,
		previewConfigurations: MarkdownPreviewConfigurationManager,
		logger: ILogger,
		topmostLineMonitor: TopmostLineMonitor,
		...
		const webview = ProX-Code.window.createWebviewPanel(
	...
	private constructor(
		...
		this._register(ProX-Code.window.onDidChangeTextEditorSelection(event => {
		...
		this._register(ProX-Code.window.onDidChangeActiveTextEditor(editor => {
	...
	private readonly _onDisposeEmitter = this._register(new ProX-Code.EventEmitter<void>());
	public readonly onDispose = this._onDisposeEmitter.event;

	private readonly _onDidChangeViewStateEmitter = this._register(new ProX-Code.EventEmitter<ProX-Code.WebviewPanelOnDidChangeViewStateEvent>());
	...
	public reveal(viewColumn: ProX-Code.ViewColumn) {
	...
	public update(newResource: ProX-Code.Uri, scrollLocation?: StartingScrollLocation) {
	...
	private static _getPreviewTitle(resource: ProX-Code.Uri, locked: boolean): string {
		const resourceLabel = uri.Utils.basename(resource);
		return locked
			? ProX-Code.l10n.t('[Preview] {0}', resourceLabel)
			: ProX-Code.l10n.t('Preview {0}', resourceLabel);
	}

	public get position(): ProX-Code.ViewColumn | undefined {
	...
	private _createPreview(resource: ProX-Code.Uri, startingScroll?: StartingScrollLocation): MarkdownPreview {
		return new MarkdownPreview(this._webviewPanel, resource, startingScroll, {
			...
			openPreviewLinkToMarkdownFile: (link: ProX-Code.Uri, fragment?: string) => {
