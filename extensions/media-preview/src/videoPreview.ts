...
class VideoPreviewProvider implements ProX-Code.CustomReadonlyEditorProvider {

	public static readonly viewType = 'ProX-Code.videoPreview';
	...
	public async resolveCustomEditor(document: ProX-Code.CustomDocument, webviewEditor: ProX-Code.WebviewPanel): Promise<void> {
...
class VideoPreview extends MediaPreview {
	...
	protected async getWebviewContents(): Promise<string> {
		...
		const configurations = ProX-Code.workspace.getConfiguration('mediaPreview.video');
		const settings = {
...
<body class="loading" data-ProX-Code-context='{ "preventDefaultContextMenuItems": true }'>
	<div class="loading-indicator"></div>
	<div class="loading-error">
		<p>${ProX-Code.l10n.t("An error occurred while loading the video file.")}</p>
	...
	private async getResourcePath(webviewEditor: ProX-Code.WebviewPanel, resource: ProX-Code.Uri, version: string): Promise<string | null> {
...
export function registerVideoPreviewSupport(context: ProX-Code.ExtensionContext, binarySizeStatusBarEntry: BinarySizeStatusBarEntry): ProX-Code.Disposable {
	const provider = new VideoPreviewProvider(context.extensionUri, binarySizeStatusBarEntry);
	return ProX-Code.window.registerCustomEditorProvider(VideoPreviewProvider.viewType, provider, {
