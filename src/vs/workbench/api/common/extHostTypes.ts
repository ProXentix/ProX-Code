...
export class ChatResponseMarkdownPart {
	...
	constructor(value: string | vscode.MarkdownString) {
		if (typeof value !== 'string' && (value as MarkdownString).isTrusted === true) {
...
export class ChatResponsePullRequestPart {
	...
	constructor(
		...
		if (URI.isUri(uriOrCommand)) {
			...
			this.command = uriOrCommand as vscode.Command;
		}
	}
}
...
export class LanguageModelDataPart implements vscode.LanguageModelDataPart {
	...
	constructor(data: Uint8Array, mimeType: string, audience?: vscode.LanguageModelPartAudience[]) {
		this.mimeType = mimeType;
		this.data = data;
		this.audience = audience;
	}
	...
	static json(value: object, mime: string = 'text/x-json'): LanguageModelDataPart {
		...
		return new LanguageModelDataPart(VSBuffer.fromString(rawStr).buffer as Uint8Array, mime);
	}

	static text(value: string, mime: string = Mimes.text): LanguageModelDataPart {
		return new LanguageModelDataPart(VSBuffer.fromString(value).buffer as Uint8Array, mime);
	}
