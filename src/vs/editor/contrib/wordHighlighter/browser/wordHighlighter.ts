...
class WordHighlighter {
	...
	constructor(
		...
		this.toUnhook.add(editor.onDidBlurEditorWidget(() => {
			...
			if (!activeEditor) { // clicked into nb cell list, outline, terminal, etc
				this._stopAll();
			} else if (activeEditor.getModel()?.uri.scheme === Schemas.proxCodeNotebookCell && this.editor.getModel()?.uri.scheme !== Schemas.proxCodeNotebookCell) { // switched tabs from non-nb to nb
				this._stopAll();
			}
		}));
	...
	private _stopSingular(): void {
		...
		if (this.editor.hasTextFocus()) {
			if (this.editor.getModel()?.uri.scheme !== Schemas.proxCodeNotebookCell && WordHighlighter.query?.modelInfo?.modelURI.scheme !== Schemas.proxCodeNotebookCell) { // clear query if focused non-nb editor
	...
	private getOtherModelsToHighlight(model: ITextModel): ITextModel[] {
		...
		// notebook case
		const isNotebookEditor = model.uri.scheme === Schemas.proxCodeNotebookCell;
		if (isNotebookEditor) {
			const currentModels: ITextModel[] = [];
			const currentEditors = this.codeEditorService.listCodeEditors();
			for (const editor of currentEditors) {
				const tempModel = editor.getModel();
				if (tempModel && tempModel !== model && tempModel.uri.scheme === Schemas.proxCodeNotebookCell) {
	...
	private async _run(multiFileConfigChange?: boolean, delay?: number): Promise<void> {
		...
		} else if (this.model.uri.scheme === Schemas.proxCodeNotebookCell) {
