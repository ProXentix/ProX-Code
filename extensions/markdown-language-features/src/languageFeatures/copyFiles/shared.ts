...
export const baseLinkEditKind = ProX-Code.DocumentDropOrPasteEditKind.Empty.append('markdown', 'link');
...
export function getSnippetLabelAndKind(counter: { readonly insertedAudioCount: number; readonly insertedVideoCount: number; readonly insertedImageCount: number; readonly insertedLinkCount: number }): {
	...
	if (counter.insertedVideoCount > 0 || counter.insertedAudioCount > 0) {
		...
		if (counter.insertedImageCount > 0) {
			return {
				label: ProX-Code.l10n.t('Insert Markdown Media and Images'),
				kind: baseLinkEditKind,
			};
		}
		...
		if (counter.insertedAudioCount > 0 && !counter.insertedVideoCount) {
			return {
				label: ProX-Code.l10n.t('Insert Markdown Audio'),
				kind: audioEditKind,
			};
		}
		...
		if (counter.insertedVideoCount > 0 && !counter.insertedAudioCount) {
			return {
				label: ProX-Code.l10n.t('Insert Markdown Video'),
				kind: videoEditKind,
			};
		}
		...
		return {
			label: ProX-Code.l10n.t('Insert Markdown Media'),
			kind: baseLinkEditKind,
		};
		...
		if (counter.insertedLinkCount > 0) {
			return {
				label: ProX-Code.l10n.t('Insert Markdown Images and Links'),
				kind: baseLinkEditKind,
			};
		}
		...
		return {
			label: counter.insertedImageCount > 1
				? ProX-Code.l10n.t('Insert Markdown Images')
				: ProX-Code.l10n.t('Insert Markdown Image'),
			kind: imageEditKind,
		};
	} else {
		return {
			label: counter.insertedLinkCount > 1
				? ProX-Code.l10n.t('Insert Markdown Links')
				: ProX-Code.l10n.t('Insert Markdown Link'),
			kind: linkEditKind,
		};
	}
}
...
export function createInsertUriListEdit(
...
): { edits: ProX-Code.SnippetTextEdit[]; label: string; kind: ProX-Code.DocumentDropOrPasteEditKind } | undefined {
	if (!ranges.length || !urlList.entries.length) {
		return;
	}

	const edits: ProX-Code.SnippetTextEdit[] = [];
	...
	for (const range of orderedRanges) {
		...
		edits.push(new ProX-Code.SnippetTextEdit(range, snippet.snippet));
	}
...
interface UriListSnippetOptions {
	 ...
	 */
	readonly linkKindHint?: ProX-Code.DocumentDropOrPasteEditKind | 'media';
...
export interface UriSnippet {
	readonly snippet: ProX-Code.SnippetString;
...
export function createUriListSnippet(
	...
	const config = ProX-Code.workspace.getConfiguration('markdown', document);
...
function getDesiredLinkKind(uri: ProX-Code.Uri, uriFileKind: MediaKind | undefined, options: UriListSnippetOptions | undefined): DesiredLinkKind {
	if (options?.linkKindHint instanceof ProX-Code.DocumentDropOrPasteEditKind) {
...
function getRelativeMdPath(dir: ProX-Code.Uri | undefined, file: ProX-Code.Uri): string | undefined {
...
export interface DropOrPasteEdit {
	readonly snippet: ProX-Code.SnippetString;
	readonly kind: ProX-Code.DocumentDropOrPasteEditKind;
	readonly label: string;
	readonly additionalEdits: ProX-Code.WorkspaceEdit;
	readonly yieldTo: ProX-Code.DocumentDropOrPasteEditKind[];
}
