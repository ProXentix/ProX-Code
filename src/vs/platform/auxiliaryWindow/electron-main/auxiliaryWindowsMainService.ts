...
export class AuxiliaryWindowsMainService extends Disposable implements IAuxiliaryWindowsMainService {
	...
	private registerListeners(): void {
		...
		validatedIpcMain.handle('ProX-Code:registerAuxiliaryWindow', async (event, mainWindowId: number) => {
			const auxiliaryWindow = this.getWindowByWebContents(event.sender);
			if (auxiliaryWindow) {
				this.logService.trace('[aux window] ProX-Code:registerAuxiliaryWindow: Registering auxiliary window to main window');
