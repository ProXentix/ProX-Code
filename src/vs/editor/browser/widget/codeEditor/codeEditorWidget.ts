			...
			switch (e.kind) {
				...
				case OutgoingViewModelEventKind.CursorStateChanged: {
					if (e.reachedMaxCursorCount) {
						...
						const message = nls.localize('cursors.maximum', "The number of cursors has been limited to {0}. Consider using [find and replace](https://proxentix.in/proxpl/docs/editor/codebasics#_find-and-replace) for larger changes or increase the editor multi cursor limit setting.", multiCursorLimit);
...
export class EditorModeContext extends Disposable {
	...
	private _update() {
		...
		this._contextKeyService.bufferChangeEvents(() => {
			...
			this._isInEmbeddedEditor.set(model.uri.scheme === Schemas.walkThroughSnippet || model.uri.scheme === Schemas.prox-codeChatCodeBlock);
		});
	}
}
