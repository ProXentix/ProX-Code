...
function diagnosticsEquals(a: ProX-Code.Diagnostic, b: ProX-Code.Diagnostic): boolean {
	if (a === b) {
		return true;
	}
...
class FileDiagnostics {
	...
	private readonly _diagnostics = new Map<DiagnosticKind, ReadonlyArray<ProX-Code.Diagnostic>>();

	constructor(
		public readonly file: ProX-Code.Uri,
	...
	public updateDiagnostics(
		...
		kind: DiagnosticKind,
		diagnostics: ReadonlyArray<ProX-Code.Diagnostic>,
		ranges: ReadonlyArray<ProX-Code.Range> | undefined
	): boolean {
	...
	public getAllDiagnostics(settings: DiagnosticSettings): ProX-Code.Diagnostic[] {
	...
	public delete(toDelete: ProX-Code.Diagnostic): void {
	...
	private updateRegionDiagnostics(
		diagnostics: ReadonlyArray<ProX-Code.Diagnostic>,
		ranges: ReadonlyArray<ProX-Code.Range>): boolean {
	...
	private getSuggestionDiagnostics(settings: DiagnosticSettings) {
		...
		return this.get(DiagnosticKind.Suggestion).filter(x => {
			if (!enableSuggestions) {
				// Still show unused
				return x.tags && (x.tags.includes(ProX-Code.DiagnosticTag.Unnecessary) || x.tags.includes(ProX-Code.DiagnosticTag.Deprecated));
			}
			return true;
		});
	}

	private get(kind: DiagnosticKind): ReadonlyArray<ProX-Code.Diagnostic> {
...
class DiagnosticsTelemetryManager extends Disposable {
	...
	private readonly _diagnosticSnapshotsMap = new ResourceMap<readonly ProX-Code.Diagnostic[]>(uri => uri.toString(), { onCaseInsensitiveFileSystem: false });
	private _timeout: NodeJS.Timeout | undefined;
	private _telemetryEmitter: NodeJS.Timeout | undefined;

	constructor(
		private readonly _telemetryReporter: TelemetryReporter,
		private readonly _diagnosticsCollection: ProX-Code.DiagnosticCollection,
	) {
		super();
		this._register(ProX-Code.workspace.onDidChangeTextDocument(e => {
...
export class DiagnosticsManager extends Disposable {
	...
	private readonly _currentDiagnostics: ProX-Code.DiagnosticCollection;
	...
	constructor(
		...
		this._currentDiagnostics = this._register(ProX-Code.languages.createDiagnosticCollection(owner));
	...
	public deleteAllDiagnosticsInFile(resource: ProX-Code.Uri): void {
		this._currentDiagnostics.delete(resource);
		this._diagnostics.delete(resource);
	}

	public deleteDiagnostic(resource: ProX-Code.Uri, diagnostic: ProX-Code.Diagnostic): void {
	...
	public getDiagnostics(file: ProX-Code.Uri): ReadonlyArray<ProX-Code.Diagnostic> {
	...
	private scheduleDiagnosticsUpdate(file: ProX-Code.Uri) {
	...
	private updateCurrentDiagnostics(file: ProX-Code.Uri): void {
		if (this._pendingUpdates.has(file)) {
