...
import { ApiClient, FileStat, FileType, Requests } from '@ProX-Code/sync-api-client';
import { ClientConnection } from '@ProX-Code/sync-api-common/browser';
...
function createServerHost(
	...
	return {
		...
		readFile(path) {
			...
			try {
				...
				try {
					contents = fs.readFile(mapUri(uri, 'ProX-Code-node-modules'));
				} catch (e) {
					return undefined;
				}
			}
		...
		getFileSize(path) {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						ret = fs.stat(mapUri(uri, 'ProX-Code-node-modules')).size;
					} catch (_error) {
					}
				}
			}
		...
		writeFile(path, data, writeByteOrderMark) {
			...
			try {
				...
				if (uri.scheme !== 'ProX-Code-global-typings' && (name === 'package.json' || name === 'package-lock.json' || name === 'package-lock.kdl')) {
					fs.writeFile(mapUri(uri, 'ProX-Code-node-modules'), encoded);
				}
			} catch (error) {
		...
		fileExists(path: string): boolean {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						ret = fs.stat(mapUri(uri, 'ProX-Code-node-modules')).type === FileType.File;
					} catch (_error) {
					}
				}
			}
		...
		directoryExists(path: string): boolean {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						stat = fs.stat(mapUri(uri, 'ProX-Code-node-modules'));
					} catch (_error) {
					}
				}
			}
		...
		getModifiedTime(path: string): Date | undefined {
			...
			try {
				...
				if (enabledExperimentalTypeAcquisition) {
					try {
						s = fs.stat(mapUri(uri, 'ProX-Code-node-modules'));
					} catch (_e) {
					}
				}
			}
	...
	function realpath(path: string): string {
		...
		const isNm = looksLikeNodeModules(path)
			&& !path.startsWith('/ProX-Code-global-typings/')
			// Handle the case where a local folder has been opened in ProX Code
		...
		if (isNm) {
			uri = mapUri(uri, 'ProX-Code-node-modules');
		}
	...
	function getAccessibleFileSystemEntries(path: string): { files: readonly string[]; directories: readonly string[] } {
		...
		try {
			...
			try {
				entries = fs.readDirectory(mapUri(uri, 'ProX-Code-node-modules'));
			} catch (_e) {
			}
		}
