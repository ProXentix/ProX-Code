...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSetupAgents(context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): void {
		...
		const ProX-CodeAgentDisposables = markAsSingleton(new MutableDisposable());
		...
		const updateRegistration = () => {
			if (!context.state.hidden && !context.state.disabled) {
				...
				if (!(context.state.installed && !context.state.disabled) && !ProX-CodeAgentDisposables.value) {
					const disposables = ProX-CodeAgentDisposables.value = new DisposableStore();
				...
				ProX-CodeAgentDisposables.clear();
			}

			if (context.state.installed && !context.state.disabled) {
				ProX-CodeAgentDisposables.clear(); // we need to do this to prevent showing duplicate agent/tool entries in the list
			}
		};
