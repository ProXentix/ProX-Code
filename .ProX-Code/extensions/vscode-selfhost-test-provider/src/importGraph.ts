 ...
 */
export class ImportGraph implements ProX-Code.TestRelatedCodeProvider {
	private graph = new Map<string, FileNode>();

	constructor(
		private readonly root: ProX-Code.Uri,
		private readonly discoverWorkspaceTests: () => Thenable<ProX-Code.TestItem[]>,
		private readonly getTestNodeForDoc: (uri: ProX-Code.Uri) => ProX-Code.TestItem | undefined,
	) { }

	/** @inheritdoc */
	public async provideRelatedCode(test: ProX-Code.TestItem, token: ProX-Code.CancellationToken): Promise<ProX-Code.Location[]> {
		...
		return [...imports].map(importPath =>
			new ProX-Code.Location(
				ProX-Code.Uri.file(join(this.root.fsPath, 'src', `${importPath}.ts`)),
				new ProX-Code.Range(0, 0, maxInt32, 0),
			),
		);
	}

	/** @inheritdoc */
	public async provideRelatedTests(document: ProX-Code.TextDocument, _position: ProX-Code.Position, token: ProX-Code.CancellationToken): Promise<ProX-Code.TestItem[]> {
		...
		const tests: ProX-Code.TestItem[] = [];
	...
	public didChange(uri: ProX-Code.Uri, deleted: boolean) {
	...
	private getNode(uri: ProX-Code.Uri | undefined): FileNode | undefined {
		const rel = this.uriToImportPath(uri);
		return rel ? this.graph.get(rel) : undefined;
	}

	/** Discover all nodes that import the file */
	private async discoverOutwards(uri: ProX-Code.Uri | undefined, seen: Set<string>, maxDistance: number, token: ProX-Code.CancellationToken): Promise<FileNode | undefined> {
		const rel = this.uriToImportPath(uri);
		if (!rel) {
			return undefined;
		}
	...
	private async discoverOutwardsInner(node: FileNode, seen: Set<string>, maxDistance: number, token: ProX-Code.CancellationToken) {
	...
	private async syncNode(node: FileNode) {
		node.isSynced = discoverLimiter.execute(async () => {
			const doc = ProX-Code.workspace.textDocuments.find(d => d.uri.toString() === node.uri.toString());
	...
	private uriToImportPath(uri: ProX-Code.Uri | undefined) {
		if (!uri) {
			return undefined;
		}

		const relativePath = ProX-Code.workspace.asRelativePath(uri).replaceAll('\\', '/');
	...
	private importPathToUri(importPath: string) {
		return ProX-Code.Uri.file(join(this.root.fsPath, 'src', `${importPath}.ts`));
	}
}
