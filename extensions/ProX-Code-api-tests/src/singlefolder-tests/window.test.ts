...
import { CancellationTokenSource, commands, MarkdownString, TabInputNotebook, Position, QuickPickItem, Selection, StatusBarAlignment, TextEditor, TextEditorSelectionChangeKind, TextEditorViewColumnChangeEvent, TabInputText, Uri, ViewColumn, window, workspace, TabInputTextDiff, UIKind, env } from 'ProX-Code';
import { assertNoRpc, closeAllEditors, createRandomFile, pathEquals } from '../utils';


suite('ProX-Code API - window', () => {
	...
	test('issue #27408 - showTextDocument & ProX-Code.diff always default to ViewColumn.One', async () => {
		const [docA, docB, docC] = await Promise.all([
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile())
	...
	test('Tabs - ProX-Code.open & ProX-Code.diff', async function () {
		...
		await commands.executeCommand('ProX-Code.open', commandFile, ViewColumn.Three);
		// Ensure active tab is correct after calling ProX-Code.open
		...
		await commands.executeCommand('ProX-Code.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Four, preview: false });
	...
	(env.uiKind === UIKind.Web ? test.skip : test)('Tabs - Ensure tabs getter is correct', async function () {
		...
		const leftDiff = await createRandomFile();
		const rightDiff = await createRandomFile();
		await commands.executeCommand('ProX-Code.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Three, preview: false });
	...
	// TODO@lramos15 https://github.com/microsoft/ProX-Code/issues/145846
	...
	*/
	...
	test('showQuickPick, keep selection (microsoft/ProX-Code-azure-account#67)', async function () {
	...
	test('createStatusBar', async function () {
		...
		assert.strictEqual(statusBarEntryWithoutId.id, 'ProX-Code.ProX-Code-api-tests');
