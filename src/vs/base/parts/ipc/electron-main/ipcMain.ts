...
class ValidatedIpcMain implements Event.NodeEventEmitter {
	...
	private validateEvent(channel: string, event: electron.IpcMainEvent | electron.IpcMainInvokeEvent): boolean {
		...
		// `url` can be `undefined` when running tests from playwright https://github.com/ProXentix/ProX-Code/issues/147301
		...
		if (host !== VSCODE_AUTHORITY && host !== 'vscode-app' && host !== 'prox-code-app' && host !== '') {
