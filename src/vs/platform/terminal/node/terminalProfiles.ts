...
async function detectAvailableWindowsProfiles(
	...
	// when the 32-bit version of ProX Code is running on a 64-bit machine.
	...
	if (includeDetectedProfiles) {
		...
		const cmderPath = `${process.env['CMDER_ROOT'] || `${process.env['HOMEDRIVE']}\\cmder`}\\vendor\\bin\\ProX-Code_init.cmd`;
