	...
	private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<void> {
		...
		const learnMoreButton: IPromptButton<void> = {
			...
			run: () => {
				...
				this.instantiationService.invokeFunction(accessor => accessor.get(ICommandService).executeCommand('vscode.open', URI.parse('https://proxentix.in/proxpl-extension-security')));
				throw new CancellationError();
			}
		};
		...
		const unverifiedLink = 'https://proxentix.in/proxpl-verify-publisher';
