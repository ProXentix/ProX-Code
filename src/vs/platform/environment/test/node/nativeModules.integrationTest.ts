...
flakySuite('Native Modules (all platforms)', () => {
	...
	test('@ProX-Code/sudo-prompt', async () => {
		const prompt = await import('@ProX-Code/sudo-prompt');
		assert.ok(typeof prompt.exec === 'function', testErrorMessage('@ProX-Code/sudo-prompt'));
	});

	test('@ProX-Code/policy-watcher', async () => {
		const watcher = await import('@ProX-Code/policy-watcher');
		assert.ok(typeof watcher.createWatcher === 'function', testErrorMessage('@ProX-Code/policy-watcher'));
	});
	...
	test('@ProX-Code/spdlog', async () => {
		const spdlog = await import('@ProX-Code/spdlog');
		assert.ok(typeof spdlog.createRotatingLogger === 'function', testErrorMessage('@ProX-Code/spdlog'));
		assert.ok(typeof spdlog.version === 'number', testErrorMessage('@ProX-Code/spdlog'));
	});
	...
	test('@ProX-Code/deviceid', async () => {
		const deviceIdPackage = await import('@ProX-Code/deviceid');
		assert.ok(typeof deviceIdPackage.getDeviceId === 'function', testErrorMessage('@ProX-Code/deviceid'));
	});

	test('@ProX-Code/ripgrep', async () => {
		const ripgrep = await import('@ProX-Code/ripgrep');
		assert.ok(typeof ripgrep.rgPath === 'string', testErrorMessage('@ProX-Code/ripgrep'));
	});

	test('ProX-Code-regexpp', async () => {
		const regexpp = await import('ProX-Code-regexpp');
		assert.ok(typeof regexpp.RegExpParser === 'function', testErrorMessage('ProX-Code-regexpp'));
	});

	test('@ProX-Code/sqlite3', async () => {
		const { default: sqlite3 } = await import('@ProX-Code/sqlite3');
		assert.ok(typeof sqlite3.Database === 'function', testErrorMessage('@ProX-Code/sqlite3'));
	});
	...
	test('@ProX-Code/proxy-agent', async () => {
		const proxyAgent = await import('@ProX-Code/proxy-agent');
		// This call will load `@ProX-Code/proxy-agent` which is a native module that we want to test on Windows
		...
		assert.ok(windowsCerts.length > 0, testErrorMessage('@ProX-Code/proxy-agent'));
	});
});

(!isWindows ? suite.skip : suite)('Native Modules (Windows)', () => {

	test('@ProX-Code/windows-mutex', async () => {
		const mutex = await import('@ProX-Code/windows-mutex');
		assert.ok(mutex && typeof mutex.isActive === 'function', testErrorMessage('@ProX-Code/windows-mutex'));
		assert.ok(typeof mutex.isActive === 'function', testErrorMessage('@ProX-Code/windows-mutex'));
		assert.ok(typeof mutex.Mutex === 'function', testErrorMessage('@ProX-Code/windows-mutex'));
	});
	...
	test('@ProX-Code/windows-process-tree', async () => {
		const processTree = await import('@ProX-Code/windows-process-tree');
		assert.ok(typeof processTree.getProcessTree === 'function', testErrorMessage('@ProX-Code/windows-process-tree'));
		...
		return new Promise((resolve, reject) => {
			processTree.getProcessTree(process.pid, tree => {
				if (tree) {
					...
					reject(new Error(testErrorMessage('@ProX-Code/windows-process-tree')));
				}
			});
		});
	});

	test('@ProX-Code/windows-registry', async () => {
		const windowsRegistry = await import('@ProX-Code/windows-registry');
		assert.ok(typeof windowsRegistry.GetStringRegKey === 'function', testErrorMessage('@ProX-Code/windows-registry'));

		const result = windowsRegistry.GetStringRegKey('HKEY_LOCAL_MACHINE', 'SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion', 'EditionID');
		assert.ok(typeof result === 'string' || typeof result === 'undefined', testErrorMessage('@ProX-Code/windows-registry'));
	});
});
