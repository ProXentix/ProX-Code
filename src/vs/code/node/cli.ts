...
export async function main(argv: string[]): Promise<any> {
	...
	for (const subcommand of NATIVE_CLI_COMMANDS) {
		if (args[subcommand]) {
			...
			// Refs https://github.com/microsoft/ProX-Code/issues/221883
	...
	// Shell integration
		...
		switch (args['locate-shell-integration-path']) {
			// Usage: `[[ "$TERM_PROGRAM" == "ProX-Code" ]] && . "$(code --locate-shell-integration-path bash)"`
			case 'bash': file = 'shellIntegration-bash.sh'; break;
			// Usage: `if ($env:TERM_PROGRAM -eq "ProX-Code") { . "$(code --locate-shell-integration-path pwsh)" }`
			case 'pwsh': file = 'shellIntegration.ps1'; break;
			// Usage: `[[ "$TERM_PROGRAM" == "ProX-Code" ]] && . "$(code --locate-shell-integration-path zsh)"`
			case 'zsh': file = 'shellIntegration-rc.zsh'; break;
			// Usage: `string match -q "$TERM_PROGRAM" "ProX-Code"; and . (code --locate-shell-integration-path fish)`
	...
	// Write File
		...
		try {
			...
			if (isWindows) {
				...
				// (see https://github.com/microsoft/ProX-Code/issues/931) and
				// prevent removing alternate data streams
				// (see https://github.com/microsoft/ProX-Code/issues/6363)
	...
	// Just Code
		...
		if (hasStdinWithoutTty()) {
			...
			// checking for stdin being connected to a TTY is not enough (https://github.com/microsoft/ProX-Code/issues/40351)
		...
		if (!isMacOSBigSurOrNewer) {
			...
			// https://github.com/microsoft/ProX-Code/issues/102975

			// The following args are for the open command itself, rather than for ProX Code:
			...
			for (const e in env) {
				...
				// https://github.com/microsoft/ProX-Code/issues/134696.
				if (e !== '_') {
					spawnArgs.push('--env');
			...
			if (env['VSCODE_DEV']) {
				...
				// ProX-Code source arg. Because the OSS app isn't bundled,
				// it needs the full ProX-Code source arg to launch properly.
				const curdir = '.';
