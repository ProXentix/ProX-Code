...
export interface IExtHostTimeline extends ExtHostTimelineShape {
	...
	$getTimeline(id: string, uri: UriComponents, options: ProX-Code.TimelineOptions, token: ProX-Code.CancellationToken): Promise<Timeline | undefined>;
}
...
export class ExtHostTimeline implements IExtHostTimeline {
	...
	private _itemsBySourceAndUriMap = new Map<string, Map<string | undefined, Map<string, ProX-Code.TimelineItem>>>();
	...
	async $getTimeline(id: string, uri: UriComponents, options: ProX-Code.TimelineOptions, token: ProX-Code.CancellationToken): Promise<Timeline | undefined> {
		const item = this._providers.get(id);
		return item?.provider.provideTimeline(URI.revive(uri), options, token);
	}

	registerTimelineProvider(scheme: string | string[], provider: ProX-Code.TimelineProvider, extensionId: ExtensionIdentifier, commandConverter: CommandsConverter): IDisposable {
	...
	private convertTimelineItem(source: string, commandConverter: CommandsConverter, disposables: DisposableStore) {
		return (uri: URI, options?: TimelineOptions) => {
			let items: Map<string, ProX-Code.TimelineItem> | undefined;
			...
			return (item: ProX-Code.TimelineItem): TimelineItem => {
