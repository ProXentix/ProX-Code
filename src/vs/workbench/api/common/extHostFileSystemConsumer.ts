...
export class ExtHostConsumerFileSystem {
	...
	readonly value: ProX-Code.FileSystem;

	private readonly _proxy: MainThreadFileSystemShape;
	private readonly _fileSystemProvider = new Map<string, { impl: ProX-Code.FileSystemProvider; extUri: IExtUri; isReadonly: boolean }>();
	...
	constructor(
		...
		this.value = Object.freeze({
			...
			async createDirectory(uri: ProX-Code.Uri): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async writeFile(uri: ProX-Code.Uri, content: Uint8Array): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
					if (provider && !provider.isReadonly) {
						// use shortcut
			...
			async delete(uri: ProX-Code.Uri, options?: { recursive?: boolean; useTrash?: boolean }): Promise<void> {
				try {
					const provider = that._fileSystemProvider.get(uri.scheme);
			...
			async rename(oldUri: ProX-Code.Uri, newUri: ProX-Code.Uri, options?: { overwrite?: boolean }): Promise<void> {
				try {
			...
			async copy(source: ProX-Code.Uri, destination: ProX-Code.Uri, options?: { overwrite?: boolean }): Promise<void> {
				try {
					// no shortcut: potentially involves different schemes, does mkdirp
	...
	private async mkdirp(provider: ProX-Code.FileSystemProvider, providerExtUri: IExtUri, directory: ProX-Code.Uri): Promise<void> {
		...
		for (let i = directoriesToCreate.length - 1; i >= 0; i--) {
			...
			try {
				...
				if (files.toFileSystemProviderErrorCode(error) !== files.FileSystemProviderErrorCode.FileExists) {
					...
					// (see also https://github.com/microsoft/ProX-Code/issues/89834)
					throw error;
				}
			}
		}
	...
	addFileSystemProvider(scheme: string, provider: ProX-Code.FileSystemProvider, options?: { isCaseSensitive?: boolean; isReadonly?: boolean | IMarkdownString }): IDisposable {
