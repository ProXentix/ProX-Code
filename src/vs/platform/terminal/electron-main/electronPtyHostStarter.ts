...
export class ElectronPtyHostStarter extends Disposable implements IPtyHostStarter {
	...
	constructor(
		...
		validatedIpcMain.on('ProX-Code:createPtyHostMessageChannel', (e, nonce) => this._onWindowConnection(e, nonce));
		this._register(toDisposable(() => {
			validatedIpcMain.removeHandler('ProX-Code:createPtyHostMessageChannel');
		}));
	}
	...
	private _onWindowConnection(e: IpcMainEvent, nonce: string) {
		...
		e.sender.postMessage('ProX-Code:createPtyHostMessageChannelResult', nonce, [port]);
	}
}
