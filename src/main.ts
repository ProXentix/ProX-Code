...
// (https://github.com/microsoft/ProX-Code/issues/56651)
...
protocol.registerSchemesAsPrivileged([
	{
		scheme: 'ProX-Code-webview',
		privileges: { standard: true, secure: true, supportFetchAPI: true, corsEnabled: true, allowServiceWorkers: true, codeCache: true }
	},
	{
		scheme: 'ProX-Code-file',
...
// https://github.com/microsoft/ProX-Code/issues/167543.
...
function configureCommandlineSwitchesSync(cliArgs: NativeParsedArgs) {
	...
	const SUPPORTED_MAIN_PROCESS_SWITCHES = [
		...
		// Persistently enable proposed api via argv.json: https://github.com/microsoft/ProX-Code/issues/99775
		'enable-proposed-api',
	...
	// `FontMatchingCTMigration` - Siwtch font matching on macOS to Appkit (Refs https://github.com/microsoft/ProX-Code/issues/224496#issuecomment-2270418470).
	// `StandardizedBrowserZoom` - Disable zoom adjustment for bounding box (https://github.com/microsoft/ProX-Code/issues/232750#issuecomment-2459495394)
	...
	// to address https://github.com/microsoft/ProX-Code/issues/213780
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to ProX Code.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of ProX Code.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in ProX Code.',
...
function getArgvConfigPath(): string {
	const ProX-CodePortable = process.env['VSCODE_PORTABLE'];
	if (ProX-CodePortable) {
		return path.join(ProX-CodePortable, 'argv.json');
	}
	...
	// Otherwise we configure the crash reporter from product.json
		...
		if (appCenter) {
			...
			if (crashReporterId && uuidPattern.test(crashReporterId)) {
				...
				// For ProX-Code this is ExtensionHost process currently.
...
async function resolveNlsConfiguration(): Promise<INLSConfiguration> {
	...
	if (!userLocale) {
		return {
			...
			// NLS: below 2 are a relic from old times only used by ProX-Code-nls and deprecated
			locale: 'en',
			availableLanguages: {}
		};
	}
