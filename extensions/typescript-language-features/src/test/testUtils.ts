...
export function withRandomFileEditor(
	...
	run: (editor: ProX-Code.TextEditor, doc: ProX-Code.TextDocument) => Thenable<void>
): Thenable<boolean> {
		...
		return ProX-Code.workspace.openTextDocument(file).then(doc => {
			return ProX-Code.window.showTextDocument(doc).then((editor) => {
				if (cursorIndex >= 0) {
					...
					editor.selection = new ProX-Code.Selection(pos, pos);
				}
...
export async function createTestEditor(uri: ProX-Code.Uri, ...lines: string[]) {
	const document = await ProX-Code.workspace.openTextDocument(uri);
	const editor = await ProX-Code.window.showTextDocument(document);
	await editor.insertSnippet(new ProX-Code.SnippetString(joinLines(...lines)), new ProX-Code.Range(0, 0, 1000, 0));
	return editor;
}

export function assertEditorContents(editor: ProX-Code.TextEditor, expectedDocContent: string, message?: string): void {
...
export async function updateConfig(documentUri: ProX-Code.Uri, newConfig: VsCodeConfiguration): Promise<VsCodeConfiguration> {
	const oldConfig: VsCodeConfiguration = {};
	const config = ProX-Code.workspace.getConfiguration(undefined, documentUri);

	for (const configKey of Object.keys(newConfig)) {
		oldConfig[configKey] = config.get(configKey);
		await new Promise<void>((resolve, reject) =>
			config.update(configKey, newConfig[configKey], ProX-Code.ConfigurationTarget.Global)
				.then(() => resolve(), reject));
	}
	return oldConfig;
}
...
export function onChangedDocument(documentUri: ProX-Code.Uri, disposables: ProX-Code.Disposable[]) {
	return new Promise<ProX-Code.TextDocument>(resolve => ProX-Code.workspace.onDidChangeTextDocument(e => {
...
export async function retryUntilDocumentChanges(
	...
	disposables: ProX-Code.Disposable[],
