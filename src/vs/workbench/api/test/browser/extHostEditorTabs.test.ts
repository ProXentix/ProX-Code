...
suite('ExtHostEditorTabs', function () {
	...
	test('Ensure tabGroup change events fires', function () {
		...
		const activeTabGroup: ProX-Code.TabGroup = extHostEditorTabs.tabGroups.activeTabGroup;
		assert.strictEqual(extHostEditorTabs.tabGroups.all.length, 1);
	...
	test('Check TabGroupChangeEvent properties', function () {
		...
		const events: ProX-Code.TabGroupChangeEvent[] = [];
	...
	test('ProX-Code.window.tagGroups is immutable', function () {
	...
	test('Update tab only sends tab change event', async function () {
		...
		const p = new Promise<ProX-Code.TabChangeEvent>(resolve => store.add(extHostEditorTabs.tabGroups.onDidChangeTabs(resolve)));
