...
export interface IFilePathToResourceConverter {
	/**
	 * Convert a typescript filepath to a ProX Code resource.
	 */
				...
				if (currentLink) {
					if (currentLink.target) {
						...
						const args: OpenJsDocLinkCommand_Args = {
							file: { ...file.toJSON(), $mid: undefined }, // Prevent ProX Code from trying to transform the uri,
...
export function documentationToMarkdown(
	...
	tags: readonly Proto.JSDocTagInfo[],
	filePathConverter: IFilePathToResourceConverter,
	baseUri: ProX-Code.Uri | undefined,
): ProX-Code.MarkdownString {
	const out = new ProX-Code.MarkdownString();
