...
export function lazilyActivateClient(
	...
	const maybeActivate = (textDocument: ProX-Code.TextDocument): boolean => {
	...
	const didActivate = ProX-Code.workspace.textDocuments.some(maybeActivate);
	if (!didActivate) {
		const openListener = ProX-Code.workspace.onDidOpenTextDocument(doc => {
