...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	protected applyState(state: IWindowState, hasMultipleDisplays = electron.screen.getAllDisplays().length > 0): void {
		...
		// correctly unless we set the bounds again (https://github.com/microsoft/ProX-Code/issues/74872)
		//
		// Extended to cover Windows as well as Mac (https://github.com/microsoft/ProX-Code/issues/146499)
		//
		// However, when running with native tabs with multiple windows we cannot use this workaround
		// because there is a potential that the new window will be added as native tab instead of being
		// a window on its own. In that case calling setBounds() would cause https://github.com/microsoft/ProX-Code/issues/75830
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			const options = instantiationService.invokeFunction(defaultBrowserWindowOptions, this.windowState, undefined, {
				...
				additionalArguments: [`--ProX-Code-window-config=${this.configObjectUrl.resource.toString()}`],
	...
	private registerListeners(): void {
		...
		// with the DOM way.
		// (https://github.com/microsoft/ProX-Code/issues/122736)
		...
		// Window Fullscreen
		this._register(this.onDidEnterFullScreen(() => {
			this.sendWhenReady('ProX-Code:enterFullScreen', CancellationToken.None);
		}));

		this._register(this.onDidLeaveFullScreen(() => {
			this.sendWhenReady('ProX-Code:leaveFullScreen', CancellationToken.None);
		}));
	...
	private async onWindowError(type: WindowError, details?: { reason?: string; exitCode?: number }): Promise<void> {
		...
		type WindowErrorClassification = {
			...
			owner: 'bpasero';
			comment: 'Provides insight into reasons the ProX-Code window had an error.';
		};
			...
			case WindowError.PROCESS_GONE:
				...
				if (type === WindowError.UNRESPONSIVE) {
					if (this.isExtensionDevelopmentHost || this.isExtensionTestHost || (this._win && this._win.webContents && this._win.webContents.isDevToolsOpened())) {
						// TODO@electron Workaround for https://github.com/microsoft/ProX-Code/issues/56994
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): void {
		...
		// Ref: https://github.com/microsoft/ProX-Code/issues/115874
	...
	serializeWindowState(): IWindowState {
		...
		if (this.isFullScreen) {
			...
			try {
				...
				// e.g. https://github.com/microsoft/ProX-Code/issues/100334 when
			...
			return {
				...
				// (https://github.com/microsoft/ProX-Code/issues/58218) so we
	...
	protected override setFullScreen(fullscreen: boolean, fromRestore: boolean): void {
		...
		// Events
		this.sendWhenReady(fullscreen ? 'ProX-Code:enterFullScreen' : 'ProX-Code:leaveFullScreen', CancellationToken.None);
	...
	private setMenuBarVisibility(visibility: MenuBarVisibility, notify: boolean = true): void {
		...
		if (visibility === 'toggle') {
			if (notify) {
				this.send('ProX-Code:showInfoMessage', localize('hiddenMenuBar', "You can still access the menu bar by pressing the Alt-key."));
			}
		}

		if (visibility === 'hidden') {
			// for some weird reason that I have no explanation for, the menu bar is not hiding when calling
			// this without timeout (see https://github.com/microsoft/ProX-Code/issues/19777). there seems to be
	...
	private createTouchBarGroup(items: ISerializableCommandAction[] = []): electron.TouchBarSegmentedControl {
		...
		const control = new electron.TouchBar.TouchBarSegmentedControl({
			...
			change: (selectedIndex) => {
				this.sendWhenReady('ProX-Code:runAction', CancellationToken.None, { id: (control.segments[selectedIndex] as ITouchBarSegment).id, from: 'touchbar' });
			}
		});
	...
	private stopCollectingJScallStacks(): void {
		...
		if (this.jsCallStackMap.size) {
			...
			logMessage += 'For full overview of the unresponsive period, capture cpu profile via https://aka.ms/ProX-Code-tracing-cpu-profile';
