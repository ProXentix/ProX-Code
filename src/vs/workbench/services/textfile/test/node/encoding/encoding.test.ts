...
suite('Encoding', () => {
		...
		return new Promise<string>((resolve, reject) => {
			fs.readFile(path, (err, data) => {
				if (err) {
					...
					resolve(importAMDNodeModule<typeof import('@ProX-Code/iconv-lite-umd')>('@ProX-Code/iconv-lite-umd', 'lib/iconv-lite-umd.js').then(iconv => iconv.decode(data, encoding.toNodeEncoding(fileEncoding))));
				}
			});
		});
	}
	...
	test('toEncodeReadable - encoding, utf16be', async function () {
		...
		const iconv = await importAMDNodeModule<typeof import('@ProX-Code/iconv-lite-umd')>('@ProX-Code/iconv-lite-umd', 'lib/iconv-lite-umd.js');
