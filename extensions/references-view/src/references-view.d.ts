 ...
 * const api = await ProX-Code.extensions.getExtension<SymbolTree>('ProX-Code.references-view').activate();
 *
...
export interface SymbolTreeInput<T> {
	 ...
	 */
	resolve(): ProX-Code.ProviderResult<SymbolTreeModel<T>>;
	 ...
	 */
	with(location: ProX-Code.Location): SymbolTreeInput<T>;
}
...
export interface SymbolTreeModel<T> {
	 ...
	 */
	provider: ProX-Code.TreeDataProvider<T>;
...
export interface SymbolItemNavigation<T> {
	 ...
	 */
	nearest(uri: ProX-Code.Uri, position: ProX-Code.Position): T | undefined;
	/**
	 ...
	 */
	location(item: T): ProX-Code.Location | undefined;
}
...
export interface SymbolItemEditorHighlights<T> {
	 ...
	 */
	getEditorHighlights(item: T, uri: ProX-Code.Uri): ProX-Code.Range[] | undefined;
}

export interface SymbolItemDragAndDrop<T> {

	getDragUri(item: T): ProX-Code.Uri | undefined;
}
