...
import { env, ExtensionKind, extensions, UIKind, Uri } from 'ProX-Code';
import { assertNoRpc } from '../utils';

suite('ProX-Code API - env', () => {
	...
	test('env.remoteName', function () {
		...
		const knownWorkspaceExtension = extensions.getExtension('ProX-Code.git');
		const knownUiAndWorkspaceExtension = extensions.getExtension('ProX-Code.media-preview');
	...
	test('env.uiKind', async function () {
		const uri = Uri.parse(`${env.uriScheme}:://ProX-Code.ProX-Code-api-tests/path?key=value&other=false`);
