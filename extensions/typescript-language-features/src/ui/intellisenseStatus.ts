...
namespace IntellisenseState {
	...
	export class Pending {
		...
		public readonly cancellation = new ProX-Code.CancellationTokenSource();
...
type CreateOrOpenConfigCommandArgs = [root: ProX-Code.Uri, projectType: ProjectType];
...
export class IntellisenseStatus extends Disposable {
	...
	private _statusItem?: ProX-Code.LanguageStatusItem;
	...
	private updateState(newState: IntellisenseState.State): void {
		...
		switch (this._state.type) {
			...
			case IntellisenseState.Type.Pending: {
				...
				statusItem.severity = ProX-Code.LanguageStatusSeverity.Information;
				statusItem.text = ProX-Code.l10n.t("Loading IntelliSense status");
			...
			case IntellisenseState.Type.Resolved: {
				const noConfigFileText = this._state.projectType === ProjectType.TypeScript
					? ProX-Code.l10n.t("No tsconfig")
					: ProX-Code.l10n.t("No jsconfig");

				const rootPath = this._client.getWorkspaceRootForResource(this._state.resource);
				if (!rootPath) {
					if (this._statusItem) {
						this._statusItem.text = noConfigFileText;
						this._statusItem.detail = !ProX-Code.workspace.workspaceFolders
							? ProX-Code.l10n.t("No opened folders")
							: ProX-Code.l10n.t("File is not part opened folders");
				...
				if (isImplicitProjectConfigFile(this._state.configFile)) {
					...
					statusItem.command = {
						...
						title: this._state.projectType === ProjectType.TypeScript
							? ProX-Code.l10n.t("Configure TSConfig")
							: ProX-Code.l10n.t("Configure JSConfig"),
						arguments: [rootPath, this._state.projectType] satisfies CreateOrOpenConfigCommandArgs,
					};
				} else {
					statusItem.text = ProX-Code.workspace.asRelativePath(this._state.configFile);
					statusItem.detail = undefined;
					statusItem.command = {
						command: this.openOpenConfigCommandId,
						title: ProX-Code.l10n.t("Open Config File"),
						arguments: [rootPath, this._state.projectType] satisfies CreateOrOpenConfigCommandArgs,
					};
				}
				break;
			...
			case IntellisenseState.Type.SyntaxOnly: {
				const statusItem = this.ensureStatusItem();
				statusItem.severity = ProX-Code.LanguageStatusSeverity.Warning;
				statusItem.text = ProX-Code.l10n.t("Partial mode");
				statusItem.detail = ProX-Code.l10n.t("Project wide IntelliSense not available");
				statusItem.busy = false;
				statusItem.command = {
					title: ProX-Code.l10n.t("Learn More"),
					command: 'ProX-Code.open',
					arguments: [
						ProX-Code.Uri.parse('https://aka.ms/ProX-Code/jsts/partial-mode'),
					]
				};
				break;
			}
	...
	private ensureStatusItem(): ProX-Code.LanguageStatusItem {
		if (!this._statusItem) {
			this._statusItem = ProX-Code.languages.createLanguageStatusItem('typescript.projectStatus', jsTsLanguageModes);
			this._statusItem.name = ProX-Code.l10n.t("JS/TS IntelliSense Status");
		}
