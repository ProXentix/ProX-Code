...
class CopyMetadata {
	...
	static parse(data: string): CopyMetadata | undefined {
		try {
			...
			const resource = ProX-Code.Uri.parse(parsedData.resource);
			const ranges = parsedData.ranges.map((range: any) => new ProX-Code.Range(range.start, range.end));
...
class TsPasteEdit extends ProX-Code.DocumentPasteEdit {
	...
	constructor() {
		super('', ProX-Code.l10n.t("Paste with imports"), DocumentPasteProvider.kind);
		this.yieldTo = [
			ProX-Code.DocumentDropOrPasteEditKind.Text.append('plain')
		];
	}
}
...
class DocumentPasteProvider implements ProX-Code.DocumentPasteEditProvider<TsPasteEdit> {

	static readonly kind = ProX-Code.DocumentDropOrPasteEditKind.TextUpdateImports.append('jsts');
	...
	async prepareDocumentPaste(document: ProX-Code.TextDocument, ranges: readonly ProX-Code.Range[], dataTransfer: ProX-Code.DataTransfer, token: ProX-Code.CancellationToken) {
		...
		if (response) {
			...
			dataTransfer.set(DocumentPasteProvider.metadataMimeType,
				new ProX-Code.DataTransferItem(new CopyMetadata(document.uri, ranges, undefined)));
		} else {
			...
			dataTransfer.set(DocumentPasteProvider.metadataMimeType,
				new ProX-Code.DataTransferItem(new CopyMetadata(document.uri, ranges, copyRequest)));
		}
	}
	...
	async provideDocumentPasteEdits(
		...
		const pasteCts = new ProX-Code.CancellationTokenSource();
	...
	async resolveDocumentPasteEdit(inEdit: TsPasteEdit, _token: ProX-Code.CancellationToken): Promise<TsPasteEdit | undefined> {
	...
	private async extractMetadata(dataTransfer: ProX-Code.DataTransfer, token: ProX-Code.CancellationToken): Promise<CopyMetadata | undefined> {
	...
	private isEnabled(document: ProX-Code.TextDocument) {
		const config = ProX-Code.workspace.getConfiguration(this._modeId, document.uri);
...
export function register(selector: DocumentSelector, language: LanguageDescription, client: ITypeScriptServiceClient, fileConfigurationManager: FileConfigurationManager) {
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerDocumentPasteEditProvider(selector.semantic, new DocumentPasteProvider(language.id, client, fileConfigurationManager), {
