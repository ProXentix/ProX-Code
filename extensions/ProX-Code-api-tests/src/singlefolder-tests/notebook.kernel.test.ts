...
async function withEvent<T>(event: ProX-Code.Event<T>, callback: (e: Promise<T>) => Promise<void>) {
...
async function assertKernel(kernel: Kernel, notebook: ProX-Code.NotebookDocument): Promise<void> {
	const success = await ProX-Code.commands.executeCommand('notebook.selectKernel', {
		extension: 'ProX-Code.ProX-Code-api-tests',
...
(ProX-Code.env.uiKind === ProX-Code.UIKind.Web ? suite.skip : suite)('Notebook Kernel API tests', function () {
	...
	suiteSetup(() => {
		suiteDisposables.push(ProX-Code.workspace.registerNotebookSerializer('notebookCoreTest', apiTestSerializer));
	});
	...
	test('cell execute command takes arguments', async () => {
		...
		assert.strictEqual(ProX-Code.window.activeNotebookEditor !== undefined, true, 'notebook first');
		const editor = ProX-Code.window.activeNotebookEditor!;
		const cell = editor.notebook.cellAt(0);

		await withEvent(ProX-Code.workspace.onDidChangeNotebookDocument, async event => {
			await ProX-Code.commands.executeCommand('notebook.execute');
			await event;
			assert.strictEqual(cell.outputs.length, 1, 'should execute'); // runnable, it worked
		});

		await withEvent(ProX-Code.workspace.onDidChangeNotebookDocument, async event => {
			await ProX-Code.commands.executeCommand('notebook.cell.clearOutputs');
			await event;
			assert.strictEqual(cell.outputs.length, 0, 'should clear');
		});

		const secondResource = await createRandomNotebookFile();
		const secondDocument = await ProX-Code.workspace.openNotebookDocument(secondResource);
		await ProX-Code.window.showNotebookDocument(secondDocument);

		await withEvent<ProX-Code.NotebookDocumentChangeEvent>(ProX-Code.workspace.onDidChangeNotebookDocument, async event => {
			await ProX-Code.commands.executeCommand('notebook.cell.execute', { start: 0, end: 1 }, notebook.uri);
			await event;
			assert.strictEqual(cell.outputs.length, 1, 'should execute'); // runnable, it worked
			assert.strictEqual(ProX-Code.window.activeNotebookEditor?.notebook.uri.fsPath, secondResource.fsPath);
		});
	});
	...
	test('cell execute command takes arguments 2', async () => {
		...
		testDisposables.push(ProX-Code.workspace.onDidChangeNotebookDocument(e => {
		...
		ProX-Code.commands.executeCommand('notebook.cell.execute', { document: notebook.uri, ranges: [{ start: 0, end: 1 }, { start: 1, end: 2 }] });
	...
	test('document execute command takes arguments', async () => {
		...
		await withEvent<ProX-Code.NotebookDocumentChangeEvent>(ProX-Code.workspace.onDidChangeNotebookDocument, async (event) => {
			await ProX-Code.commands.executeCommand('notebook.execute', notebook.uri);
			await event;
			assert.strictEqual(cell.outputs.length, 1, 'should execute'); // runnable, it worked
		});
	});
	...
	test('cell execute and select kernel', async function () {
		...
		assert.strictEqual(ProX-Code.window.activeNotebookEditor === editor, true, 'notebook first');
		...
		const alternativeKernel = new class extends Kernel {
			...
			override async _runCell(cell: ProX-Code.NotebookCell) {
				const task = this.controller.createNotebookCellExecution(cell);
				task.start();
				await task.replaceOutput([new ProX-Code.NotebookCellOutput([
					ProX-Code.NotebookCellOutputItem.text('my second output', 'text/plain')
				])]);
				task.end(true);
			}
		};
		...
		await withEvent<ProX-Code.NotebookDocumentChangeEvent>(ProX-Code.workspace.onDidChangeNotebookDocument, async (event) => {
			...
			await ProX-Code.commands.executeCommand('notebook.cell.execute');
			await event;
			assert.strictEqual(cell.outputs.length, 1, 'should execute'); // runnable, it worked
	...
	test('Output changes are applied once the promise resolves', async function () {
		...
		const verifyOutputSyncKernel = new class extends Kernel {
			...
			override async _execute(cells: ProX-Code.NotebookCell[]) {
				const [cell] = cells;
				const task = this.controller.createNotebookCellExecution(cell);
				task.start();
				await task.replaceOutput([new ProX-Code.NotebookCellOutput([
					ProX-Code.NotebookCellOutputItem.text('Some output', 'text/plain')
				])]);
	...
	test('execution cancelled when delete while executing', async () => {
		...
		edit.set(cell!.notebook.uri, [ProX-Code.NotebookEdit.replaceCells(new ProX-Code.NotebookRange(cell!.index, cell!.index + 1), [])]);
	...
	test('appendOutput to different cell', async function () {
		...
		const nextCellKernel = new class extends Kernel {
			...
			override async _runCell(cell: ProX-Code.NotebookCell) {
				const task = this.controller.createNotebookCellExecution(cell);
				task.start();
				await task.appendOutput([new ProX-Code.NotebookCellOutput([
					ProX-Code.NotebookCellOutputItem.text('my output')
				])], cell1);
				await task.appendOutput([new ProX-Code.NotebookCellOutput([
					ProX-Code.NotebookCellOutputItem.text('my output 2')
				])], cell1);
				task.end(true);
			}
		};
