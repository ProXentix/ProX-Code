...
export class Response extends AbstractResponse implements IDisposable {
	...
	updateContent(progress: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit | IChatTask | IChatUndoStop, quiet?: boolean): void {
		if (progress.kind === 'markdownContent') {
			...
			const useOldApproachForInlineNotebook = progress.uri.scheme === Schemas.ProX-CodeNotebookCell && !this._responseParts.find(part => part.kind === 'notebookEditGroup');
		...
		try {
			return requests.map((raw: ISerializableChatRequestData) => {
				...
				if (raw.response || raw.result || (raw as any).responseErrorDetails) {
					...
					if (raw.usedContext) { // @ulugbekna: if this's a new ProX-Code sessions, doc versions are incorrect anyway?
