...
class FsLinkProvider {
	...
	provideDocumentLinks(document: ProX-Code.TextDocument): ProX-Code.ProviderResult<ProX-Code.DocumentLink[]> {
		this._initStateMachine();

		const result: ProX-Code.DocumentLink[] = [];
...
export class ExtHostFileSystem implements ExtHostFileSystemShape {
	...
	private readonly _fsProvider = new Map<number, ProX-Code.FileSystemProvider>();
	...
	registerFileSystemProvider(extension: IExtensionDescription, scheme: string, provider: ProX-Code.FileSystemProvider, options: { isCaseSensitive?: boolean; isReadonly?: boolean | ProX-Code.MarkdownString } = {}) {
	...
	private static _validateFileSystemProvider(provider: ProX-Code.FileSystemProvider) {
		if (!provider) {
	...
	private static _asIStat(stat: ProX-Code.FileStat): files.IStat {
	...
	private _getFsProvider(handle: number): ProX-Code.FileSystemProvider {
