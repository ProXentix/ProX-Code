...
class ManageTrustedMcpServersForAccountActionImpl {
	...
	private async _getItems(accountQuery: IAccountQuery) {
		...
		return [
			...
			{ type: 'separator', label: localize('trustedMcpServers', "Trusted by ProXentix") } satisfies IQuickPickSeparator,
	...
	private _toQuickPickItem(mcpServer: AllowedMcpServer): TrustedMcpServersQuickPickItem {
		...
		if (mcpServer.trusted) {
			tooltip = localize('trustedMcpServerTooltip', "This MCP server is trusted by ProXentix and\nalways has access to this account");
			disabled = true;
		}
		return {
