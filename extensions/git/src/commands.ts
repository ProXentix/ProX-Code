...
export class CommandCenter {
		...
		try {
			...
			*/
			...
			if (action === PostCloneAction.Open) {
				commands.executeCommand('ProX-Code.openFolder', uri, { forceReuseWindow: true });
			} else if (action === PostCloneAction.AddToWorkspace) {
				workspace.updateWorkspaceFolders(workspace.workspaceFolders!.length, 0, { uri });
			} else if (action === PostCloneAction.OpenNewWindow) {
				commands.executeCommand('ProX-Code.openFolder', uri, { forceNewWindow: true });
			}
		} catch (err) {
	...
	async continueInLocalClone(): Promise<Uri | void> {
		...
		if (uri !== undefined) {
			let target = `${env.uriScheme}://ProX-Code.git/clone?url=${encodeURIComponent(uri)}`;
	...
	async init(skipFolderPrompt = false): Promise<void> {
		...
		if (result === open) {
			commands.executeCommand('ProX-Code.openFolder', uri);
		} else if (result === addToWorkspace) {
			workspace.updateWorkspaceFolders(workspace.workspaceFolders!.length, 0, { uri });
		} else if (result === openNewWindow) {
			commands.executeCommand('ProX-Code.openFolder', uri, true);
		} else {
			await this.model.openRepository(repositoryPath);
		}
	}
	...
	async openHEADFile(arg?: Resource | Uri): Promise<void> {
		...
		return await commands.executeCommand<void>('ProX-Code.open', HEAD, opts, title);
	}
	...
	async rename(repository: Repository, fromUri: Uri | undefined): Promise<void> {
		...
		await commands.executeCommand('ProX-Code.open', Uri.file(path.join(repository.root, to)), { viewColumn: ViewColumn.Active });
	}
	...
	async timelineCompareWithSelected(item: TimelineItem, uri: Uri | undefined, _source: string) {
		...
		await commands.executeCommand('ProX-Code.diff', selected.ref === '' ? uri : toGitUri(uri, selected.ref), item.ref === '' ? uri : toGitUri(uri, item.ref), title);
	}
			...
			return result.catch(err => {
				...
				switch (err.gitErrorCode) {
					...
					case GitErrorCodes.NoUserEmailConfigured:
						...
						choices.set(l10n.t('Learn More'), () => commands.executeCommand('ProX-Code.open', Uri.parse('https://aka.ms/ProX-Code-setup-git')));
						break;
