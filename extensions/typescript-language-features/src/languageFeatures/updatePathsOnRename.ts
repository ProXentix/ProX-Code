...
async function isDirectory(resource: ProX-Code.Uri): Promise<boolean> {
	try {
		return (await ProX-Code.workspace.fs.stat(resource)).type === ProX-Code.FileType.Directory;
	} catch {
		return false;
	}
}
...
interface RenameAction {
	...
	readonly jsTsFileThatIsBeingMoved: ProX-Code.Uri;
}
...
class UpdateImportsOnFileRenameHandler extends Disposable {
	...
	public constructor(
		...
		private readonly _handles: (uri: ProX-Code.Uri) => Promise<boolean>,
	) {
		super();
			...
			for (const { newUri, oldUri } of e.files) {
				...
				this._delayer.trigger(() => {
					ProX-Code.window.withProgress({
						...
						title: ProX-Code.l10n.t("Checking for update of JS/TS imports")
					}, () => this.flushRenames());
				});
			}
		}));
	...
	private async flushRenames(): Promise<void> {
		...
		for (const group of this.groupRenames(renames)) {
			const edits = new ProX-Code.WorkspaceEdit();
			const resourcesBeingRenamed: ProX-Code.Uri[] = [];

			for (const { oldUri, newUri, newFilePath, oldFilePath, jsTsFileThatIsBeingMoved } of group) {
				const document = await ProX-Code.workspace.openTextDocument(jsTsFileThatIsBeingMoved);
			...
			if (edits.size) {
				if (await this.confirmActionWithUser(resourcesBeingRenamed)) {
					await ProX-Code.workspace.applyEdit(edits, { isRefactoring: true });
				}
			}
		}
	}

	private async confirmActionWithUser(newResources: readonly ProX-Code.Uri[]): Promise<boolean> {
	...
	private getConfiguration(resource: ProX-Code.Uri) {
		return ProX-Code.workspace.getConfiguration(doesResourceLookLikeATypeScriptFile(resource) ? 'typescript' : 'javascript', resource);
	}

	private async promptUser(newResources: readonly ProX-Code.Uri[]): Promise<boolean> {
		if (!newResources.length) {
			return false;
		}
		...
		const response = await ProX-Code.window.showInformationMessage(
			newResources.length === 1
				? ProX-Code.l10n.t("Update imports for '{0}'?", path.basename(newResources[0].fsPath))
				: this.getConfirmMessage(ProX-Code.l10n.t("Update imports for the following {0} files?", newResources.length), newResources), {
			modal: true,
	...
	private async getJsTsFileBeingMoved(resource: ProX-Code.Uri): Promise<ProX-Code.Uri | undefined> {
		if (resource.scheme !== fileSchemes.file) {
			return undefined;
		}

		if (await isDirectory(resource)) {
			const files = await ProX-Code.workspace.findFiles(new ProX-Code.RelativePattern(resource, '**/*.{ts,tsx,js,jsx}'), '**/node_modules/**', 1);
			return files[0];
		}
	...
	private getConfirmMessage(start: string, resourcesToConfirm: readonly ProX-Code.Uri[]): string {
	...
	private getConfigTargetScope(config: ProX-Code.WorkspaceConfiguration, settingsName: string): ProX-Code.ConfigurationTarget {
...
export function register(
	...
	handles: (uri: ProX-Code.Uri) => Promise<boolean>,
) {
