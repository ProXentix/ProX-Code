...
suite('WindowsFinder', () => {
	...
	const testWorkspaceFolders = toWorkspaceFolders([{ path: join(fixturesFolder, 'ProX-Code_workspace_1_folder') }, { path: join(fixturesFolder, 'ProX-Code_workspace_2_folder') }], testWorkspace.configPath, extUriBiasedIgnorePathCase);
	...
	const ProX-CodeFolderWindow: ICodeWindow = createTestCodeWindow({ lastFocusTime: 1, openedFolderUri: URI.file(join(fixturesFolder, 'ProX-Code_folder')) });
	const lastActiveWindow: ICodeWindow = createTestCodeWindow({ lastFocusTime: 3, openedFolderUri: undefined });
	const noVscodeFolderWindow: ICodeWindow = createTestCodeWindow({ lastFocusTime: 2, openedFolderUri: URI.file(join(fixturesFolder, 'no_ProX-Code_folder')) });
	const windows: ICodeWindow[] = [
		ProX-CodeFolderWindow,
		lastActiveWindow,
	...
	test('New window without folder when no windows exist', async () => {
		...
		assert.strictEqual(await findWindowOnFile([], URI.file(join(fixturesFolder, 'no_ProX-Code_folder', 'file.txt')), localWorkspaceResolver), undefined);
	});

	test('Existing window with folder', async () => {
		assert.strictEqual(await findWindowOnFile(windows, URI.file(join(fixturesFolder, 'no_ProX-Code_folder', 'file.txt')), localWorkspaceResolver), noVscodeFolderWindow);

		assert.strictEqual(await findWindowOnFile(windows, URI.file(join(fixturesFolder, 'ProX-Code_folder', 'file.txt')), localWorkspaceResolver), ProX-CodeFolderWindow);

		const window: ICodeWindow = createTestCodeWindow({ lastFocusTime: 1, openedFolderUri: URI.file(join(fixturesFolder, 'ProX-Code_folder', 'nested_folder')) });
		assert.strictEqual(await findWindowOnFile([window], URI.file(join(fixturesFolder, 'ProX-Code_folder', 'nested_folder', 'subfolder', 'file.txt')), localWorkspaceResolver), window);
	});

	test('More specific existing window wins', async () => {
		const window: ICodeWindow = createTestCodeWindow({ lastFocusTime: 2, openedFolderUri: URI.file(join(fixturesFolder, 'no_ProX-Code_folder')) });
		const nestedFolderWindow: ICodeWindow = createTestCodeWindow({ lastFocusTime: 1, openedFolderUri: URI.file(join(fixturesFolder, 'no_ProX-Code_folder', 'nested_folder')) });
		assert.strictEqual(await findWindowOnFile([window, nestedFolderWindow], URI.file(join(fixturesFolder, 'no_ProX-Code_folder', 'nested_folder', 'subfolder', 'file.txt')), localWorkspaceResolver), nestedFolderWindow);
	});
	...
	test('Workspace folder wins', async () => {
		...
		assert.strictEqual(await findWindowOnFile([window], URI.file(join(fixturesFolder, 'ProX-Code_workspace_2_folder', 'nested_ProX-Code_folder', 'subfolder', 'file.txt')), localWorkspaceResolver), window);
	});
