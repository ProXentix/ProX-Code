...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private readonly validExtensions = new Set(['.svg', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.mp4', '.otf', '.ttf', '.woff', '.woff2', '.eot']); // https://github.com/ProXentix/ProX-Code/issues/119384
	...
	constructor(
		...
		// - storage    : all files in global and workspace storage (https://github.com/ProXentix/ProX-Code/issues/116735)
	...
	private handleProtocols(): void {
		...
		if ((Schemas.proxCodeFileResource as string) !== 'vscode-file') {
	...
	private handleResourceRequest(request: Electron.ProtocolRequest, callback: ProtocolCallback): void {
		...
		// Refs https://github.com/ProXentix/ProX-Code/issues/148541#issuecomment-2670891511
