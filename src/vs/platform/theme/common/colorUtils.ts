 ...
 * everything is prefixed with `--ProX-Code-`.
 *
 * @sample `editorSuggestWidget.background` is `--ProX-Code-editorSuggestWidget-background`.
 */
export function asCssVariableName(colorIdent: ColorIdentifier): string {
	return `--ProX-Code-${colorIdent.replace(/\./g, '-')}`;
}
...
export const workbenchColorsSchemaId = 'ProX-Code://schemas/workbench-colors';
