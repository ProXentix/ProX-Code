...
function isWorkspaceFolder(thing: any): thing is ProX-Code.WorkspaceFolder {
	return thing
		&& thing.uri instanceof URI
...
function scopeToOverrides(scope: ProX-Code.ConfigurationScope | undefined | null): IConfigurationOverrides | undefined {
	if (isUri(scope)) {
...
export class ExtHostConfigProvider {
	...
	private readonly _onDidChangeConfiguration = new Emitter<ProX-Code.ConfigurationChangeEvent>();
	private readonly _proxy: MainThreadConfigurationShape;
	private readonly _extHostWorkspace: ExtHostWorkspace;
	...
	get onDidChangeConfiguration(): Event<ProX-Code.ConfigurationChangeEvent> {
	...
	getConfiguration(section?: string, scope?: ProX-Code.ConfigurationScope | null, extensionDescription?: IExtensionDescription): ProX-Code.WorkspaceConfiguration {
		...
		const result: ProX-Code.WorkspaceConfiguration = {
	...
	private _toConfigurationChangeEvent(change: IConfigurationChange, previous: { data: IConfigurationData; workspace: Workspace | undefined }): ProX-Code.ConfigurationChangeEvent {
		...
		return Object.freeze({
			affectsConfiguration: (section: string, scope?: ProX-Code.ConfigurationScope) => event.affectsConfiguration(section, scopeToOverrides(scope))
		});
	}
