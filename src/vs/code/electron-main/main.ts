...
/**
 * The main ProX Code entry point.
 *
 * Note: This class can exist more than once for example when ProX Code is already
 * running and a second instance is started from the command line. It will always
 * try to communicate with an existing instance to prevent that 2 ProX Code instances
...
class CodeMain {
	...
	private async startup(): Promise<void> {
		...
		try {
			...
			await instantiationService.invokeFunction(async accessor => {
				...
				// instance of ProX Code running and so we would quit.
				const mainProcessNodeIpcServer = await this.claimInstance(logService, environmentMainService, lifecycleMainService, instantiationService, productService, true);

				// Write a lockfile to indicate an instance is running
				// (https://github.com/microsoft/ProX-Code/issues/127861#issuecomment-877417451)
				...
				// Delay creation of spdlog for perf reasons (https://github.com/microsoft/ProX-Code/issues/72906)
	...
	private createServices(): [IInstantiationService, IProcessEnvironment, IEnvironmentMainService, ConfigurationService, StateService, BufferLogger, IProductService, UserDataProfilesMainService] {
		...
		// log file access on Windows (https://github.com/microsoft/ProX-Code/issues/41218)
	...
	private async claimInstance(logService: ILogService, environmentMainService: IEnvironmentMainService, lifecycleMainService: ILifecycleMainService, instantiationService: IInstantiationService, productService: IProductService, retry: boolean): Promise<NodeIPCServer> {
		...
		try {
			...
			// indicates another instance of ProX Code is running)
	...
	private showStartupWarningDialog(message: string, detail: string, productService: IProductService): void {
		...
		// https://github.com/microsoft/ProX-Code/issues/104493
	...
	private resolveArgs(): NativeParsedArgs {
		...
		// added as argument. This can happen if ProX Code was started from CLI.
	...
	private preparePath(cwd: string, path: string): string {
		...
		if (isWindows) {
			path = rtrim(path, '"'); // https://github.com/microsoft/ProX-Code/issues/1498
		}
