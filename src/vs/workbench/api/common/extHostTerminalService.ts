...
export interface IExtHostTerminalService extends ExtHostTerminalServiceShape, IDisposable {
	...
	activeTerminal: ProX-Code.Terminal | undefined;
	terminals: ProX-Code.Terminal[];

	readonly onDidCloseTerminal: Event<ProX-Code.Terminal>;
	readonly onDidOpenTerminal: Event<ProX-Code.Terminal>;
	readonly onDidChangeActiveTerminal: Event<ProX-Code.Terminal | undefined>;
	readonly onDidChangeTerminalDimensions: Event<ProX-Code.TerminalDimensionsChangeEvent>;
	readonly onDidChangeTerminalState: Event<ProX-Code.Terminal>;
	readonly onDidWriteTerminalData: Event<ProX-Code.TerminalDataWriteEvent>;
	readonly onDidExecuteTerminalCommand: Event<ProX-Code.TerminalExecutedCommand>;
	readonly onDidChangeShell: Event<string>;

	createTerminal(name?: string, shellPath?: string, shellArgs?: readonly string[] | string): ProX-Code.Terminal;
	createTerminalFromOptions(options: ProX-Code.TerminalOptions, internalOptions?: ITerminalInternalOptions): ProX-Code.Terminal;
	createExtensionTerminal(options: ProX-Code.ExtensionTerminalOptions): ProX-Code.Terminal;
	attachPtyToTerminal(id: number, pty: ProX-Code.Pseudoterminal): void;
	getDefaultShell(useAutomationShell: boolean): string;
	getDefaultShellArgs(useAutomationShell: boolean): string[] | string;
	registerLinkProvider(provider: ProX-Code.TerminalLinkProvider): ProX-Code.Disposable;
	registerProfileProvider(extension: IExtensionDescription, id: string, provider: ProX-Code.TerminalProfileProvider): ProX-Code.Disposable;
	registerTerminalQuickFixProvider(id: string, extensionId: string, provider: ProX-Code.TerminalQuickFixProvider): ProX-Code.Disposable;
	...
	getTerminalIdByApiObject(apiTerminal: ProX-Code.Terminal): number | null;
	registerTerminalCompletionProvider(extension: IExtensionDescription, provider: ProX-Code.TerminalCompletionProvider<ProX-Code.TerminalCompletionItem>, ...triggerCharacters: string[]): ProX-Code.Disposable;
}

interface IEnvironmentVariableCollection extends ProX-Code.EnvironmentVariableCollection {
	getScoped(scope: ProX-Code.EnvironmentVariableScope): ProX-Code.EnvironmentVariableCollection;
}
...
export class ExtHostTerminal extends Disposable {
	...
	private _exitStatus: ProX-Code.TerminalExitStatus | undefined;
	private _state: ProX-Code.TerminalState = { isInteractedWith: false, shell: undefined };
	private _selection: string | undefined;

	shellIntegration: ProX-Code.TerminalShellIntegration | undefined;

	public isOpen: boolean = false;

	readonly value: ProX-Code.Terminal;
	...
	constructor(
		...
		private readonly _creationOptions: ProX-Code.TerminalOptions | ProX-Code.ExtensionTerminalOptions,
		...
		this.value = {
			...
			get creationOptions(): Readonly<ProX-Code.TerminalOptions | ProX-Code.ExtensionTerminalOptions> {
				return that._creationOptions;
			},
			get exitStatus(): ProX-Code.TerminalExitStatus | undefined {
				return that._exitStatus;
			},
			get state(): ProX-Code.TerminalState {
				return that._state;
			},
			get selection(): string | undefined {
				return that._selection;
			},
			get shellIntegration(): ProX-Code.TerminalShellIntegration | undefined {
			...
			get dimensions(): ProX-Code.TerminalDimensions | undefined {
	...
	public async create(
		options: ProX-Code.TerminalOptions,
	...
	public async createExtensionTerminal(location?: TerminalLocation | ProX-Code.TerminalEditorLocationOptions | ProX-Code.TerminalSplitLocationOptions, internalOptions?: ITerminalInternalOptions, parentTerminal?: ExtHostTerminalIdentifier, iconPath?: TerminalIcon, color?: ThemeColor): Promise<number> {
		if (typeof this._id !== 'string') {
			throw new Error('Terminal has already been created');
		}
		await this._proxy.$createTerminal(this._id, {
	...
	private _serializeParentTerminal(location?: TerminalLocation | ProX-Code.TerminalEditorLocationOptions | ProX-Code.TerminalSplitLocationOptions, parentTerminal?: ExtHostTerminalIdentifier): TerminalLocation | { viewColumn: EditorGroupColumn; preserveFocus?: boolean } | { parentTerminal: ExtHostTerminalIdentifier } | undefined {
...
class ExtHostPseudoterminal implements ITerminalChildProcess {
	...
	constructor(private readonly _pty: ProX-Code.Pseudoterminal) { }
	...
	acknowledgeDataEvent(charCount: number): void {
		...
		// implemented it will need new pause and resume ProX Code APIs.
	}
...
interface ICachedLinkEntry {
	provider: ProX-Code.TerminalLinkProvider;
	link: ProX-Code.TerminalLink;
}
...
export abstract class BaseExtHostTerminalService extends Disposable implements IExtHostTerminalService, ExtHostTerminalServiceShape {
	...
	private readonly _linkProviders: Set<ProX-Code.TerminalLinkProvider> = new Set();
	private readonly _completionProviders: Map<string, ProX-Code.TerminalCompletionProvider<ProX-Code.TerminalCompletionItem>> = new Map();
	private readonly _profileProviders: Map<string, ProX-Code.TerminalProfileProvider> = new Map();
	private readonly _quickFixProviders: Map<string, ProX-Code.TerminalQuickFixProvider> = new Map();
	private readonly _terminalLinkCache: Map<number, Map<number, ICachedLinkEntry>> = new Map();
	private readonly _terminalLinkCancellationSource: Map<number, CancellationTokenSource> = new Map();

	public get activeTerminal(): ProX-Code.Terminal | undefined { return this._activeTerminal?.value; }
	public get terminals(): ProX-Code.Terminal[] { return this._terminals.map(term => term.value); }

	protected readonly _onDidCloseTerminal = new Emitter<ProX-Code.Terminal>();
	readonly onDidCloseTerminal = this._onDidCloseTerminal.event;
	protected readonly _onDidOpenTerminal = new Emitter<ProX-Code.Terminal>();
	readonly onDidOpenTerminal = this._onDidOpenTerminal.event;
	protected readonly _onDidChangeActiveTerminal = new Emitter<ProX-Code.Terminal | undefined>();
	readonly onDidChangeActiveTerminal = this._onDidChangeActiveTerminal.event;
	protected readonly _onDidChangeTerminalDimensions = new Emitter<ProX-Code.TerminalDimensionsChangeEvent>();
	readonly onDidChangeTerminalDimensions = this._onDidChangeTerminalDimensions.event;
	protected readonly _onDidChangeTerminalState = new Emitter<ProX-Code.Terminal>();
	...
	protected readonly _onDidWriteTerminalData = new Emitter<ProX-Code.TerminalDataWriteEvent>({
	...
	protected readonly _onDidExecuteCommand = new Emitter<ProX-Code.TerminalExecutedCommand>({
	...
	public abstract createTerminal(name?: string, shellPath?: string, shellArgs?: string[] | string): ProX-Code.Terminal;
	public abstract createTerminalFromOptions(options: ProX-Code.TerminalOptions, internalOptions?: ITerminalInternalOptions): ProX-Code.Terminal;
	...
	public createExtensionTerminal(options: ProX-Code.ExtensionTerminalOptions, internalOptions?: ITerminalInternalOptions): ProX-Code.Terminal {
	...
	protected _serializeParentTerminal(options: ProX-Code.TerminalOptions, internalOptions?: ITerminalInternalOptions): ITerminalInternalOptions {
	...
	public attachPtyToTerminal(id: number, pty: ProX-Code.Pseudoterminal): void {
		...
		if (terminal) {
			if (terminal.setDimensions(cols, rows)) {
				this._onDidChangeTerminalDimensions.fire({
					...
					dimensions: terminal.value.dimensions as ProX-Code.TerminalDimensions
				});
			}
		}
	}
	...
	public $acceptTerminalOpened(id: number, extHostTerminalId: string | undefined, name: string, shellLaunchConfigDto: IShellLaunchConfigDto): void {
		...
		const creationOptions: ProX-Code.TerminalOptions = {
	...
	public registerProfileProvider(extension: IExtensionDescription, id: string, provider: ProX-Code.TerminalProfileProvider): ProX-Code.Disposable {
	...
	public registerTerminalCompletionProvider(extension: IExtensionDescription, provider: ProX-Code.TerminalCompletionProvider<TerminalCompletionItem>, ...triggerCharacters: string[]): ProX-Code.Disposable {
	...
	public registerTerminalQuickFixProvider(id: string, extensionId: string, provider: ProX-Code.TerminalQuickFixProvider): ProX-Code.Disposable {
	...
	public registerLinkProvider(provider: ProX-Code.TerminalLinkProvider): ProX-Code.Disposable {
	...
	public async $provideLinks(terminalId: number, line: string): Promise<ITerminalLinkDto[]> {
		...
		const context: ProX-Code.TerminalLinkContext = { terminal: terminal.value, line };
		const promises: ProX-Code.ProviderResult<{ provider: ProX-Code.TerminalLinkProvider; links: ProX-Code.TerminalLink[] }>[] = [];
	...
	public getTerminalIdByApiObject(terminal: ProX-Code.Terminal): number | null {
...
class UnifiedEnvironmentVariableCollection extends Disposable {
	...
	getScopedEnvironmentVariableCollection(scope: ProX-Code.EnvironmentVariableScope | undefined): IEnvironmentVariableCollection {
	...
	replace(variable: string, value: string, options: ProX-Code.EnvironmentVariableMutatorOptions | undefined, scope: ProX-Code.EnvironmentVariableScope | undefined): void {
	...
	append(variable: string, value: string, options: ProX-Code.EnvironmentVariableMutatorOptions | undefined, scope: ProX-Code.EnvironmentVariableScope | undefined): void {
	...
	prepend(variable: string, value: string, options: ProX-Code.EnvironmentVariableMutatorOptions | undefined, scope: ProX-Code.EnvironmentVariableScope | undefined): void {
	...
	private _setIfDiffers(variable: string, mutator: ProX-Code.EnvironmentVariableMutator & { scope: ProX-Code.EnvironmentVariableScope | undefined }): void {
	...
	get(variable: string, scope: ProX-Code.EnvironmentVariableScope | undefined): ProX-Code.EnvironmentVariableMutator | undefined {
	...
	private getKey(variable: string, scope: ProX-Code.EnvironmentVariableScope | undefined) {
		const scopeKey = this.getScopeKey(scope);
		return scopeKey.length ? `${variable}:::${scopeKey}` : variable;
	}

	private getScopeKey(scope: ProX-Code.EnvironmentVariableScope | undefined): string {
		return this.getWorkspaceKey(scope?.workspaceFolder) ?? '';
	}

	private getWorkspaceKey(workspaceFolder: ProX-Code.WorkspaceFolder | undefined): string | undefined {
		return workspaceFolder ? workspaceFolder.uri.toString() : undefined;
	}

	public getVariableMap(scope: ProX-Code.EnvironmentVariableScope | undefined): Map<string, ProX-Code.EnvironmentVariableMutator> {
		const map = new Map<string, ProX-Code.EnvironmentVariableMutator>();
	...
	delete(variable: string, scope: ProX-Code.EnvironmentVariableScope | undefined): void {
		const key = this.getKey(variable, scope);
		this.map.delete(key);
		this._onDidChangeCollection.fire();
	}

	clear(scope: ProX-Code.EnvironmentVariableScope | undefined): void {
	...
	setDescription(description: string | ProX-Code.MarkdownString | undefined, scope: ProX-Code.EnvironmentVariableScope | undefined): void {
	...
	public getDescription(scope: ProX-Code.EnvironmentVariableScope | undefined): string | ProX-Code.MarkdownString | undefined {
		const key = this.getScopeKey(scope);
		return this.descriptionMap.get(key)?.description;
	}

	private clearDescription(scope: ProX-Code.EnvironmentVariableScope | undefined): void {
		const key = this.getScopeKey(scope);
...
class ScopedEnvironmentVariableCollection implements IEnvironmentVariableCollection {
	...
	constructor(
		...
		private readonly scope: ProX-Code.EnvironmentVariableScope | undefined
	) {
	}

	getScoped(scope: ProX-Code.EnvironmentVariableScope | undefined) {
		return this.collection.getScopedEnvironmentVariableCollection(scope);
	}

	replace(variable: string, value: string, options?: ProX-Code.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.replace(variable, value, options, this.scope);
	}

	append(variable: string, value: string, options?: ProX-Code.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.append(variable, value, options, this.scope);
	}

	prepend(variable: string, value: string, options?: ProX-Code.EnvironmentVariableMutatorOptions | undefined): void {
		this.collection.prepend(variable, value, options, this.scope);
	}

	get(variable: string): ProX-Code.EnvironmentVariableMutator | undefined {
		return this.collection.get(variable, this.scope);
	}

	forEach(callback: (variable: string, mutator: ProX-Code.EnvironmentVariableMutator, collection: ProX-Code.EnvironmentVariableCollection) => any, thisArg?: any): void {
		this.collection.getVariableMap(this.scope).forEach((value, variable) => callback.call(thisArg, variable, value, this), this.scope);
	}

	[Symbol.iterator](): IterableIterator<[variable: string, mutator: ProX-Code.EnvironmentVariableMutator]> {
	...
	set description(description: string | ProX-Code.MarkdownString | undefined) {
		this.collection.setDescription(description, this.scope);
	}

	get description(): string | ProX-Code.MarkdownString | undefined {
...
function asTerminalIcon(iconPath?: ProX-Code.Uri | { light: ProX-Code.Uri; dark: ProX-Code.Uri } | ProX-Code.ThemeIcon): TerminalIcon | undefined {
...
function asTerminalColor(color?: ProX-Code.ThemeColor): ThemeColor | undefined {
	return ThemeColor.isThemeColor(color) ? color as ThemeColor : undefined;
}

function convertMutator(mutator: IEnvironmentVariableMutator): ProX-Code.EnvironmentVariableMutator {
	const newMutator = { ...mutator };
	delete newMutator.scope;
	newMutator.options = newMutator.options ?? undefined;
	delete (newMutator as any).variable;
	return newMutator as ProX-Code.EnvironmentVariableMutator;
}
