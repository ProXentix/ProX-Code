...
function shouldTransformUri(uri: string): boolean {
	...
	return /^(file|ProX-Code-remote):/i.test(uri);
}
...
if ((<any>self).Worker) {
	...
	Worker = <any>function (stringUrl: string | URL, options?: WorkerOptions) {
		...
		} else if (/^ProX-Code-remote:/i.test(stringUrl.toString())) {
			// Supporting transformation of ProX-Code-remote URIs requires an async call to the main thread,
...
interface IInitMessage {
	readonly type: 'ProX-Code.init';
	readonly data: ReadonlyMap<string, MessagePort>;
}

function isInitMessage(a: any): a is IInitMessage {
	return !!a && typeof a === 'object' && a.type === 'ProX-Code.init' && a.data instanceof Map;
}
