...
export class TelemetryService implements ITelemetryService {
	...
	constructor(
		...
		// static cleanup pattern for: `ProX-Code-file:///DANGEROUS/PATH/resources/app/Useful/Information`
		this._cleanupPatterns = [/(ProX-Code-)?file:\/\/\/.*?\/resources\/app\//gi];
...
function getTelemetryLevelSettingDescription(): string {
	...
	const externalLinksStatement = !product.privacyStatementUrl ?
		localize("telemetry.docsStatement", "Read more about the [data we collect]({0}).", 'https://aka.ms/ProX-Code-telemetry') :
		localize("telemetry.docsAndPrivacyStatement", "Read more about the [data we collect]({0}) and our [privacy statement]({1}).", 'https://aka.ms/ProX-Code-telemetry', product.privacyStatementUrl);
