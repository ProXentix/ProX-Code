...
export function removeTag() {
	...
	const finalRangesToRemove = Array.from(editor.selections).reverse()
		.reduce<ProX-Code.Range[]>((prev, selection) =>
 ...
 */
function getRangesToRemove(document: ProX-Code.TextDocument, rootNode: HtmlFlatNode, selection: ProX-Code.Selection): ProX-Code.Range[] {
		...
		if (closeTagRange) {
			...
			for (let i = openTagRange.start.line + 1; i < closeTagRange.start.line; i++) {
				if (!document.lineAt(i).isEmptyOrWhitespace) {
					rangesToRemove.push(new ProX-Code.Range(i, 0, i, indentAmountToRemove));
			...
			if (entireLineIsTag(document, closeTagRange) && lastInnerNonEmptyLine) {
				rangesToRemove.push(new ProX-Code.Range(
			...
			if (entireLineIsTag(document, openTagRange) && firstInnerNonEmptyLine) {
				rangesToRemove[1] = new ProX-Code.Range(
					openTagRange.start.line,
					openTagRange.start.character,
...
function entireLineIsTag(document: ProX-Code.TextDocument, range: ProX-Code.Range): boolean {
 ...
 */
function calculateIndentAmountToRemove(document: ProX-Code.TextDocument, openRange: ProX-Code.Range, closeRange: ProX-Code.Range): number {
