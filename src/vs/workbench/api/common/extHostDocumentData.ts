...
export class ExtHostDocumentData extends MirrorTextModel {
	...
	private _document?: ProX-Code.TextDocument;
	...
	get document(): ProX-Code.TextDocument {
		if (!this._document) {
			...
			this._document = {
				...
				lineAt(lineOrPos: number | ProX-Code.Position) { return that._lineAt(lineOrPos); },
	...
	private _getTextInRange(_range: ProX-Code.Range): string {
	...
	private _lineAt(lineOrPosition: number | ProX-Code.Position): ProX-Code.TextLine {
	...
	private _offsetAt(position: ProX-Code.Position): number {
		position = this._validatePosition(position);
		this._ensureLineStarts();
		return this._lineStarts!.getPrefixSum(position.line - 1) + position.character;
	}

	private _positionAt(offset: number): ProX-Code.Position {
	...
	private _validateRange(range: ProX-Code.Range): ProX-Code.Range {
	...
	private _validatePosition(position: ProX-Code.Position): ProX-Code.Position {
	...
	private _getWordRangeAtPosition(_position: ProX-Code.Position, regexp?: RegExp): ProX-Code.Range | undefined {
...
export class ExtHostDocumentLine implements ProX-Code.TextLine {
