...
suite('resolveCopyDestination', () => {
	...
	test('Relative destinations should resolve next to document', async () => {
		const documentUri = ProX-Code.Uri.parse('test://projects/project/sub/readme.md');

		{
			const dest = resolveCopyDestination(documentUri, 'img.png', '${fileName}', () => ProX-Code.Uri.parse('test://projects/project/'));
	...
	test('If path ends in /, we should automatically add the fileName', async () => {
		{
			const documentUri = ProX-Code.Uri.parse('test://projects/project/sub/readme.md');
			const dest = resolveCopyDestination(documentUri, 'img.png', 'images/', () => ProX-Code.Uri.parse('test://projects/project/'));
			assert.strictEqual(dest.toString(), 'test://projects/project/sub/images/img.png');
		}
		{
			const documentUri = ProX-Code.Uri.parse('test://projects/project/sub/readme.md');
			const dest = resolveCopyDestination(documentUri, 'img.png', './', () => ProX-Code.Uri.parse('test://projects/project/'));
			assert.strictEqual(dest.toString(), 'test://projects/project/sub/img.png');
		}
		{
			const documentUri = ProX-Code.Uri.parse('test://projects/project/sub/readme.md');
