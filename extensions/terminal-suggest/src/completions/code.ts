...
export const extensionManagementOptions = (cliName: string): Fig.Option[] => [
	...
	},
	{
		...
		description:
			`Installs or updates an extension. The argument is either an extension id or a path to a VSIX. The identifier of an extension is '\${ publisher }.\${ name }'. Use '--force' argument to update to latest version. To install a specific version provide '@\${version}'. For example: 'ProX-Code.csharp@1.2.3'`,
		args: {
...
export const codeTunnelSubcommands = [
	{
		name: 'tunnel',
		description: 'Create a tunnel that\'s accessible on ProX-Code.dev from anywhere. Run`code tunnel --help` for more usage info',
		subcommands: [
			{
				name: 'prune',
