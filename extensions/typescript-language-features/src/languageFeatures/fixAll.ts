...
async function buildIndividualFixes(
	...
	edit: ProX-Code.WorkspaceEdit,
	client: ITypeScriptServiceClient,
	file: string,
	diagnostics: readonly ProX-Code.Diagnostic[],
...
abstract class SourceAction extends ProX-Code.CodeAction {
	abstract build(
		client: ITypeScriptServiceClient,
		file: string,
		diagnostics: readonly ProX-Code.Diagnostic[],
...
class SourceFixAll extends SourceAction {
	...
	static readonly kind = ProX-Code.CodeActionKind.SourceFixAll.append('ts');

	constructor() {
		super(ProX-Code.l10n.t("Fix all fixable JS/TS issues"), SourceFixAll.kind);
	}

	async build(client: ITypeScriptServiceClient, file: string, diagnostics: readonly ProX-Code.Diagnostic[], token: ProX-Code.CancellationToken): Promise<void> {
		this.edit = new ProX-Code.WorkspaceEdit();
...
class SourceRemoveUnused extends SourceAction {
	...
	static readonly kind = ProX-Code.CodeActionKind.Source.append('removeUnused').append('ts');

	constructor() {
		super(ProX-Code.l10n.t("Remove all unused code"), SourceRemoveUnused.kind);
	}
...
class SourceAddMissingImports extends SourceAction {
	...
	static readonly kind = ProX-Code.CodeActionKind.Source.append('addMissingImports').append('ts');

	constructor() {
		super(ProX-Code.l10n.t("Add all missing imports"), SourceAddMissingImports.kind);
	}
...
class TypeScriptAutoFixProvider implements ProX-Code.CodeActionProvider {
	...
	public get metadata(): ProX-Code.CodeActionProviderMetadata {
		return {
	...
	public async provideCodeActions(
		...
		_range: ProX-Code.Range,
		context: ProX-Code.CodeActionContext,
		token: ProX-Code.CancellationToken
	): Promise<ProX-Code.CodeAction[] | undefined> {
		if (!context.only || !ProX-Code.CodeActionKind.Source.intersects(context.only)) {
			return undefined;
		}
	...
	private getFixAllActions(only: ProX-Code.CodeActionKind): SourceAction[] {
...
export function register(
	...
	return conditionalRegistration([
		...
		return ProX-Code.languages.registerCodeActionsProvider(selector.semantic, provider, provider.metadata);
	});
}
