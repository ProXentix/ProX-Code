...
suite('MarkdownRenderer', () => {
	...
	suite('Images', () => {
		...
		test('image with file uri should render as same origin uri', () => {
			...
			assertNodeEquals(result, '<div><p><img src="ProX-Code-file://ProX-Code-app/images/cat.gif" alt="image"></p></div>');
		});
	});
...
`);
	...
	suite('supportHtml', () => {
		...
		test('Should render html images with file uri as same origin uri', () => {
			...
			const result = store.add(renderMarkdown(mds)).element;
			assert.strictEqual(result.innerHTML, `<img src="ProX-Code-file://ProX-Code-app/images/cat.gif">`);
		});
	});
	...
	- item 2
		...
		suite('link', () => {
			...
			test('incomplete link target with incomplete arg 2', () => {
				const incomplete = '[text](command:ProX-Code.openRelativePath "arg';
				const tokens = marked.marked.lexer(incomplete);
				const newTokens = fillInIncompleteTokens(tokens);
