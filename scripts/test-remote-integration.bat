...
IF "%~1" == "" (
	set AUTHORITY=ProX-Code-remote://test+test/
...
IF "%VSCODEUSERDATADIR%" == "" (
	set VSCODEUSERDATADIR=%TMP%\ProX-Codeuserfolder-%RANDOM%-%TIME:~6,5%
)
...
if "%VSCODE_REMOTE_SERVER_PATH%"=="" (
	...
	set TESTRESOLVER_INSTALL_BUILTIN_EXTENSION=ms-ProX-Code.ProX-Code-smoketest-check
...
if "%INTEGRATION_TEST_ELECTRON_PATH%"=="" (
	...
	set API_TESTS_EXTRA_ARGS_BUILT=--extensions-dir=%EXT_PATH% --enable-proposed-api=ProX-Code.ProX-Code-test-resolver --enable-proposed-api=ProX-Code.ProX-Code-api-tests
...
call "%INTEGRATION_TEST_ELECTRON_PATH%" --folder-uri=%REMOTE_EXT_PATH%/ProX-Code-api-tests/testWorkspace --extensionDevelopmentPath=%REMOTE_EXT_PATH%/ProX-Code-api-tests --extensionTestsPath=%REMOTE_EXT_PATH%/ProX-Code-api-tests/out/singlefolder-tests %API_TESTS_EXTRA_ARGS% %API_TESTS_EXTRA_ARGS_BUILT%
if %errorlevel% neq 0 exit /b %errorlevel%

echo.
echo ### API tests (workspace)
call "%INTEGRATION_TEST_ELECTRON_PATH%" --file-uri=%REMOTE_EXT_PATH%/ProX-Code-api-tests/testworkspace.code-workspace --extensionDevelopmentPath=%REMOTE_EXT_PATH%/ProX-Code-api-tests --extensionTestsPath=%REMOTE_EXT_PATH%/ProX-Code-api-tests/out/workspace-tests %API_TESTS_EXTRA_ARGS% %API_TESTS_EXTRA_ARGS_BUILT%
if %errorlevel% neq 0 exit /b %errorlevel%
