...
export class MarkdownPreviewConfiguration {
	public static getForResource(resource: ProX-Code.Uri | null) {
	...
	private constructor(resource: ProX-Code.Uri | null) {
		const editorConfig = ProX-Code.workspace.getConfiguration('editor', resource);
		const markdownConfig = ProX-Code.workspace.getConfiguration('markdown', resource);
		const markdownEditorConfig = ProX-Code.workspace.getConfiguration('[markdown]', resource);
...
export class MarkdownPreviewConfigurationManager {
	...
	private _getKey(
		...
		const folder = ProX-Code.workspace.getWorkspaceFolder(resource);
