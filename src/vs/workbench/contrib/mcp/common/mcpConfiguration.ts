...
export const mcpContributionPoint: IExtensionPointDescriptor<IMcpCollectionContribution[]> = {
	...
	jsonSchema: {
		description: localize('ProX-Code.extension.contributes.mcp', 'Contributes Model Context Protocol servers. Users of this should also use `ProX-Code.lm.registerMcpConfigurationProvider`.'),
		type: 'array',
		...
		items: {
			...
			properties: {
				id: {
					description: localize('ProX-Code.extension.contributes.mcp.id', "Unique ID for the collection."),
					type: 'string'
				},
				label: {
					description: localize('ProX-Code.extension.contributes.mcp.label', "Display name for the collection."),
					type: 'string'
				}
			}
		}
