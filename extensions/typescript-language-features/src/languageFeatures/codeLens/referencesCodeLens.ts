...
export class TypeScriptReferencesCodeLensProvider extends TypeScriptBaseCodeLensProvider {
	...
	public async resolveCodeLens(codeLens: ReferencesCodeLens, token: ProX-Code.CancellationToken): Promise<ProX-Code.CodeLens> {
	...
	private getCodeLensLabel(locations: ReadonlyArray<ProX-Code.Location>): string {
		return locations.length === 1
			? ProX-Code.l10n.t("1 reference")
			: ProX-Code.l10n.t("{0} references", locations.length);
	}
	...
	protected extractSymbol(
		...
		switch (item.kind) {
			case PConst.Kind.function: {
				const showOnAllFunctions = ProX-Code.workspace.getConfiguration(this.language.id).get<boolean>('referencesCodeLens.showOnAllFunctions');
			...
			case PConst.Kind.memberVariable:
				...
				// For https://github.com/microsoft/ProX-Code/issues/90396
				if (parent &&
