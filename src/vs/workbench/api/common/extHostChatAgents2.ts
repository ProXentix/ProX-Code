...
class ChatAgentResponseStream {
	...
	private _apiObject: ProX-Code.ChatResponseStream | undefined;
	...
	get apiObject() {
		...
		if (!this._apiObject) {
			...
			const _report = (progress: IChatProgressDto, task?: (progress: ProX-Code.Progress<ProX-Code.ChatResponseWarningPart | ProX-Code.ChatResponseReferencePart>) => Thenable<string | void>) => {
				...
				if (task) {
					...
					const progressReporter = {
						report: (p: ProX-Code.ChatResponseWarningPart | ProX-Code.ChatResponseReferencePart) => {
							progressReporterPromise.then(() => {
								if (extHostTypes.MarkdownString.isMarkdownString(p.value)) {
									send(typeConvert.ChatResponseWarningPart.from(<ProX-Code.ChatResponseWarningPart>p), myHandle);
								} else {
									send(typeConvert.ChatResponseReferencePart.from(<ProX-Code.ChatResponseReferencePart>p), myHandle);
								}
							});
						}
					};
			...
			this._apiObject = Object.freeze<ProX-Code.ChatResponseStream>({
				markdown(value) {
				...
				progress(value, task?: ((progress: ProX-Code.Progress<ProX-Code.ChatResponseWarningPart>) => Thenable<string | void>)) {
				...
				codeCitation(value: ProX-Code.Uri, license: string, snippet: string): void {
...
interface InFlightChatRequest {
	requestId: string;
	extRequest: ProX-Code.ChatRequest;
}
...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	transferActiveChat(newWorkspace: ProX-Code.Uri): void {
		this._proxy.$transferActiveChatSession(newWorkspace);
	}

	createChatAgent(extension: IExtensionDescription, id: string, handler: ProX-Code.ChatExtendedRequestHandler): ProX-Code.ChatParticipant {
	...
	createDynamicChatAgent(extension: IExtensionDescription, id: string, dynamicProps: ProX-Code.DynamicChatParticipantProps, handler: ProX-Code.ChatExtendedRequestHandler): ProX-Code.ChatParticipant {
		const handle = ExtHostChatAgents2._idPool++;
		const agent = new ExtHostChatAgent(extension, id, this._proxy, handle, handler);
		this._agents.set(handle, agent);
	...
	registerChatParticipantDetectionProvider(extension: IExtensionDescription, provider: ProX-Code.ChatParticipantDetectionProvider): ProX-Code.Disposable {
	...
	registerRelatedFilesProvider(extension: IExtensionDescription, provider: ProX-Code.ChatRelatedFilesProvider, metadata: ProX-Code.ChatRelatedFilesProviderMetadata): ProX-Code.Disposable {
	...
	async $detectChatParticipant(handle: number, requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[] }, options: { location: ChatAgentLocation; participants?: ProX-Code.ChatParticipantMetadata[] }, token: CancellationToken): Promise<ProX-Code.ChatParticipantDetectionResult | null | undefined> {
	...
	private async _createRequest(requestDto: Dto<IChatAgentRequest>, context: { history: IChatAgentHistoryEntryDto[] }, extension: IExtensionDescription) {
		...
		let location: ProX-Code.ChatRequestEditorData | ProX-Code.ChatRequestNotebookData | undefined;
	...
	private async getModelForRequest(request: IChatAgentRequest, extension: IExtensionDescription): Promise<ProX-Code.LanguageModelChat> {
		let model: ProX-Code.LanguageModelChat | undefined;
	...
	private getToolsForRequest(extension: IExtensionDescription, request: Dto<IChatAgentRequest>): ProX-Code.ChatRequestToolSelection | undefined {
	...
	private async prepareHistoryTurns(extension: Readonly<IRelaxedExtensionDescription>, agentId: string, context: { history: IChatAgentHistoryEntryDto[] }): Promise<(ProX-Code.ChatRequestTurn | ProX-Code.ChatResponseTurn)[]> {
		const res: (ProX-Code.ChatRequestTurn | ProX-Code.ChatResponseTurn)[] = [];

		for (const h of context.history) {
			const ehResult = typeConvert.ChatAgentResult.to(h.result);
			const result: ProX-Code.ChatResult = agentId === h.request.agentId ?
				ehResult :
	...
	$acceptFeedback(handle: number, result: IChatAgentResult, voteAction: IChatVoteAction): void {
		...
		const feedback: ProX-Code.ChatResultFeedback = {
			result: ehResult,
			kind,
...
class ExtHostParticipantDetector {
	constructor(
		...
		public readonly provider: ProX-Code.ChatParticipantDetectionProvider,
	) { }
}
...
class ExtHostRelatedFilesProvider {
	constructor(
		public readonly extension: IExtensionDescription,
		public readonly provider: ProX-Code.ChatRelatedFilesProvider,
	) { }
}

class ExtHostChatAgent {

	private _followupProvider: ProX-Code.ChatFollowupProvider | undefined;
	private _iconPath: ProX-Code.Uri | { light: ProX-Code.Uri; dark: ProX-Code.Uri } | ProX-Code.ThemeIcon | undefined;
	private _helpTextPrefix: string | ProX-Code.MarkdownString | undefined;
	private _helpTextVariablesPrefix: string | ProX-Code.MarkdownString | undefined;
	private _helpTextPostfix: string | ProX-Code.MarkdownString | undefined;
	private _onDidReceiveFeedback = new Emitter<ProX-Code.ChatResultFeedback>();
	private _onDidPerformAction = new Emitter<ProX-Code.ChatUserActionEvent>();
	private _supportIssueReporting: boolean | undefined;
	private _agentVariableProvider?: { provider: ProX-Code.ChatParticipantCompletionItemProvider; triggerCharacters: string[] };
	private _additionalWelcomeMessage?: string | ProX-Code.MarkdownString | undefined;
	private _titleProvider?: ProX-Code.ChatTitleProvider | undefined;
	private _requester: ProX-Code.ChatRequesterInformation | undefined;
	private _pauseStateEmitter = new Emitter<ProX-Code.ChatParticipantPauseStateEvent>();
	...
	constructor(
		...
		private _requestHandler: ProX-Code.ChatExtendedRequestHandler,
	) { }

	acceptFeedback(feedback: ProX-Code.ChatResultFeedback) {
		this._onDidReceiveFeedback.fire(feedback);
	}

	acceptAction(event: ProX-Code.ChatUserActionEvent) {
		this._onDidPerformAction.fire(event);
	}

	setChatRequestPauseState(pauseState: ProX-Code.ChatParticipantPauseStateEvent) {
		this._pauseStateEmitter.fire(pauseState);
	}

	async invokeCompletionProvider(query: string, token: CancellationToken): Promise<ProX-Code.ChatCompletionItem[]> {
	...
	async provideFollowups(result: ProX-Code.ChatResult, context: ProX-Code.ChatContext, token: CancellationToken): Promise<ProX-Code.ChatFollowup[]> {
	...
	async provideTitle(context: ProX-Code.ChatContext, token: CancellationToken): Promise<string | undefined> {
	...
	get apiAgent(): ProX-Code.ChatParticipant {
		let disposed = false;
		...
		} satisfies ProX-Code.ChatParticipant;
	}

	invoke(request: ProX-Code.ChatRequest, context: ProX-Code.ChatContext, response: ProX-Code.ChatResponseStream, token: CancellationToken): ProX-Code.ProviderResult<ProX-Code.ChatResult | void> {
