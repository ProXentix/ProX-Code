...
suite('ProX-Code API - workspace events', () => {
	...
	test('onWillCreate/onDidCreate', withLogDisabled(async function () {
		...
		let onWillCreate: ProX-Code.FileWillCreateEvent | undefined;
		let onDidCreate: ProX-Code.FileCreateEvent | undefined;

		disposables.push(ProX-Code.workspace.onWillCreateFiles(e => onWillCreate = e));
		disposables.push(ProX-Code.workspace.onDidCreateFiles(e => onDidCreate = e));
	...
	test('onWillCreate/onDidCreate, make changes, edit another file', withLogDisabled(async function () {
		...
		const base = await createRandomFile();
		const baseDoc = await ProX-Code.workspace.openTextDocument(base);

		const newUri = base.with({ path: base.path + '-foo' });

		disposables.push(ProX-Code.workspace.onWillCreateFiles(e => {
			const ws = new ProX-Code.WorkspaceEdit();
			ws.insert(base, new ProX-Code.Position(0, 0), 'HALLO_NEW');
	...
	test('onWillCreate/onDidCreate, make changes, edit new file fails', withLogDisabled(async function () {
		...
		disposables.push(ProX-Code.workspace.onWillCreateFiles(e => {
			const ws = new ProX-Code.WorkspaceEdit();
			ws.insert(e.files[0], new ProX-Code.Position(0, 0), 'nope');
			e.waitUntil(Promise.resolve(ws));
		}));
		...
		assert.strictEqual((await ProX-Code.workspace.fs.readFile(newUri)).toString(), '');
		assert.strictEqual((await ProX-Code.workspace.openTextDocument(newUri)).getText(), '');
	}));
	...
	test('onWillDelete/onDidDelete', withLogDisabled(async function () {
		...
		let onWilldelete: ProX-Code.FileWillDeleteEvent | undefined;
		let onDiddelete: ProX-Code.FileDeleteEvent | undefined;

		disposables.push(ProX-Code.workspace.onWillDeleteFiles(e => onWilldelete = e));
		disposables.push(ProX-Code.workspace.onDidDeleteFiles(e => onDiddelete = e));
	...
	test('onWillDelete/onDidDelete, make changes', withLogDisabled(async function () {
		...
		disposables.push(ProX-Code.workspace.onWillDeleteFiles(e => {

			const edit = new ProX-Code.WorkspaceEdit();
			edit.createFile(newUri);
			edit.insert(newUri, new ProX-Code.Position(0, 0), 'hahah');
	...
	test('onWillRename/onDidRename', withLogDisabled(async function () {
		...
		let onWillRename: ProX-Code.FileWillRenameEvent | undefined;
		let onDidRename: ProX-Code.FileRenameEvent | undefined;

		disposables.push(ProX-Code.workspace.onWillRenameFiles(e => onWillRename = e));
		disposables.push(ProX-Code.workspace.onDidRenameFiles(e => onDidRename = e));
	...
	async function testOnWillRename(withDirtyFile: boolean): Promise<void> {
		...
		if (withDirtyFile) {
			const edit = new ProX-Code.WorkspaceEdit();
			edit.insert(oldUri, new ProX-Code.Position(0, 0), 'BAR');

			const success = await ProX-Code.workspace.applyEdit(edit);
			assert.ok(success);

			const oldDocument = await ProX-Code.workspace.openTextDocument(oldUri);
		...
		disposables.push(ProX-Code.workspace.onWillRenameFiles(e => {
			onWillRename = e;
			const edit = new ProX-Code.WorkspaceEdit();
			edit.insert(e.files[0].oldUri, new ProX-Code.Position(0, 0), 'FOO');
			edit.replace(anotherFile, new ProX-Code.Range(0, 0, 0, 3), 'FARBOO');
			e.waitUntil(Promise.resolve(edit));
		}));
		...
		const newDocument = await ProX-Code.workspace.openTextDocument(newUri);
		const anotherDocument = await ProX-Code.workspace.openTextDocument(anotherFile);
