...
export async function reopenAsText(resource: ProX-Code.Uri, viewColumn: ProX-Code.ViewColumn | undefined): Promise<void> {
	await ProX-Code.commands.executeCommand('ProX-Code.openWith', resource, 'default', viewColumn);
}
...
export abstract class MediaPreview extends Disposable {
	...
	constructor(
		extensionRoot: ProX-Code.Uri,
		protected readonly _resource: ProX-Code.Uri,
		protected readonly _webviewEditor: ProX-Code.WebviewPanel,
		...
		const watcher = this._register(ProX-Code.workspace.createFileSystemWatcher(new ProX-Code.RelativePattern(_resource, '*')));
	...
	protected updateBinarySize() {
		ProX-Code.workspace.fs.stat(this._resource).then(({ size }) => {
