...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): void {
		...
		ipcRenderer.on('ProX-Code:runAction', async (event: unknown, request: INativeRunActionInWindowRequest) => {
		...
		ipcRenderer.on('ProX-Code:runKeybinding', (event: unknown, request: INativeRunKeybindingInWindowRequest) => {
		...
		ipcRenderer.on('ProX-Code:reportSharedProcessCrash', (event: unknown, error: string) => {
		...
		ipcRenderer.on('ProX-Code:openFiles', (event: unknown, request: IOpenFileRequest) => { this.onOpenFiles(request); });

		// Support addRemoveFolders event for workspace management
		ipcRenderer.on('ProX-Code:addRemoveFolders', (event: unknown, request: IAddRemoveFoldersRequest) => this.onAddRemoveFoldersRequest(request));

		// Message support
		ipcRenderer.on('ProX-Code:showInfoMessage', (event: unknown, message: string) => this.notificationService.info(message));

		// Shell Environment Issue Notifications
		ipcRenderer.on('ProX-Code:showResolveShellEnvError', (event: unknown, message: string) => {
			this.notificationService.prompt(
				Severity.Error,
				message,
				[{
		...
		ipcRenderer.on('ProX-Code:showCredentialsError', (event: unknown, message: string) => {
			this.notificationService.prompt(
				Severity.Error,
		...
		ipcRenderer.on('ProX-Code:showTranslatedBuildWarning', () => {
			this.notificationService.prompt(
				Severity.Warning,
		...
		ipcRenderer.on('ProX-Code:showArgvParseWarning', (event: unknown, message: string) => {
			this.notificationService.prompt(
				Severity.Warning,
		...
		// Fullscreen Events
		ipcRenderer.on('ProX-Code:enterFullScreen', () => setFullscreen(true, mainWindow));
		ipcRenderer.on('ProX-Code:leaveFullScreen', () => setFullscreen(false, mainWindow));

		// Proxy Login Dialog
		ipcRenderer.on('ProX-Code:openProxyAuthenticationDialog', async (event: unknown, payload: { authInfo: AuthInfo; username?: string; password?: string; replyChannel: string }) => {
		...
		ipcRenderer.on('ProX-Code:accessibilitySupportChanged', (event: unknown, accessibilitySupportEnabled: boolean) => {
			this.accessibilityService.setAccessibilitySupport(accessibilitySupportEnabled ? AccessibilitySupport.Enabled : AccessibilitySupport.Disabled);
		});

		// Allow to update security settings around allowed UNC Host
		ipcRenderer.on('ProX-Code:configureAllowedUNCHost', async (event: unknown, host: string) => {
			if (!isWindows) {
		...
		ipcRenderer.on('ProX-Code:disablePromptForProtocolHandling', (event: unknown, kind: 'local' | 'remote') => {
	...
	private async handleWarnings(): Promise<void> {
		...
		if (this.environmentService.isBuilt) {
			...
			if (isMacintosh) {
				// appRoot = /Applications/ProX Code - Insiders.app/Contents/Resources/app
				installLocationUri = dirname(dirname(dirname(URI.file(this.nativeEnvironmentService.appRoot))));
			} else {
				// appRoot = C:\Users\<name>\AppData\Local\Programs\Microsoft ProX Code Insiders\resources\app
	...
	private handleOnDidChangeZoomLevel(targetWindowId: number): void {
		...
		if (targetWindowId === mainWindow.ProX-CodeWindowId) {
			...
			ipcRenderer.invoke('ProX-Code:notifyZoomLevel', notifyZoomLevel);
		}
	}
